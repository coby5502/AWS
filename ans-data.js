window.ANS_QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "BC",
    "explanation": "【핵심 용어】\n▸ VPC PrivateLink 엔드포인트 — AWS 서비스를 프라이빗으로 접근하기 위한 VPC 내 가상 인터페이스\n▸ DNS 이름 해석 — enableDnsSupport와 엔드포인트별 DNS 지원으로 프라이빗 IP로 변환\n▸ 엔드포인트 정책 — 기본 deny-all, 명시적 Allow 필요\n\n【정답 포인트】\n▸ enableDnsSupport(VPC) + DNS 지원(엔드포인트) 활성화 → AWS 서비스 DNS를 프라이빗 IP로 변환\n(B) ▸ 엔드포인트 정책 검증 → 기본값 deny-all이므로 명시적으로 통신 허용 설정\n(C) 【오답 체크】\n(A) 라우트 테이블에 경로 추가 — 불필요. PrivateLink는 DNS 해석으로 작동하며 라우팅이 아님\n(D) Route 53 공개 호스팅 영역 — 공개 인터넷 경로로 역효과. VPC 엔드포인트의 DNS 지원이 정답\n(E) 프라이빗 호스팅 영역 생성 — 불필요. VPC 엔드포인트의 기본 DNS 지원으로 충분\n\n【시험 포인트】\n▸ PrivateLink DNS 흐름 → enableDnsSupport(VPC) → 엔드포인트 DNS 활성화 → 정책 확인\n▸ 함정: 라우팅\n(A) 이나 Route 53(D,E)은 VPC 엔드포인트의 기본 기능을 간과한 오답",
