// AWS Certified Advanced Networking - Specialty (ANS-C01) 한국어 문제 데이터
// 수집일: 2026-04-21 · 출처: ExamTopics (via blog.naver.com/imaokiro)
// 총 289문제
...
window.ANS_QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Gateway VPC Endpoint (S3) — S3 접근 시 NAT 우회\n▸ Interface VPC Endpoint — ENI 기반 엔드포인트 (비용 추가)\n▸ VPC Route Table — 라우팅 정책\n▸ Data Transfer Cost — NAT 게이트웨이 통과 비용\n\n【정답 포인트】\n▸ S3 접근 → Gateway Endpoint로 NAT 우회 → 데이터 전송 비용 제거\n▸ Gateway Endpoint는 S3 전용 → 라우트 기반 자동 리다이렉션\n▸ Interface Endpoint 대비 무료 (S3만 해당)\n▸ 라우트 테이블에 S3 프리픽스 리스트 추가 → 자동 라우팅\n\nOption D: Gateway Endpoint는 라우트 테이블 기반으로 S3 트래픽을 자동으로 리다이렉션. NAT 게이트웨이 통과 불필요 → 비용 제거. Private Subnet EC2도 직접 S3 접근 가능 (권한만 있으면).\n\n【오답 체크】\n(A) Direct Connect 프라이빗 VIF는 온프레미스 연결용 → S3 (AWS internal) 트래픽과 무관.\n(B) VPN Tunnel은 encrypted overhead만 추가 → NAT 비용 미해결.\n(C) Interface Endpoint는 ENI 생성으로 추가 비용 + 관리 오버헤드.\n\n【시험 포인트】\n▸ S3 접근 비용 최적화: Gateway Endpoint 필수\n▸ Endpoint 유형 선택: S3/DynamoDB → Gateway, 기타 → Interface\n▸ 라우트 테이블 자동 업데이트: Prefix List 기반",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Transit Gateway Network Manager — TGW 네트워크 모니터링\n▸ CloudWatch Logs Insights — 로그 분석/쿼리\n▸ EventBridge — 이벤트 기반 알림\n▸ BGP Route Change — 라우팅 테이블 변경 감지\n\n【정답 포인트】\n▸ TGW Network Manager: BGP 경로 변경 이벤트 자동 수집\n▸ CloudWatch Logs Insights: 경로 변경 로그 쿼리\n▸ EventBridge: 로그 패턴 기반 실시간 알림\n▸ Lambda Polling 대비 실시간 이벤트 기반 아키텍처\n\nOption B: TGW Network Manager가 Direct Connect 연결의 BGP 이벤트 (새 경로 광고 등)를 CloudWatch Logs로 전송. EventBridge가 로그 패턴을 감지하여 실시간 알림. 네이티브 AWS 솔루션으로 관리 오버헤드 최소.\n\n【오답 체크】\n(A) CloudWatch Metrics는 수치 메트릭 (대역폭 등)만 추적 → 경로 변경 이벤트 감지 불가.\n(C) Lambda Polling은 주기적 체크 → 실시간성 낮음 + 비용 + 관리 복잡.\n(D) Transit VIF 로그는 존재하지 않음 → CloudWatch Logs에 경로 정보 미전송.\n\n【시험 포인트】\n▸ TGW Network Manager: Hybrid 네트워크 통합 모니터링\n▸ CloudWatch Logs + EventBridge: 이벤트 기반 자동화\n▸ 실시간 라우팅 변경 감지: 네트워크 가시성 핵심",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ MACsec (Media Access Control Security) — Layer 2 암호화\n▸ Direct Connect Encryption — AWS-to-Colocation 암호화\n▸ Operational Overhead — 운영 복잡성\n▸ Bandwidth Preservation — 기존 10 Gbps 유지\n\n【정답 포인트】\n▸ MACsec은 Direct Connect 네이티브 암호화 방식 → VPN 대비 오버헤드 적음\n▸ Layer 2 암호화 → 성능 영향 최소\n▸ 새로운 연결로 배포 → 기존 연결 유지 중 증설 가능\n▸ 엣지 라우터에서 MACsec 설정 → 초기 구성 후 자동화\n\nOption C: MACsec이 포함된 새 DX 연결 2개 배포 → 10 Gbps 유지. 엣지 라우터에 MACsec 구성 (초기만). 기존 연결에서 새 연결로 점진적 전환. VPN 오버헤드 없음 → 최소 오버헤드.\n\n【오답 체크】\n(A) Public VIF 암호화는 미존재 → 실질적 암호화 불가.\n(B) VPN은 IPsec 오버헤드 + 성능 저하 + 대역폭 감소 위험.\n(D) 복잡한 다층 구조 (DX + Public VIF + VPN) → 운영 복잡성 극대화.\n\n【시험 포인트】\n▸ Direct Connect MACsec: Layer 2 암호화의 표준\n▸ 대역폭 효율성: VPN 대비 우월\n▸ 점진적 마이그레이션: 기존 연결 유지 중 증설",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ ALB Access Logs — Application Load Balancer 로그\n▸ Amazon Athena — S3 기반 SQL 쿼리\n▸ Amazon S3 — 로그 저장소\n▸ Operational Efficiency — 관리 오버헤드 최소\n\n【정답 포인트】\n▸ ALB 로그는 S3에 자동 저장 지원\n▸ Athena: S3에 직접 SQL 쿼리 (ETL 불필요)\n▸ 서버리스 → 인프라 관리 불필요\n▸ 클라이언트 IP, 대상 IP, 포트, User-Agent 모두 ALB 로그에 포함됨\n\nOption D: ALB Access Logs 활성화 → S3 저장. Athena로 S3 로그를 직접 SQL 쿼리. \"SELECT * WHERE client_ip = 'X'\" 등 강력한 필터링. 추가 인프라 없음 → 비용 효율적.\n\n【오답 체크】\n(A) 수동 다운로드 + 스프레드시트 → 확장성 낮음 + 반복 작업 비효율.\n(B) Kinesis Data Streams → ALB 직접 지원 안 함 (custom integration 필요) + 비용.\n(C) Kinesis + OpenSearch → 복잡한 파이프라인 + 유지보수 오버헤드.\n\n【시험 포인트】\n▸ ALB 로그 분석: Athena의 표준 패턴\n▸ 서버리스 로그 분석: S3 + Athena 조합\n▸ 운영 효율성: 기존 AWS 서비스 조합으로 신속 구현",
  ...
  },
  {
    ...
    "answer": "BDE",
    "explanation": "【핵심 용어】\n▸ CloudFront Edge Encryption — CloudFront to Client (TLS)\n▸ Origin HTTPS — CloudFront to ALB (TLS)\n▸ Certificate Chain — public(CF) + ALB 인증서\n▸ HTTP Redirect — 강제 암호화\n\n【정답 포인트】\n▸ CloudFront → Client: service.example.com 인증서 필요\n▸ CloudFront → ALB: ALB가 HTTPS 리스너, ALB 인증서 필요\n▸ ACM 공인 인증서 필수 (자체 서명 불가)\n▸ 양쪽 HTTP → HTTPS 리다이렉트\n\nOption B: ACM 공인 인증서(service.example.com) + CloudFront 설정 + HTTP→HTTPS 리다이렉트.\nOption D: ALB EC2 인스턴스 HTTPS + 공인 제3자 인증서 (wildcard 가능) + 새 대상 그룹.\nOption E: ALB HTTPS 리스너 + ACM 인증서(service-alb.example.com) + CloudFront HTTPS only.\n\n조합 BDE: B는 CloudFront 엣지 암호화, D는 백엔드 인스턴스 암호화, E는 ALB 암호화 → 3단계 암호화 완성.\n\n【오답 체크】\n(A) 자체 서명 인증서 → CloudFront 브라우저 경고 → 부적절.\n(C) EC2 \"인스턴스 대상\" + \"모든 도메인\" 인증서 → 도메인 불일치 + 인증서 유효성 문제.\n(F) 자체 서명 인증서 → ALB와 CloudFront 간 검증 실패.\n\n【시험 포인트】\n▸ Multi-tier HTTPS: 각 구간마다 독립 인증서\n▸ CloudFront + ALB: 도메인 명확한 구분 필수\n▸ ACM 공인 인증서: 자체 서명 금지",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ NLB Availability Zone — NLB 리스너가 활성화된 AZ\n▸ Target Registration — 대상 그룹에 인스턴스 등록\n▸ Cross-AZ Load Balancing — 다중 AZ 로드 밸런싱\n\n【정답 포인트】\n▸ NLB 생성 시 기본값: 현재 AZ만 활성화\n▸ 새로운 인스턴스가 있는 AZ도 활성화 필요\n▸ 대상 그룹에 이미 등록됨 → 새 대상 그룹 불필요\n▸ AZ 활성화 설정 1회 → 문제 해결\n\nOption A: NLB 구성에서 두 번째 AZ를 활성화하면, 기존 대상 그룹(이미 인스턴스 등록됨)에서 자동으로 2개 AZ에 트래픽 분산. 단순한 설정 변경.\n\n【오답 체크】\n(B) 새로운 NLB 생성 → 비효율적 + 비용 증가.\n(C) Proxy Protocol은 클라이언트 IP 전달 → 로드 밸런싱 문제 미해결.\n(D) 대상은 이미 등록됨 → 새 대상 그룹 불필요.\n\n【시험 포인트】\n▸ NLB Multi-AZ: AZ 활성화 설정 필수\n▸ 기본 동작: 최초 생성 AZ만 활성화",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Bring Your Own IP (BYOIP) — 고객 소유 IP 풀\n▸ Elastic IP (from BYOIP) — BYOIP 풀 기반 공인 IP\n▸ User Data Script — 인스턴스 시작 시 실행 스크립트\n▸ Secondary Network Interface — 추가 ENI\n\n【정답 포인트】\n▸ 시작 템플릿에는 기본 ENI만 정의 → 2차 ENI는 부팅 후 추가\n▸ user data script로 cloud-init 실행 → ENI 생성 + Elastic IP 할당\n▸ BYOIP 풀에서 할당받은 Elastic IP 사용\n▸ Auto Scaling 그룹에서 서브넷 선택 (프라이머리용)\n\nOption D: 시작 템플릿은 프라이머리 ENI만 (또는 프라이빗 서브넷). 부팅 후 user data가 2차 ENI 생성 + BYOIP Elastic IP 할당. Auto Scaling이 자동으로 각 인스턴스마다 반복.\n\n【오답 체크】\n(A) 시작 템플릿에서 2개 ENI 정의는 가능하지만, BYOIP pool ID를 직접 선택할 수 없음 (subnet level 설정 부족).\n(B) cloud-init 실행은 맞지만, \"자동 할당\" 옵션은 BYOIP 풀 미지원 → Elastic IP 명시 필요.\n(C) Lambda + Global Accelerator는 과도한 복잡성 + 목표 미달성.\n\n【시험 포인트】\n▸ Auto Scaling + 다중 ENI: user data 기반 자동화 필수\n▸ BYOIP: Elastic IP로 할당받아야 함\n▸ 시작 템플릿 vs Auto Scaling: 각 역할 구분",
  ...
  },
  {
    ...
    "answer": "BCD",
    "explanation": "【핵심 용어】\n▸ Route 53 Private Hosted Zone (PHZ) — VPC 내부 DNS\n▸ Split-View DNS — 같은 도메인, 내부/외부 다른 답변\n▸ DNS Hostname Resolution — VPC 내 호스트네임 활성화\n▸ Route 53 Record Sync — 퍼블릭과 프라이빗 영역 동기화\n\n【정답 포인트】\n▸ VPC 내부의 인스턴스가 같은 호스트명으로 내부 IP 접근 필요\n▸ PHZ 생성 + VPC 연결 → VPC 내 DNS 해석\n▸ DNS Hostname 활성화 → EC2 내부 호스트명 지원\n▸ 퍼블릭 호스팅 영역의 각 레코드를 프라이빗 IP로 복제\n▸ 향후 변경 자동화는 선택사항 (수동 유지보수 가능)\n\nOption B: PHZ 생성 + VPC 연결 → VPC 내부 DNS 가능.\nOption C: DNS Hostname 활성화 → EC2 내부에서 프라이빗 호스트명 사용 가능.\nOption D: PHZ에 공개 호스팅 영역의 레코드를 프라이빗 IP로 복제 → 내부와 외부가 같은 이름으로 다른 IP 응답.\n\n【오답 체크】\n(A) 지오프록시미티는 지역 기반 라우팅 → 목표 미달성.\n(E) EventBridge + Lambda는 자동화이지만, 요구사항에는 \"현재\"만 명시되고 \"자동화 필수\"는 아님 → 선택사항.\n(F) 퍼블릭 호스팅 영역에 프라이빗 IP 추가는 부적절 → 퍼블릭 DNS로 프라이빗 IP 응답 불가.\n\n【시험 포인트】\n▸ Split-View DNS: PHZ + 퍼블릭 호스팅 영역 조합\n▸ VPC DNS 설정: 호스트명 활성화 필수\n▸ 수동 레코드 복제: 변경 주기가 낮으면 충분",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ NLB (Network Load Balancer) — SSL/TLS 연결 지원\n▸ VPC Endpoint Service — Cross-Account 프라이빗 연결\n▸ ECS Fargate — 컨테이너 오케스트레이션\n▸ Private Link — VPC 엔드포인트 기반 접근\n\n【정답 포인트】\n▸ SSL 연결 필요 → NLB (Layer 4)\n▸ Cross-Account 프라이빗 연결 → VPC Endpoint Service (VPL 기반)\n▸ 다중 소비자 관리 가능 → Endpoint Service로 접근 제어\n▸ 스케일링 → ECS Auto Scaling + NLB 자동 매핑\n\nOption D: NLB는 TCP/TLS 네이티브 지원. ECS 서비스 정의에 NLB 지정 → 작업이 대상 그룹에 자동 등록. NLB를 VPC Endpoint Service로 노출 → 다른 계정이 프라이빗하게 접근 가능. Auto Scaling으로 관리 가능 확장.\n\n【오답 체크】\n(A) GLB는 어플라이언스 통합용 → ECS 작업과 불일치.\n(B) ALB는 Layer 7 (HTTP/HTTPS) → SSL 스트림 대신 HTTP 요구 + VPC Peering이 더 간단.\n(C) ALB + VPC Peering → Layer 7 미흡 + Peering은 프라이빗 연결 미흡 (양쪽 VPC 간접 노출).\n\n【시험 포인트】\n▸ SSL/TLS 스트림 처리: NLB 필수\n▸ Cross-Account Access: VPC Endpoint Service + Private Link\n▸ ECS Auto Scaling: NLB + Target Group 자동 연동",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ VPC Endpoint Service — NLB 기반 서비스 노출\n▸ Interface VPC Endpoint — 프라이빗 연결\n▸ PrivateLink — AWS 네이티브 프라이빗 연결\n▸ Minimum Disruption — 기존 환경 변경 최소\n\n【정답 포인트】\n▸ 5일 급속 통합 → 인프라 변경 최소화\n▸ VPC Peering/TGW는 양쪽 라우팅 재구성 필요 → 시간 소요\n▸ Endpoint Service는 NLB만 노출 → 기존 프로덕션 환경 변경 불필요\n▸ Interface Endpoint 생성만 → 빠른 구현\n▸ 테스트 후 통합 가능 → 선택적 연결\n\nOption C: 웹 서비스 VPC에서 VPC Endpoint Service 생성 + NLB 연결 → 프로덕션 VPC에서 Interface Endpoint 생성. 프로덕션 환경은 로컬 라우팅만 변경 (endpoint 추가) → 최소 중단. Private Link 기반 안전한 연결.\n\n【오답 체크】\n(A) VPC Peering은 양쪽 경로 추가 + 보안 그룹 변경 → 기존 환경 영향.\n(B) 재배포는 5일 내 불가능 + 프로덕션 환경 변경 많음.\n(D) Transit Gateway는 중앙 집중식 → 설정 복잡 + 기존 환경 라우팅 대폭 재구성.\n\n【시험 포인트】\n▸ 급속 통합: VPC Endpoint Service 최적화 선택\n▸ 기존 환경 보호: 한쪽 VPC만 변경\n▸ PrivateLink: 프라이빗 + 보안 + 관리 용이",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ IPv6 Egress-Only Gateway — 아웃바운드 IPv6만 허용\n▸ Direct Connect Transit VIF (새) — IPv6 BGP 피어링\n▸ VPN IPv6 (새) — Site-to-Site VPN IPv6 지원\n▸ No Infrastructure Change — 기존 IPv4 경로/VPN 유지\n\n【정답 포인트】\n▸ \"현재 인프라 변경 방지\" → Update 아닌 Create (새 VIF/VPN)\n▸ Egress-Only IGW: IPv6 아웃바운드만 허용, 인바운드 차단\n▸ NAT Gateway는 IPv6 미지원 → Egress-Only 필수\n▸ 새 Transit VIF + 새 VPN → 기존 IPv4 경로와 병렬 운영\n\nOption C: 새로운 Direct Connect Transit VIF 생성 (IPv6 BGP) → 기존 VIF 유지. 새로운 VPN 연결 생성 (IPv6) → 기존 VPN 유지. Egress-Only IGW 추가 → IPv6 아웃바운드만 허용. 라우트 테이블 추가 (IPv6) → 기존 IPv4 경로 미변경.\n\n【오답 체크】\n(A) \"업데이트\" 기존 Transit VIF → 현재 IPv4 경로 영향 위험.\n(B) \"업데이트\" 기존 VPN → 서비스 중단 위험.\n(D) NAT Gateway는 IPv6 미지원 → IPv6 아웃바운드 불가능.\n\n【시험 포인트】\n▸ IPv6 하이브리드: 새 VIF/VPN으로 평행 운영\n▸ Egress-Only IGW: IPv6 인바운드 차단, 아웃바운드만\n▸ Zero Downtime Migration: 기존 경로 유지 필수",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Forward Secrecy (FS) — 세션별 고유 키 생성\n▸ Perfect Forward Secrecy (PFS) — 과거 세션 키 노출 방지\n▸ TLS Security Policy — ALB의 TLS 암호 설정\n▸ Session Key Uniqueness — 각 연결마다 다른 키\n\n【정답 포인트】\n▸ \"고유한 임의 세션 키\" → Forward Secrecy의 정의\n▸ TLS 1.2 only는 오래된 버전 제한 → FS 보장 안 함\n▸ KMS 세션 키 암호화는 이미 encrypted 데이터 추가 암호화 → 불필요\n▸ WAF는 Layer 7 규칙 적용 → TLS Layer 아님\n▸ Security Policy 변경: FS 지원 암호 스위트 활성화\n\nOption D: ALB Security Policy를 \"Forward Secrecy\" 지원 암호 스위트 포함 정책으로 변경. 예: ELBSecurityPolicy-TLS-1-2-2017-01 → ECDHE/DHE 기반 TLS 암호화. 각 세션마다 고유 키 자동 생성.\n\n【오답 체크】\n(A) TLS 1.2 only는 FS와 무관 (1.3도 FS 필수지만, 1.2 non-FS도 가능).\n(B) KMS는 서버 private key 암호화용 → 세션 키 생성 메커니즘 아님.\n(C) WAF는 ALB의 암호화 설정 제어 불가 → Layer 7 규칙만.\n\n【시험 포인트】\n▸ Perfect Forward Secrecy: TLS 정책 수준 설정\n▸ ECDHE/DHE: FS 지원 키 교환 알고리즘\n▸ ALB Security Policy: AWS 관리 암호 스위트 선택",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Transit Gateway Connect — SD-WAN 통합\n▸ AS_PATH Prepend — BGP 경로 길이 조정으로 우선순위 변경\n▸ BGP Route Selection — 가장 짧은 AS_PATH 선호\n▸ Active-Passive Routing — 단일 경로만 활성\n\n【정답 포인트】\n▸ \"한 번에 하나만\" → 액티브-패시브 구성\n▸ 2개 SD-WAN 어플라이언스: 기본 (Primary) + 보조 (Backup)\n▸ BGP AS_PATH Prepend: 보조 경로 길이 증가 → 우선순위 낮춤\n▸ BGP는 자동으로 더 짧은 AS_PATH 선택 → 기본만 활성\n▸ 기본 장애 시 자동 페일오버\n\nOption C: 보조 SD-WAN에서 Transit Gateway로 가는 BGP 경로에 AS_PATH Prepend 적용. 예: 원래 AS_PATH \"65001\" → Prepend 후 \"65001 65001 65001\" (또는 자신 AS 반복). TGW가 경로 선택 시 더 짧은 경로(기본) 선호 → 기본만 트래픽 수신.\n\n【오답 체크】\n(A) 정적 경로 기반 → BGP 동적 우선순위 미흡 + 장애 자동 감지 불가.\n(B) BGP Community 7224:7300은 기능 정의 불명확 (일반 라우팅 정책).\n(D) ECMP 비활성화는 동등 경로 제거하지만, 여전히 단일 경로 선택 필요 → 추가 설정 필요.\n\n【시험 포인트】\n▸ SD-WAN Active-Passive: BGP 속성 활용\n▸ AS_PATH Prepend: 경로 길이로 우선순위 조정\n▸ Transit Gateway Connect: BGP 동적 라우팅 지원",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Transit Gateway Connect — GRE 터널 + BGP를 통한 5Gbps 이상 처리량 지원\n▸ VPC Attachment — 표준 연결로는 처리량 제약\n▸ GRE Encapsulation — 고속 데이터 전송 프로토콜\n\n【정답 포인트】\n▸ 5Gbps 요구사항 → VPN(A,C)은 처리량 제약 발생\n▸ Transit Gateway Connect 어태치먼트 필수 → GRE 기반 고속 연결\n▸ Connect 피어 생성 시 GRE 프로토콜 선택 → VXLAN 아님\n▸ BGP 동적 라우팅으로 HA 구성\n\n【오답 체크】\n(A) IPsec VPN은 처리량 제약, 5Gbps 미달\n(C) VPC 어태치먼트만으로는 허브-스포크 특성상 5Gbps 보장 불가\n(D) VXLAN은 Connect에서 지원하지 않음, GRE 사용\n\n【시험 포인트】\n▸ Transit Gateway Connect: GRE, 5Gbps+\n▸ Transit Gateway VIF: IPsec VPN, 처리량 제약\n▸ SD-WAN 허브는 고처리량 필요 → Connect 필수",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ IGMP — 동적 멀티캐스트 그룹 멤버십 관리 프로토콜\n▸ Static Source Multicast — 사전 정의된 발신자만 허용\n▸ UDP — 멀티캐스트 트래픽 프로토콜(TCP 아님)\n\n【정답 포인트】\n▸ 동적 등록 요구 → IGMP 필수 (정적 아님)\n▸ 멀티캐스트는 UDP 기반 → TCP 불가\n▸ IGMP: 호스트가 동적으로 그룹 조인/탈퇴\n▸ Transit Gateway의 IGMP 도메인 기능\n\n【오답 체크】\n(A) 정적 소스 = 동적 등록 불가\n(B) TCP는 멀티캐스트 미지원\n(D) TCP 사용 시도\n\n【시험 포인트】\n▸ 동적 멀티캐스트 → IGMP 도메인\n▸ 멀티캐스트 프로토콜 = UDP\n▸ Transit Gateway 멀티캐스트 도메인 유형 구분",
  ...
  },
  {
    ...
    "answer": "BE",
    "explanation": "【핵심 용어】\n▸ X-Forwarded-For — ALB가 추가하는 클라이언트 IP 헤더\n▸ Application Load Balancer — 경로 기반 라우팅, HTTPS 지원\n▸ NLB — L4, 클라이언트 IP 직접 전달 불가\n\n【정답 포인트】\n▸ 다양한 경로 + HTTPS → ALB 필수\n▸ 고객 소스 IP 필요 → X-Forwarded-For 헤더\n▸ ALB는 자동으로 X-Forwarded-For 추가\n▸ ECS와 경로 기반 라우팅 연동\n\n【오답 체크】\n(A) NLB는 경로 기반 라우팅 불가\n(C) App Mesh는 서비스 메시 관리용\n(D) X-IP-Source는 표준 헤더 아님\n\n【시험 포인트】\n▸ ALB vs NLB: 경로 기반 = ALB\n▸ 클라이언트 IP 추출 = X-Forwarded-For\n▸ 마이크로서비스 라우팅 = ALB 리스너 규칙",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ VPC Peering — 비용 최소(데이터 전송료 없음)\n▸ Transit Gateway — 다중 VPC 관리, 비용 추가\n▸ Zonal DNS — 같은 AZ 내 통신으로 비용 절감\n▸ 10TB+ 월간 데이터 전송\n\n【정답 포인트】\n▸ 10TB+ 월간 → 비용이 최우선\n▸ VPC Peering: 같은 리전, 같은 AZ 내 무료 데이터 전송\n▸ Transit Gateway: NLB 데이터 전송료 추가 비용\n▸ 10개 VPC, 1:1 피어링 관리 가능\n▸ Zonal DNS로 교차 AZ 트래픽 회피\n\n【오답 체크】\n(A) TGW 데이터 전송료 발생\n(B) PrivateLink 인터페이스 엔드포인트 비용\n(D) TGW 사용 시 비용 증가\n\n【시험 포인트】\n▸ 비용 최소화 = 같은 AZ 내 VPC Peering\n▸ 10+ VPC = Peering 관리 vs TGW 편의성 트레이드오프\n▸ Zonal DNS 활용으로 비용 극대화",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Appliance Mode — Transit Gateway 어태치먼트의 옵션\n▸ Stateful Inspection — 양방향 트래픽 필요\n▸ 교차 AZ 트래픽 드롭 — 대칭 경로 부재\n\n【정답 포인트】\n▸ 다중 AZ의 상태 저장 어플라이언스 배포\n▸ Appliance Mode 미활성화 → 비대칭 라우팅\n▸ 트래픽: A VPC → Firewall → B VPC\n▸ 응답: B VPC → Transit Gateway → A VPC (direct)\n▸ 상태 추적 실패 → 패킷 드롭\n\n【오답 체크】\n(A) 별도 서브넷은 정상, 어플라이언스 모드 관련 아님\n(C) 같은 서브넷도 가능, 핵심은 어플라이언스 모드\n(D) 애플리케이션 VPC는 어플라이언스 없음\n\n【시험 포인트】\n▸ Appliance Mode: 공유 서비스 VPC 어태치에 필수\n▸ 상태 저장 방화벽 = 대칭 경로 필요\n▸ 교차 AZ 트래픽 문제 = Appliance Mode 확인",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Appliance Mode — 방화벽 위치 VPC의 어태치먼트에서 활성화\n▸ 상태 저장 방화벽 — 대칭 경로 필수\n▸ 교차 AZ 드롭 — 비대칭 라우팅 문제\n\n【정답 포인트】\n▸ 공유 VPC의 방화벽 어플라이언스\n▸ A → Firewall → B 경로 필요\n▸ Appliance Mode on 공유 VPC 어태치먼트\n▸ 모든 트래픽이 방화벽을 통해 검사됨\n▸ 최소 변경으로 해결 (B 옵션보다 효율적)\n\n【오답 체크】\n(A) VPN은 처리량, 복잡성 증가\n(B) A, B VPC의 어태치는 이미 정상\n(D) Peering은 방화벽 검사 우회\n\n【시험 포인트】\n▸ 어플라이언스 위치 VPC의 어태치 = Appliance Mode\n▸ 공유 서비스 VPC 패턴 핵심\n▸ 최소 관리 오버헤드 = C 선택",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ VPC Reachability Analyzer — 네트워크 경로 분석 자동화\n▸ 공인 인터넷 → EC2 포트 443 검증\n▸ EventBridge + Lambda — 보안 그룹 변경 감지\n\n【정답 포인트】\n▸ 공인 인터넷에서의 접근성 확인 필수\n▸ Reachability Analyzer 소스: IGW (공인 대표)\n▸ 대상: EC2 인스턴스, 포트: 443\n▸ EventBridge로 보안 그룹 변경 자동 감지\n▸ Lambda에서 분석 실행 및 SNS 알림\n\n【오답 체크】\n(A) VPC Flow Logs는 사후 분석용, 사전 검증 불가\n(B) Flow Logs 수집만으로는 자동화 부족\n(C) 보안 그룹을 소스로 지정 불가, IGW 필요\n\n【시험 포인트】\n▸ 자동화 검증 = Reachability Analyzer\n▸ 공인 인터넷 소스 = IGW\n▸ 변경 감지 = EventBridge + 보안 그룹 rule 변경",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ pkt-srcaddr/pkt-dstaddr — Pod IP 주소 추출\n▸ srcaddr/dstaddr — Node IP 주소 (Pod IP 불가)\n▸ 애플리케이션 서브넷 — 두 EKS 클러스터 위치\n\n【정답 포인트】\n▸ Pod IP 주소 추적 필요 → pkt-srcaddr 사용\n▸ 노드 필터링 → 로그 수 감소 불가 (노드는 여러 Pod)\n▸ 서브넷 리소스 선택 → 두 EKS 클러스터만 추적\n▸ 사용자 지정 형식 → pkt-* 필드 필요\n\n【오답 체크】\n(A) srcaddr = Node IP, Pod IP 추출 불가\n(B) EKS 노드 필터 → 로그 감소 효과 부족\n(D) 노드 필터 + pkt-* 조합이 부족\n\n【시험 포인트】\n▸ VPC CNI: Pod = ENI secondary IP\n▸ Pod IP 추출 = pkt-srcaddr/pkt-dstaddr\n▸ 로그 수 최소화 = 서브넷 레벨 리소스 선택",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Route 53 Resolver — 서버리스 DNS 해석 서비스\n▸ Outbound Endpoints — VPC → 온프레미스 DNS 전달\n▸ 확장성 — 5~7배 성장 수용\n\n【정답 포인트】\n▸ EC2 기반 DNS는 크기 제약, 관리 부담\n▸ Route 53 Resolver는 자동 확장성\n▸ Outbound endpoints로 온프레미스 DNS 통합\n▸ VPC resolver로 AWS 서비스 해석\n▸ 비용 절감: EC2 DNS 서버 제거\n\n【오답 체크】\n(A) 스케일 아웃은 한계, 관리 부담 증가\n(B) 스케일 업은 한계점 도달\n(D) Inbound endpoints는 온프레미스 → VPC용\n\n【시험 포인트】\n▸ Outbound: VPC → 온프레미스\n▸ Inbound: 온프레미스 → VPC\n▸ Route 53 Resolver = 서버리스 확장성",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ BGP Community Tag — AS경로 우선순위 제어\n▸ 7224:7300 — 기본 (낮은 local preference)\n▸ 7224:7100 — 장애 조치 (높은 local preference)\n\n【정답 포인트】\n▸ AWS → 기본 DC: BGP local preference 높음\n▸ AWS → 장애 조치 DC: BGP local preference 낮음\n▸ 7224:7300 → 200 local preference (기본)\n▸ 7224:7100 → 100 local preference (백업)\n▸ BGP는 높은 local preference 우선\n\n【오답 체크】\n(A) 태그 값 반대 (우선순위 역전)\n(C) 잘못된 커뮤니티 값\n(D) 우선순위 반대\n\n【시험 포인트】\n▸ Direct Connect BGP community\n▸ 7224:7100 vs 7224:7300 구분\n▸ Local preference 제어로 경로 선택",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Gateway Endpoint — S3, DynamoDB (비용 무료)\n▸ Interface Endpoint — SQS (IP 기반)\n▸ VPC 엔드포인트 — NAT 불필요\n\n【정답 포인트】\n▸ 프라이빗 서브넷 EC2 (공용 IP 불필요)\n▸ S3/DynamoDB = Gateway endpoint (비용 절감)\n▸ SQS = Interface endpoint (라우팅 제어)\n▸ NAT 게이트웨이 비용 회피 (시간당 청구)\n▸ 데이터 전송 비용 절감\n\n【오답 체크】\n(A) 공용 IP 요구, 설정 불일치\n(B) NAT 게이트웨이 비용 추가\n(D) SQS는 Gateway endpoint 없음, 반대 매핑\n\n【시험 포인트】\n▸ S3/DynamoDB = Gateway endpoint\n▸ 기타 AWS 서비스 = Interface endpoint\n▸ VPC endpoint로 NAT 제거 = 비용 최소",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ SiteLink — Direct Connect 게이트웨이 간 온프레미스 통신\n▸ Transit VIF — 다중 VPC 연결용\n▸ Private VIF — 단일 VPC 연결용\n▸ Transit Gateway Peering — 리전 간 연결\n\n【정답 포인트】\n▸ 동일 Direct Connect 게이트웨이 사용 (SiteLink 활성화)\n▸ 두 리전 각각 Transit Gateway (다중 VPC 지원)\n▸ Transit VIF로 온프레미스 연결\n▸ Transit Gateway Peering으로 리전 간 연결\n▸ SiteLink: US DC ↔ EU DC 직접 통신\n\n【오답 체크】\n(A) Private VIF는 단일 VPC용, 유연성 부족\n(C) 별도 Direct Connect 게이트웨이 = SiteLink 불가\n(D) Private VIF + 별도 DX GW\n\n【시험 포인트】\n▸ SiteLink = 같은 DX 게이트웨이\n▸ 다중 VPC = Transit VIF 필수\n▸ 리전 간 = Transit Gateway Peering",
  ...
  },
  {
    ...
    "answer": "AD",
    "explanation": "【핵심 용어】\n▸ MACsec — 링크 레벨 암호화\n▸ must_encrypt — 의무화 모드\n\n【정답 포인트】\n▸ 온프레미스 라우터 설정(A) — 양쪽 끝점 필요\n▸ must_encrypt 모드(D) — 요구사항에서 \"항상\" 암호화\n\n【오답 체크】\n(B/C) should_encrypt는 선택적\n(E) should_encrypt 부분 암호화만\n\n【시험 포인트】\n▸ MACsec 양쪽 설정 필수\n▸ 암호화 모드 선택 = 요구사항 수준",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Private NAT Gateway — VPC 내부 전용, 인터넷 불필요\n▸ Public NAT Gateway — 인터넷 게이트웨이 필수 (VPC에 IGW 없음)\n▸ Transit Gateway → 온프레미스 액세스\n▸ 고가용성 → 다중 AZ 배치\n\n【정답 포인트】\n▸ VPC에 인터넷 게이트웨이 없음 → Public NAT 불가\n▸ Private NAT Gateway → Transit Gateway 경로 가능\n▸ 각 AZ에 NAT 게이트웨이 → 고가용성\n▸ 소스 IP 10.1.0.0/24 유지 → NAT (Private/Public 구분 안 함)\n▸ Transit Gateway에 연결되어 있으므로 프라이빗 NAT로 충분\n\n【오답 체크】\n(A) VPC에 인터넷 게이트웨이 없음 → Public NAT 불가능\n(C) 단일 서브넷 → 가용 영역 1개만 커버. 고가용성 미충족\n(D) Public NAT Gateway 필요 (VPC에 IGW 없음)\n\n【시험 포인트】\nPrivate NAT Gateway = Transit Gateway와만 통신 (인터넷 불필요)\nPublic NAT Gateway = 인터넷 게이트웨이 필수\nVPC에 IGW 없음 = Private NAT 필수\n고가용성 = 다중 AZ 각각 NAT 배치",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ VPC IPAM — IP 주소 관리 및 모니터링\n▸ 자동 가져오기 — VPC/서브넷 IP 자동 추적\n▸ CloudWatch 경보 — 임계값 기반 알림\n▸ 운영 오버헤드 최소화 — 최소 구성\n\n【정답 포인트】\n▸ IPAM 설정 → VPC 계층별 풀 자동 생성\n▸ 자동 가져오기 활성화 → VPC/서브넷 IP 자동 추적\n▸ CloudWatch 경보 → 가용성 제한(availability limit) 기반 알림\n▸ SNS 통지 → 임계값 도달 시 즉시 알림\n▸ 수동 Lambda 구성 불필요 (자동화)\n\n【오답 체크】\n(B) CloudWatch Logs + Lambda 수동 구성 → 높은 오버헤드\n(C) 커스텀 메트릭 + Lambda 스케줄링 → 5분 간격 실행, 비효율적\n(D) EventBridge 규칙 필요 (CloudWatch 경보만으로 충분)\n\n【시험 포인트】\nVPC IPAM = IP 주소 모니터링 전문 서비스\n자동 가져오기 = VPC/Subnet IP 자동 추적\nCloudWatch 경보 = IPAM 가용성 임계값 기반\nLambda/커스텀 메트릭 = 불필요한 복잡도",
  ...
  },
  {
    ...
    "answer": "AD",
    "explanation": "【핵심 용어】\n▸ Dual-stack — IPv4 + IPv6 동시 지원\n▸ Direct Connect VIF — 물리 연결 재사용\n▸ Site-to-Site VPN — 백업 연결 (IPv6 추가)\n▸ 최소 시간 전환 → 기존 인프라 활용\n\n【정답 포인트】\n▸ (A) 기존 VPN 연결에 IPv6 터널 추가 (빠른 설정)\n▸ (D) Direct Connect VIF에 IPv6 피어 추가 (BGP 세션 확장)\n▸ 기존 물리 연결 재사용 → 배포 시간 단축\n▸ VPN + Direct Connect 모두 업그레이드 → 완전한 이중 스택\n\n【오답 체크】\n(B) 새 VPN 연결 생성 → 시간 소요. 기존 연결 재사용 아님.\n(C) 새 VIF 생성 → 추가 시간. 기존 VIF 활용 최적.\n(E) IPv6 터널링은 구식. 네이티브 IPv6 우선 (D 방식)\n\n【시험 포인트】\n기존 연결 활용 = 최단 시간\nDirect Connect IPv6 = 피어 추가 (새 VIF 불필요)\nVPN IPv6 = 새 터널 (기존 연결 확장)\nDual-stack = VPN + DX 모두 IPv6 지원",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Network Firewall 로깅 — Alert Log + Flow Log\n▸ 완벽한 감시 — 모든 트래픽 기록\n▸ 감사 및 경고 — Alert Log 필수\n\n【정답 포인트】\n▸ Network Firewall 로깅 설정 → Alert Log + Flow Log 활성화\n▸ Alert Log = 정책 매칭 이벤트 (감지됨 트래픽)\n▸ Flow Log = 모든 네트워크 흐름 기록\n▸ CloudWatch, VPC Flow Logs, CloudTrail은 불완전\n\n【오답 체크】\n(A) CloudWatch만으로는 흐름 로그 캡처 불가. Alert만 기록.\n(C) VPC Flow Logs는 방화벽 동작 기록 안 함. 엔드포인트 트래픽만.\n(D) CloudTrail은 API 호출 기록. 방화벽 트래픽 검사 로깅 안 함.\n\n【시험 포인트】\nNetwork Firewall = 자체 로깅 시스템\nAlert + Flow 로그 = 완전한 감시\nCloudWatch/CloudTrail = Network Firewall 로깅 기능 아님\n모든 트래픽 기록 = Network Firewall 설정에서 구성",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ NAT 게이트웨이 중복화 — 각 AZ에 배치\n▸ AZ 격리 — 가용 영역별 라우팅\n▸ 단일 장애점 제거 — 다중 NAT + 독립 경로\n\n【정답 포인트】\n▸ 각 AZ에 NAT 게이트웨이 배치 (AZ1 제외, 기존 위치)\n▸ AZ별 독립 라우팅 테이블 → 각 AZ 프라이빗 서브넷이 자신의 AZ NAT 사용\n▸ AZ2 장애 시 → AZ2 서브넷만 영향 (AZ3 독립)\n▸ AZ 간 NAT 의존성 없음 → 격리된 장애\n\n【오답 체크】\n(A) 단일 라우팅 테이블로 VIP 수동 관리 → 복잡, 자동 장애 조치 어려움\n(B) 단일 라우팅 테이블 사용 (동일 테이블, 두 AZ) → 정책 구분 불가능\n(C) 다중 VPC → 비용 증가, 복잡성 높음. 불필요.\n\n【시험 포인트】\n고가용성 NAT = 각 AZ에 배치 필수\n라우팅 격리 = AZ별 독립 라우팅 테이블\nAZ 장애 = 해당 AZ만 영향 (다른 AZ 독립)\nVIP 수동 관리 < AZ별 라우팅 (자동화)",
  ...
  },
  {
    ...
    "answer": "ABE",
    "explanation": "【핵심 용어】\n▸ Virtual Private Gateway — 단일 VPC 액세스용\n▸ Direct Connect Gateway — 다중 VPC/지역 액세스\n▸ Transit VIF vs Private VIF — Transit Gateway 지원 여부\n▸ 비용 최적화 — VPC별 Direct Connect 연결 불필요\n\n【정답 포인트】\n▸ (A) 4개 VPC에 각각 VPG 생성 (단일 VPC 연결용)\n▸ (B) Direct Connect 게이트웨이 중앙화 (다중 VPC 지원)\n▸ (E) 각 Direct Connect에 4개 프라이빗 VIF (VPG 연결)\n▸ VPG는 Transit Gateway와 연결 안 됨 (Direct Connect Gateway 거쳐야 함)\n▸ 총 4개의 Direct Connect로 4개 VPC 모두 지원\n\n【오답 체크】\n(C) Transit VIF → Transit Gateway 연결용. VPG는 Transit VIF 미지원.\n(D) Transit VIF를 VPG와 직접 연결 불가능. DX Gateway 거쳐야 함.\n(F) Transit Gateway 연결 불필요 (4개 VPC만 선택적 액세스)\n\n【시험 포인트】\n다중 지역 4개 VPC 선택 액세스 → VPG + DX Gateway 조합\nPrivate VIF = VPG 연결 (Direct Connect 당 4개)\nTransit VIF = Transit Gateway 연결 (여기서는 불필요)\nDirect Connect Gateway = 중앙 허브 역할\nTransit Gateway = 30개 VPC 내부 통신용 (DX 액세스 아님)",
  ...
  },
  {
    ...
    "answer": "BC",
    "explanation": "PrivateLink 엔드포인트 배포 후 통신이 불가능한 경우 두 가지 주요 문제가 발생할 수 있습니다. 첫째, DNS 해석 문제로 enableDnsSupport와 엔드포인트별 DNS 지원이 필수입니다(B). VPC 엔드포인트는 AWS 서비스의 DNS 이름을 프라이빗 IP로 확인해야 하므로, 이 설정이 없으면 통신이 실패합니다. 둘째, 엔드포인트 정책(C)이 실제 통신을 허용해야 합니다. PrivateLink 엔드포인트는 기본적으로 deny-all 정책을 적용하므로 명시적으로 허용해야 합니다. (A)는 불필요합니다 — 엔드포인트는 라우팅이 아닌 DNS 해석으로 작동합니다. (D)와 (E)는 공개/프라이빗 호스팅 영역이 아닌 VPC 엔드포인트의 DNS 지원 설정이 필요합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "하이브리드 멀티 리전 네트워크에서 최적의 경로 선택을 위해서는 로컬 및 원격 경로를 명확하게 구분해야 합니다. (B)가 정답인 이유는 로컬 리전의 VPC CIDR 접두사를 구체적으로 광고하면서 동시에 집계 경로를 유지하기 때문입니다. 이렇게 하면 가장 짧은 경로 우선(shortpath first) 원칙이 작동합니다: 온프레미스 데이터 센터는 로컬 리전의 VPC들에 대해 더 구체적인 경로를, 원격 리전의 VPC들에 대해 집계 경로를 받습니다. BGP 선택 로직(longest prefix match)에 따라 로컬 Direct Connect가 선호됩니다. (A)는 트래픽이 항상 집계 경로만 사용하므로 로컬 최적화를 놓칩니다. (C)는 모든 15개 접두사를 광고하면 경로 경합(path explosion)이 발생합니다. (D)의 BGP 커뮤니티는 경로 선택을 조정하지만 구체적인 접두사 전략보다 덜 효율적입니다.",
  ...
  },
  {
    ...
    "answer": "BCE",
    "explanation": "AWS Firewall Manager는 조직 단위의 중앙 집중식 firewall 정책 관리 솔루션입니다. 정답 조합 (B)(C)(E)를 분석하면: (B) 모든 계정이 조직에 가입해야 Firewall Manager가 모든 계정의 firewall을 관리할 수 있습니다. (C) 한 계정을 Firewall Manager 관리자로 지정하여 다른 계정의 firewall 정책을 중앙에서 관리합니다. (E) AWS Config는 Firewall Manager의 규정 준수 모니터링에 필수적이며, 리소스가 있는 모든 리전에서 활성화되어야 합니다. (A)는 잘못되었습니다 — Firewall Manager가 관리할 계정도 조직에 가입해야 합니다. (D) Firewall Manager는 계정을 \"자식 계정\"으로 부르지 않습니다. (F) 관리 계정만으로는 부족합니다 — 모든 계정에 AWS Config가 필요합니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "문제는 \"네트워크 계층\"에서 구현하라고 명시했으므로 보안 그룹(Layer 4) 또는 네트워크 ACL(Layer 3-4)이어야 합니다. (A) 보안 그룹이 정답인 이유는 AWS 관리형 CloudFront 접두사 목록을 사용하여 CloudFront IP 범위를 동적으로 추적하기 때문입니다. CloudFront IP가 변경되면 AWS가 접두사 목록을 자동으로 업데이트하므로 \"가장 운영 효율적\"입니다. 관리 오버헤드가 없습니다. (B) 네트워크 ACL도 기술적으로 작동하지만, 보안 그룹이 더 간단하고 권장됩니다. (C) HTTP 헤더는 애플리케이션 계층이므로 요구사항에 맞지 않습니다. (D) AWS WAF는 애플리케이션 계층(Layer 7)이고, Lambda 함수로 IP 세트를 수동 관리해야 하므로 운영 오버헤드가 증가합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "이 문제는 사전 예방(preventive) 제어가 필요합니다. SCP(Service Control Policy)는 조직 수준에서 API 작업 자체를 차단하는 가장 강력한 메커니즘입니다. (B)가 정답인 이유는 CreateVpc와 AssociateVpcCidrBlock 작업 시 Ipv4IpamPoolId 컨텍스트 키를 조건으로 검사하기 때문입니다. 사용자가 IPAM 풀에서 할당된 CIDR만 사용하도록 강제합니다. 이는 정책 수준에서 구현되므로 우회 불가능합니다. (A)(C) Config 규칙과 Lambda는 이미 생성된 VPC를 감지하고 삭제하는 반응형 제어로, 부정확성이 높고 운영 비용이 많이 듭니다. (D) EventBridge 기반 자동 삭제도 사후 제어로 데이터 손실 위험이 있습니다. SCP는 사전에 생성을 방지하므로 가장 효율적입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "문제의 핵심은 \"암호화 + 프라이빗 IP + 최소 오버헤드\"입니다. (B)가 정답인 이유는 transit gateway와 transit VIF의 조합이 가장 효율적이기 때문입니다. Direct Connect gateway는 여러 VPC를 중앙에서 관리할 수 있으므로 향후 확장성이 뛰어나고, Site-to-Site VPN은 IPsec 암호화를 제공합니다. (A) 단일 VPC만 지원하므로 비확장적입니다. (C) 공개 VIF는 인터넷 기반이므로 \"프라이빗 IP\" 요구사항을 충족하지 않습니다. (D)는 불필요한 서드파티 방화벽이 추가되어 오버헤드가 증가합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "Tokyo 마이그레이션은 5일 내 신속 구성이 필요하고, \"인터넷 비노출\"과 \"기존 Paris 네트워크 액세스\" 조건이 있습니다. (C)가 정답인 이유: (1) Tokyo 리전에 별도의 transit gateway 구성으로 리전 독립성 확보, (2) transit gateway 피어링으로 Paris-Tokyo 간 프라이빗 통신 기반 마련, (3) Site-to-Site VPN이 5일 내 빠르게 배포 가능한 옵션 (Direct Connect는 수주 소요), (4) VPN의 IPsec 암호화로 보안 확보. (B) Direct Connect는 설치 기간이 5일을 초과할 수 있습니다. (A) 공개 서브넷과 Internet Gateway는 \"인터넷 비노출\" 요구사항에 위배됩니다. (D) 단일 Paris transit gateway로 Tokyo VPC를 관리하면 확장성이 떨어지고 피어링 최적화가 불가능합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "다중 계정 통합 시나리오에서 핵심은 Company A의 기존 Direct Connect 연결과 gateway를 활용하는 것입니다. (B)가 정답인 이유: Direct Connect gateway 연결 제안(association proposal) 메커니즘은 AWS의 크로스 계정 연결 기능입니다. Company B 계정이 제안을 생성하고 Company A가 승인하므로 회사 간 관리 책임을 명확히 합니다. 기존 Direct Connect 연결과 Company A의 Direct Connect gateway를 재사용하므로 중복 인프라가 없고, 추가 비용도 없습니다. (A) 새로운 Direct Connect gateway와 transit VIF를 생성하면 중복 리소스가 생기고 설정이 복잡합니다. (C) 여러 virtual private gateway와 VIF는 Company A의 Direct Connect gateway와 호환성이 떨어집니다. (D) 역시 새로운 gateway를 중복 생성합니다.",
  ...
  },
  {
    ...
    "answer": "BD",
    "explanation": "대규모 데이터(수백 MB) 전송과 고객 계정별 선택적 액세스가 핵심입니다. (B)(D) 조합이 정답인 이유: (B) PrivateLink 엔드포인트 서비스는 서비스 프로바이더가 고객의 엔드포인트 연결 요청을 수동으로 승인/거부할 수 있는 기능을 제공합니다. 이는 고객 선택이 매우 유연합니다. (D) NLB는 높은 처리량(수백 MB)과 낮은 지연 시간을 지원하므로 ALB보다 적합합니다. PrivateLink는 NLB를 대상으로만 지원합니다. ALB 단독 (E)는 PrivateLink와 호환되지 않습니다. (A) VPC 피어링은 1:1 관계로 고객마다 설정해야 하므로 운영 오버헤드가 높습니다. (C) HTTP 인증은 애플리케이션 계층이고 대규모 데이터 전송 시 오버헤드가 있습니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "Auto Scaling 환경에서 인스턴스 개수가 변해도 공급업체 ACL 업데이트가 최소화되어야 합니다. (A) 프라이빗 NAT gateway가 정답인 이유: NAT gateway는 고정 탄력적 IP를 가진 고정 엔드포인트이므로, 인스턴스가 증가해도 NAT gateway IP는 변하지 않습니다. 따라서 공급업체 허용 목록을 한 번만 설정하면 됩니다. 프라이빗 NAT는 VPC 내부에서 NAT 기능을 제공합니다. (B) elastic network interface는 고정 IP이지만 Auto Scaling 그룹과 의미적 연결이 약하고, 인스턴스가 고정되지 않으므로 실제로는 작동하지 않습니다. (C) 고정 EC2 인스턴스로 proxy 역할을 하는 방식은 이 인스턴스 자체가 SPOF(Single Point of Failure)가 되고, 운영 복잡도가 높습니다. (D) ALB 경로를 통한 로드 밸런싱은 외부 호스팅 데이터 서비스와 호환되지 않습니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "멀티 VPC 환경에서 대규모 파일 전송 시 가장 효율적인 아키텍처는 transit gateway를 중심으로 합니다. (D)가 정답인 이유: (1) transit gateway는 모든 VPC를 중앙에서 관리하므로 메시 네트워크(VPC 피어링)가 필요 없어 운영 오버헤드가 감소합니다. (2) transit VIF는 Direct Connect gateway와 통합되어 온프레미스 데이터 센터 연결이 간단합니다. (3) BGP 동적 라우팅은 경로 교환이 자동이므로 정적 라우팅 관리 비용이 없습니다. (4) MTU 8500은 Jumbo Frame을 지원하면서 Direct Connect 표준 오버헤드를 고려한 안전한 값입니다. 9001은 일부 환경에서 호환성 문제가 있습니다. (A)(B)는 여전히 VPC 피어링을 사용하므로 VPC 개수가 증가하면 N*(N-1)/2 개의 피어링이 필요합니다. (C)는 MTU 9001로 설정하여 호환성 이슈 위험이 있습니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "BGP(Border Gateway Protocol) 경로 선택 로직에 따른 최장 접두사 일치(longest prefix match) 원칙입니다. 172.16.1.0/24는 두 VIF 모두에서 광고되지만, VIF 1은 /16 (172.16.0.0/16), VIF 2는 /24 (172.16.1.0/24)를 광고합니다. BGP는 더 구체적인 경로(/24)를 선호하므로 VIF 2가 선택됩니다. AS_PATH 길이의 차이(1 vs 3)는 최장 접두사 일치 이후에 고려되는 이차 선택 기준입니다. 따라서 (B)가 정답입니다. (A) 덜 구체적인 경로는 사용되지 않습니다. (C)(D) 로드 밸런싱은 BGP에서 기본 동작이 아닙니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "멀티 계층 보안 아키텍처에서 중앙 관리와 탈중앙화 관리의 균형을 맞춰야 합니다. (D)가 정답인 이유: (1) CloudFormation으로 초기 배포 (IaC — Infrastructure as Code) — 인프라 버전 관리와 일관성 확보. (2) AWS Firewall Manager는 Network Firewall과 WAF를 조직 전체에서 중앙 관리하면서도, 애플리케이션 팀은 자신의 보안 그룹을 관리할 수 있는 정책 기반 접근. (3) GuardDuty는 지나치게 허용적인 규칙(0.0.0.0/0)을 모니터링하여 감시 기능 추가. (A) CloudFormation 수동 업데이트는 변경 추적이 어렵고, 대규모 조직에 비확장성입니다. (B) Console/CLI 수동 관리는 운영 오버헤드가 매우 높고, GuardDuty 기반 자동 삭제는 위험합니다. (C) Firewall Manager만으로는 초기 배포 프로세스가 부재하고, Network Firewall 배포 위치 설정이 불명확합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "Availability Zone 간 트래픽을 최소화하면서 failover를 지원하는 DNS 기반 솔루션이 필요합니다. (C) 프라이빗 호스팅 영역의 장애 조치 라우팅(failover routing)이 정답인 이유: Route 53 장애 조치 라우팅은 기본 레코드의 상태 검사 실패 시 자동으로 보조 레코드로 전환합니다. 각 Availability Zone 별로 장애 조치 레코드를 설정하면, 로컬 Availability Zone NLB가 건강할 때는 그곳으로 트래픽이 가고, 실패할 때만 지역 NLB로 failover됩니다. (A) 가중치 라우팅은 가중치 비율로 트래픽을 분배하므로, 로컬 대상이 있어도 항상 일부 트래픽이 원격으로 갑니다. (B) 크로스 존 로드 밸런싱 비활성화는 로컬 대상이 없을 때 대체 경로를 제공하지 않습니다. (D) 스티키 세션은 이미 연결된 세션만 유지하고, 새로운 연결은 제어하지 못하므로 원격 대상으로 갈 수 있습니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "botnet \"명령 제어\" 트래픽은 DNS 쿼리를 통해 C&C 서버와 통신하는 특성이 있습니다. (B) Amazon Route 53 Resolver DNS Firewall이 정답인 이유: AWS Managed Domains - Botnet Command and Control 목록은 이미 알려진 botnet C&C 도메인의 목록이며, DNS 쿼리 시점에 이 도메인으로의 요청을 차단합니다. 이는 감염된 인스턴스가 명령을 받기 전에 통신 자체를 차단하는 사전 예방 방식입니다. (A) Shield Advanced는 DDoS 공격 방어로, botnet C&C 통신과 무관합니다. (C) AWS WAF Bot Control은 HTTP/HTTPS 애플리케이션 계층 봇 탐지로, DNS 쿼리 차단과 무관합니다. (D) Systems Manager는 인스턴스 에이전트 수준의 제어로, 이미 감염된 악성 소프트웨어를 제거하지 못합니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "Direct Connect SiteLink는 AWS Direct Connect 위치 간에 직접 데이터 센터 간 연결을 제공하는 기능입니다. (D)가 정답인 이유: 이미 구축된 아키텍처에서 두 데이터 센터 간의 연결 안정성을 개선하려면 프라이빗 WAN에 의존하지 않고 Direct Connect 인프라 자체를 활용해야 합니다. transit VIF에서 SiteLink를 활성화하면, 두 Direct Connect 위치(us-east-1과 us-east-2) 간에 AWS 인프라 내 프라이빗 경로가 생성되어 프라이빗 WAN의 불안정성을 우회할 수 있습니다. (A) 새로운 Direct Connect gateway 생성은 불필요합니다 — 기존 gateway로 충분합니다. (B) 공개 VIF는 공인 IP와 BGP를 사용하므로, 데이터 센터 간 프라이빗 연결 요구에 맞지 않습니다. (C) SiteLink는 VIF 수준에서 활성화되며, 직접 연결(connection) 수준이 아닙니다.",
  ...
  },
  {
    ...
    "answer": "ACD",
    "explanation": "Control Tower 기반 멀티 계정 자동화를 위해서는 IaC, 이벤트 기반 트리거, 크로스 계정 접근이 필요합니다. 정답 조합 (A)(C)(D)를 분석하면: (A) Lambda 함수가 멤버 계정의 역할을 가정하여 VPC와 subnet을 프로비저닝합니다. (C) CloudFormation 템플릿이 VPC, transit gateway attachment 등의 인프라 정의를 제공하므로, Lambda가 템플릿을 사용하여 배포합니다. (D) EventBridge 규칙이 Control Tower의 CreateManagedAccount 이벤트를 감지하여 Lambda를 자동 호출하므로, 새로운 계정이 추가될 때 자동으로 네트워크 인프라가 프로비저닝됩니다. 조합의 시너지: 새 계정 생성 (EventBridge) → Lambda 함수 실행 (A) → CloudFormation 템플릿 배포 (C). (B) AFC(Account Factory Customization)는 Control Tower의 계정 프로비저닝 커스터마이징이지만, 기존 계정에는 적용되지 않으므로 부족합니다. (E) AWSControlTowerBlueprintAccess는 Control Tower 블루프린트 기능과 무관합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "문제의 핵심: \"지리적 위치 기반 지역화\", \"GET/POST 메서드\", \"1분 이내 failover\". (B) AWS Global Accelerator + ALB 조합이 정답인 이유: (1) Global Accelerator는 Anycast IP를 사용하여 사용자의 지리적 위치에 가장 가까운 엔드포인트 그룹으로 라우팅합니다 — \"지리적 위치\" 요구사항 충족. (2) ALB는 HTTP/HTTPS를 지원하고 GET/POST 모두 처리합니다. (3) Global Accelerator는 엔드포인트 상태 검사를 통해 failover를 지원하며, 1분 이내 빠른 failover가 가능합니다. (A) NLB는 Layer 4(TCP/UDP)로 작동하므로, HTTP 메서드를 구분할 수 없고, \"지리적 위치\" 인식이 제한적입니다. (C) Route 53 DNS 기반 failover는 DNS 캐싱으로 인해 1분 이내 failover가 보장되지 않습니다. (D) CloudFront는 정적 콘텐츠 전송에 최적화되고, POST 메서드 지원이 제한적입니다.",
  ...
  },
  {
    ...
    "answer": "CDE",
    "explanation": "AWS Network Firewall 정책과 규칙 그룹을 조직 전체에서 효율적으로 관리하려면 중앙 집중식 생성과 크로스 계정 공유가 필수입니다. 정답 조합 (C)(D)(E)를 분석하면: (C) 관리 계정(Management Account)에서 단 하나의 firewall 정책과 규칙 그룹을 생성합니다 — 중복 제거. (D) AWS RAM(Resource Access Manager)이 이러한 리소스를 다른 계정과 공유합니다. RAM은 AWS Organizations의 리소스 공유 메커니즘입니다. (E) Organizations 내에서 공유(Enable sharing within Organizations)를 활성화해야 RAM이 조직 내 계정들과 자동으로 공유할 수 있습니다. 조합의 시너지: 관리 계정에서 1개 정책 생성 (C) → Organizations 공유 활성화 (E) → RAM으로 50개 계정에 자동 공유 (D). (A) 각 계정에서 생성하면 50개 중복이 생깁니다. (B) SCP(Service Control Policy)는 리소스 공유 메커니즘이 아닙니다. (F) OU(Organizational Unit)도 공유 메커니즘이 아닙니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】 ▸ VPC Traffic Mirroring — 네트워크 트래픽을 캡처하여 검사 엔드포인트로 전송하는 기능 ▸ Network Load Balancer — 레이어 4에서 작동하는 로드밸런서\n【정답 포인트】 ▸ 트래픽 미러링 대상으로 NLB 사용 가능 → NLB를 직접 선택하지 않고 NLB 뒤의 EC2 인스턴스를 대상으로 함 ▸ UDP 리스너 필요 → VPC Traffic Mirroring은 UDP 포트 4789(VXLAN)를 사용하여 캡슐화된 트래픽 전달 ▸ NLB는 트래픽 미러링의 대상 인터페이스로 작동하려면 UDP 리스너가 필요\n【오답 체크】 (A) NLB를 소스로 선택할 수 없음. 미러링은 EC2 인스턴스나 ENI에서 시작 (B) TCP와 UDP 모두 필요 없음. UDP만 필요 (C) TCP 리스너만으로는 부족. 트래픽 미러링은 UDP VXLAN 캡슐화 사용\n【시험 포인트】 VPC Traffic Mirroring의 기술적 요구사항 이해 → NLB 통합 시 적절한 리스너 구성 필수 → VXLAN 캡슐화 방식과 UDP 포트의 관계",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】 ▸ Appliance Mode — Transit Gateway의 기능으로, 트래픽이 항상 같은 경로로 돌아오도록 강제하여 대칭 라우팅 보장 ▸ Transit Gateway Peering — 여러 리전의 TGW 간 연결\n【정답 포인트】 ▸ 리전 간 통신 실패 원인 → 트래픽이 origin region의 검사 VPC를 통과하지만, 응답이 destination region의 검사 VPC를 통과하려고 시도 → 비대칭 라우팅 발생 ▸ Appliance Mode 활성화 → 검사 VPC의 ENI에서 모든 트래픽이 동일 경로로 반환되도록 강제 → 요청과 응답이 동일한 검사 VPC를 통과하도록 보장\n【오답 체크】 (A) OSPF는 라우팅 문제 해결 못함. Transit Gateway Peering이 이미 경로 전파 (B) RAM 공유는 통신 대칭 문제 해결 못함 (C) 설명은 맞지만, 실제 구현은 Appliance Mode를 통함\n【시험 포인트】 리전 간 트래픽 흐름 이해 필수 → Network Firewall 검사 시 대칭 라우팅 요구 → Appliance Mode가 양방향 경로 보장의 핵심",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】 ▸ VPC Peering — 두 VPC 간의 직접 네트워크 연결, 데이터 전송 비용 없음 ▸ Virtual Private Gateway — VPN 연결용 AWS 끝점\n【정답 포인트】 ▸ 비용 분석 필수 → VPC 간 트래픽(50TB/월) 매우 많음 → VPC Peering 사용 시 데이터 전송 비용 약 4배 절감 ▸ VPN을 통한 리전 간 통신 비용이 높음 → 온프레미스 방화벽이 라우터 역할 → VPC Peering으로 직접 연결 권장 ▸ 온프레미스 연결(10TB/월)은 VPN으로 충분\n【오답 체크】 (A) Transit Gateway Peering도 비용 발생. VPC Peering보다 비쌈 (B) 온프레미스 방화벽을 라우터로 사용하면 모든 VPC 간 트래픽이 VPN 통과 → 비용 증가 (D) VPG 간 VPN은 불가능. VPC Peering으로 대체 필요\n【시험 포인트】 AWS 네트워크 비용 이해 필수 → 리전 간 통신 시 트래픽 패턴 분석 → VPC Peering이 비용 최적화의 핵심",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 ▸ Route Propagation — VPC의 CIDR 범위를 자동으로 라우팅 테이블에 전파하는 기능 ▸ BGP — 동적 라우팅 프로토콜 ▸ Transit Gateway — 중앙 라우팅 허브\n【정답 포인트】 ▸ 동적 라우팅이 이미 구성됨 → BGP가 VPN 연결에서 활성화됨 ▸ Route Propagation 활성화 → Transit Gateway는 자동으로 VPC의 CIDR 범위 변경을 감지하고 라우트 전파 ▸ 향후 유지보수 불필요 → 운영 효율성 최고\n【오답 체크】 (B) 수동 업데이트는 매번 필요. 운영 효율성 낮음 (C)(D) EventBridge/CloudWatch 및 Lambda로 자동화하지만, 복잡성 증가. Route Propagation이 더 간단하고 효율적\n【시험 포인트】 Transit Gateway의 Route Propagation 이해 필수 → BGP 동적 라우팅과의 조합 → 자동화 메커니즘 선택 시 단순성 우선",
  ...
  },
  {
    ...
    "answer": "ABC",
    "explanation": "【핵심 용어】 ▸ MACsec — Media Access Control Security, Direct Connect의 계층 2 암호화 기술 ▸ CKN/CAK — MACsec 인증서 키 쌍\n【정답 포인트】 ▸ Direct Connect 자체 암호화 → MACsec 포트를 요청하여 물리적 포트 레벨에서 암호화 활성화 (A) ▸ CKN/CAK 생성 및 관련 → 각 연결에 고유의 키 쌍 할당으로 암호화 설정 (B) ▸ 온프레미스 라우터 구성 → 라우터도 동일한 MACsec 및 CKN/CAK 쌍으로 구성하여 양쪽 암호화 일치 (C)\n【오답 체크】 (D)(E)(F) IPsec은 Direct Connect의 표준 암호화가 아님. MACsec이 권장됨. IPsec은 VPN용이고, Direct Connect는 MACsec으로 암호화\n【시험 포인트】 Direct Connect 암호화 방식 이해 → MACsec의 계층 2 암호화 특성 → CKN/CAK의 역할과 양쪽 라우터 구성 필요성",
  ...
  },
  {
    ...
    "answer": "BCF",
    "explanation": "【핵심 용어】 ▸ NLB Target — IP 또는 인스턴스 ID로 등록 가능 ▸ VPC Peering — 다른 VPC의 인스턴스를 대상으로 사용 시 IP 기반 등록 필요 ▸ Network ACL — 서브넷 수준의 상태 비저장 방화벽\n【정답 포인트】 ▸ VPC Peering을 통한 등록 → IP 주소 기반 등록 필수 (인스턴스 ID는 같은 VPC 내에서만 작동) (B) ▸ NLB 가용 영역 설정 → 새 애플리케이션이 있는 AZ에 NLB 활성화 필요 (C) ▸ Network ACL 규칙 → 인바운드(응답) 및 아웃바운드(요청) 모두 허용 필요 (F)\n【오답 체크】 (A) 인스턴스 ID는 VPC Peering 대상으로 작동 안 함 (D) Zonal Shift는 이 시나리오에 불필요. AZ 활성화가 필요 (E) 아웃바운드만으로는 부족. 인바운드도 필요\n【시험 포인트】 NLB의 VPC Peering 지원 메커니즘 이해 → IP 기반 등록의 필수성 → Network ACL의 양방향 구성 필요",
  ...
  },
  {
    ...
    "answer": "ACE",
    "explanation": "【핵심 용어】 ▸ Transit Gateway — 여러 VPC를 중앙으로 연결 ▸ Direct Connect Gateway — 여러 리전의 DX 연결을 중앙화 ▸ Private VIF — 비공개 IP 사용 ▸ Transit VIF — Transit Gateway 연결용\n【정답 포인트】 ▸ 중앙 집중식 아키텍처 → Transit Gateway와 DX Gateway 조합으로 15개 VPC를 효율적으로 관리 (A) ▸ 프라이빗 IP CIDR 블록 → Transit Gateway 자체의 프라이빗 IP 주소 공간 필요 (C) ▸ Transit VIF 생성 → DX Gateway와 Transit Gateway 간의 연결 (E) ▸ 프라이빗 IP VPN → 공개 IP 피함 (E)\n【오답 체크】 (B) 각 VPC마다 VIF를 생성하면 운영 오버헤드 증가 (D) 공개 IP는 요구사항에 위배 (F) 공개 VIF와 공개 IP VPN은 금지됨\n【시험 포인트】 Transit Gateway와 Direct Connect Gateway의 조합 이해 → 중앙 집중식 라우팅 아키텍처 → 운영 효율성과 보안 균형",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】 ▸ S3 Gateway Endpoint — 라우트 테이블 기반, VPC 내 EC2용, 비용 무료 ▸ S3 Interface Endpoint — ENI 기반, VPC 외부 접근 가능, 비용 발생\n【정답 포인트】 ▸ 온프레미스 서버 접근 → VPN을 통해 S3에 접근. Gateway Endpoint는 VPC 내만 지원 → Interface Endpoint 필요 (B) ▸ EC2 인스턴스 접근 → Gateway Endpoint가 비용 효율적이고 성능 우수 (A) ▸ 조합 활용 → Interface Endpoint(온프레미스용) + Gateway Endpoint(EC2용)로 가장 비용 효율적 (C)\n【오답 체크】 (A) Gateway Endpoint만으로는 온프레미스 서버 불가능. VPN 트래픽은 Gateway Endpoint를 통과하지 않음 (B) Interface Endpoint만 사용하면 비용이 높음 (D) 프록시 서버 플릿은 비용과 복잡성 증가\n【시험 포인트】 S3 Endpoint 종류의 차이 이해 → Gateway vs Interface의 적용 범위 → 혼합 아키텍처로 비용과 성능 최적화",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】 ▸ Resource Group — 태그 기반으로 AWS 리소스를 그룹화 ▸ Network Firewall Stateful Rule — 지속적인 상태를 유지하는 방화벽 규칙 ▸ Auto Scaling — 동적 인스턴스 생성/제거\n【정답 포인트】 ▸ Resource Group ARN 참조 → Network Firewall 규칙이 동적으로 Resource Group의 멤버를 인식 ▸ Auto Scaling 자동 추적 → EC2 인스턴스가 시작/종료될 때 태그에 따라 Resource Group 자동 업데이트 ▸ 최소 관리 노력 → Lambda 함수나 manual update 불필요. 완전 자동화\n【오답 체크】 (A) Network ACL은 동적 멤버십 지원 안 함 (B) Prefix List도 수동 업데이트 필요 (C) Lambda 함수는 지속적 관리 필요. 이벤트 기반 실행도 복잡\n【시험 포인트】 AWS Resource Group의 동적 멤버십 기능 이해 → Network Firewall의 ARN 기반 참조 → 태그 기반 자동화의 강력함",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 ▸ Accelerated VPN — Transit Gateway 기반의 성능 최적화 VPN ▸ Global Accelerator — 전역 네트워크 최적화 (하지만 VPN에는 미지원) ▸ Transit Gateway — 중앙 라우팅 허브\n【정답 포인트】 ▸ Virtual Private Gateway 제약 → 성능 최적화 부족. Legacy 솔루션 ▸ Accelerated VPN 필요 → Transit Gateway 기반으로 AWS의 글로벌 네트워크 인프라 활용 → 지연 시간 대폭 감소 ▸ 성능 개선 최고 → Accelerated VPN이 목표 달성\n【오답 체크】 (A) Global Accelerator는 VPN 최적화 지원 안 함 (C) IPv6으로 변경해도 성능 개선 안 됨 (D) PrivateLink는 VPN 대체 불가능\n【시험 포인트】 Accelerated VPN과 Transit Gateway의 조합 이해 → VPN 성능 최적화 메커니즘 → AWS 글로벌 네트워크 인프라 활용",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 ▸ Flow Log — 네트워크 트래픽의 메타데이터 기록 ▸ Network ACL — 서브넷 수준의 상태 비저장 방화벽 ▸ Dropped Traffic — Transit Gateway에서 거부된 트래픽\n【정답 포인트】 ▸ 트래픽 차단 목표 → VPC를 떠나는 트래픽 차단 ▸ 아웃바운드 규칙 필요 → VPC 내부에서 외부로 나가는 트래픽 차단 ▸ 대상 IP 주소 사용 → 차단하려는 외부 대상의 IP를 식별 필요\n【오답 체크】 (B) 인바운드 규칙은 VPC로 들어오는 트래픽 차단 (C) 소스 IP는 VPC 내 발신지. 외부 대상 IP가 아님 (D) 인바운드 규칙으로 아웃바운드 트래픽 차단 불가\n【시험 포인트】 Network ACL의 방향성 이해 → Flow Log의 소스/대상 필드 의미 → 아웃바운드 차단을 위한 규칙 구성",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 ▸ DNS64 — IPv6 클라이언트가 IPv4 주소를 IPv6 형식으로 변환 ▸ NAT64 — VPC 수준의 IPv6-to-IPv4 변환 (커뮤니티 기능 필요) ▸ Egress-only IGW — IPv6 전용 아웃바운드 게이트웨이\n【정답 포인트】 ▸ DNS64 활성화 → IPv6 인스턴스가 IPv4 서비스를 IPv6 형식으로 액세스 가능하게 함 ▸ NAT Gateway 라우팅 → Route Table에서 NAT Gateway로 트래픽 전송하여 실제 NAT 처리 ▸ 표준 솔루션 → AWS에서 권장하는 IPv6-to-IPv4 통신 방식\n【오답 체크】 (B) NAT64는 VPC 레벨에서 지원 안 함. 기존 NAT Gateway도 IPv6 지원 안 함 (C) DNS64만으로는 부족. NAT 처리 필요. Egress-only IGW는 IPv6 전용 (D) 각 라우트에 NAT64 불가. NAT Gateway도 IPv6 전용 지원 안 함\n【시험 포인트】 DNS64와 NAT Gateway의 조합 이해 → IPv6-to-IPv4 통신 메커니즘 → AWS의 IPv6 전환 지원 방식",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】 ▸ Direct Connect — 전용 네트워크 연결, 높은 성능 및 일관된 대역폭 ▸ Site-to-Site VPN — 공개 인터넷 기반 암호화 연결 ▸ BGP — Border Gateway Protocol, 동적 라우팅 ▸ OSPF — Open Shortest Path First, 라우팅 프로토콜\n【정답 포인트】 ▸ 운영 오버헤드 최소화 → BGP 동적 라우팅이 자동으로 경로 학습 및 장애 조치 ▸ VPN 충분성 → 800Mbps는 VPN으로 가능 (1Gbps 이상 권장) ▸ 정적 경로의 단점 → VPC CIDR 변경 시 수동 업데이트 필요 ▸ 운영 효율성 → BGP가 자동 경로 관리로 최적\n【오답 체크】 (A)(B) Direct Connect는 2Gbps 비용이 높음. VPN으로 충분하고 비용 저렴 (A) 정적 경로는 수동 관리 필요 (B) OSPF도 작동하지만, VPN은 BGP 기본 지원\n【시험 포인트】 비용과 운영 효율성의 균형 → BGP의 자동 경로 관리 → VPN으로 충분한 대역폭 평가",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 ▸ Interface VPC Endpoint — ENI 기반 AWS 서비스 연결. VPC 내부에서만 접근 가능 ▸ Security Group — Stateful 방화벽. 인바운드와 아웃바운드 통제\n【정답 포인트】 ▸ 인바운드 규칙 1 제거 → 0.0.0.0/0은 불필요. VPC 내부 리소스만 액세스 필요 ▸ 인바운드 규칙 2 유지 → VPC CIDR에서만 443 포트 허용으로 충분 ▸ 아웃바운드 규칙 1 제거 → 모든 트래픽 허용은 과도. Stateful이므로 응답 자동 허용 ▸ 아웃바운드 규칙 필요 없음 → Interface VPC Endpoint는 VPC 내부이므로 아웃바운드 규칙 불필요\n【오답 체크】 (A) 아웃바운드 규칙 2가 주어지지 않음 (C) 인바운드 규칙 2도 불필요한가? 아니, VPC CIDR만 필요 (D) 아웃바운드 규칙 1만으로는 부족. 인바운드 규칙 1도 제거 필요\n【시험 포인트】 Interface VPC Endpoint의 액세스 패턴 이해 → VPC 내부 통신만 필요 → Stateful 방화벽의 응답 자동 허용 특성",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 ▸ Prefix List — VPC CIDR 범위들을 그룹화하여 관리 ▸ Customer-managed Prefix List — 사용자가 직접 관리하는 접두사 목록 ▸ Route Propagation — Transit Gateway가 라우트 자동 전파\n【정답 포인트】 ▸ 운영 효율성 최고 → Prefix List 생성 후 추가 라우트 테이블마다 재사용 ▸ 확장성 → 새 VPC 추가 시 Prefix List에만 추가하면 모든 라우트 테이블이 자동 적용 ▸ 중앙 집중식 관리 → Prefix List가 단일 지점에서 VPC CIDR 관리\n【오답 체크】 (B) Route Propagation도 작동하지만, VPC 라우트 테이블에서 각각 활성화해야 함. 초기 구성 비용 높음 (C) 0.0.0.0/0은 모든 트래픽을 Transit Gateway로 보냄. NAT Gateway 등의 로컬 라우팅 불가능 (D) Association은 전파와 다른 개념. 경로 제한 증가\n【시험 포인트】 Prefix List의 재사용성 → 많은 VPC 추가 시 단일 관리 지점 → 수동 업데이트 최소화",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 ▸ Site-to-Site VPN — AWS와 온프레미스 간의 암호화 연결 ▸ IPv6 Support — VPN 터널이 IPv6 트래픽 지원 필요 ▸ Transit Gateway — 중앙 라우팅\n【정답 포인트】 ▸ 기존 VPN IPv6 미지원 → IPv4 기반 VPN은 IPv6 트래픽 통과 불가 ▸ 새 VPN 연결 생성 → IPv6 지원 VPN을 별도로 생성 ▸ 최소 오버헤드 → 기존 IPv4 VPN 유지하면서 IPv6 전용 VPN 추가 ▸ 병렬 운영 → IPv4와 IPv6 동시 지원\n【오답 체크】 (B) 자체 관리 EC2는 복잡성 증가. AWS 관리형이 나음 (C) 기존 VPN은 IPv6 업그레이드 불가능. 프로토콜 수준의 제약 (D) Public IP 변경만으로는 VPN IPv6 지원 안 됨\n【시험 포인트】 VPN 터널의 프로토콜별 지원 → IPv6 전환 시 병렬 구성 필요 → 기존 리소스 영향 최소화",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 ▸ Resource Access Manager(RAM) — 계정 간 AWS 리소스 공유 ▸ Transit Gateway Sharing — RAM을 통한 TGW 공유 ▸ Least Operational Overhead — 최소한의 변경\n【정답 포인트】 ▸ TGW-A 재사용 → 이미 구성된 TGW-A를 Account B와 공유하여 활용 ▸ Account A 중심 구조 → TGW-A의 리소스 공유만으로 충분 ▸ 최소 변경 → Account A의 라우팅은 유지. Account B만 TGW-A 연결 추가 ▸ 가장 간단한 마이그레이션 → TGW-B만 제거\n【오답 체크】 (B) 자동 복제 기능 없음. TGW 설정은 수동 구성 필요 (C) 새 TGW 생성으로 운영 오버헤드 증가. Account A의 라우팅도 변경 필요 (D) 별도 Account C 생성은 불필요한 복잡성. Account A 중심이 더 간단\n【시험 포인트】 Least Operational Overhead의 의미 → 기존 리소스 최대 활용 → 최소한의 구성 변경으로 목표 달성",
  ...
  },
  {
    ...
    "answer": "BE",
    "explanation": "【핵심 용어】 ▸ Certificate-based VPN — 인증서를 사용한 VPN 인증 (PSK 대체) ▸ Private Certificate — ACM Private CA에서 발급한 프라이빗 인증서 ▸ Customer Gateway — 온프레미스 VPN 엔드포인트\n【정답 포인트】 ▸ 프라이빗 인증서 생성 → AWS Certificate Manager(ACM)의 Private CA를 사용하여 프라이빗 인증서 발급 (B) ▸ 동적 IP 주소 지원 → Customer Gateway 생성 시 IP 주소를 지정하지 않으면 동적 IP 변경 시에도 작동 (E) ▸ 인증서 기반 → PSK 대신 인증서 사용으로 더 강력한 인증\n【오답 체크】 (A) 공개 인증서는 인터넷 기반 HTTPS용. VPN에는 프라이빗 인증서 필요 (C) 인증서 기반이므로 PSK 불필요 (D) 동적 IP이므로 IP를 지정할 수 없음. 지정하면 변경 시 작동 안 함\n【시험 포인트】 인증서 기반 VPN의 이점 → 동적 IP 지원으로 유연성 증가 → Private Certificate의 역할과 생성 방식",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 ▸ AWS Config Rule — AWS 리소스의 준수 상태를 모니터링 ▸ Auto Remediation — 자동 수정 기능. 규칙 위반 시 자동 조치 ▸ AWS Organizations — 여러 계정의 중앙 관리\n【정답 포인트】 ▸ AWS Config 규칙 생성 → VPC Flow Logs 설정 상태 자동 모니터링 ▸ 자동 수정 기능 → VPC 생성 시 Flow Logs가 자동 설정됨 ▸ 조직 적용 → 모든 계정과 리전에 일괄 적용 ▸ 최소 관리 노력 → 일단 설정되면 지속적 관리 불필요\n【오답 체크】 (A) IAM은 사전 예방이지만, 모든 사용자의 권한 관리 복잡 (C) Transit Gateway만 설정. VPC 개별 설정 필요 (D) CloudTrail + Lambda는 복잡하고 관리 오버헤드 높음\n【시험 포인트】 AWS Config의 조직 규모 적용 → Auto Remediation의 자동화 이점 → 리소스 생성 시 자동 준수 설정",
