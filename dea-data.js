// AWS Certified Data Engineer - Associate (DEA-C01) 한국어 문제 데이터
// 수집일: 2026-04-21 · 출처: ExamTopics Community (via blog.naver.com/imaokiro)
// 총 337문제 (다지선다 형식 전체)
// 각 문제: 한글 번역 + 【핵심 용어】【정답 포인트】【오답 체크】【시험 포인트】 상세 해설
// AWS 서비스명·기술 용어는 원어 유지
...
window.DEA_QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "이 문제는 여러 이기종 데이터 소스를 조인하는 일회성 분석의 최적화에 관한 것입니다. 핵심 요구사항은 \"데이터 이동 최소화\"와 \"쿼리 실행 시간 최적화\"입니다.\n\n정답 D(Athena Federated Query)가 최적인 이유:\n- Federated Query는 여러 데이터 소스(DynamoDB, RDS, Redshift, S3)에 원본 위치에서 직접 쿼리\n- 데이터를 중앙 저장소로 이동할 필요 없음 → 네트워크 오버헤드 제거\n- 일회성 보고서에 매우 효율적 (초기 스캔 비용 최소)\n- Lambda 기반 커넥터로 다양한 데이터 소스 지원\n\n오답 검토:\n- A: DMS 마이그레이션 + Spectrum은 과도한 데이터 이동 발생 (일회성에 부적합)\n- B: Glue ETL로 S3 중앙화 → 데이터 이동 비용 높음\n- C: EMR 클러스터는 항상 켜야 하므로 일회성 분석에 비용 비효율적\n\n시험 포인트: Federated Query는 \"데이터를 옮기지 않고\" 여러 소스를 쿼리해야 할 때 선택. 일회성 vs 반복 쿼리 구분 중요.",
  ...
  },
  {
    ...
    "answer": "AB",
    "explanation": "서버리스 멀티 스텝 ETL 워크플로우 오케스트레이션 선택 문제입니다. 두 가지 정답을 판별해야 합니다.\n\n정답 A(Step Functions): 가장 강력한 오케스트레이션 도구\n- 복잡한 워크플로우 정의 가능 (상태 머신 기반)\n- 자동 재시도, 오류 처리 (Catch, Retry)\n- 작업 간 의존성 관리 용이\n- 완전 서버리스\n\n정답 B(Glue Workflows): Glue 자체 오케스트레이션\n- ETL 작업 간 의존성을 그래프로 시각화\n- 작업 트리거 관리\n- Glue 작업에 최적화됨\n\n오답 검토:\n- C: EC2는 \"항상 켜져 있음\" → 비용 문제 + \"서버리스\" 요구사항 위배\n- D: EventBridge는 이벤트 기반이지만 복잡한 순차 의존성 관리 부족\n- E: CodePipeline은 배포 파이프라인용이지, ETL 오케스트레이션 전문 도구 아님\n\n시험 포인트: Step Functions(일반) vs Glue Workflows(Glue 특화). 둘 다 서버리스 ETL 오케스트레이션에 적합.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "90일 유지 후 삭제해야 하는 일시적 저장 데이터의 가장 비용 효율적인 S3 라이프사이클 구성입니다.\n\n정답 C가 최적인 이유:\n- 90일 동안 S3 Standard로 유지 (접근 가능한 상태)\n- Lifecycle Expiration 규칙으로 정확히 90일 후 자동 삭제\n- 추가 비용 없음 (Glacier 전환, Object Lock 오버헤드 제거)\n- 가장 단순하고 직관적\n\n오답 검토:\n- A: Glacier Flexible Retrieval로 전환 → 불필요한 비용 (재처리 용도라 접근성 필요)\n- B: Object Lock은 규정 준수(데이터 보호)용이지 90일 \"만료\"는 아님. 비용 증가\n- D: Intelligent-Tiering은 자동 티어링 비용 발생. 90일 후 단순 삭제가 목표면 불필요\n\n핵심:\n- \"90일 후 삭제\" = Expiration 규칙\n- \"재처리 가능해야 함\" = Standard 스토리지 유지\n- 다른 버킷에 처리된 데이터 저장하므로 원본은 저비용 옵션\n\n시험 포인트: S3 라이프사이클 정책의 목적을 정확히 파악. 요구사항이 명확하면 가장 간단한 솔루션이 비용 효율적.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "교과서 기반 RAG(Retrieval-Augmented Generation)로 소스 인용을 포함한 다국어 피드백을 제공해야 합니다. \"최소 운영 오버헤드\" 요구사항이 중요합니다.\n\n정답 B(Bedrock Knowledge Bases): 최고의 운영 효율성\n- Knowledge Bases는 AWS의 완전 관리형 RAG 솔루션\n- 교과서를 자동으로 임베드하고 인덱싱\n- LLM(Claude 등)의 기본 인용 능력 활용\n- 다국어 응답 생성 내장\n- 서버 관리 불필요\n\n오답 검토:\n- A: OpenSearch + Lambda → 벡터 DB 구축 및 운영 오버헤드 높음\n- C: Comprehend + Kendra + Lambda → 여러 서비스 연동 필요 (복잡함)\n- D: SageMaker 미세 조정 → 훈련/배포 운영 비용 매우 높음\n\n핵심 차별 요소:\n- Knowledge Bases = \"미리 구성된 RAG\" (빠른 배포)\n- 소스 인용 자동 추적\n- 다국어 지원 (Bedrock 모델 기능)\n- AWS의 최신 생성 AI 통합\n\n시험 포인트: \"최소 운영 오버헤드\" + \"소스 인용\" + \"다국어\" = Bedrock Knowledge Bases. 완전 관리형 서비스 우선.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "S3 액세스 제어, 암호화, 자격증명 관리 및 자동 로테이션을 모두 포함하는 통합 보안 솔루션입니다.\n\n정답 B의 구성 요소:\n- IAM 정책: S3 액세스 제어의 표준 (버킷 정책보다 세분화)\n- SSE-KMS: AWS 관리형 키 (고급 암호화, KMS 권한 관리)\n- Secrets Manager: 자동 로테이션 전문 도구 (Lambda 기반 내장)\n\n오답 검토:\n- A: Lambda 환경 변수에 자격증명 저장 → 보안 위험 (평문 노출 가능)\n- C: Parameter Store는 정적 저장만 가능, 자동 로테이션 기본 미지원 (추가 구현 필요)\n- D: SSE-S3는 AWS 관리형이지만 키 로테이션 통제 불가, Parameter Store는 자동 로테이션 부족\n\n핵심 차별:\n- Secrets Manager = 자격증명 전문 도구 (자동 로테이션 네이티브)\n- SSE-KMS = 전체 암호화 키 제어 가능\n- IAM = 세분화된 액세스 제어\n\n시험 포인트: 자격증명 자동 로테이션 = Secrets Manager. Parameter Store는 수동 로테이션만 지원.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "밀리초 응답 시간(ms 단위)이 필요한 고성능 읽기 지향 워크로드입니다.\n\n정답 D(DynamoDB + DAX): 최고의 성능과 운영 효율성\n- DynamoDB: 완전 서버리스 NoSQL DB (프로비저닝 불필요)\n- DAX(DynamoDB Accelerator): 인메모리 캐시 (밀리초 응답)\n- 반정형 데이터에 최적 (JSON 문서 저장)\n- 자동 확장 (읽기 부하 자동 처리)\n\n오답 검토:\n- A: S3는 초 단위 응답 (밀리초 불가). 분석용이지 OLTP용 아님\n- B: Iceberg는 데이터 레이크 포맷 (밀리초 응답 안 됨)\n- C: RDS는 서버리스 아님 (Aurora Serverless라도 ms 보장 어려움)\n\n성능 비교:\n- DynamoDB 직접: 10ms\n- DAX 캐시: 1ms (인메모리)\n- S3: 수백ms\n- RDS: 10~50ms\n\n핵심:\n- \"밀리초\" = 인메모리 캐시 필수\n- \"서버리스\" = DynamoDB\n- \"자주 읽기\" = DAX 캐시\n\n시험 포인트: 초저지연 + 서버리스 + 반정형 = DynamoDB + DAX. 높은 읽기 부하는 캐시 필수.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "동시 ETL 및 분석 워크로드로 인한 테이블 잠금(table-level lock) 경합 문제를 해결하는 솔루션입니다.\n\n정답 D(별도 클러스터 + 데이터 공유): 근본 해결\n- 별도 Redshift 클러스터: ETL과 분석 격리\n- 각 클러스터는 독립적인 잠금 영역\n- 교차 데이터베이스 쿼리: 원본 데이터 접근\n- 데이터 공유: 복제 없이 실시간 동기화\n- 데이터 일관성: 스냅샷 격리 레벨\n\n오답 검토:\n- A: Materialized Views + 오프피크 일정 → 실제 해결 아님 (시간대 제한)\n- B: Federated Query + Read Replicas → Redshift는 읽기 복제본 미지원\n- C: Spectrum은 S3 데이터용이지 Redshift 내부 잠금 해결 못함\n\n핵심 차별:\n- \"테이블 수준 잠금\" = 동일 데이터에 동시 접근 불가\n- 해결책 = 워크로드 분리 (클러스터 분리)\n- Redshift 데이터 공유 = Spectrum 대비 성능 우수\n\nRedshift 차원:\n- 단일 클러스터: 모든 워크로드 경합\n- 다중 클러스터 + 공유: 병렬 실행 가능\n\n시험 포인트: 동시성 경합(concurrency contention) = 리소스(클러스터) 분리. Redshift의 데이터 공유가 최신 기능.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "S3 데이터 레이크에서 센서 카테고리별 복잡한 쿼리의 최고 성능 구성입니다.\n\n정답 C(Parquet + 센서 카테고리 분할 + 날짜 정렬): 최적화\n\n포맷 비교:\n- Parquet: 컬럼 지향 (선택적 컬럼 읽기 → 빠름)\n- ORC: 컬럼 지향이나 Hadoop 최적화 (S3 쿼리에는 Parquet 선호)\n- CSV: 전체 행 읽기 (가장 느림)\n\n분할 전략:\n- 센서 카테고리별 분할: \"센서 카테고리 특화\" 쿼리 → 파티션 프루닝 극대화\n- 날짜별 분할 (X): 모든 카테고리 스캔 필요\n\n정렬:\n- 날짜별 정렬: 같은 카테고리 내 시계열 최적화\n\nS3 쿼리 성능:\n1. Parquet (컬럼 압축)\n2. 분할 전략 (파티션 프루닝): 모든 카테고리 대신 해당 분할만 읽음\n3. 정렬: 압축률 향상\n\n오답 검토:\n- A: ORC는 S3 + Athena보다는 Hadoop 기반 선택 (Spark는 OK)\n- B/D: CSV는 전체 행 읽기 필수 (매우 느림)\n\n시험 포인트: S3 데이터 레이크 최적화 = Parquet + 주요 필터 컬럼으로 분할. \"복잡한 쿼리\"는 분할 전략이 핵심.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "Redshift 내부의 DELETE 및 INSERT 작업 (UPSERT 패턴)을 구현하는 가장 직접적이고 효율적인 방법입니다.\n\n정답 D(Redshift 저장 프로시저): 최적의 선택\n- 데이터베이스 내 트랜잭션 처리\n- DELETE 및 INSERT 원자성 보장\n- 네트워크 오버헤드 없음\n- 성능 최고 (직접 실행)\n- 다운스트림 앱에 즉시 반영\n\n오답 검토:\n- A: Spectrum은 S3 데이터용이지 Redshift 테이블 수정 불가\n- B: Unload → Athena 수정 → Copy = 과도한 라운드 트립 (느림)\n- C: Federated Query는 읽기 최적 (쓰기/삭제 성능 부족)\n\n트레이드오프:\n- A: Spectrum (읽기 전용)\n- B: 다단계 (복잡, 느림)\n- C: Federated (읽기 쿼리 최적)\n- D: Native Redshift (쓰기 최적, 가장 빠름)\n\nRedshift UPSERT 패턴:\n- Stored Procedure 내 MERGE 또는 DELETE+INSERT\n- 트랜잭션 격리\n- 롤백 가능\n\n시험 포인트: \"Redshift 테이블 업데이트\" = 저장 프로시저 또는 MERGE. Spectrum, Athena, Federated는 읽기 워크로드용.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "CloudWatch Logs에서 Firehose로 필터링된 로그를 수집하고 포맷 변환하는 파이프라인입니다.\n\n정답 B: 정확한 아키텍처\n1. 구독 필터(Subscription Filter): CloudWatch → Firehose 직접 연결\n   - 원하는 로그만 필터링 (관련 로그)\n2. Firehose: 스트리밍 수집 (신뢰성 높음)\n3. Lambda 변환: Firehose 내장 변환 기능\n   - 데이터 형식 변환 (예: JSON, Parquet)\n   - S3 직접 저장\n\n오답 검토:\n- A: S3 버킷 규칙 = 수신 후 필터링 (비효율, 이미 S3에 있음)\n- C: Firehose 스트림 모니터링 불명확 (구독 필터 + Firehose가 표준)\n- D: 태그 기반 필터는 CloudWatch 지원 안 함. Firehose 형식도 S3 직접 지원\n\n핵심 흐름:\nCloudWatch → 구독 필터 → Firehose → Lambda 변환 → S3\n\nFirehose 변환 Lambda:\n- Firehose가 직접 호출 (\"Data Transformation\" 기능)\n- 배치 처리\n- 신뢰성 높음\n\n시험 포인트: CloudWatch → Firehose 연결 = 구독 필터. Firehose 변환 Lambda = 내장 기능. S3 필터 처리 (A) = 후처리 (비효율).",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "실시간 스트리밍 분석의 지연 시간, 확장성, 비용을 종합적으로 최적화하는 솔루션입니다.\n\n정답 A(Kinesis Data Streams + Lambda + Redshift): 최고의 균형\n\nKinesis Data Streams:\n- 실시간 스트리밍 (밀리초 지연)\n- 자동 확장 (수백만 이벤트/초 처리)\n- 내구성 (3개 가용영역 복제)\n- 비용: 초당 레코드 기반 청구 (사용량 기반)\n\nLambda:\n- 서버리스 처리\n- 자동 확장\n- 비용: 호출 기반\n\nRedshift:\n- 분석 저장소\n\n오답 검토:\n- B: Glue 10분 폴링 → 실시간 아님 (지연 10분)\n- C: S3 Event Notifications → 문제: 로그 파일 도착 지연 (배치)\n- D: MSK는 최고 성능이나 운영 복잡도/비용 높음\n  - 자체 관리 소비자 = 인프라 관리 필요\n  - \"최소 운영 오버헤드\" 아님\n\nKinesis vs MSK:\n- Kinesis: AWS 완전 관리형 (서버리스)\n- MSK: 오픈소스 Kafka (성능 우수나 관리 필요)\n\n실시간 정의:\n- 밀리초 단위: Kinesis\n- 분 단위: Glue/EventBridge\n\n시험 포인트: \"실시간\" + \"최소 지연\" + \"비용 효율\" = Kinesis. MSK는 초고성능 필요할 때만.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "동일한 매개변수로 반복 실행되는 쿼리의 성능과 비용 최적화에 관한 것입니다.\n\n정답 C(쿼리 결과 재사용): 가장 직접적인 최적화\n\nAthena 쿼리 결과 재사용(Query Result Reuse):\n- 동일 쿼리(SQL 텍스트, 매개변수) 재실행 시 이전 결과 캐시에서 반환\n- 스캔 비용 제거 (S3 데이터 읽기 안 함)\n- 응답 시간 밀리초 단위\n- Workgroup 설정으로 활성화\n- 구성 방법: 쿼리 결과 재사용 기간 설정 (예: 1시간)\n\nWorkgroup 설정:\n```\nQuery result reuse by engine\nMax age of query results: 1 hour (또는 원하는 기간)\n```\n\n오답 검토:\n- A: JSON 변환 → 포맷 개선이나 \"반복 쿼리\" 최적화 아님\n- B: EMR 사전 처리 → 복잡하고 비용 증가 (일회성)\n- D: Redshift Spectrum → 다른 쿼리 엔진 (Athena 결과 재사용 미지원)\n\n핵심 메커니즘:\n- 첫 실행: 풀 스캔 (비용 발생)\n- 재실행: 캐시 반환 (비용 제로)\n- 캐시 TTL 지나면 다시 스캔\n\nAthena 최적화 계층:\n1. 쿼리 결과 재사용 (가장 빠름, 가장 저렴)\n2. 파티셔닝 (스캔 범위 축소)\n3. 컬럼 포맷 (Parquet > ORC > CSV)\n4. 메타데이터 최적화\n\n시험 포인트: \"동일 매개변수로 여러 번\" = 쿼리 결과 재사용. Athena의 기본 최적화 기능.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "사용자 정의 라이브러리가 있는 복잡한 PySpark 변환을 확장 가능하게 처리하고 가장 적은 운영 오버헤드로 스케줄링해야 합니다.\n\n정답 A(AWS Glue Studio + 데이터 연결): 최고 운영 효율\n\nAWS Glue Studio:\n- 서버리스 ETL (자동 확장)\n- 비주얼 + PySpark 코드 모두 지원\n- Glue 데이터 연결: 외부 라이브러리 의존성 관리\n- 스케줄링 내장\n- 운영 인프라 불필요\n\nGlue 데이터 연결:\n- Python 패키지(pip) 또는 .whl 파일 등록\n- Glue 작업 실행 시 자동 로드\n- 버전 관리 가능\n\n오답 검토:\n- B: EC2 Auto Scaling = 인프라 관리 (Terraform, 모니터링, 패치)\n  - \"최소 운영 오버헤드\" 위배\n- C: EMR = 클러스터 프로비저닝, 부트스트랩 스크립트 관리\n  - 클러스터 수명 주기 관리 필요\n  - Glue 대비 운영 복잡\n- D: SageMaker Processing = \"네이티브 라이브러리만\" 지원\n  - 사용자 정의 라이브러리 제한\n\n운영 오버헤드 비교:\n1. Glue Studio (최저): 코드 + 스케줄만\n2. EMR (중간): 클러스터 관리\n3. EC2 (높음): 인프라 + 스케일링 정책\n4. SageMaker Processing (제한): 라이브러리 호환성\n\nGlue의 강점:\n- 서버리스 확장\n- 의존성 관리 자동화\n- 기본 PySpark 지원\n- Redshift 로더 내장\n\n시험 포인트: \"최소 운영 오버헤드\" + \"사용자 정의 라이브러리\" + \"확장 가능\" = Glue Studio. 서버리스 ETL의 기본 선택.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "AWS Lambda의 구조화된 로깅을 통해 세분화된 로그 레벨 필터링을 구현하는 문제입니다.\n\n정답 C(JSON 로그 형식): AWS Lambda 구조화된 로깅\n\nAWS Lambda 구조화된 로깅 (Lambda Insights):\n- JSON 형식 로그로 스트럭처드 정보 전달\n- 각 로그 항목에 로그 레벨(ERROR, WARN, INFO 등) 메타데이터 포함\n- CloudWatch Logs Insights 쿼리로 로그 레벨 필터링 가능\n- 로그그룹이 애플리케이션과 시스템 로그를 구분할 수 있게 함\n\nJSON 형식 예시:\n```json\n{\n  \"level\": \"ERROR\",\n  \"type\": \"application\",\n  \"message\": \"Application error\"\n}\n```\n\nCloudWatch Logs Insights 쿼리:\n```\nfields @level, type, @message\n| filter @level = \"ERROR\" and type = \"application\"\n| stats count() by @level\n```\n\n오답 검토:\n- A: IAM 권한 → 로그 필터링과 무관 (로그 레벨 제어 아님)\n- B: Lambda 함수 코드 수준 설정 → 관리되는 런타임에서 환경 레벨 로깅만 가능\n  - 애플리케이션 + 시스템 로그 분리 불가\n- D: 사용자 정의 로그 그룹 → 로그 레벨 필터링 구현 안 함\n\n핵심:\n- \"관리되는 런타임\" = Lambda 자체 로그 + 애플리케이션 로그 혼재\n- JSON 형식 + 메타데이터 = CloudWatch 쿼리로 필터링 가능\n- \"ERROR 레벨\" + \"WARN 레벨\" 분리 = JSON 구조화 필요\n\nAWS Lambda 로깅 모드:\n1. 기본 텍스트: \"애플리케이션 로그\\n시스템 로그\" (분리 불가)\n2. JSON (구조화): {\"level\": \"...\", \"type\": \"...\"} (필터링 가능)\n\n시험 포인트: \"로그 수준 필터링\" + \"애플리케이션 vs 시스템\" = JSON 로그 형식. CloudWatch Logs Insights로 세분화 쿼리.",
