window.DEA_QUESTIONS = [
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ VPC 게이트웨이 엔드포인트 — Amazon S3에 프라이빗하게 접근하기 위한 VPC 엔드포인트로, 인터넷 게이트웨이나 NAT를 거치지 않고 S3에 연결\n▸ 경로 테이블 — VPC 내 네트워크 트래픽을 라우팅하기 위한 규칙 집합\n▸ AWS Glue 연결 — 데이터 소스(S3, RDS 등)에 접근하기 위한 연결 설정\n\n【정답 포인트】\n▸ S3 VPC 게이트웨이 엔드포인트 사용 시, VPC의 경로 테이블에 엔드포인트로의 경로가 반드시 설정되어야 함\n▸ 오류가 \"S3 VPC 게이트웨이 엔드포인트 관련\"이라는 것은 라우팅 문제를 의미\n▸ 경로 테이블에 S3 트래픽을 게이트웨이 엔드포인트로 라우팅하는 규칙 추가 필요\n\n【오답 체크】\n(A) 보안 그룹 업데이트는 VPC 게이트웨이 엔드포인트와 직접 관련이 없음. 엔드포인트는 보안 그룹을 사용하지 않음\n(B) S3 버킷 정책은 IAM 권한 문제를 해결하는 것이지, VPC 엔드포인트 라우팅 문제와 무관\n(C) 정규화된 도메인 이름은 Glue 연결 설정 시 선택사항이며, VPC 엔드포인트 라우팅 문제와 무관\n\n【시험 포인트】\nVPC 엔드포인트 문제 → 경로 테이블 확인 (라우팅 규칙 검증)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Glue FLEX 실행 클래스 — 예약 용량(Spark jobs)을 사용하여 비용 절감. 시간이 유연한 작업에 최적화\n▸ Glue STANDARD 실행 클래스 — 온디맨드 용량 사용. 즉시 실행 필요 시 사용\n▸ Glue Spark Job — Python/Scala 기반 대규모 데이터 처리 작업\n\n【정답 포인트】\n▸ \"특정 시간에 실행될 필요 없음\" → 유연한 스케줄링 가능 = FLEX 실행 클래스\n▸ FLEX는 예약 용량(Reserved Capacity Pricing)을 활용하여 온디맨드보다 40% 저렴\n▸ 매일 실행하는 배치 작업 → FLEX 이상적 (SLA 불필요)\n\n【오답 체크】\n(B) Spot 인스턴스 — Glue는 Spot 인스턴스 옵션 없음 (EMR에만 있음)\n(C) STANDARD — 온디맨드 가격 적용. FLEX보다 비용 높음\n(D) GlueVersion 최신화 — 성능/기능 개선이지, 비용 절감과 무관\n\n【시험 포인트】\nGlue 배치 작업 비용 최적화 → FLEX 실행 클래스 (유연한 시간 기반)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ S3 이벤트 알림 — S3 객체의 생성, 삭제 등의 이벤트를 트리거하여 Lambda, SNS, SQS 등으로 전송\n▸ s3:ObjectCreated:* — 객체가 생성되는 모든 이벤트 (PutObject, PostObject, CopyObject 등)\n▸ 필터 규칙 — 객체 키 접두사, 접미사 기반으로 이벤트 필터링\n\n【정답 포인트】\n▸ \".csv 파일 업로드 시만 Lambda 실행\" → s3:ObjectCreated:* + 접미사 필터 필요\n▸ 직접 Lambda 호출이 가장 간단 (SNS 중개 불필요)\n▸ 필터 규칙으로 .csv 파일만 선택하면 불필요한 Lambda 호출 방지\n\n【오답 체크】\n(B) ObjectTagging:* — 파일 업로드가 아닌 태그 변경 이벤트. 업로드 시점 감지 불가\n(C) s3:* — 모든 S3 이벤트(삭제, 태그, 메타데이터 변경 등) 캡처. 불필요한 Lambda 호출 증가\n(D) SNS 중개 → 불필요한 추가 서비스. 직접 Lambda 호출이 더 간단하고 빠름\n\n【시험 포인트】\nS3 파일 업로드 트리거 → ObjectCreated + 접미사 필터 + Lambda 직접 호출",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Apache Parquet — 컬럼 지향(columnar) 파일 형식. 특정 열만 읽으므로 성능 최적화\n▸ CSV — 행 지향(row-oriented) 형식. 전체 행을 읽어야 하므로 비효율적\n▸ Snappy 압축 — 빠른 압축/해제. 높은 압축률은 아니지만 성능 우선\n\n【정답 포인트】\n▸ \"대부분의 쿼리에서 특정 열 선택\" → 컬럼 지향 포맷(Parquet) 필수\n▸ Parquet는 필요한 열만 읽으므로 I/O 대폭 감소 (CSV는 모든 행 읽음)\n▸ Snappy 압축으로 저장 크기 감소 + 빠른 쿼리 처리\n▸ 조합 효과: 포맷 변경(Parquet) > 압축(Snappy)\n\n【오답 체크】\n(A) JSON — 반정형 형식. CSV보다 오버헤드 증가. 컬럼 지향 아님. 느림\n(B) CSV + Snappy — 행 지향 포맷은 변하지 않음. 전체 행 읽어야 하므로 성능 개선 미미\n(D) CSV + gzip — gzip은 느린 압축/해제. CSV 행 지향 문제 해결 안 됨\n\n【시험 포인트】\n특정 열 선택 쿼리 최적화 → Parquet(컬럼 지향) + Snappy(빠른 압축)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Amazon Managed Service for Apache Flink — 실시간 스트림 처리 (낮은 지연 시간)\n▸ Amazon Timestream — 시계열 데이터베이스. 실시간 데이터 쿼리 최적화\n▸ Kinesis Data Firehose — 배치 로딩(분 단위 지연). 실시간 아님\n▸ AWS Glue 북마크 — 배치 기반 증분 로드. 실시간 처리 아님\n\n【정답 포인트】\n▸ \"실시간 보기\" + \"가장 낮은 지연 시간\" → 스트림 처리 필수\n▸ Kinesis Data Streams → Apache Flink (MFAS) → Timestream (시계열 DB)\n▸ Flink는 마이크로초/밀리초 단위 지연. Firehose는 분 단위 지연\n▸ Timestream은 시계열 데이터에 특화되어 있음 (빠른 조회)\n\n【오답 체크】\n(B) S3 이벤트 → Lambda → Aurora — S3에 저장될 때까지 대기(분 단위 지연). 실시간 아님\n(C) Flink + Firehose → Timestream — Firehose가 배치 로딩(분 단위 지연). Flink의 실시간 처리 이점 상실\n(D) Glue 북마크 — S3에서 주기적으로 데이터 읽음. 배치 기반, 실시간 아님\n\n【시험 포인트】\n실시간 저지연 스트림 → Kinesis Streams + Flink + Timestream",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Glue 크롤러 — 데이터 소스를 자동으로 스캔하여 스키마를 감지하고 Data Catalog에 메타데이터 저장\n▸ AWSGlueServiceRole — Glue 서비스의 최소 권한 역할. S3, CloudWatch, Data Catalog 접근 기본 포함\n▸ AmazonS3FullAccess — 과도한 권한. 모든 S3 액세스 허용 (보안 취약)\n▸ Glue Data Catalog — 메타데이터 저장소 (테이블, 스키마, 파티션)\n\n【정답 포인트】\n▸ 크롤러의 출력 → Data Catalog의 \"데이터베이스\"에 테이블 메타데이터 저장\n▸ AWSGlueServiceRole로 충분 (S3 읽기, Catalog 쓰기 포함)\n▸ 일일 \"스케줄\" (DPU 할당 아님) — 스케줄로 자동 실행 설정\n▸ 데이터베이스 이름 지정 → Catalog에 메타데이터 저장\n\n【오답 체크】\n(A) AmazonS3FullAccess — 과도한 권한. \"S3 버킷의 새 경로\" 출력 → 크롤러 목적 배반 (원본 데이터 스캔만 필요)\n(C) DPU 할당 → 크롤러 실행 비용 증가. 일일 스케줄로 충분 (온디맨드 방식)\n(D) DPU 할당 + 새 경로 출력 → 비용 증가 + 목적 배반\n\n【시험 포인트】\nGlue 크롤러 Data Catalog 메타데이터 저장 → AWSGlueServiceRole + 일일 스케줄 + 데이터베이스 출력",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Amazon Redshift Data API — HTTP REST API로 Redshift에서 SQL 실행. 완료 시 이벤트 발행 가능\n▸ Amazon EventBridge — AWS 서비스에서 발행한 이벤트를 수신하여 다른 서비스 호출\n▸ Amazon SQS — 메시지 큐. EventBridge보다 느린 처리\n▸ CloudWatch 이벤트 — EventBridge의 이전 이름 (현재는 EventBridge 사용)\n\n【정답 포인트】\n▸ Redshift 작업 완료 후 자동으로 Lambda 호출 → EventBridge 규칙 기반 트리거 필요\n▸ Redshift Data API는 작업 완료 시 EventBridge에 이벤트 발행\n▸ EventBridge 규칙으로 이벤트를 필터링하고 Lambda 호출\n▸ 이것은 자동 / 거의 실시간 처리\n\n【오답 체크】\n(A) CloudWatch 이벤트로 두 번째 Lambda 호출 → Redshift 작업 완료를 감지 못함. 별도 모니터링 필요\n(C) SQS 메시지 게시 → 지연 증가. EventBridge보다 비효율적\n(D) CloudTrail 이벤트 → Redshift 작업 완료 감지 아님. 감사 로그 기반\n\n【시험 포인트】\nRedshift 작업 완료 후 Lambda 트리거 → Redshift Data API + EventBridge 규칙",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS DataSync — 온프레미스 ↔ AWS 간 대규모 데이터 동기화. 변경된 데이터만 전송(증분 동기화)\n▸ AWS Glue — ETL 작업 처리. 데이터 전송 전문 서비스 아님\n▸ AWS Direct Connect — 전용 네트워크 연결. 초기 설정 비용 높음\n▸ S3 Transfer Acceleration — S3 업로드 속도 향상. 스케줄링 기능 없음\n\n【정답 포인트】\n▸ \"5TB + 매일 5% 변경 + 주기적 전송\" → 증분 동기화 필수\n▸ DataSync는 자동 변경 감지 + 증분 전송 (대역폭 절감)\n▸ DataSync는 스케줄링 지원 (주기적 자동 실행)\n▸ 여러 파일 형식 지원 (메타데이터 보존)\n▸ 이중화, 체크섬, 암호화 포함\n\n【오답 체크】\n(B) Glue — 데이터 변환/ETL 서비스. 단순 파일 전송에는 오버헤드 높음\n(C) Direct Connect — 초기 설정 복잡, 비용 높음. DataSync만으로 충분\n(D) S3 Transfer Acceleration — 업로드 속도만 개선. 스케줄링, 증분 동기화 불가\n\n【시험 포인트】\n온프레미스 → S3 증분 동기화 + 스케줄링 → DataSync",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Database Migration Service(DMS) — 데이터베이스 마이그레이션 전문 서비스. 이질형 DB 간 마이그레이션 지원(SQL Server → RDS)\n▸ 동기화 마이그레이션(CDC) — 마이그레이션 후에도 원본 DB 변경사항 계속 동기화\n▸ AWS DataSync — 파일 시스템/스토리지 동기화. DB 마이그레이션 전문 아님\n▸ AWS Direct Connect — 네트워크 연결 전용. 데이터 마이그레이션 도구 아님\n\n【정답 포인트】\n▸ \"SQL Server → RDS for SQL Server\" = 이질형 데이터베이스 마이그레이션\n▸ DMS는 Live Data Migration 지원 → 가동 중지 최소화\n▸ Change Data Capture(CDC) 기능으로 마이그레이션 후에도 변경사항 계속 동기화\n▸ DMS는 기존 마이그레이션 방법(수동 복사, 스크립트)보다 비용 절감\n\n【오답 체크】\n(A) Lambda — 코드 실행 서비스. 데이터베이스 마이그레이션 기능 없음\n(C) Direct Connect — 전용 네트워크 연결. 마이그레이션 도구 아님. 비용 높음\n(D) DataSync — 스토리지 동기화(NFS, SMB). 데이터베이스 마이그레이션 아님\n\n【시험 포인트】\n이질형 데이터베이스 마이그레이션 + 최소 다운타임 → AWS DMS (CDC 기반 실시간 동기화)",
  ...
  },
  {
    ...
    "answer": "AD",
    "explanation": "【핵심 용어】\n▸ AWS Glue 트리거 — Glue 작업의 실행 일정 및 조건 정의. 시간 기반 트리거 지원\n▸ AWS Glue 연결 — 데이터 소스(RDS, MongoDB) ↔ Glue 간 연결 설정\n▸ AWS Glue DataBrew — 데이터 정제 전문 도구 (별도 비용)\n▸ Redshift Data API — SQL 실행 용도. 데이터 로드 목적 아님\n\n【정답 포인트】\n▸ \"매시간 ETL 작업 실행\" → A(Glue 트리거) — Glue 네이티브 스케줄링\n▸ \"데이터 소스 ↔ Redshift 연결\" → D(Glue 연결) — RDS, MongoDB 접근 설정\n▸ A + D 조합 = 최소 운영 오버헤드 (Glue 내 모든 기능)\n\n【오답 체크】\n(B) DataBrew — 데이터 정제 도구이지만, 질문에 \"정제\" 요구사항 없음. 불필요한 추가 비용\n(C) Lambda 함수 — Glue 작업 호출 가능하지만, Glue 트리거보다 복잡. Lambda 추가 비용\n(E) Redshift Data API — SQL 쿼리 실행용. INSERT/COPY로 데이터 로드하는 것이 표준\n\n【시험 포인트】\nGlue 기반 매시간 ETL → Glue 트리거 + Glue 연결 (네이티브 오케스트레이션)",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Redshift 동시성 확장(Concurrency Scaling) — 쿼리 대기 큐가 쌓일 때 임시 클러스터로 확장\n▸ 워크로드 관리(WLM) — Redshift 쿼리 큐, 우선순위, 리소스 할당 관리\n▸ Redshift Serverless — 완전 서버리스 모드 (WLM 설정 다름)\n▸ RA3 노드 — Redshift 상위 노드 유형 (Redshift Managed Storage 지원)\n\n【정답 포인트】\n▸ Redshift 클러스터(프로비저닝형) + RA3 노드 → WLM 큐 레벨에서 동시성 확장 활성화\n▸ WLM의 특정 큐에 concurrency scaling을 적용하면, 그 큐의 쿼리 대기 시 자동 확장\n▸ RA3는 Redshift Managed Storage 기반으로 동시성 확장 효율적\n\n【오답 체크】\n(A) Serverless 워크그룹 — Serverless는 자동으로 스케일링됨. 별도 동시성 확장 설정 불필요\n(C) 클러스터 생성 중 설정 — 클러스터 생성 후 WLM 큐 설정에서 활성화 (생성 시 아님)\n(D) 일일 사용 할당량 — 동시성 확장과 무관. WLM 설정\n\n【시험 포인트】\nRedshift RA3 클러스터 동시성 확장 → WLM 큐 레벨 설정 (Serverless 아님)",
  ...
  },
  {
    ...
    "answer": "AB",
    "explanation": "【핵심 용어】\n▸ AWS Step Functions — 상태 머신 기반 오케스트레이션. Wait 상태로 긴 작업 폴링 가능\n▸ AWS Lambda — 짧은 작업용(15분 제한). Athena 쿼리 호출만 가능\n▸ AWS Glue Python Shell — Glue 작업. Athena 쿼리 오케스트레이션 전문 아님\n▸ Amazon MWAA — Apache Airflow 관리형. Athena 오케스트레이션 가능하지만 고비용\n\n【정답 포인트】\n▸ \"각 쿼리 15분 이상\" → Lambda 15분 제한 초과 가능\n▸ A(Lambda + start_query_execution) — Athena 쿼리 비동기 호출\n▸ B(Step Functions + Wait 상태 + get_query_execution) — 쿼리 완료 대기 후 다음 쿼리 호출\n▸ A + B 조합 = 비용 효율적 (Lambda + Step Functions 기본 비용만)\n\n【오답 체크】\n(C) Glue Python Shell — 가능하지만 Lambda보다 비용 높음 (DPU 사용료)\n(D) Sleep 타이머 폴링 — 비효율적. 5분마다 불필요한 처리. 장시간 작업 시 비용 증가\n(E) MWAA + AWS Batch — 과도한 오버헤드. 비용 높음. Step Functions로 충분\n\n【시험 포인트】\n장시간 Athena 쿼리 오케스트레이션 → Lambda + Step Functions (Wait 상태 폴링)",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Apache Spark, HBase, Flink — 대규모 분산 처리 프레임워크\n▸ Amazon EMR — 클러스터 기반 빅데이터 처리\n\n【정답 포인트】\n▸ 기존 프레임워크 지원 → EMR은 Spark, HBase, Flink 네이티브 지원\n▸ 페타바이트 규모 + 고성능 → EMR 클러스터의 확장성 활용\n▸ Serverless 호환 → EMR Serverless로 관리 오버헤드 최소화\n\n【오답 체크】\n(A) Glue는 서버리스 ETL이나 Spark/HBase 같은 복잡한 프레임워크 미지원, 중소 규모 처리에만 적합\n(C) Lambda는 페타바이트 처리 불가능, 타임아웃 제약\n(D) Redshift는 DW(데이터 웨어하우스)이지 ETL 엔진이 아님\n\n【시험 포인트】\n서버리스 + 기존 프레임워크 호환성 = EMR Serverless\n대규모 분산 처리 성능 요구 = EMR 클러스터 모드",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Detect PII Transform — Glue Studio의 내장 PII 검출 기능\n▸ Obfuscate — PII 난독화 처리\n▸ Step Functions — 데이터 파이프라인 오케스트레이션\n\n【정답 포인트】\n▸ 운영 노력 최소화 → Glue Studio의 네이티브 Detect PII 변환\n▸ PII 난독화 → Glue의 Obfuscate 변환으로 추가 코딩 불필요\n▸ 오케스트레이션 → Step Functions로 워크플로우 자동화\n\n【오답 체크】\n(A) Lambda에서 PII 감지 로직 수동 구현 필요 → 운영 노력 증가\n(C) Data Quality는 품질 규칙이지 난독화 기능 아님\n(D) DynamoDB 중간 저장소 추가 → 복잡성 증가, 불필요한 단계\n\n【시험 포인트】\n Glue Studio 네이티브 변환 = 최소 운영 오버헤드\n최소 코딩 = 기본 제공 기능 활용",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Step Functions — 서버리스 워크플로우 오케스트레이션\n▸ Glue + EMR 통합 — 다양한 ETL 엔진 작업 조정\n▸ 최소 운영 오버헤드 — 관리형 서버리스 선택\n\n【정답 포인트】\n▸ Glue/EMR 모두 지원 → Step Functions은 두 서비스 네이티브 통합\n▸ 서버리스 오케스트레이션 → 인프라 관리 불필요\n▸ 복잡한 DAG 처리 → Step Functions의 상태 머신으로 선언적 정의\n\n【오답 체크】\n(A) Glue 워크플로우는 Glue 작업 중심, EMR 통합 약함\n(C) Lambda는 오케스트레이션 아님, 스크립트 실행 도구\n(D) MWAA는 강력하지만 Airflow 서버 유지 필요 → 운영 오버헤드 증가\n\n【시험 포인트】\n최소 운영 오버헤드 = Step Functions (서버리스 상태 머신)\nGlue + EMR 통합 필요 = Step Functions Tasks로 해결",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ S3 Standard-IA — 다중 AZ 고가용성, 낮은 접근 빈도\n▸ S3 One Zone-IA — 단일 AZ, 최저 비용\n▸ S3 Glacier Flexible Retrieval — 중기 아카이빙\n▸ S3 Glacier Deep Archive — 장기 아카이빙, 최저 비용\n\n【정답 포인트】\n▸ 높은 가용성 유지 필요 → One Zone-IA 제외\n▸ 중기(6-24개월) 접근 → Standard-IA (다중 AZ)\n▸ 장기(2년+) 접근 → Deep Archive (최저 아카이브 비용)\n▸ 최악의 경우 연간 1회 접근 → Deep Archive 적합\n\n【오답 체크】\n(A) One Zone-IA는 단일 AZ 가용성 → 요구사항 불충족\n(B) Glacier Flexible Retrieval은 Deep Archive보다 비쌈 (2년+)\n(D) One Zone-IA 사용 → 고가용성 요구사항 위반\n\n【시험 포인트】\n높은 가용성 = Standard-IA/Glacier 계열 (One Zone-IA 제외)\n아카이빙 전략 = 접근 빈도에 따른 계층화 (Standard → IA → Glacier)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Redshift Data Sharing — 클러스터 간 데이터 공유 기능\n▸ Producer/Consumer 모델 — 리소스 격리된 데이터 공유\n▸ 컴퓨팅 리소스 격리 — ETL 작업 영향 없음\n\n【정답 포인트】\n▸ 중요 작업 무중단 → Data Sharing은 읽기 전용 공유\n▸ ETL 리소스 보호 → 데이터 복제 없이 메타데이터 공유\n▸ 관리 오버헤드 최소 → 자동 동기화\n\n【오답 체크】\n(B) \n(C) 직접 액세스는 ETL 클러스터 리소스 경쟁 → 성능 저하\n(D) 매주 언로드 → 수작업 반복, Spectrum 쿼리 느림\n\n【시험 포인트】\nRedshift Data Sharing = 클러스터 간 고성능 공유\n리소스 격리 + 동적 공유 = Consumer 클러스터의 독립적 쿼리",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Athena Federated Query — 여러 소스 통합 쿼리\n▸ 일회성 분석 — 단기 임시 작업\n▸ 프로비저닝 최소화 → 서버리스 쿼리 선택\n\n【정답 포인트】\n▸ 일회성 작업 → 프로비저닝 인프라 불필요\n▸ 다중 소스 조인 → Athena Federated Query로 가능\n▸ 비용 효율성 → 쿼리 기반 과금, 초기 비용 없음\n▸ 데이터 복제 불필요 → 각 소스에서 직접 접근\n\n【오답 체크】\n(A) EMR 클러스터 프로비저닝 비용 발생 (일회성에 부담)\n(B) 데이터 복제 과정 추가, S3 스토리지 비용\n(D) Spectrum은 S3 데이터 중심, DynamoDB/RDS는 어려움\n\n【시험 포인트】\n일회성 + 다중 소스 = Athena Federated Query\n최소 프로비저닝 = 서버리스 쿼리 엔진",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 AWS Glue(완전관리형 ETL 서비스), Apache Parquet(컬럼 기반 저장 형식), 운영 효율(Operational Efficiency)\n\n【정답 포인트】 선택지 A가 정답입니다. AWS Glue는 AWS에서 제공하는 완전관리형 ETL 서비스로, SQL Server 데이터베이스에서 직접 뷰를 통해 조인된 데이터에 접근할 수 있으며, Parquet 형식으로의 자동 변환을 지원합니다. EC2 기반 데이터베이스에 조인 결과를 포함하는 뷰를 생성한 후 AWS Glue ETL 작업으로 이 뷰를 직접 쿼리하여 S3로 전송하는 구조는 추가 변환 로직 없이 가장 간단합니다. Glue의 내장 스케줄러로 매일 자동 실행 가능합니다.\n\n【오답 체크】 B는 SQL Server Agent로 CSV 추출 후 Lambda를 사용하여 Parquet 변환을 진행하므로 복잡하고, CSV에서 Parquet 변환의 성능 오버헤드가 발생합니다. C는 불필요한 Glue 크롤러를 추가하여 실행 시간을 연장하고 추가 비용이 발생합니다. D는 Lambda의 메모리 제약(최대 10GB)과 15분 타임아웃으로 대용량 데이터 처리에 부적합하며, JDBC 연결의 네트워크 오버헤드가 병목입니다.\n\n【시험 포인트】 AWS Glue는 네이티브 Parquet 지원으로 ETL 파이프라인에 최적화되어 있습니다. 일일 배치 작업의 스케줄링은 Glue의 내장 스케줄러로 처리 가능하며, 빅 데이터 처리의 탄력성과 비용 효율성이 우수합니다. DMS 대안으로도 고려될 수 있는 강력한 솔루션입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 Amazon Redshift 시스템 테이블(STL_ALERT_EVENT_LOG), 성능 이상(Performance Anomalies), 쿼리 옵티마이저\n\n【정답 포인트】 선택지 B가 정답입니다. STL_ALERT_EVENT_LOG는 Amazon Redshift의 시스템 테이블로, 쿼리 옵티마이저가 성능 문제를 감지했을 때 경고 이벤트를 자동으로 기록합니다. 누락된 조인 조건, 병렬화 불가능한 연산, 스캔 효율성 저하, 메모리 부족 등 성능 최적화와 관련된 모든 이상 현상을 추적하며, 이를 통해 성능 저하의 근본 원인을 사전에 파악할 수 있습니다.\n\n【오답 체크】 A의 STL_USAGE_CONTROL은 리소스 사용률을 제어하고 WLM(Workload Management) 정책을 관리하는 테이블로, 성능 이상 감지와는 무관합니다. C의 STL_QUERY_METRICS는 쿼리 실행 후 메트릭을 기록하지만 경고 이벤트가 아니므로 사전 감지가 불가능합니다. D의 STL_PLAN_INFO는 단순 쿼리 실행 계획만 저장하므로 성능 문제 해석이 어렵습니다.\n\n【시험 포인트】 Redshift 모니터링에서 STL_ALERT_EVENT_LOG의 사전 감지 기능이 핵심입니다. 정기적인 모니터링을 통해 성능 저하의 근본 원인(테이블 통계 부재, 부적절한 정렬 키, 누락된 인덱스)을 파악할 수 있으며, 이는 데이터 웨어하우스의 운영 효율을 크게 향상시킵니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】 Apache Parquet(컬럼 기반 저장 형식), 컬럼 프루닝(Column Pruning), Amazon Athena 비용 최적화\n\n【정답 포인트】 선택지 D가 정답입니다. Parquet은 컬럼 기반 저장 형식으로, 필요한 컬럼만 읽을 수 있는 특성이 있습니다. 15개 컬럼 중 1~2개만 쿼리하는 사용 사례에서 Parquet의 컬럼 프루닝 기능으로 Athena 스캔 비용을 85~90% 절감할 수 있습니다. AWS Glue ETL 작업으로 CSV를 Parquet으로 변환하여 저장하면, Athena 쿼리 성능이 비약적으로 개선되고 비용도 극적으로 감소합니다.\n\n【오답 체크】 A의 CSV 형식은 컬럼 선택 최적화가 불가능하므로 항상 전체 행을 스캔해야 하여 비용이 높습니다. B의 JSON도 행 지향 처리 특성으로 특정 컬럼 필터링 효율이 떨어지며 CSV보다 파일 크기가 큽니다. C의 Apache Avro는 행 기반 직렬화 형식으로 컬럼 프루닝을 지원하지 않으므로 Parquet보다 비효율적입니다.\n\n【시험 포인트】 데이터 레이크 구축에서 저장 형식 선택은 비용과 성능을 좌우하는 결정적 요소입니다. 분석 쿼리 패턴이 특정 컬럼에만 집중하는 경우, Parquet 활용이 비용 최적화의 핵심 전략입니다. Athena의 스캔 비용 모델(GB당 $5 기준)에서 Parquet 사용 시 ROI가 매우 높습니다.",
  ...
  },
  {
    ...
    "answer": "BD",
    "explanation": "【핵심 용어】 AWS Lake Formation, 세분화된 액세스 제어(Fine-Grained Access Control), 데이터 필터(Data Filters)\n\n【정답 포인트】 정답은 B와 D입니다. S3 경로를 AWS Lake Formation 위치로 등록\n(B) 한 후, Lake Formation의 세분화된 액세스 제어를 활성화\n(D) 하여 리전별 데이터 필터를 설정하면, 중앙 집중식으로 각 IAM 역할의 액세스를 제어할 수 있습니다. 이는 개별 IAM 정책을 수정하지 않고도 Lake Formation 콘솔에서 통합 관리 가능하므로 운영 오버헤드가 최소화됩니다.\n\n【오답 체크】 A는 필터만 설정하고 위치 등록을 하지 않아 Lake Formation의 거버넌스 체계가 작동하지 않으므로 불완전합니다. C는 각 IAM 역할을 개별적으로 수정해야 하므로 운영 오버헤드가 높고 확장성이 떨어집니다. E는 리전별로 별도의 S3 버킷을 생성 및 관리해야 하므로 관리 복잡도와 운영 비용이 급격히 증가합니다.\n\n【시험 포인트】 Lake Formation은 데이터 레이크 거버넌스의 핵심 서비스로, 위치 등록과 세분화된 액세스 제어의 조합으로 확장 가능하고 유지보수가 용이한 액세스 관리를 구현합니다. 다중 계정 환경에서의 권한 관리 효율성이 높습니다.",
  ...
  },
  {
    ...
    "answer": "BD",
    "explanation": "【핵심 용어】 AWS Step Functions, IAM 권한(IAM Permissions), VPC 네트워크 보안(Security Groups)\n\n【정답 포인트】 정답은 B와 D입니다. Step Functions가 EMR 작업을 실행하지 못하는 경우 두 가지 핵심 원인을 확인해야 합니다. 첫째, Step Functions의 IAM 역할이 EMR 작업 생성과 S3 버킷 접근 권한을 가져야 하며, Access Analyzer를 활용하여 S3 접근 속성을 검증\n(B) 해야 합니다. 둘째, VPC의 보안 그룹이 EMR 클러스터의 데이터 소스 접근을 허용해야 하며, VPC 흐름 로그를 쿼리하여 네트워크 연결성을 검증\n(D) 해야 합니다.\n\n【오답 체크】 A는 배포 자동화와 알림 설정으로, 실제 문제 해결보다는 사후 관리에 초점이 있습니다. C는 EMR on EKS로의 아키텍처 변경을 제시하는데, 현재 EMR 환경의 문제 원인을 파악하지 못한 상태에서의 마이그레이션은 적절하지 않습니다. E는 재시도 정책과 SNS 알림 설정으로 근본 원인을 해결하지 못합니다.\n\n【시험 포인트】 데이터 파이프라인 오케스트레이션 문제 해결의 핵심은 IAM 권한 체인과 네트워크 연결성 검증입니다. Access Analyzer와 VPC Flow Logs는 엔드투엔드 디버깅의 필수 도구이며, 이를 통해 체계적인 문제 해결이 가능합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】 AWS Database Migration Service(DMS), Change Data Capture(CDC), 점진적 마이그레이션(Incremental Migration)\n\n【정답 포인트】 정답은 A와 C입니다. AWS DMS의 CDC 기능으로 초기 전체 로드 이후 증분 변경사항만 캡처하여 전송합니다\n(A) . 마이그레이션 중 지속적인 복제를 지원하려면 DMS 작업의 Ongoing replication 옵션을 활성화하고, 마이그레이션 완료 후 cutover 전까지 데이터 일관성을 검증해야 합니다\n(C) . 이 조합으로 RDS로의 부드럽고 안정적인 전환이 가능하며, 다운타임을 최소화할 수 있습니다.\n\n【오답 체크】 B는 전체 데이터를 매일 재로드하는 방식으로 비효율적이며, 대량 데이터의 반복 전송으로 네트워크 대역폭을 낭비합니다. D는 기존 복제 인스턴스의 설정이 CDC에 최적화되지 않았을 수 있으므로 신뢰할 수 없습니다. E는 검증 단계를 생략하여 데이터 손실이나 불일치의 위험이 높습니다.\n\n【시험 포인트】 DMS는 제로 다운타임 마이그레이션의 핵심 서비스로, CDC를 통한 점진적 복제 전략이 운영 가용성을 보장합니다. 마이그레이션 작업의 배치 설정과 검증 정책이 데이터 무결성 확보의 핵심 요소입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 AWS Data Exchange, SaaS 데이터 공급자, AWS Glue 카탈로그 통합\n\n【정답 포인트】 선택지 C가 정답입니다. AWS Data Exchange를 통해 구독한 데이터셋을 AWS Glue 카탈로그에 자동으로 등록할 수 있으며, 이는 데이터 레이크의 메타데이터 관리를 통합합니다. Data Exchange의 자산 연동 기능으로 외부 데이터 공급자의 데이터를 Glue 작업에서 바로 사용할 수 있어 메타데이터 동기화 오버헤드가 없습니다. 공급자의 데이터 업데이트가 자동으로 반영됩니다.\n\n【오답 체크】 A는 수동 카탈로그 등록으로 메타데이터 갱신이 번거롭고 자동화 수준이 낮습니다. B는 S3 복제본을 별도로 관리해야 하므로 저장소 비용 증가와 데이터 동기화 복잡도가 높습니다. D는 Data Exchange 활용을 생략하여 공급자의 정기 업데이트를 놓칠 수 있고 수동 갱신 부담이 발생합니다.\n\n【시험 포인트】 AWS Data Exchange는 데이터 마켓플레이스 활용 시 Glue 카탈로그와의 통합이 데이터 거버넌스를 단순화합니다. 구독 기반 데이터 활용 모델에서 자동 메타데이터 갱신이 운영 효율과 데이터 신선도를 동시에 향상시킵니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】 Amazon Kinesis Data Streams, 실시간 데이터 처리, 온디맨드 용량 모드(On-Demand Capacity Mode)\n\n【정답 포인트】 정답은 B와 D입니다. Kinesis의 온디맨드 용량 모드\n(B) 를 활성화하면 처리량이 급변해도 자동으로 스케일링되어 수동 샤드 관리가 불필요합니다. 실시간 스트림 모니터링을 위해 CloudWatch 메트릭(Iterator Age, GetRecords 레이턴시, 샤드 병목 지표)을 설정하고\n(D) , 이를 기반으로 성능 병목을 식별하고 예방합니다.\n\n【오답 체크】 A의 프로비저닝 모드는 고정 샤드 수를 수동으로 관리해야 하므로 비효율적입니다. C는 개별 샤드 모니터링보다 전체 스트림의 성능 지표가 더 중요하고 의미 있습니다. E는 재시도 정책 조정으로 근본적인 throughput 부족 문제를 해결하지 못합니다.\n\n【시험 포인트】 Kinesis의 온디맨드 모드는 변동하는 처리량 대응에 최적화되어 있으며, 용량 계획 부담을 제거합니다. 적절한 CloudWatch 메트릭 모니터링으로 성능 저하를 조기에 감지하는 것이 실시간 데이터 처리의 안정성을 확보하는 핵심입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 Apache Spark Adaptive Query Execution, 동적 파티션 프루닝(Dynamic Partition Pruning), 성능 최적화\n\n【정답 포인트】 정답은 A와 C입니다. Spark의 Adaptive Query Execution\n(A) 은 런타임에 통계를 수집하여 실행 계획을 동적으로 최적화하므로, 조인 순서와 브로드캐스트 배포 방식을 자동으로 개선합니다. Dynamic Partition Pruning\n(C) 을 활성화하면 필터 조건 기반으로 불필요한 파티션 스캔을 사전에 제거하여 I/O를 크게 절감합니다.\n\n【오답 체크】 B의 수동 파티션 관리는 유지보수 부담이 높고 실시간 데이터 분포 변화에 동적으로 응답하지 못합니다. D는 전체 데이터 리샤플링으로 네트워크 오버헤드가 급격히 증가합니다. E는 정렬 최적화만으로는 복잡한 조인 성능 병목을 근본적으로 해결하지 못합니다.\n\n【시험 포인트】 Spark의 Adaptive Execution과 Dynamic Partition Pruning은 대규모 데이터 조인 성능 최적화의 핵심 기법입니다. 동적 최적화 활성화는 데이터 분포의 불균형을 자동으로 보정하여 안정적인 성능을 제공합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 Amazon EMR, 스팟 인스턴스(Spot Instances), 비용 최적화 전략(Cost Optimization Strategy)\n\n【정답 포인트】 선택지 C가 정답입니다. EMR에서 스팟 인스턴스를 활용하면 온디맨드 인스턴스 대비 70~90% 비용 절감이 가능합니다. 마스터 노드는 온디맨드로 유지하여 클러스터 안정성과 상태 정보 보존을 보장하고, 코어 및 태스크 노드는 스팟으로 구성하면 작업 중단 시 데이터 손실 위험을 최소화하면서도 비용을 극적으로 절감할 수 있습니다. 마스터 노드의 온디맨드 유지가 전체 클러스터의 복원력을 확보합니다.\n\n【오답 체크】 A는 온디맨드 인스턴스만 사용하여 비용 절감 효과가 전혀 없습니다. B는 스팟 인스턴스의 불안정성으로 장시간 배치 작업에 부적합하며 데이터 손실 위험이 높습니다. D는 인스턴스 유형 변경보다 가격 모델(온디맨드 vs 스팟) 선택이 비용에 훨씬 더 큰 영향을 미칩니다. 인스턴스 타입 최적화는 이미 선행되어야 합니다.\n\n【시험 포인트】 EMR에서 스팟 인스턴스 전략은 비용 최적화의 핵심입니다. 마스터-코어-태스크의 세 계층 구조에서 각 계층의 특성에 맞는 용량 모드 선택이 안정성과 경제성을 동시에 달성합니다. 장기 비용 절감 효과가 매우 크며, AWS 베스트 프랙티스의 표준 권장사항입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 AWS Glue Schema Registry, 데이터 품질 관리, 스키마 진화(Schema Evolution)\n\n【정답 포인트】 선택지 B가 정답입니다. Glue Schema Registry는 스키마 버전 관리와 호환성 검증을 중앙에서 처리하며, 프로듀서와 컨슈머 간의 스키마 일관성을 자동으로 강제합니다. 호환성 검사(backward/forward compatible) 설정으로 스키마 변경 시 기존 데이터와의 호환성을 보장합니다. 직렬화된 데이터에 스키마 레지스트리 ID를 포함시켜 버전 관리를 자동화합니다.\n\n【오답 체크】 A는 스키마 문제를 Glue Job의 예외 처리로만 관리하여 근본적인 데이터 품질 관리가 불가능합니다. C는 카탈로그에 스키마 메타데이터만 저장하고 호환성 검증을 하지 않으므로 불완전합니다. D는 Glue 크롤러의 자동 스키마 감지로 인한 예상치 못한 스키마 변경과 그에 따른 관리 부작용이 발생합니다.\n\n【시험 포인트】 Glue Schema Registry는 데이터 품질 관리의 필수 도구로, 다양한 데이터 소스의 스키마 통일성을 보장합니다. 스키마 버전 관리로 마이그레이션 과정에서의 데이터 손상을 사전에 방지하고, 데이터 계약(data contract) 개념을 구현합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】 Amazon Redshift Spectrum, S3 외부 데이터 쿼리, 비용 효율성(Cost-Efficiency)\n\n【정답 포인트】 선택지 A가 정답입니다. Redshift Spectrum을 활용하면 S3에 저장된 콜드 데이터를 Redshift에 로드하지 않고도 직접 쿼리할 수 있습니다. 데이터 이동 비용 없이 S3의 Parquet 파일을 쿼리하고, 필요한 결과만 Redshift로 반환되므로 네트워크 대역폭과 스토리지 비용을 크게 절감할 수 있습니다. 외부 스키마 생성으로 투명한 쿼리 처리가 가능합니다.\n\n【오답 체크】 B는 데이터를 모두 Redshift로 로드해야 하므로 스토리지 비용과 I/O 오버헤드가 급격히 증가합니다. C는 S3 Select로 단순 필터링만 가능하고 복잡한 분석 쿼리와 조인이 불가능합니다. D는 Glue를 거친 변환 단계가 추가되어 불필요한 처리와 비용이 발생합니다.\n\n【시험 포인트】 Redshift Spectrum은 데이터 웨어하우스의 핫/콜드 데이터 분리 아키텍처를 구현합니다. 자주 조회되지 않는 역사 데이터는 S3에 저장하고 필요시에만 쿼리하는 비용 최적화 전략의 핵심 서비스입니다. 장기 데이터 보관 비용을 크게 절감합니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 AWS Glue Data Brew, 데이터 프로파일링(Data Profiling), 데이터 품질 검증(Data Quality Validation)\n\n【정답 포인트】 정답은 A와 C입니다. Data Brew의 프로파일 작업\n(A) 으로 데이터 분포, 결측값, 이상치, 데이터 타입 불일치를 자동으로 식별합니다. 이후 품질 규칙 기반 필터링\n(C) 으로 불완전한 레코드를 자동으로 제거하여 데이터 품질을 보장합니다. 이 조합으로 수동 검증 부담 없이 자동화된 데이터 정제가 가능하며, 반복 가능한 품질 표준을 구현할 수 있습니다.\n\n【오답 체크】 B는 샘플 기반 검증으로 전체 데이터의 품질을 보장하지 못하고 이상치를 놓칠 수 있습니다. D는 Glue Job의 수동 코딩으로 개발과 유지보수 부담이 높습니다. E는 기본 통계 함수만으로는 복잡한 데이터 패턴 분석과 이상 감지가 어렵습니다.\n\n【시험 포인트】 Glue Data Brew는 비즈니스 사용자도 쉽게 데이터 품질을 관리할 수 있도록 설계된 노코드 도구입니다. 프로파일링과 규칙 기반 필터링의 조합으로 데이터 정제의 자동화도를 높이고 데이터 거버넌스를 강화합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 AWS Lake Formation, 크로스 계정 공유(Cross-Account Resource Sharing), 데이터 거버넌스\n\n【정답 포인트】 선택지 B가 정답입니다. Lake Formation의 리소스 공유 기능으로 다른 AWS 계정의 사용자에게 S3 기반 데이터에 대한 세분화된 액세스 권한을 부여할 수 있습니다. 이는 각 계정의 IAM 정책을 개별적으로 관리하는 것보다 훨씬 간단하며, Lake Formation이 모든 액세스 제어를 중앙에서 처리하여 일관성을 보장합니다. 권한 관리의 복잡도가 크게 감소합니다.\n\n【오답 체크】 A는 개별 S3 버킷 정책 관리로 확장성이 떨어지고 정책 충돌 위험이 있습니다. C는 권한을 위임함으로써 중앙 거버넌스의 통제가 약화되고 감시 추적이 어려워집니다. D는 복제본을 유지해야 하므로 데이터 동기화 문제와 추가 비용이 발생합니다.\n\n【시험 포인트】 Lake Formation의 크로스 계정 리소스 공유는 조직 내 다중 계정 아키텍처에서 거버넌스를 단순화합니다. 중앙 집중식 권한 관리로 보안과 규정 준수를 동시에 달성하며, 운영 복잡도를 크게 줄입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】 Apache Spark RDD(Resilient Distributed Dataset), 데이터 캐싱, 메모리 최적화\n\n【정답 포인트】 선택지 D가 정답입니다. Spark RDD의 persist() 메서드로 자주 사용되는 중간 데이터셋을 메모리에 캐싱하면, 반복되는 연산 시 디스크 I/O를 크게 줄일 수 있습니다. 캐싱 저장소로 MEMORY_AND_DISK를 선택하면 메모리 부족 시 자동으로 디스크로 스필(spill)되므로 OOM 오류 위험을 제거하면서도 성능을 향상시킵니다.\n\n【오답 체크】 A는 재분할로 네트워크 오버헤드가 증가하고 파티션 수 최적화만으로는 반복 연산 성능 개선이 미미합니다. B는 파티션 병합으로 병렬 처리 효율이 감소하므로 전체 성능이 악화됩니다. C는 문제 분석 없이 팩터만 증가시켜 메모리 압력을 높일 뿐 근본 해결이 아닙니다.\n\n【시험 포인트】 Spark의 캐싱 전략은 메모리와 디스크의 효율적 활용을 통해 반복 연산 성능을 극대화합니다. 적절한 저장소 레벨 선택이 메모리 오버헤드와 성능의 균형을 맞추는 핵심이며, DAG 최적화의 중요한 부분입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 Amazon S3 Intelligent-Tiering, 자동 비용 최적화, 저장소 클래스 전환(Storage Class Transitions)\n\n【정답 포인트】 정답은 B와 D입니다. S3 Intelligent-Tiering\n(B) 은 접근 패턴을 자동으로 분석하여 저장소 클래스를 전환하므로 설정 후 운영 개입이 최소화됩니다. 라이프사이클 정책으로 오래된 데이터를 Glacier로 자동 전환하고\n(D) , 선택적으로 Archive 및 Deep Archive 계층을 추가 설정하여 장기 보관 비용을 더욱 절감합니다.\n\n【오답 체크】 A는 수동 전환으로 운영 오버헤드가 높고 일관성 유지가 어렵습니다. C는 정책 설정만으로 자동 전환이 이루어지지 않으므로 불충분합니다. E는 비용 분석 없이 삭제 정책을 적용하여 규제 준수나 감사 요구사항 위반으로 데이터 손실 위험이 있습니다.\n\n【시험 포인트】 S3 저장소 최적화는 데이터 레이크의 장기 운영 비용을 좌우하는 핵심 요소입니다. Intelligent-Tiering과 라이프사이클 정책의 조합으로 자동화된 비용 절감을 구현하며, 규정 준수 요구사항도 함께 충족합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】 AWS Glue 크롤러(Crawler), 자동 스키마 감지(Schema Discovery), 메타데이터 카탈로그 관리\n\n【정답 포인트】 선택지 A가 정답입니다. Glue 크롤러는 S3의 데이터 구조를 자동으로 스캔하여 스키마를 감지하고 Glue 카탈로그에 메타데이터를 등록합니다. 이를 통해 수동 메타데이터 정의 작업을 제거하고, Amazon Athena, Redshift Spectrum 등의 쿼리 엔진이 바로 사용할 수 있는 테이블 구조를 생성합니다. 정기적 실행으로 스키마 변경을 지속적으로 추적합니다.\n\n【오답 체크】 B는 스키마 검사만으로 카탈로그 등록이 자동되지 않으므로 불완전합니다. C는 수동 JSON 스키마 정의로 자동화 효과가 없고 유지보수 부담이 높습니다. D는 Lambda로 별도 변환 로직을 개발해야 하므로 불필요한 복잡도를 증가시킵니다.\n\n【시험 포인트】 Glue 크롤러는 데이터 거버넌스의 시작점으로, 자동 메타데이터 발견을 통해 데이터 카탈로그 관리의 자동화도를 높입니다. 정기적인 크롤러 실행으로 스키마 변경을 추적하고 다운스트림 쿼리의 안정성을 보장합니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】 Amazon Kinesis Data Firehose, 버퍼링(Buffering), 배치 처리 최적화(Batch Optimization)\n\n【정답 포인트】 선택지 B가 정답입니다. Kinesis Firehose의 버퍼 설정(크기 또는 시간)을 조정하여 S3로 전달되는 배치 크기를 최적화합니다. 버퍼 크기를 128MB 이상으로 설정하고 버퍼 시간을 300초 이상으로 설정하면, 리전별 S3 PUT 요청 수를 크게 줄여 API 호출 비용을 극적으로 절감할 수 있습니다. S3의 요청 가격 모델(PUT 요청당 $0.005)에서 큰 효과를 발휘합니다.\n\n【오답 체크】 A는 Firehose의 용도와 맞지 않으며, 수동 Lambda 처리로 복잡도가 증가하고 신뢰성이 감소합니다. C는 데이터 압축 설정으로 네트워크 대역폭만 줄일 뿐 PUT 요청 수에는 영향이 없습니다. D는 파티션 설정의 잘못된 적용으로 성능 개선이 미미하고 쿼리 효율성이 악화될 수 있습니다.\n\n【시험 포인트】 Kinesis Firehose는 높은 처리량의 스트림 데이터를 S3로 안정적으로 전달하는 매니지드 서비스입니다. 버퍼 파라미터 튜닝으로 비용과 레이턴시의 균형을 맞추는 것이 운영 효율의 핵심입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】 AWS DataSync, 데이터 마이그레이션(Data Migration), 온프레미스-클라우드 연결(On-Premises to Cloud)\n\n【정답 포인트】 정답은 A와 C입니다. AWS DataSync\n(A) 는 온프레미스 저장소와 AWS 간의 대규모 데이터 마이그레이션을 가속화하는 매니지드 서비스입니다. 자동 대역폭 최적화와 데이터 암호화로 안전성을 보장합니다. Amazon S3, EFS, FSx 등 다양한 AWS 저장소로 직접 전송 가능하므로\n(C) , 유연한 아키텍처 구성이 가능합니다. 자동 검증 및 재시도 메커니즘으로 데이터 무결성을 확보합니다.\n\n【오답 체크】 B의 Direct Connect만으로는 데이터 마이그레이션 소프트웨어를 별도로 개발하고 운영해야 합니다. D는 S3 전송 가속 기능만으로는 온프레미스 소스 최적화가 부족하고 프로토콜 변환이 필요합니다. E는 Snowball로 초기 로드 후 DataSync가 불필요하다는 주장은 증분 동기화와 지속적 복제 시나리오에서 부정확합니다.\n\n【시험 포인트】 DataSync는 하이브리드 클라우드 환경에서 대규모 데이터 이동의 표준 솔루션입니다. 자동화된 검증과 재시도 메커니즘으로 신뢰할 수 있는 마이그레이션을 보장하며, 대역폭 최적화로 전송 시간을 단축합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Glue Data Catalog — 중앙화된 메타데이터 저장소, Hive metastore 자동 수집 가능\n▸ EMR — Hive metastore 호스팅용 컴퓨팅, 메타데이터 관리용 아님\n▸ Ranger — 접근 제어(보안), 메타데이터 저장소 아님\n\n【정답 포인트】\n▸ 다중 쿼리 엔진(EMR Hive, Athena) 지원 → Glue Data Catalog 필수\n▸ Hive metastore 자동 수집(크롤러 기능) → 개발 노력 최소화\n▸ 중앙 메타데이터 저장소 = Glue Data Catalog의 핵심 기능\n\n【오답 체크】\n(A) Ranger는 권한 관리 도구이며 메타데이터 저장소 아님\n(B) Hive metastore는 EMR 내부용이며 외부 Athena 통합 제한적\n(D) RDS는 메타데이터 수집 자동화 불가, 수동 ETL 필요\n\n【시험 포인트】\n다중 소스 메타데이터 통합 → Glue Data Catalog (표준)\n기존 Hive 메타데이터 활용 → 자동 크롤러 + 마이그레이션 지원",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Lake Formation — 행/열 수준 접근 제어, 다중 쿼리 엔진 통합\n▸ 행 수준 접근(Row-level) — 특정 레코드만 노출\n▸ 열 수준 접근(Column-level) — 특정 컬럼만 노출\n\n【정답 포인트】\n▸ Lake Formation은 Athena, Redshift Spectrum, EMR Hive 모두 지원\n▸ 세분화된 접근 제어(행/열 수준) 기본 기능\n▸ 데이터 필터링 자동 적용 → 운영 오버헤드 최소화\n\n【오답 체크】\n(A) S3 정책은 객체 수준만 지원, 행/열 수준 불가\n(B) Ranger는 EMR 전용, 외부 쿼리 엔진(Athena, Spectrum) 미지원\n(C) Redshift는 데이터 레이크 스토리지가 아님, 별도 DW 관리 필요\n\n【시험 포인트】\nS3 + 행/열 접근 제어 → Lake Formation (표준)\n다중 쿼리 엔진 통합 → Lake Formation 필수\n최소 운영 오버헤드 → 자동화된 접근 제어 프레임워크",
  ...
  },
  {
    ...
    "answer": "CE",
    "explanation": "【핵심 용어】\n▸ Parquet — 컬럼 형식, 선택적 블록 읽기 지원\n▸ JSON — 행 형식, 문서 수준 처리\n▸ 파티션 프루닝 — 쿼리 범위 외 데이터 스킵\n\n【정답 포인트】\n(C) 같은 리전 배치 → 네트워크 레이턴시 제거, 비용 절감\n(E) Parquet 형식 → 컬럼 선택적 읽기, 데이터 전송량 감소\n▸ 두 가지 모두 쿼리 성능 개선 (I/O + 네트워크)\n\n【오답 체크】\n(A) 무작위 프리픽스는 분산 시스템에서 처리량 증대하지만, 쿼리 성능 미향상\n(B) 같은 계정은 기본 요구사항이며 최적화 효과 없음\n(D) JSON은 텍스트 형식, 선택적 읽기 불가, Parquet보다 큼\n\n【시험 포인트】\nAthena 성능 최적화 = 지역 배치 + 컬럼 형식 (2가지 조합)\nParquet이 CSV/JSON보다 효율적 (컬럼 프루닝 가능)",
  ...
  },
  {
    ...
    "answer": "AD",
    "explanation": "【핵심 용어】\n▸ Performance Insights — 데이터베이스 성능 모니터링, 병목 쿼리 식별\n▸ CPU 사용률 최적화 — 쿼리 최적화 vs 인스턴스 확장\n\n【정답 포인트】\n(A) Performance Insights로 병목 쿼리 식별 및 최적화 → 근본 원인 해결\n(D) 더 큰 인스턴스로 확장 → CPU 처리량 증대, 즉각적 개선\n▸ 쓰기 위주 워크로드 + 높은 CPU → 쿼리 효율성 문제 + 리소스 부족\n\n【오답 체크】\n(B) 테이블/인덱스 추가는 쓰기 성능 저하, CPU 증가 위험\n(C) 주간 재부팅은 캐시 초기화, 성능 악화 가능\n(E) 캐싱은 읽기 최적화인데 워크로드가 쓰기 위주 → 제한적 효과\n\n【시험 포인트】\nRDS 성능 문제 = Performance Insights + 인스턴스 확장 조합\n워크로드 분석 후 대응 (쓰기 위주면 쿼리 최적화 우선)",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ VACUUM — Redshift 디스크 정리, 삭제 행 제거\n▸ REINDEX — 인터리브 정렬 키 재구성\n▸ 인터리브 정렬 키 — 다중 컬럼 정렬 최적화\n\n【정답 포인트】\n▸ VACUUM REINDEX → 삭제 행 제거 + 정렬 키 재구성\n▸ 인터리브 정렬 키는 REINDEX로만 최적화 가능\n▸ 두 요구사항 모두 충족: 디스크 공간 회수 + 정렬 키 분석 가능\n\n【오답 체크】\n(A) FULL은 모든 행 재정렬하지만 인터리브 키 분석 미지원\n(B) DELETE ONLY는 삭제 행만 제거, 정렬 키 재구성 없음\n(D) SORT ONLY는 존재하지 않는 명령어\n\n【시험 포인트】\nREDSHIFT VACUUM 변형:\n- VACUUM FULL: 전체 재정렬\n- VACUUM DELETE ONLY: 삭제 행만 정리\n- VACUUM REINDEX: 정렬 키 재구성\n- 인터리브 정렬 키 최적화 = REINDEX 필수",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Kinesis Data Streams — 실시간 스트리밍, 낮은 지연시간\n▸ DynamoDB — NoSQL, 밀리초 지연시간, 중첩 JSON 지원\n▸ 거의 실시간(Near real-time) — 마이크로초~밀리초 범위\n\n【정답 포인트】\n▸ Kinesis + DynamoDB = 완전 관리형, 운영 오버헤드 최소\n▸ DynamoDB는 중첩 JSON 네이티브 지원 (Document DB 특성)\n▸ DynamoDB 지연시간 < 10ms (on-demand 모드)\n▸ Serverless 솔루션, 자동 확장\n\n【오답 체크】\n(A) Kafka는 자체 관리, 운영 오버헤드 높음 + S3는 10ms 쿼리 불가능\n(B) Lambda는 배치 처리 적합, 실시간 수집에 부적절\n(D) SQS+RDS는 지연시간 10ms 달성 어려움, 운영 복잡\n\n【시험 포인트】\n실시간 스트리밍 + NoSQL 쿼리 = Kinesis + DynamoDB\n중첩 JSON 저장소 = DynamoDB (RDBMS 아님)\n10ms 이하 지연시간 = DynamoDB 필수",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Lake Formation — S3 데이터 레이크 통합 접근 제어\n▸ 데이터 필터 — Lake Formation의 행/열 수준 제한\n▸ PII 보호 — 세분화된 접근 제어 필수\n\n【정답 포인트】\n▸ Lake Formation 데이터 필터 = PII 열 단위 마스킹 자동 지원\n▸ IAM 역할 기반 접근 정책 = 자동 적용\n▸ Athena 통합 = 투명한 필터링\n▸ 최소 노력 = 기본 제공 기능 활용\n\n【오답 체크】\n(B) QuickSight는 시각화 도구, Athena 백단 필터링 미지원\n(C) 커스텀 UI 구축 = 개발 노력 높음, 권장하지 않음\n(D) IAM 정책만으로는 열 수준 필터링 자동화 불가\n\n【시험 포인트】\nS3 데이터 레이크 + PII 접근 제어 = Lake Formation 필수\n다중 그룹 PII 격리 = 데이터 필터 (표준 패턴)\nAthena와의 투명한 통합 = Lake Formation 장점",
  ...
  },
  {
    ...
    "answer": "BD",
    "explanation": "【핵심 용어】\n▸ AWS Glue Crawler — 스키마 자동 감지\n▸ AWS Glue Workflow — 작업 간 의존성 관리\n▸ EventBridge — 시간 기반/이벤트 기반 트리거\n\n【정답 포인트】\n(B) EventBridge(15분 정기) + Workflow(크롤러→작업)\n▸ 크롤러가 스키마 변경 자동 감지\n▸ 워크플로우는 크롤러 완료 후 작업 실행 → 순차 처리\n▸ 정기적 실행 + 스키마 적응성 모두 충족\n(D) Lambda(S3 이벤트) + Workflow(크롤러→작업)\n▸ S3 파일 업로드 시 즉시 트리거\n▸ 워크플로우가 크롤러와 작업 오케스트레이션\n▸ 이벤트 기반 + 스키마 자동 감지\n\n【오답 체크】\n(A) EventBridge 일정은 OK이나 크롤러 없음 → 스키마 변경 미감지\n(C) 여러 Lambda 함수 체인 → 복잡성 높음, 오류 처리 어려움\n(E) Firehose는 배치용, 15분 정기 처리에 부적절\n\n【시험 포인트】\n스키마 변경 대응 = Glue Crawler 필수\n정기/이벤트 기반 선택지 = B(정기), D(이벤트)\nGlue Workflow가 Crawler+Job 의존성 관리",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ 쿼리 결과 재사용 — Athena의 결과 캐싱, 비용 절감\n▸ 1시간 새로고침 — 1시간 이내 재사용 가능한 결과 활용\n▸ 추가 인프라 — ElastiCache 같은 별도 서비스 불가\n\n【정답 포인트】\n▸ Athena 쿼리 결과 자동 캐싱 (기본 기능)\n▸ 1시간 내 같은 쿼리 재실행 → 캐시 히트\n▸ 데이터는 1일 1회 업데이트 → 결과 재사용 효과 높음\n▸ 추가 비용/인프라 없음\n\n【오답 체크】\n(A) Glacier는 쿼리 성능 미관련, 검색 지연시간 증대\n(C) ElastiCache는 추가 비용 발생, \"인프라 비용 없이\" 요구사항 위반\n(D) Parquet은 좋지만 1시간 내 쿼리 재사용 미지원\n\n【시험 포인트】\nAthena 비용 최적화 = 쿼리 결과 재사용 (표준 답)\n1시간 새로고침 주기 = 결과 캐시 유효 기간 내\n추가 인프라 금지 = 완전 관리형 솔루션만 선택",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ ALL 분배 — 모든 노드에 전체 복제\n▸ EVEN 분배 — 해시 기반 분산\n▸ 조인 성능 — 분배 키 일치 시 최적\n\n【정답 포인트】\n▸ 소형 테이블(10MB 미만) ALL 분배 → 조인 시 재분배 불필요\n▸ 대형 테이블은 EVEN 유지 → 저장 공간 절약\n▸ \"거의 업데이트되지 않음\" → ALL 분배 오버헤드 최소\n▸ 클러스터 확장 불가 → 분배 최적화로 성능 향상\n\n【오답 체크】\n(A) EVEN만으로는 소형 테이블 조인 시 재분배 발생\n(B) 대형 테이블 ALL 분배 → 스토리지 낭비, 메모리 부담\n(D) 파티션은 Redshift에서 거의 사용 안 함\n\n【시험 포인트】\nRedshift 분배 전략:\n- ALL: 소형, 읽기 위주, 변경 최소\n- EVEN/KEY: 대형, 변경 빈번\n- 소형 테이블 ALL 분배 = 조인 성능 극대화",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ NEST_TO_MAP — 컬럼을 맵/딕셔너리로 변환\n▸ NEST_TO_ARRAY — 컬럼을 배열로 변환\n▸ JSON 구조 — key-value 쌍 = 맵(딕셔너리)\n\n【정답 포인트】\n▸ 목표 형식 = {\"key\": \"value\"} 쌍\n▸ NEST_TO_MAP = 명명된 컬럼을 맵 구조로 변환\n▸ DataBrew는 코딩 없는 변환 (최소 노력)\n\n【오답 체크】\n(A) NEST_TO_ARRAY = [값1, 값2, ...] 배열 형식 (요구사항 불일치)\n(C) PIVOT = 행/열 재정렬, 키-값 쌍 생성 불가\n(D) Lambda + Python = 코딩 필요, DataBrew보다 노력 많음\n\n【시험 포인트】\nDataBrew 변환:\n- NEST_TO_MAP: 구조적 맵/딕셔너리 생성\n- NEST_TO_ARRAY: 순차적 배열 생성\n- 최소 코딩 = DataBrew (로우코드 플랫폼)",
  ...
  },
  {
    ...
    "answer": "CD",
    "explanation": "【핵심 용어】\n▸ QuickSight IAM 역할 — 서비스 역할(선택사항)\n▸ S3 버킷 접근 — 데이터 읽기 권한\n▸ KMS 복호화 — S3 암호화 시 필수\n\n【정답 포인트】\n(C) QuickSight → S3 읽기 권한 필수\n(D) SSE-KMS 암호화 시 KMS 복호화 권한 필수\n▸ 두 가지 모두 \"접근 권한\" 부족 = 오류 원인\n\n【오답 체크】\n(A) \"연결 없음\"은 존재하지 않는 개념, 자동 통합\n(B) Athena 테이블 카탈로그 = 권한과 무관 (스키마 문제)\n(E) IAM 역할은 선택사항 (연합 인증 사용 가능)\n\n【시험 포인트】\nQuickSight + Athena + S3 권한:\n- S3 접근 = 필수\n- KMS 복호화 = 암호화 시 필수\n- Athena 연결 = 자동 지원\n- IAM 역할 = 선택",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Glue Data Catalog — 다중 소스 메타데이터 통합\n▸ Athena — SQL 연합 쿼리(Federated Query)\n▸ PartiQL — JSON 쿼리 언어\n\n【정답 포인트】\n▸ Glue Crawler → RDS/DynamoDB/S3 메타데이터 자동 수집\n▸ Athena + PartiQL → 통합 쿼리 인터페이스\n▸ SQL 유사 구문 = Athena(SQL) + PartiQL\n▸ 최소 운영 오버헤드 = 완전 관리형 서비스\n\n【오답 체크】\n(B) Redshift Spectrum = S3 외부 데이터만 지원, DynamoDB/RDS 미지원\n(C) 수동 변환 = 운영 오버헤드 높음, 지속적 유지보수\n(D) Lake Formation = S3 데이터 레이크용, RDS/DynamoDB 직접 통합 어려움\n\n【시험 포인트】\n다중 소스 통합 쿼리 = Glue Catalog + Athena\nSQL + JSON 쿼리 = Athena(SQL) + PartiQL\nRDS/DynamoDB/S3 모두 지원 = Athena 연합 쿼리",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAWS Glue 대화형 세션, SageMaker Studio, IAM 역할(Role), 신뢰 정책(Trust Policy), 역할 수임(AssumeRole), 서비스 프린시팔(Service Principal), 크로스 서비스 인증\n\n【정답 포인트】\nSageMaker Studio에서 AWS Glue 대화형 세션을 사용할 때 접근 거부 오류가 발생하는 근본 원인은 SageMaker 실행 역할이 Glue 역할을 수임(Assume)할 수 있는 권한이 없기 때문입니다. 해결책은 데이터 엔지니어의 IAM 사용자에게 AWS Glue와 SageMaker 서비스 프린시팔에 대한 신뢰 정책 내에서 sts:AssumeRole 작업을 명시적으로 포함하는 정책을 추가하는 것입니다.\n\n【오답 체크】\nA(AWSGlueServiceRole 추가)는 역할을 직접 부여하는 것으로 AssumeRole 권한이 없으면 불충분합니다. C(AmazonSageMakerFullAccess)는 최소 권한 원칙을 위반하는 과도한 권한입니다. D(sts:AddAssociation)는 AWS IAM에 존재하지 않는 작업입니다.\n\n【시험 포인트】\nAWS 시험에서 중요한 것은 신뢰 정책과 권한 정책의 차이를 이해하는 것입니다. 신뢰 정책은 역할을 수임할 수 있는 주체를 지정하고, 권한 정책은 역할이 수행할 수 있는 작업을 정의합니다. 크로스 서비스 통합 시나리오에서 AssumeRole 권한은 필수입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAWS Glue, 스키마 감지(Schema Detection), 분류자(Classifier), ETL(Extract-Transform-Load), 운영 오버헤드, 완전 관리형 서비스, 대역폭 최적화\n\n【정답 포인트】\n일일 약 1TB의 데이터를 여러 이기종 소스에서 추출하고 15분 SLA를 충족해야 하는 요구사항에서 AWS Glue가 최적입니다. AWS Glue는 자동 스키마 감지 분류자를 내장하고 있어 변경되는 스키마도 동적으로 감지할 수 있습니다. 또한 Spark 기반의 ETL 작업으로 효율적인 변환을 제공하며, 완전 관리형 서비스이므로 인프라 관리 오버헤드가 최소화됩니다.\n\n【오답 체크】\nA(Amazon EMR)은 클러스터 프로비저닝과 관리 오버헤드가 상당하고 복잡합니다. C(AWS Lambda)는 15분 내에 1TB를 처리하기에 메모리와 타임아웃 제약이 있습니다. D(Amazon Redshift)는 데이터 웨어하우스로 설계되어 ETL 수행에 부적합합니다.\n\n【시험 포인트】\n'최소 운영 오버헤드'가 조건인 경우 완전 관리형 서비스를 우선적으로 고려합니다. Glue의 스키마 감지와 Spark 통합은 대규모 다중 소스 데이터 통합의 표준 솔루션입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nS3 Object Lambda, PII(개인식별정보), 데이터 마스킹(Data Redaction), 동적 변환(Dynamic Transformation), 규정 준수, 저장 공간 최적화\n\n【정답 포인트】\nS3 Object Lambda는 객체가 S3에서 검색될 때 Lambda 함수를 통해 데이터를 실시간으로 변환하는 혁신적인 AWS 서비스입니다. 이를 통해 단일 원본 데이터셋을 S3에 유지하면서도 각 애플리케이션의 필요에 따라 다른 수준의 PII 마스킹을 동적으로 적용할 수 있습니다. 규정 준수 요구사항을 충족하면서도 저장 공간을 효율적으로 활용합니다.\n\n【오답 체크】\nA와 C는 애플리케이션마다 여러 데이터셋 복사본을 생성하여 저장 공간을 낭비하고 데이터 동기화 문제를 야기합니다. D(API Gateway)는 추가 구성과 네트워크 레이턴시 오버헤드가 발생하며 비용이 증가합니다.\n\n【시험 포인트】\n'동적 변환' 또는 '마스킹' 키워드가 나올 때 S3 Object Lambda가 최우선 선택지입니다. 현대적 클라우드 아키텍처에서 데이터 마스킹을 분리하는 패턴을 이해해야 합니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nAWS Glue Python Shell, PySpark, pandas, DPU(Data Processing Unit), 비용 효율성, 소규모 데이터 처리\n\n【정답 포인트】\n각 S3 객체의 크기가 100MB 미만인 소규모 데이터셋에는 복잡한 분산 처리 엔진이 불필요합니다. AWS Glue Python Shell은 pandas를 사용한 행 기반 처리에 최적화되어 있으며, DPU(Data Processing Unit) 시간 기반의 청구 방식에서 소규모 작업에 가장 저렴한 비용으로 운영됩니다. 간단한 CSV 파일 처리에는 Spark 오버헤드가 낭비적입니다.\n\n【오답 체크】\nA(Amazon EKS)는 Kubernetes 클러스터 관리로 과도한 인프라 비용이 발생합니다. B(Amazon EMR)은 Hadoop 클러스터 프로비저닝과 관리 비용이 매우 높습니다. C(Glue PySpark)는 분산 처리 오버헤드로 100MB 파일 처리에 불필요한 리소스를 소비합니다.\n\n【시험 포인트】\n데이터 크기에 따른 서비스 선택이 중요합니다. 소규모(100MB 미만)는 Python Shell, 중간(GB)은 PySpark, 대규모(TB)는 EMR을 선택하는 것이 표준입니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\nAWS Glue Crawler, 파티션(Partition), ALTER TABLE, MSCK REPAIR TABLE, DDL(Data Definition Language), 메타스토어\n\n【정답 포인트】\n새로운 파티션을 명시적으로 추가하면서 모든 폴더를 스캔하지 않으려면 ALTER TABLE ... ADD PARTITION 명령을 사용해야 합니다. 이 DDL 문은 메타스토어에 파티션 정보를 직접 추가하며, 정확한 S3 경로를 지정하여 신규 파티션을 빠르고 효율적으로 등록할 수 있습니다.\n\n【오답 체크】\nB(MSCK REPAIR TABLE)은 실제로 S3의 모든 폴더와 파일을 검사하는 명령으로 대규모 버킷에서는 시간이 오래 걸리고 요구사항에 맞지 않습니다. C(REPAIR TABLE)은 표준 Hive DDL이 아닌 오류입니다. D(MODIFY PARTITION)은 기존 파티션의 위치를 수정하는 것이지 새 파티션을 추가하는 것이 아닙니다.\n\n【시험 포인트】\n파티션 관리 DDL의 용도를 정확히 구분해야 합니다. ADD PARTITION은 신규 추가, MSCK REPAIR는 누락된 파티션 발견, MODIFY는 기존 파티션 수정입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAWS Glue Data Catalog, 중앙화된 메타데이터, 데이터 거버넌스, 스키마 레지스트리, 자동 크롤링\n\n【정답 포인트】\nAWS Glue Data Catalog는 전사적 데이터 자산에 대한 완전 관리형 메타데이터 저장소를 제공합니다. 데이터 레이크의 모든 계층에서 생성되는 데이터에 대해 중앙화된 카탈로그를 구축하고, 스키마 레지스트리와 함께 데이터 거버넌스를 강화합니다. 이를 통해 데이터 발견성과 일관성을 확보할 수 있습니다.\n\n【오답 체크】\nAWS Data Exchange는 제3자 데이터 구매 플랫폼이고, Amazon MQ는 메시지 브로커 서비스이며, AWS Lake Formation은 중앙 거버넌스 도구이지만 내부적으로 Glue를 기반으로 합니다.\n\n【시험 포인트】\n'중앙화된 메타데이터'나 '데이터 거버넌스' 키워드가 나오면 AWS Glue Data Catalog가 최우선 선택지입니다. 전사 데이터 레이크 구축의 필수 기초 요소입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAmazon Redshift, 데이터 웨어하우스(Data Warehouse), 컬럼 저장소(Columnar Storage), 쿼리 성능 최적화, 압축 기술\n\n【정답 포인트】\nAmazon Redshift는 컬럼 기반 저장 아키텍처로 분석 쿼리에 최적화되어 있습니다. 고급 압축 기술을 통해 저장 공간을 효율적으로 사용하고, 집계 및 분석 워크로드에서 매우 빠른 쿼리 성능을 제공합니다. 프로비저닝된 용량 방식으로 비용을 예측 가능하게 관리할 수 있습니다.\n\n【오답 체크】\nAthena는 임시 쿼리 서비스로 지속적인 분석 워크로드에는 비용이 높습니다. AWS Glue는 메타데이터와 ETL 처리 서비스이지 쿼리 엔진이 아닙니다. Amazon DynamoDB는 NoSQL로 설계되어 대규모 분석 쿼리에 부적합합니다.\n\n【시험 포인트】\n분석 워크로드에서 Redshift(프로비저닝된 용량), 임시 쿼리에 Athena(온디맨드)를 선택하는 것이 표준 패턴입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAmazon Kinesis Data Streams, 실시간 스트리밍, 레이턴시(Latency), 처리량(Throughput), 완전 관리형 서비스\n\n【정답 포인트】\nAmazon Kinesis Data Streams은 AWS의 완전 관리형 실시간 스트리밍 서비스로 밀리초 단위의 초저 레이턴시로 대규모 데이터 스트림을 처리합니다. 자동 스케일링과 높은 처리량을 제공하며 AWS 서비스와의 네이티브 통합이 매우 우수합니다. 실시간 데이터 수집과 분석이 필요한 시나리오에 이상적입니다.\n\n【오답 체크】\nAmazon SQS는 메시지 큐 서비스로 실시간 분석에 부적합합니다. 자체 관리 Apache Kafka는 프로비저닝과 운영 오버헤드가 상당합니다. Amazon S3는 배치 저장소이지 실시간 스트림 처리에 설계되지 않았습니다.\n\n【시험 포인트】\n'실시간'이나 '밀리초' 키워드가 나오면 Amazon Kinesis가 최우선 선택지입니다. AWS 네이티브 서비스의 장점을 이해해야 합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n데이터 검증(Data Validation), 데이터 품질(Data Quality), AWS Glue Data Quality, 규칙 기반 검증, 자동화\n\n【정답 포인트】\nAWS Glue Data Quality는 내장된 검증 프레임워크로 데이터 품질 규칙을 자동으로 실행합니다. 이상 감지, 통계 기반 검증, 스키마 준수 확인 등을 포괄적으로 관리하며, ETL 파이프라인에 seamlessly 통합되어 데이터 품질 게이트 역할을 수행합니다.\n\n【오답 체크】\n수동 SQL 쿼리로 검증하면 자동화되지 않아 운영 오버헤드가 높습니다. Lambda 기반 검증은 커스텀 로직이 필요하여 유지보수가 복잡합니다. Amazon Athena는 쿼리 엔진일 뿐 품질 관리 도구가 아닙니다.\n\n【시험 포인트】\n'자동화된 데이터 품질'이나 '자동 검증' 키워드가 나오면 Glue Data Quality를 고려합니다. 품질 게이트는 모던 데이터 파이프라인의 필수 요소입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAmazon EMR, Hadoop, Apache Spark, HBase, 분산 처리, 빅데이터 분석, 클러스터 프로비저닝\n\n【정답 포인트】\nAmazon EMR은 Hadoop, Spark, HBase 등 다양한 오픈소스 프레임워크를 사용하여 대규모 분산 데이터 처리를 제공합니다. EC2 인스턴스 기반의 완전히 커스터마이징 가능한 클러스터로 복잡한 데이터 처리 요구사항과 다중 프레임워크 조합이 필요한 시나리오를 충족합니다.\n\n【오답 체크】\nAWS Glue는 Spark만 지원하고 Hadoop이나 HBase를 지원하지 않습니다. Amazon Athena는 SQL 쿼리만 가능하며 커스텀 프레임워크 실행이 불가능합니다. Amazon Redshift는 웨어하우스이지 일반적인 처리 프레임워크가 아닙니다.\n\n【시험 포인트】\n여러 오픈소스 프레임워크 조합이나 완전한 클러스터 커스터마이징이 필요하면 Amazon EMR을 선택합니다.",
  ...
  },
  {
    ...
    "explanation": "【핵심 용어】\n데이터 거버넌스, 접근 제어(Access Control), 암호화(Encryption), 감사 로깅, 규정 준수, 세밀한 권한\n\n【정답 포인트】\nAWS Lake Formation은 데이터 레이크의 중앙화된 거버넌스를 제공하여 세밀한 접근 제어, 암호화, 감사 로깅을 통합 관리합니다. IAM과 독립적으로 데이터 기반의 접근 정책을 설정할 수 있어 컬럼 레벨까지의 세밀한 접근 제어가 가능하며 보안과 규정 준수를 강화합니다.\n\n【오답 체크】\nAWS Glue는 메타데이터 관리이고 접근 제어 기능이 제한적입니다. IAM 정책만으로는 데이터 기반 거버넌스 요구사항 충족이 어렵습니다. AWS KMS는 암호화만 담당하고 접근 제어는 제공하지 않습니다.\n\n【시험 포인트】\n'데이터 거버넌스'나 '세밀한 접근 제어' 키워드가 있으면 Lake Formation이 최적의 솔루션입니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n데이터 카탈로그 마이그레이션(Migration), 메타데이터 이전, AWS Glue, Apache Hive Metastore, 호환성\n\n【정답 포인트】\nAWS Glue는 Apache Hive Metastore 형식과 완벽하게 호환되어 기존 메타데이터를 쉽게 마이그레이션할 수 있습니다. Glue Crawler를 통한 자동 스캔이나 직접 import를 통해 중앙화된 카탈로그로 통합하며, 기존 쿼리와의 호환성을 유지하여 마이그레이션 위험을 최소화합니다.\n\n【오답 체크】\n수동 마이그레이션은 오류 가능성이 높고 시간이 오래 걸립니다. Amazon Athena만으로는 메타데이터 통합 관리가 불가능합니다. AWS S3 sync는 데이터만 복사하고 메타데이터는 처리하지 않습니다.\n\n【시험 포인트】\nHive 기반 환경에서 AWS로의 마이그레이션 시 Glue의 호환성과 자동화 기능이 중요한 역할을 합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAmazon S3 Select, 부분 데이터 추출(Partial Retrieval), 네트워크 트래픽 감소, 비용 최적화, 선택적 검색\n\n【정답 포인트】\nAmazon S3 Select를 사용하면 S3에 저장된 객체에서 필요한 컬럼과 행만 선택적으로 추출하여 네트워크 전송 데이터를 획기적으로 최소화합니다. 전체 객체를 다운로드하지 않아 대역폭 비용을 절감하고 쿼리 응답 성능을 향상시킵니다.\n\n【오답 체크】\n전체 객체 다운로드는 대역폭과 비용을 낭비합니다. Amazon Athena는 스캔한 데이터량에 따라 쿼리 비용이 발생합니다. S3 Intelligent-Tiering은 저장소 계층화 기능일 뿐 쿼리 최적화가 아닙니다.\n\n【시험 포인트】\n'부분 데이터 추출'이나 '대역폭 감소' 키워드가 나오면 Amazon S3 Select가 정답입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAmazon Athena, SQL 쿼리, Presto 엔진, 스키마온리드(Schema-on-read), 임시 분석, 온디맨드 청구\n\n【정답 포인트】\nAmazon Athena는 S3의 데이터에 대해 표준 SQL로 직접 쿼리할 수 있는 서버리스 쿼리 서비스입니다. 인프라 관리 없이 스키마온리드 방식으로 유연한 스키마 처리가 가능하며, 스캔한 데이터량 기반의 온디맨드 가격 모델로 비용을 효율적으로 관리합니다.\n\n【오답 체크】\nAWS Glue는 메타데이터와 ETL에 중점을 두고 있습니다. Amazon Redshift는 웨어하우스로 프로비저닝 비용이 지속적으로 발생합니다. Amazon EMR은 클러스터 관리 오버헤드가 상당합니다.\n\n【시험 포인트】\n'임시 분석'이나 '서버리스' 키워드가 있으면 Amazon Athena를 선택합니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n데이터 파이프라인 오케스트레이션(Orchestration), AWS Step Functions, 워크플로우 자동화, 상태 관리\n\n【정답 포인트】\nAWS Step Functions는 분산 애플리케이션을 마이크로서비스로 조정하고 자동화하는 서비스입니다. 시각적으로 정의된 워크플로우로 복잡한 데이터 파이프라인을 관리하며, 내장된 상태 관리와 재시도 로직으로 안정적이고 신뢰할 수 있는 실행을 보장합니다.\n\n【오답 체크】\nAmazon CloudWatch Events는 단순 트리거 메커니즘일 뿐 복잡한 워크플로우 조정이 불가능합니다. AWS Glue Jobs는 개별 작업 실행이지 파이프라인 오케스트레이션 기능이 아닙니다. AWS Lambda는 서버리스 함수일 뿐 복잡한 워크플로우 관리가 어렵습니다.\n\n【시험 포인트】\n'워크플로우 조정'이나 '오케스트레이션' 키워드가 나오면 Step Functions가 최적입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n데이터 파이프라인 모니터링, AWS Glue Job Bookmarks, 증분 처리(Incremental Processing), 상태 추적, 중복 방지\n\n【정답 포인트】\nAWS Glue Job Bookmarks는 이전 실행 상태를 기억하여 새로운 데이터만 처리하게 합니다. 중복 처리를 방지하고 효율적인 증분 ETL을 구현하며, 전체 데이터 재처리를 피해 비용을 획기적으로 절감합니다.\n\n【오답 체크】\nAmazon CloudWatch 로그는 모니터링 기능일 뿐 증분 처리 추적이 아닙니다. Amazon S3 versioning은 파일 버전 관리이지 처리 상태 추적이 아닙니다. Amazon DynamoDB는 상태 저장소로 사용 가능하나 Glue 네이티브 기능이 아닙니다.\n\n【시험 포인트】\n'중복 방지'나 '증분 처리' 키워드가 있으면 AWS Glue Job Bookmarks가 정답입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nAmazon S3 파티셔닝(Partitioning), 쿼리 성능 최적화(Query Performance), 파티션 프루닝(Partition Pruning), key=value\n\n【정답 포인트】\nS3에서 s3://prefix/key=value 형식의 파티션 스키마를 사용하면 쿼리 엔진이 필요한 폴더만 스캔합니다(파티션 프루닝). 이를 통해 쿼리 성능을 향상시키고 네트워크 비용을 절감하며 전체 처리량을 증가시킵니다.\n\n【오답 체크】\n평면 구조는 모든 파일을 스캔하므로 비효율적입니다. 시간 기반만의 파티션은 부분적이고 제약이 있습니다. 데이터베이스 기반 구조는 S3의 객체 저장소 이점을 활용하지 못합니다.\n\n【시험 포인트】\n'파티션 프루닝'이나 '쿼리 성능' 키워드가 나오면 key=value 파티셔닝이 표준 답입니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n데이터 포맷(Data Format), Apache Parquet, ORC, CSV, JSON, 압축 효율, 쿼리 성능\n\n【정답 포인트】\nApache Parquet은 컬럼 기반 저장 포맷으로 압축률이 매우 높고 분석 쿼리 성능이 우수합니다. 부분 컬럼 읽기가 가능하여 네트워크 트래픽을 최소화하며, 대규모 분석 워크로드에 최적화되어 있습니다.\n\n【오답 체크】\nCSV는 행 기반으로 압축이 안 되고 쿼리 성능이 현저히 낮습니다. JSON은 중첩 구조로 파싱 오버헤드가 있습니다. Avro는 스키마 변경에 유리하나 분석에는 Parquet이 더 효율적입니다.\n\n【시험 포인트】\n'분석 최적화'나 '대규모 데이터' 키워드가 있으면 Apache Parquet 포맷을 선택합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n데이터 선호도(Data Locality), Spark 클러스터, 파티션 배치(Partition Placement), 가용 영역, 네트워크 레이턴시\n\n【정답 포인트】\nSpark 클러스터의 노드와 동일한 가용 영역에 데이터를 배치하면 로컬 데이터 접근이 가능하여 네트워크 레이턴시를 극적으로 최소화합니다. 데이터 선호도를 고려한 배치는 Spark 작업의 처리 성능과 처리량을 크게 향상시킵니다.\n\n【오답 체크】\n다른 리전에 배치하면 데이터 전송 비용이 높고 네트워크 레이턴시가 급증합니다. Amazon RDS는 관계형 데이터베이스로 Spark와의 선호도가 낮습니다. Amazon CloudFront는 콘텐츠 캐싱 목적이지 Spark 처리와 무관합니다.\n\n【시험 포인트】\n'데이터 선호도'나 '로컬 처리' 키워드가 나오면 동일 가용 영역 배치가 정답입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nCDC(변경 데이터 캡처), CDCLatencySource 메트릭, DMS 모니터링, Redshift 실시간 동기화\n\n【정답 포인트】\nCDCLatencySource는 소스 데이터베이스에서 DMS로 데이터가 도달하는 시간을 정량적으로 측정합니다. 이 메트릭이 높으면 PostgreSQL이 병목이고, 낮으면 Redshift 쪽 병목을 의미합니다. CloudWatch 대시보드를 통해 지연의 정확한 출처를 특정할 수 있어 문제 해결이 신속합니다. DMS 복제 작업 상태뿐만 아니라 메트릭 기반 분석이 근본 원인 파악에 필수적입니다.\n\n【오답 체크】\nA: CDCIncomingChanges는 변경사항 개수만 나타내며 실제 지연 시간을 반영하지 않습니다. B: 논리적 복제 설정 확인은 구성 검증일 뿐 성능 지연 측정이 아닙니다. C: CloudWatch Logs는 오류 진단용이지 지연 성능 측정 도구가 아닙니다.\n\n【시험 포인트】\nDMS CDC 지연 문제 해결 시 각 메트릭의 용도를 정확히 이해해야 합니다. CDCLatencySource, CDCLatencyTarget, CDCIncomingChanges, CDCChangesMemorySource 등이 있으며, 소스 지연 확인이 목표일 때는 CDCLatencySource만이 정확한 답입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nKinesis Data Streams, Kinesis Client Library, 버퍼 간격, 배치 처리, 지연 최소화\n\n【정답 포인트】\n매 10초 100KB 데이터 수집에 최적화되려면 Kinesis Data Streams의 기본 버퍼(1MB, 1초)를 조정해야 합니다. KCL로 애플리케이션 수준에서 5초 버퍼를 설정하면, 최대 5초 지연으로 효율적인 배치 처리가 가능하며 30초 다운스트림 읽기와 완벽하게 동기화됩니다. 스트림 + 커스텀 버퍼링 조합이 지연과 처리량의 최적 균형을 제공합니다.\n\n【오답 체크】\nA: Firehose 기본 버퍼(60초)는 너무 길어 지연이 증가합니다. B: 샤드 개수는 처리량 제어일 뿐 지연 감소와 무관합니다. D: Flink 추가는 불필요한 복잡성을 증가시키며 비용도 높습니다.\n\n【시험 포인트】\nKinesis 지연 최적화 문제에서 버퍼 간격 설정이 핵심입니다. Streams만으로는 S3 직접 쓰기 불가능하고, Firehose는 기본 버퍼 조정 제약이 있으며, KCL의 커스텀 버퍼링이 세밀한 제어를 가능하게 합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAWS Glue Data Quality, 사용자 지정 규칙 세트, 데이터 검증, ETL 품질 관리\n\n【정답 포인트】\n\"특정 유효성 검사 규칙\" 요구사항에 정확히 부합하는 것이 Glue Data Quality의 사용자 지정 규칙 세트입니다. 이를 통해 비즈니스 로직에 맞는 검증 조건(범위 확인, 패턴 매칭, NULL 값 검증, 중복 확인, 데이터 타입 검증)을 정의할 수 있습니다. 규칙 세트는 Glue 작업 실행 전후 자동으로 검증하여 데이터 품질을 보장합니다.\n\n【오답 체크】\nA: Job Bookmarks는 처리된 파일 추적용으로 데이터 품질 검증이 아닙니다. C: 기본 제공 변환은 표준 검증에만 제한적이며 맞춤형 규칙 정의가 불가능합니다. D: Data Catalog는 메타데이터 관리 도구일 뿐 검증 로직 구현이 아닙니다.\n\n【시험 포인트】\nGlue의 여러 기능 중 데이터 품질 요구사항 충족 도구를 정확히 선택해야 합니다. Job Bookmarks(추적), Data Catalog(메타데이터), Data Quality(검증)의 용도 구분이 중요합니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\nGlacier Flexible Retrieval, Glacier Select, 저장소 아카이빙, 압축 데이터 쿼리, 비용 최적화\n\n【정답 포인트】\n\"가끔씩 감시\" 요구사항은 낮은 접근 빈도를 의미하며, 이는 장기 아카이빙 목적입니다. Glacier Flexible Retrieval이 스토리지 비용을 가장 절감하고, Glacier Select는 Glacier 내 압축 파일을 직접 쿼리하여 전체 검색 없이 필요한 데이터만 추출합니다. 빠른 응답이 필요하지 않은 감시용 쿼리에 이상적입니다.\n\n【오답 체크】\nB: S3 스토리지는 월 비용이 Glacier의 약 20배 높아 비용 비효율적입니다. C: Athena는 S3만 지원하므로 여전히 높은 스토리지 비용이 발생합니다. D: Glacier Instant는 스토리지 비용이 Flexible보다 비싸고 불필요한 즉시 접근을 강제합니다.\n\n【시험 포인트】\nGlacier 저장소 클래스의 비용-성능 트레이드오프를 이해해야 합니다. Flexible(가장 저비용), Standard(중간), Deep Archive(최저 비용, 느림), Instant(고비용, 빠름) 특성을 구분하고 쿼리 도구 선택도 함께 고려합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nRedshift Query Editor v2, 저장 프로시저 스케줄링, 네이티브 예약 기능, 운영 오버헤드 최소화\n\n【정답 포인트】\nRedshift Query Editor v2는 저장 프로시저의 예약 실행을 네이티브로 지원하며, UI에서 간단한 설정만으로 일정을 구성할 수 있습니다. 별도 컴퓨팅 리소스 관리가 불필요하며, Redshift 클러스터만으로 전체 기능을 수행합니다. 비용은 저렴하고 관리 복잡도도 최소입니다.\n\n【오답 체크】\nA: Lambda 함수는 추가 코드 작성과 IAM 권한 설정이 필요하며 복잡성이 증가합니다. B: EC2 Spot은 비용이 저렴하지만 인스턴스 관리, 상태 관리, 네트워크 설정이 필요합니다. D: Glue Python shell은 ETL 작업용이므로 단순 프로시저 실행에는 오버스펙이고 비용도 높습니다.\n\n【시험 포인트】\nRedshift 관리형 스케줄링 기능의 존재를 알고 있어야 합니다. Redshift 내부 기능으로 해결 가능한 경우, 외부 서비스 조합보다 Redshift 네이티브 기능이 항상 최적입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nApache Airflow, 하이브리드 오케스트레이션, 오픈 소스, DAG(Directed Acyclic Graph), 이식성\n\n【정답 포인트】\nAmazon MWAA는 오픈 소스 Apache Airflow 기반이므로, AWS 클라우드와 온프레미스 환경에서 동일한 DAG 코드를 실행할 수 있습니다. Airflow의 포터빌리티 덕분에 온프레미스에는 직접 설치하고 AWS에는 MWAA를 사용하여 일관된 워크플로우 정의를 유지할 수 있습니다. 오픈 소스 생태계와 커뮤니티 지원도 장점입니다.\n\n【오답 체크】\nA: AWS Data Exchange는 데이터 마켓플레이스이며 오케스트레이션 도구가 아닙니다. B: SWF는 AWS 전용 레거시 서비스이며 온프레미스 실행이 불가능합니다. D: AWS Glue는 AWS 전용 ETL 서비스로 온프레미스 호환성이 없습니다.\n\n【시험 포인트】\n하이브리드 환경에서 오픈 소스와 이식성을 우선시할 때 Apache Airflow를 선택해야 합니다. AWS 관리형 서비스 중 오픈 소스 기반 솔루션을 식별하는 능력이 중요합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nDynamoDB, NoSQL, OLTP, 한 자리 밀리초 지연, 글로벌 테이블, 자동 확장\n\n【정답 포인트】\nAmazon DynamoDB는 높은 OLTP 워크로드, 한 자리 밀리초 지연, 글로벌 고가용성을 모두 제공하는 완전 관리형 NoSQL 데이터베이스입니다. 자동 확장, DAX 캐시, 글로벌 테이블(멀티 리전 복제)로 운영 오버헤드를 최소화합니다. 기존 NoSQL 데이터베이스에서의 마이그레이션도 간단합니다.\n\n【오답 체크】\nA: Keyspaces는 Cassandra 호환이지만 설정이 더 복잡하고 관리 오버헤드가 증가합니다. B: DocumentDB는 MongoDB 호환성에 중점이며 게임 고객 정보 저장에는 과도하지 않습니다. D: Timestream은 시계열 데이터 전용이므로 고객 정보 저장에 부적합합니다.\n\n【시험 포인트】\nAWS NoSQL 데이터베이스 선택 시 워크로드 특성(OLTP vs 분석), 성능 요구사항(지연시간), 관리 복잡도를 종합적으로 평가해야 합니다. DynamoDB는 일반 OLTP에 최적화, Keyspaces는 대규모 분산 시스템, DocumentDB는 MongoDB 호환성이 필요할 때입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nEventBridge, Lambda, IAM 권한, 리소스 기반 정책, AccessDeniedException, 크로스 서비스 호출\n\n【정답 포인트】\nEventBridge에서 Lambda를 호출할 때 AccessDeniedException이 발생하면, 두 가지 권한을 모두 확인해야 합니다: 1) EventBridge 측: EventBridge가 사용하는 IAM 역할이 Lambda 호출 권한(lambda:InvokeFunction)을 가져야 합니다. 2) Lambda 측: Lambda 함수의 리소스 기반 정책이 EventBridge 서비스(또는 구체적인 EventBridge 규칙)를 신뢰하고 호출을 허용해야 합니다.\n\n【오답 체크】\nA: 신뢰 정책만으로는 부족하며, Lambda의 리소스 기반 정책도 필요합니다. C: 서브넷은 동기식 호출에 영향이 없으며, Lambda VPC 설정과도 무관합니다. D: 스키마 유효성은 권한 문제와 관련이 없으며 이벤트 구조만 검증합니다.\n\n【시험 포인트】\nAWS 서비스 간 호출 권한 설정 시 양쪽 권한을 모두 검토하는 습관이 중요합니다. IAM 역할(주체 권한) + 리소스 기반 정책(대상 신뢰) 조합이 크로스 서비스 호출의 필수 구성입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nDSSE-KMS(Dual-layer Server-Side Encryption), 이중 암호화, KMS, 규정 준수, S3 객체 보호\n\n【정답 포인트】\n\"두 개의 암호화 계층\" 요구사항은 DSSE-KMS가 정확히 충족합니다. DSSE-KMS는 AWS KMS를 활용한 두 단계 암호화를 제공하여 HIPAA, PCI-DSS 등 규제 준수 요구사항을 만족시킵니다. Lambda 함수에서 S3 PutObject API 호출 시 ServerSideEncryption 파라미터를 'aws:kms'로 설정하면 자동으로 이중 암호화가 적용됩니다.\n\n【오답 체크】\nA: SSE-KMS와 S3 Encryption Client는 단일 계층 암호화일 뿐 이중 계층이 아닙니다. C: SSE-C는 고객 관리 키로 별도 관리 부담이 있고 이중 계층 아닙니다. D: SSE-KMS는 단일 계층 암호화로 규정의 \"두 개 계층\" 요구사항을 충족하지 못합니다.\n\n【시험 포인트】\nS3 암호화 옵션의 계층 구조를 이해해야 합니다. SSE-S3(기본), SSE-KMS(강화), DSSE-KMS(이중 강화), SSE-C(고객 관리) 순서로 보안 수준이 높아지며, 규제 요구사항에 따라 선택합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAthena 프로비저닝된 용량, DPU(Data Processing Unit), 동시 실행 제약, 온디맨드 용량, 쿼리 큐\n\n【정답 포인트】\nAthena에서 쿼리가 큐에 대기하는 것은 동시 실행 가능한 쿼리 수 제한 때문입니다. 프로비저닝된 용량을 구성하면, 워크그룹이 예약된 DPU 리소스를 확보하여 동시 실행 제약을 제거합니다. 기본적으로 Athena는 온디맨드 모드로 동시성이 제한되지만, 프로비저닝된 용량 구매로 일정한 리소스를 보장받을 수 있습니다.\n\n【오답 체크】\nA: 쿼리 결과 제한은 반환 행 수와만 관련이며 대기 문제와 무관합니다. C: 페더레이션된 쿼리는 다른 데이터 소스 접근용이며 대기 시간과 무관합니다. D: 사용자 권한은 액세스 제어일 뿐 용량 문제와 무관합니다.\n\n【시험 포인트】\nAthena의 성능 병목 원인을 정확히 파악해야 합니다. 쿼리 복잡도, 스캔 데이터량, 동시 실행 제약 등 다양한 원인 중에서 용량 제약이 원인일 때는 프로비저닝된 용량이 정답입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nGlue Job Bookmarks, 상태 커밋, job.commit(), 파일 추적, 중복 처리 방지\n\n【정답 포인트】\nGlue Job Bookmarks는 처리된 파일의 상태를 저장하여 반복 처리를 방지합니다. 그러나 Spark 또는 Glue 스크립트 마지막에 job.commit() 또는 job.bookmark()를 호출하지 않으면, 북마크가 상태를 영구 저장하지 않아 다음 실행 시 동일 파일이 다시 처리됩니다. 커밋 문은 북마크 메커니즘의 필수 조건입니다.\n\n【오답 체크】\nA: s3:GetObjectAcl은 북마크 동작보다는 S3 접근 권한 관련이며, 일반적으로 필수 권한이 아닙니다. B: 최대 동시성 1은 처리 속도에만 영향이며, 파일 반복 처리와는 무관합니다. C: Glue 버전은 호환성이나 기능 문제일 수 있지만 커밋 문 부재보다 일반적이지 않습니다.\n\n【시험 포인트】\nGlue Job Bookmarks 설정 시 enabled 플래그만으로는 부족하며, 스크립트 수준에서 커밋 문을 명시적으로 작성해야 합니다. 북마크 활성화 + 커밋 문 조합이 파일 추적의 필수 요소입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAmazon MWAA, Apache Airflow, DAG, 스크립트 오케스트레이션, 코드 마이그레이션, 서버리스\n\n【정답 포인트】\nAmazon MWAA는 Apache Airflow의 관리형 서비스로, 기존 Python/Bash 스크립트를 DAG(방향성 비순환 그래프)로 정의하여 거의 그대로 마이그레이션할 수 있습니다. 서버 관리 불필요(완전 관리형), 기존 Airflow 코드 호환성이 높아 리팩터링 최소화, 타사 도구와의 전환이 간단합니다. Python 스크립트 + Bash 조율에 최적화됩니다.\n\n【오답 체크】\nA: Lambda는 함수 기반이므로 스크립트 기반 오케스트레이션에 부적합하고, 15분 타임아웃 제약도 있습니다. C: Step Functions는 상태 머신 정의가 필요하여 상당한 코드 리팩터링을 요구합니다. D: Glue는 주로 ETL 작업용이며 스크립트 오케스트레이션에는 제한적입니다.\n\n【시험 포인트】\n온프레미스 도구에서 AWS로의 마이그레이션 시 코드 수정 최소화를 우선하면 Airflow 기반 솔루션을 선택합니다. 타사 도구의 오케스트레이션 메커니즘과 가장 가까운 AWS 서비스를 찾아야 합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAWS DMS, CDC(변경 데이터 캡처), 전체 로드, 지속적 복제, 거의 실시간, Redshift 동기화\n\n【정답 포인트】\n\"거의 실시간\" 변경 데이터 복제는 AWS DMS의 CDC 기능이 최적화되어 있습니다. DMS의 전체 로드 + CDC 작업은 초기 데이터를 한 번에 로드하고, 이후 MySQL의 바이너리 로그를 추적하여 변경사항을 지속적으로 Redshift로 복제합니다. Direct Connect 연결이 이미 있어 네트워크 구성이 단순하고, DMS 복제 인스턴스 설정만으로 시작 가능합니다.\n\n【오답 체크】\nA: Glue 예약 작업은 배치 처리(시간 단위)이므로 \"거의 실시간\" 요구사항을 충족하지 못합니다. C: AppFlow SDK는 커스텀 커넥터 개발이 필요하여 개발 노력이 증가합니다. D: DataSync는 스토리지 동기화용이며, 변경사항 추적 및 실시간 동기화 기능이 없습니다.\n\n【시험 포인트】\nDMS가 데이터 마이그레이션 + 실시간 동기화 목적에 최적화되어 있음을 알아야 합니다. 배치 처리(Glue)와 실시간 스트리밍(DMS CDC)의 구분이 핵심입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAthena, Apache Iceberg, ACID 트랜잭션, 서버리스 쿼리, S3 데이터 분할, 비용 효율성\n\n【정답 포인트】\nAmazon Athena는 S3의 다중 버킷 데이터를 SQL JOIN으로 쿼리하면서, Apache Iceberg 테이블 포맷(또는 Hudi, Delta Lake)을 지원하여 ACID 트랜잭션을 보장합니다. 서버 관리 불필요(완전 서버리스), 분할 데이터 쿼리 최적화, 스캔한 데이터에 대해서만 비용 청구하므로 비용 효율적입니다. KPI 생성을 위한 실시간 쿼리에 이상적입니다.\n\n【오답 체크】\nA: S3 Select는 단일 객체 내 데이터만 쿼리 가능하며 JOIN 불가능하고 ACID 미지원입니다. B: Redshift Spectrum은 Redshift 클러스터 운영 비용이 필요하여 비용 효율적이지 않습니다. D: EMR은 서버리스가 아니며 클러스터 관리 오버헤드가 증가합니다.\n\n【시험 포인트】\nS3 기반 분석에서 ACID 요구사항이 추가되면 Athena의 Iceberg 지원을 고려합니다. 전통적인 Parquet/CSV 포맷은 ACID 미지원이므로, 트랜잭션 보증이 필요하면 ACID 호환 테이블 포맷 선택이 필수입니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\nDMS 복제 인스턴스, 크로스 계정 마이그레이션, 리전 선택, 네트워크 최적화, IAM 권한\n\n【정답 포인트】\nDMS 복제 인스턴스는 소스(Account_A, eu-east-1)와 대상(Account_B, eu-west-1)을 모두 접근해야 합니다. 대상 쪽(Account_B, eu-west-1)에 배치하는 것이 최적입니다: 1) Redshift 쓰기 성능 최적화, 2) Account_B의 IAM 역할로 Redshift에 직접 접근 가능, 3) 대역폭 효율성. 크로스 계정 크레덴셜로 소스 PostgreSQL에 접근하고, 대상은 로컬 권한으로 처리합니다.\n\n【오답 체크】\nB: Account_B이지만 eu-east-1은 대상 리전에서 멀어 네트워크 지연 증가합니다. C: 새 계정은 불필요한 복잡성을 추가하며, 두 계정 모두에 권한 설정이 필요합니다. D: Account_A에 배치하면 Redshift 쓰기 시 크로스 리전 + 크로스 계정 홉이 증가하여 성능 저하합니다.\n\n【시험 포인트】\nDMS 복제 인스턴스 위치는 성능과 권한에 큰 영향을 미칩니다. 일반적으로 대상 데이터베이스 쪽에 배치하는 것이 쓰기 성능과 권한 관리 측면에서 최적입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nRedshift COPY 명령, 매니페스트 파일, 병렬 수집, 네트워크 최적화, 비용 효율성\n\n【정답 포인트】\nRedshift의 COPY 명령은 매니페스트 파일(manifest.json)을 지원하여 여러 S3 객체 경로를 한 번의 COPY로 처리합니다. 장점: 1) 다중 파일의 병렬 수집, 2) 네트워크 왕복(round-trip) 횟수 최소화, 3) Redshift 클러스터만 사용하여 추가 비용 없음, 4) 메타데이터 자동 관리. 기존 인프라 활용으로 개발 노력도 최소입니다.\n\n【오답 체크】\nA: EMR 클러스터 프로비저닝은 추가 비용이 발생하며, 파일 통합은 불필요한 중간 단계입니다. B: Aurora 중간 로드는 복잡성과 비용을 증가시키며 이점이 없습니다. C: Glue 작업으로 파일 통합하면 추가 ETL 비용이 발생하고 이미 수거된 데이터입니다.\n\n【시험 포인트】\nRedshift 데이터 로드 성능 향상 시 매니페스트 파일 활용이 핵심입니다. 다중 S3 경로의 병렬 수집을 하나의 COPY 명령으로 구현하는 가장 경제적인 방법입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ AWS Transfer Family — SFTP, FTPS, SSH 프로토콜을 지원하는 완전 관리형 파일 전송 서비스\n▸ Security Policy — 프로토콜 버전과 암호화 알고리즘을 제어하는 Transfer Family 설정\n\n【정답 포인트】\n▸ TLS 1.2 이상 요구 → Security Policy에서 최소 프로토콜 버전 설정\n▸ Transfer Family의 내장 정책 기능으로 데이터 전송 암호화 수준 관리\n\n【오답 체크】\n(A) SSH 키는 인증 메커니즘이며, TLS 버전 제어와 무관\n(B) 보안 그룹은 네트워크 접근 제어이며, 프로토콜 버전 강제와 무관\n(D) SSL 인증서는 이미 Transfer Family에 포함되어 있으며, 정책 설정이 필요함\n\n【시험 포인트】\n▸ Transfer Family 정책 기능 — 프로토콜 버전 제어의 올바른 방식\n▸ TLS 암호화 관리 — 인프라 레벨이 아닌 서비스 설정에서 구성",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Replatform 전략 — 기존 애플리케이션을 최소한의 변경으로 AWS 매니지드 서비스로 이동\n▸ Amazon MSK Serverless — 용량 관리 없이 자동 스케일링되는 Kafka 서비스\n▸ Provisioned vs Serverless — 프로비저닝 방식의 차이에 따른 관리 복잡도\n\n【정답 포인트】\n▸ 기존 Kafka 애플리케이션 호환성 → MSK 선택 (Kinesis 아님)\n▸ 최소 관리 오버헤드 → MSK Serverless (프로비저닝 X)\n▸ 리플랫폼 전략 — 애플리케이션 코드 변경 최소화\n\n【오답 체크】\n(A) Kinesis — 완전히 다른 API, 애플리케이션 리팩터 필요\n(B) MSK 프로비저닝 — 클러스터 용량/성능 관리 필요\n(C) Firehose — 스트리밍이 아닌 배치 전달, Kafka 호환성 X\n\n【시험 포인트】\n▸ 마이그레이션 전략별 서비스 선택 — Replatform은 최소 코드 변경\n▸ MSK Serverless의 자동 스케일링 — 용량 관리 불필요",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Job Bookmarks — 이전에 처리한 데이터의 위치를 추적하여 증분 처리를 지원\n▸ 증분 데이터 처리 — 새로운 또는 변경된 레코드만 처리하는 방식\n▸ Glue Crawler Bookmark — 마지막 처리 위치를 저장하여 중복 처리 방지\n\n【정답 포인트】\n▸ 증분 처리 요구 → Job Bookmarks로 처리 진행도 추적\n▸ 이전 실행 상태 저장 → 다음 실행에서 새로운 파일만 처리\n▸ 압축 파일 지원 — Glue는 gzip, bzip2 등 자동 처리\n\n【오답 체크】\n(A) Workflows — 여러 작업 간 의존성 오케스트레이션, 증분 추적 X\n(B) Triggers — 이벤트 기반 작업 실행, 상태 추적 X\n(D) Classifiers — 데이터 형식 분류, 증분 처리와 무관\n\n【시험 포인트】\n▸ Job Bookmarks 메커니즘 — Glue ETL의 증분 처리 핵심\n▸ 압축 파일 처리 — Bookmarks는 자동으로 지원",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Exactly-Once Semantics — 네트워크 장애에도 각 레코드가 정확히 한 번만 처리됨\n▸ Idempotent Processing — 중복 데이터를 처리해도 최종 결과가 동일\n▸ Unique ID (Business Key) — 중복 감지를 위한 레코드 식별자\n\n【정답 포인트】\n▸ 네트워크 장애 대비 → 소스에서 고유 ID 포함 (PutRecord 재시도 시 중복 발생)\n▸ 처리 단계에서 중복 제거 → 비즈니스 로직으로 idempotent 달성\n▸ 전체 파이프라인 보호 — 소스부터 싱크까지 중복 제어\n\n【오답 체크】\n(B) Checkpoint — 장애 복구용이지만 네트워크 중복 전송은 방지 불가\n(C) 소스 설계 — PutRecord 자체가 재시도 가능하므로 100% 방지 불가\n(D) EMR 전환 — Kinesis의 기본 한계가 아니라 설계 문제\n\n【시험 포인트】\n▸ Exactly-Once는 서비스 보증이 아닌 애플리케이션 설계 책임\n▸ Idempotency 패턴 — 비즈니스 키로 중복 감지/제거",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ S3 Versioning — 삭제된 객체의 이전 버전을 자동으로 보존\n▸ Delete Marker — 객체 삭제 시 마크 생성, 버전 히스토리 유지\n▸ 의도하지 않은 삭제 복구 — Versioning으로 이전 버전 복원 가능\n\n【정답 포인트】\n▸ 자동 보호 메커니즘 → Versioning 활성화로 모든 버전 자동 유지\n▸ 최소 운영 오버헤드 → 설정 후 자동 동작, 추가 관리 불필요\n▸ 즉시 복구 가능 → Delete Marker를 지우거나 이전 버전 복원\n\n【오답 체크】\n(A) 수동 백업 — 운영 오버헤드 많음, 일관성 보장 X\n(C) 복제 — 대상 계정도 동일하게 삭제되는 문제 있음\n(D) Glacier — 아카이브 용도이지, 실시간 복구 솔루션이 아님\n\n【시험 포인트】\n▸ Versioning의 Delete Marker 메커니즘 — 실제 파일 제거 X\n▸ 의도하지 않은 삭제 방지 — Versioning이 최적 솔루션",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ 실시간 감지 — 밀리초 단위 지연으로 이상 감지\n▸ Amazon Kinesis Data Streams — 고속 스트리밍 데이터 처리\n▸ Apache Flink — 시계열 데이터의 윈도우 기반 집계 및 패턴 감지\n\n【정답 포인트】\n▸ 지연 시간 최소화 → 폴링(EventBridge)이 아닌 실시간 스트리밍\n▸ 급격한 감소 감지 → Flink의 시계열 윈도우 함수 활용\n▸ 초당 수천 포인트 처리 → Kinesis의 고처리량 특성\n\n【오답 체크】\n(A) 매분 폴링 — 급격한 변화 감지 지연(최대 1분)\n(C) DynamoDB + 매분 폴링 — Aurora보다 더 느린 폴링 방식\n(D) Database Activity Streams — 감지용이 아닌 감시용 기능\n\n【시험 포인트】\n▸ 실시간 이상 감지 — 스트리밍 아키텍처 필수\n▸ 시계열 데이터의 윈도우 기반 분석 — Flink 전문 영역",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Glue DataBrew — 시각적 데이터 정제, 코드 불필요\n▸ AWS Glue ETL Job — 데이터 변환 및 로드 자동화\n▸ 최소 운영 오버헤드 — 완전 관리형 서비스 사용\n\n【정답 포인트】\n▸ 데이터 준비 → DataBrew의 시각적 인터페이스로 복잡한 ETL 불필요\n▸ Redshift 로드 → Glue의 DPU 기반 병렬 처리로 효율적\n▸ 쿼리 실행 → Redshift의 고성능 분석 쿼리\n▸ 인프라 관리 불필요 → 모두 서버리스/관리형\n\n【오답 체크】\n(A) EMR + Step Functions — EMR 클러스터 관리 필요, 오버헤드 많음\n(C) Lambda + Firehose + Athena — Lambda는 준비에 부적합, Athena는 S3 쿼리용\n(D) Glue + DMS — DMS는 DB 마이그레이션용, Spectrum은 S3 직접 쿼리\n\n【시험 포인트】\n▸ DataBrew — 코드 없는 데이터 정제 도구\n▸ 최소 오버헤드 조합 — 완전 관리형 서비스만 선택",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ VPC Gateway Endpoint — S3에 대한 무료, 낮은 지연 시간 엔드포인트\n▸ VPC Interface Endpoint — S3 접근용 인터페이스 (비용 발생)\n▸ NAT Gateway — 인터넷 연결용, S3 VPC 트래픽에 비효율적\n▸ 인터넷 게이트웨이 — 공개 인터넷 노출, 보안 우려\n\n【정답 포인트】\n▸ 보안 (VPC 내) + 비용 효율 → Gateway Endpoint (무료)\n▸ VPC 활성화 Lambda → VPC 내에서 S3 접근 필요\n▸ 타임아웃 원인 → NAT Gateway/IGW 부재로 S3 접근 불가\n\n【오답 체크】\n(A) NAT Gateway — 월간 비용, S3 접근 시에도 데이터 처리 비용\n(C) Interface Endpoint — 시간당 비용(Gateway보다 비쌈)\n(D) IGW — 공개 인터넷 노출, 보안 요구사항 미충족\n\n【시험 포인트】\n▸ Gateway vs Interface Endpoint — S3는 Gateway 무료 사용 가능\n▸ VPC Lambda → S3 접근 — Endpoint 필수",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Glue FindMatches — 머신러닝 기반 중복 레코드 감지 (불일치 필드 처리)\n▸ 최소 운영 오버헤드 — 완전 관리형 Glue 솔루션\n▸ 스키마 불일치 — 필드명이 다르지만 의미는 같은 데이터\n\n【정답 포인트】\n▸ FindMatches Transform — 필드명 불일치 자동 처리\n▸ Glue Crawler — RDS 스키마 자동 발견\n▸ 서버리스 처리 — EMR 클러스터 관리 불필요\n▸ 평가 및 튜닝 — 머신러닝 모델의 정확도 개선\n\n【오답 체크】\n(A) EMR + Zeppelin — 클러스터 관리 오버헤드, FindMatches는 Glue 전용\n(C) SageMaker — 사용자 정의 ML 파이프라인 구축 필요, 오버헤드 많음\n(D) EMR + Spark ML — Spark ML은 데이터 불일치 자동 처리 X, 오버헤드 최대\n\n【시험 포인트】\n▸ FindMatches Transform — 필드 불일치 문제의 해결책\n▸ Glue의 완전 관리형 특성 — 최소 운영 오버헤드",
  ...
  },
  {
    ...
    "answer": "AD",
    "explanation": "【핵심 용어】\n▸ Glue Crawler 테이블 생성 규칙 — 스키마와 형식이 같으면 같은 테이블로 그룹화\n▸ 압축 유형 — 데이터 포맷(CSV, JSON 등)과 무관하게 처리 가능\n▸ S3 접두사 — 디렉토리 구조로 테이블 분할 제어\n\n【정답 포인트】\n(A) 객체 형식(CSV/JSON) + 압축 유형(gzip/bzip2) + 스키마 — 모두 같아야 같은 테이블\n(D) 접두사 구조 일관성 — Crawler의 테이블 분할 제어 기준\n▸ 조합 AD — 스키마 일치 + 접두사 일관성으로 단일 테이블 생성\n\n【오답 체크】\n(B) 압축 유형 무시 — gzip과 uncompressed 파일이 함께 있으면 테이블 분할됨\n(C) 형식 무시 — CSV와 JSON이 함께 있으면 별도 테이블 생성\n(E) 파일명 패턴만 — S3 객체 키 패턴은 Crawler 분할 기준이 아님 (접두사만 해당)\n\n【시험 포인트】\n▸ Glue Crawler 테이블 통합 조건 — 형식+압축+스키마 모두 일치\n▸ S3 접두사 구조 — 테이블 분할의 핵심 제어 메커니즘",
  ...
  },
  {
    ...
    "answer": "AC",
    "explanation": "【핵심 용어】\n▸ Message Retention Period — 메시지가 삭제되기 전까지의 대기 시간 (기본 4일)\n▸ Dead-Letter Queue (DLQ) — 처리 실패 메시지의 별도 저장소\n▸ Visibility Timeout — 메시지 처리 중 다른 소비자로부터 숨김 시간\n\n【정답 포인트】\n(A) 메시지 보존 기간 증가 — 다운타임 복구 전까지 메시지 유지\n(C) DLQ 연결 — 처리 실패 메시지 별도 보존, 나중에 복구 가능\n▸ 조합 AC — 장시간 보존 + 실패 메시지 보호\n\n【오답 체크】\n(B) Visibility Timeout — 메시지 처리 중 숨김 시간, 만료 방지와 무관\n(D) 지연 큐 — 메시지 배달 지연용, 다운타임 중 손실 방지 X\n(E) 처리 시간 단축 — 다운타임 자체를 해결하지 못함\n\n【시험 포인트】\n▸ Message Retention Period — 만료 시간 연장\n▸ DLQ — 실패 메시지 보존으로 복구 가능성 확대",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ OpenSearch Dashboards — OpenSearch Service의 내장 시각화 도구, 실시간 대시보드 지원\n▸ 거의 실시간 — 밀리초~수초의 지연으로 최신 데이터 표시\n▸ Streaming → 쿼리 지연 — S3/Athena 추가 레이턴시 발생\n\n【정답 포인트】\n▸ OpenSearch Service에 직접 저장 → Dashboards로 최소 지연\n▸ 내장 시각화 → 별도 도구 없이 실시간 대시보드\n▸ MSK → OpenSearch → Dashboards (가장 짧은 경로)\n\n【오답 체크】\n(B) Athena — S3 쿼리로 지연 발생 (초 단위)\n(C) Athena + QuickSight — 쿼리 레이턴시 증가\n(D) S3 Select + QuickSight — S3 객체 쿼리로 추가 레이턴시\n\n【시험 포인트】\n▸ 실시간 대시보드 — 인-메모리 검색 엔진(OpenSearch) 필수\n▸ 데이터 흐름 최단화 — 저장소와 시각화 도구 통합",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Amazon Kinesis Data Firehose — 자동 버퍼링, 배치 전달, 변환 지원\n▸ Lambda 변환 — Firehose 내장 Lambda 지원으로 코드 실행\n▸ 최소 운영 오버헤드 — 완전 관리형 서비스\n\n【정답 포인트】\n▸ Kinesis Data Stream → Firehose (자동 배치 및 버퍼)\n▸ Lambda 변환 — Firehose의 내장 변환 기능\n▸ 자동 전달 — OpenSearch에 직접 로드\n▸ 수백만 이벤트 처리 — Firehose의 고처리량 특성\n\n【오답 체크】\n(B) Logstash — 외부 도구 관리 필요, 오버헤드 증가\n(C) Kinesis Agent — 이미 Kinesis Data Stream 사용, 중복 도구\n(D) KCL — 직접 관리 필요, Firehose의 자동화 이점 상실\n\n【시험 포인트】\n▸ Firehose의 Lambda 변환 기능 — 자동 배치 및 변환\n▸ 최소 운영 오버헤드 — Firehose의 완전 관리형 특성",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Row-Level Filter — 특정 조건의 행을 필터링하여 액세스 제어\n▸ Column-Level Filter — 특정 열을 숨기는 액세스 제어 (행 필터링 X)\n▸ Lake Formation 정책 — 테이블/열/행 단위 액세스 제어\n\n【정답 포인트】\n▸ 캐나다 고객 행 제외 → Row-Level Filter (country = 'Canada' 제외)\n▸ Lake Formation의 내장 기능 — 정책 기반 액세스 제어\n▸ 최소 운영 노력 → IAM 정책 작성 불필요\n\n【오답 체크】\n(B) IAM 역할 — Lake Formation 정책과 분리된 방식, 복잡도 증가\n(C) 열 수준 필터 — 주소 열을 숨기는 것이지, 특정 행 제외 X\n(D) 태깅 — 행별 태그 적용 오버헤드, Lake Formation 필터보다 복잡\n\n【시험 포인트】\n▸ Row-Level Filter — 특정 조건의 행을 조건부로 필터링\n▸ Lake Formation의 정책 기반 제어 — 최소 운영 노력",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Redshift Query Editor v2 — 브라우저 기반 쿼리 도구\n▸ Third-party IdP (SAML, OIDC) — 외부 인증 공급자\n▸ Redshift 내 IdP 등록 — Query Editor의 인증 설정\n\n【정답 포인트】\n▸ Query Editor v2 인증 설정 → Redshift 관리 콘솔에서 IdP 등록\n▸ SAML 2.0 또는 OIDC 지원 — 표준 인증 프로토콜\n▸ 첫 번째 단계 — Redshift 내에서 IdP 등록 (선행 작업)\n\n【오답 체크】\n(A) 클러스터 설정 — Query Editor v2 인증과 별개의 영역\n(C) Secrets Manager — 사용자 자격 증명 저장소이지, IdP 등록 서비스 아님\n(D) Certificate Manager — SSL 인증서 관리, IdP 등록과 무관\n\n【시험 포인트】\n▸ Redshift Query Editor v2 — 내부에서 IdP 등록 필요\n▸ SAML/OIDC 구성 — 외부 IdP와의 신뢰 설정",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Compute Optimized (C5, C6 등) — CPU 최적화, 메모리 적음\n▸ Memory Optimized (R5, R6 등) — 메모리 최적화, CPU 덜 중요\n▸ General Purpose (M5, M6 등) — 균형잡힌 리소스\n\n【정답 포인트】\n▸ CPU 최대 사용 + 메모리 30% 미만 → CPU 병목 현상\n▸ Compute Optimized로 전환 → CPU 성능/비용 비율 개선\n▸ 노드 수 증가 불필요 → 같은 워크로드로 비용 절감\n\n【오답 체크】\n(A) 최대 노드 10으로 증가 — CPU 병목이므로 스케일 아웃 도움 안 됨\n(B) 메모리 최적화 → 메모리 문제 없으므로 비용만 증가\n(D) 쿨다운 단축 → 스케일링 속도와 무관, CPU 병목 미해결\n\n【시험 포인트】\n▸ 인스턴스 유형 선택 — 병목 리소스 기반 최적화\n▸ CPU 병목 → Compute Optimized 인스턴스 전환",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Staging Table — 임시 로드 테이블로 기존 데이터와 비교\n▸ Upsert 패턴 — Update (기존) + Insert (신규) 작업\n▸ SQL 기반 제어 — Redshift의 MERGE 또는 DELETE/INSERT 사용\n\n【정답 포인트】\n▸ 다시 실행 시 중복 발생 → 스테이징 테이블로 기존 데이터 먼저 삭제\n▸ SQL UPDATE — 스테이징 테이블의 신규 데이터로 기존 레코드 갱신\n▸ Redshift MERGE — UPDATE + INSERT 원자적 수행\n\n【오답 체크】\n(B) MySQL 중간 단계 — 불필요한 추가 단계, 성능 저하\n(C) dropDuplicates() — 메모리 기반 중복 제거, 이미 로드된 데이터 미처리\n(D) ResolveChoice — Glue 데이터 타입 선택 변환, 중복 제거와 무관\n\n【시험 포인트】\n▸ Redshift 중복 방지 — 스테이징 테이블 패턴\n▸ 멱등성(Idempotency) — 여러 번 실행해도 같은 결과",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Redshift COPY 명령 — S3 객체 병렬 로드 명령\n▸ 단일 vs 다중 COPY — 네트워크 레이턴시와 처리량 차이\n▸ 클러스터 리소스 최적화 — 모든 노드의 병렬 처리\n\n【정답 포인트】\n▸ 최대 처리량 → 단일 COPY로 모든 노드 동시 활용\n▸ 다중 COPY의 문제 — 순차 실행 또는 경합으로 오버헤드 증가\n▸ S3 prefixes — 단일 COPY가 자동으로 병렬화\n\n【오답 체크】\n(A) 여러 COPY — 순서대로 실행되어 처리량 감소\n(B) S3DistCp + HDFS — 불필요한 중간 단계, Redshift와 연결 비효율\n(C) 다중 INSERT — COPY보다 훨씬 느림, 병렬 처리 미흡\n\n【시험 포인트】\n▸ Redshift 로드 최적화 — 단일 COPY 명령의 병렬 처리\n▸ 처리량 극대화 — 클러스터의 모든 노드 활용",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ FindMatches Transform — 머신러닝 기반 유사 레코드 매칭\n▸ 고유 식별자 없음 — 필드값 유사성으로 레코드 연결\n▸ Fuzzy Matching — 오타, 형식 차이 등 자동 처리\n\n【정답 포인트】\n▸ 공통 식별자 부재 → FindMatches로 데이터 품질 개선\n▸ 머신러닝 기반 — 패턴 인식 및 학습\n▸ Lake Formation 통합 — Glue ETL과 원활한 연결\n\n【오답 체크】\n(A) Macie — 민감한 정보 검출용, 레코드 매칭과 무관\n(B) PySpark Filter — 조건 기반 필터링, 유사성 매칭 X\n(C) Partitioning — 데이터 조직, 레코드 매칭 로직 제공 X\n\n【시험 포인트】\n▸ FindMatches Transform — 필드 불일치 및 중복 제거\n▸ 고유 식별자 없는 환경 — Fuzzy Matching 필수",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Glue Crawler — 메타데이터 카탈로깅 도구, 데이터 스키마 자동 감지\n▸ Athena — S3 데이터의 SQL 쿼리 엔진\n▸ 파일 경로 분리 — 쿼리 시 특정 경로만 스캔하도록 제한\n\n【정답 포인트】\n▸ Crawler 설정은 메타데이터만 제어, 실제 데이터 스캔은 제어 불가 → Crawler 설정만으로 불충분\n▸ Athena 콘솔에서 파일 제외는 쿼리마다 수동 지정 필요 → 자동화 안 됨\n▸ 파일 물리적 이동 → S3 경로 기반 파티셔닝으로 Athena가 자동으로 스캔 경로 제한 가능\n▸ S3 버킷 정책은 JSON 파일 접근 완전 차단, CSV 파일 요구 사항 위반\n\n【오답 체크】\n(A) Crawler는 카탈로그 메타데이터만 제외, 실제 S3 데이터 스캔은 독립적 → Athena는 전체 경로 스캔\n(B) 쿼리마다 수동으로 WHERE 절 추가 필요 → 운영 오버헤드 증가, 자동화 불가\n(D) 정책으로 JSON 접근 차단하면 CSV 파일도 영향 가능, 과도한 제한\n\n【시험 포인트】\n▸ Glue Catalog ≠ 실제 데이터 스캔 제어 → 메타데이터와 데이터 스캔은 독립\n▸ S3 파티셔닝 → Athena의 파티션 프루닝(Partition Pruning) 활용으로 성능 향상\n▸ 최단 쿼리 시간 → 물리적 데이터 분리로 스캔 대상 최소화",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ KMS 암호화 — S3 객체 서버 측 암호화, 복호화 권한 필수\n▸ IAM 실행 역할 — Lambda 함수의 권한 집합\n▸ 두 계층 권한 — S3 접근 + KMS 복호화 권한 모두 필요\n\n【정답 포인트】\n▸ S3 접근 권한만으로 불충분 → KMS 암호화 데이터 복호화는 별도 권한 필요\n▸ Lambda 역할에 kms:Decrypt, kms:DescribeKey 권한 필요\n▸ KMS 키 정책에서도 Lambda 역할을 허용해야 함\n▸ 이는 매우 일반적인 에러 패턴 → DEA 시험 핵심 주제\n\n【오답 체크】\n(A) S3 권한이 있더라도 KMS 복호화 권한 부재 → 제한적 설명\n(B) 최신 SDK도 KMS 권한 없으면 복호화 불가\n(C) 리전 차이는 지연만 유발, 오류 원인 아님\n\n【시험 포인트】\n▸ KMS 암호화 + IAM 권한 → 이중 권한 검증 필수\n▸ Lambda 역할 = AssumeRole + 리소스 권한\n▸ 암호화 데이터 처리 → 복호화 권한 항상 확인",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Glue Data Quality — Glue에 내장된 규칙 기반 품질 검사 서비스\n▸ 프로그래밍식 관리 — API를 통한 대량 수정\n▸ 운영 오버헤드 — 자동화 수준과 관리 복잡도\n\n【정답 포인트】\n▸ 1,000개 테이블 = 대량 반복 작업 → 자동화 필수\n▸ AWS Glue Data Quality API → 한 번의 Lambda 호출로 모든 규칙 일괄 수정 가능\n▸ Glue Data Quality는 Glue 서비스에 네이티브 통합\n▸ 코드 개발 최소, API 호출만으로 대규모 관리 가능\n\n【오답 체크】\n(A) Glue ETL 작업 1,000개 생성/관리 → 과도한 오버헤드\n(C) EMR 클러스터 구성 → Glue보다 복잡, 비용 높음\n(D) 수동 콘솔 편집 → 1,000개 테이블 불가능, 운영 불가\n\n【시험 포인트】\n▸ Glue Data Quality API — 규칙 대량 관리의 최우선 선택\n▸ Lambda + API 조합 → 최소 오버헤드 자동화\n▸ Glue 서비스 생태계 활용 → 외부 도구 불필요",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ git rebase — 현재 브랜치의 커밋을 다른 브랜치의 최신 상태 위에 재적용\n▸ git pull — fetch + merge 동시 수행\n▸ 커밋 히스토리 — 선형 vs. 병렬 구조\n\n【정답 포인트】\n▸ Branch A가 이미 master로 배포됨 → master 최신 커밋 이후 변경사항 발생\n▸ rebase는 Branch B의 커밋을 master 최신 상태 위에 재배치\n▸ 선형 히스토리 유지 → PR 병합 시 충돌 최소화\n▸ Pull Request 전 rebase는 업스트림 변경사항 동기화 베스트 프랙티스\n\n【오답 체크】\n(A) git diff는 차이점만 표시, 병합 준비 아님\n(B) pull은 merge 포함 → 불필요한 merge 커밋 생성, 히스토리 복잡\n(D) fetch -b는 유효한 git 옵션 아님\n\n【시험 포인트】\n▸ DEA에서 간헐적 출제 — Git 기본 이해도 검증\n▸ PR 전 동기화 → rebase가 표준 방식\n▸ 커밋 히스토리 관리 — 운영 효율성 영향",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ 작은 파일 문제 — 1KB 파일 수백만 개 → 메타데이터 오버헤드, 느린 처리\n▸ Glue 동적 프레임 파일 그룹화 — 자동 파일 병합, 내장 최적화\n▸ I/O 효율 — 파일 시스템 오버헤드 감소\n\n【정답 포인트】\n▸ 파일 그룹화가 핵심 → 수백만 개 소규모 파일 처리 불가\n▸\n(A) Lambda 추가 개발 필요 + S3 중간 쓰기 레이턴시\n▸\n(B) Glue 네이티브 파일 그룹화 → 별도 Lambda 없이 자동 최적화\n▸ Glue의 동적 프레임 = 분산 처리 프레임워크 활용\n▸ 최소 개발 노력 + 빠른 처리\n\n【오답 체크】\n(A) Lambda로 그룹화 → 추가 개발, 추가 S3 쓰기 비용/레이턴시\n(C) Redshift COPY는 로드만 담당, 처리 아님 → 형식 변환 불가\n(D) EMR 클러스터 오버헤드 > Glue, 복잡도 증가\n\n【시험 포인트】\n▸ 작은 파일 안티패턴 — Glue 파일 그룹화로 해결\n▸ Glue 동적 프레임 = Spark DataFrame 추상화\n▸ 데이터 레이크 효율성 — Parquet + 적정 파일 크기 필수",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ MWAA 로그 타입 — 각 컴포넌트별 동작 기록\n▸ Task 로그 — 개별 작업 실행 상태 및 오류\n▸ Airflow 구성 요소 — Scheduler, WebServer, DAG Processor, Task\n\n【정답 포인트】\n▸ Task 로그 = 실제 작업(DAG 내 Task) 실행 로그\n▸ 작업 실패 원인 진단 → Task 로그에서 직접 확인 가능\n▸ stderr, stdout, 실행 결과 모두 기록\n▸ 가장 상세하고 구체적인 오류 정보 포함\n\n【오답 체크】\n(A) WebServer — UI 접근 로그, 파이프라인 실행 정보 X\n(B) Scheduler — DAG 스케줄링 결정, Task 분배 로그 (간접적)\n(C) DAGProcessing — DAG 해석/검증 로그 (작업 실행 로그 아님)\n\n【시험 포인트】\n▸ MWAA 로그 선택 — 문제 영역별 타겟팅 필수\n▸ Task 실패 → 즉시 Task 로그 확인\n▸ Scheduler/DAGProcessor 로그는 스케줄링 이슈 진단용",
  ...
  },
  {
    ...
    "answer": "AC",
    "explanation": "【핵심 용어】\n▸ Enhanced VPC Routing — Redshift의 데이터 경로를 VPC 내부로 제한\n▸ Direct Connect — 전용 네트워크 연결, AWS와 온프레미스 간 보안\n▸ Spectrum — S3의 외부 데이터 쿼리\n▸ 규정 준수 경계 — 회사 네트워크 내에서만 데이터 접근\n\n【정답 포인트】\n▸\n(A) 프라이빗 서브넷 + 인터페이스 VPC 엔드포인트 → Redshift 퍼블릭 접근 차단\n▸\n(C) Enhanced VPC Routing + Direct Connect → 모든 데이터 경로가 회사 네트워크 내부 경로를 통과\n▸ Direct Connect = 인터넷 경로 우회, 전용 회선\n▸ 조합 (A+C) = 외부 접근 완전 차단\n\n【오답 체크】\n(B) HSM은 암호화 키 관리 도구, 접근 제어 X\n(D) 테이블 제약은 데이터 거버넌스, 네트워크 보안 X\n(E) Federated queries는 아키텍처 변경, 권장 방식 아님 + 게이트웨이 VPC 엔드포인트는 S3 접근용 아님\n\n【시험 포인트】\n▸ Redshift 보안 경계 → VPC 라우팅 + Direct Connect 표준\n▸ 규정 준수 = 네트워크 격리 + 암호화\n▸ Spectrum 보안 → Enhanced VPC Routing 필수",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ S3 Event Notifications — S3 객체 생성/변경 이벤트 트리거\n▸ Near real-time — 즉각적 반응, 지연 최소화\n▸ 이벤트 기반 아키텍처 — 폴링 불필요, 자동 트리거\n\n【정답 포인트】\n▸ 파일 도착 시 즉시 처리 = 이벤트 기반 설계 필수\n▸ S3 Event Notification → Lambda → Redshift COPY\n▸ 지연 시간 최소 (초 단위)\n▸ 자동 스케일링, 비용 효율적\n\n【오답 체크】\n(A) 쿼리 편집기 COPY 스케줄 = 고정 시간 기반, 즉시성 X, 파일 도착 감지 X\n(B) Zero-ETL은 Aurora 전용, S3에 적용 불가\n(C) Glue 북마크 = 배치 처리, 스케줄 기반, 거의 실시간 아님\n\n【시험 포인트】\n▸ Near real-time 수집 = S3 Event + Lambda 표준 패턴\n▸ Redshift 로딩 → COPY 명령이 가장 효율적\n▸ 이벤트 기반 데이터 파이프라인 구축 방식",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Redshift 스트리밍 수집 — Kinesis/MSK에서 직접 스트리밍 데이터 수집\n▸ 구체화된 보기(Materialized View) — 실시간 집계 쿼리 최적화\n▸ 운영 오버헤드 최소 — 관리형 서비스 선호\n\n【정답 포인트】\n▸ Redshift 스트리밍 수집 (신기능) = Kinesis → Redshift 직접 통합\n▸ Firehose 불필요 (중간 계층), 직접 연결로 레이턴시 감소\n▸ 구체화된 보기 = 스트리밍 데이터의 실시간 집계 뷰\n▸ 데이터 보강 + 실시간 쿼리 모두 지원\n▸ 관리형 서비스로 오버헤드 최소\n\n【오답 체크】\n(A) Firehose는 배치 버퍼 → 진정한 실시간 아님\n(B) S3 거쳐서 추가 지연 + 분단위 배치 로딩\n(C) Flink는 데이터 변환 도구, Redshift 직접 로딩 아니며 추가 개발 필요\n\n【시험 포인트】\n▸ Redshift 스트리밍 수집 — DEA 신규 핵심 서비스\n▸ Kinesis → Redshift 직접 경로 = 최소 지연\n▸ 실시간 데이터 파이프라인 아키텍처",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Change Data Capture(CDC) — 증분 데이터 추출\n▸ Job Bookmark — Glue의 상태 저장 메커니즘\n▸ 단조 증가 값 — CDC 추적에 최적 (timestamp 등)\n▸ 운영 효율성 — 자동화, 관리 최소화\n\n【정답 포인트】\n▸ 크기 < 50GB, 테이블 수 적음 → 증분 로드 효율\n▸ 단조 증가 컬럼 = 증분 데이터 추출의 천연 마커\n▸ Glue 북마크 = Job에서 마지막 처리 위치 자동 추적\n▸ JDBC 연결 + 북마크 = 자동화된 증분 로드\n▸ 전체 로드보다 비용/시간 효율\n\n【오답 체크】\n(A) DMS CDC는 전체 테이블 스캔 후 변경사항 추적 → 복잡\n▸ 추가 커스텀 Glue 로직 필요 → 개발 오버헤드\n(C) 매일 전체 로드 덮어쓰기 = 비효율, 불필요한 복제\n(D) \n(C) 와 동일 문제\n\n【시험 포인트】\n▸ Glue 북마크 = 증분 처리의 표준 방식\n▸ 단조 증가 컬럼 활용 = CDC 최적화\n▸ 온프레미스 → S3 마이그레이션 패턴\n▸ 적정 기술 선택 = 비용 + 성능 균형",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ DMS 지속적 복제(CDC) — 온프레미스 DB → AWS 지속 동기화\n▸ 변경 데이터 — 소스의 데이터 변경을 자동 추적\n▸ 최소 노력 — 설정 후 자동 운영\n\n【정답 포인트】\n▸ 70개 테이블 + 변경 감지 필요 → 자동 CDC 필수\n▸ DMS는 Oracle → S3 직접 복제 지원\n▸ Parquet 형식 출력 = DMS 네이티브 지원\n▸ 기본 키 존재 = CDC 추적 가능 (DMS 요구사항)\n▸ \"지속적 복제\" 모드 = 증분 변경 자동 동기화\n▸ 개발 없이 설정만으로 완성\n\n【오답 체크】\n(A) Sqoop = 배치 도구, 증분 변경 추적 약함, EMR 관리 필요\n(B) Glue 북마크 = 배치식 증분, DMS의 연속 CDC보다 지연\n(D) RDS 추가 계층 불필요, Lambda 트리거로 복잡, 과도한 설계\n\n【시험 포인트】\n▸ 온프레미스 DB → 데이터 레이크 표준 → DMS 선택\n▸ 지속 복제 = CDC 모드의 자동화\n▸ Parquet 변환 = DMS 직접 지원 (Sqoop 대비 장점)\n▸ 최소 노력 = 관리형 서비스 우선",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ KPL — Kinesis Producer Library, 배치 처리 + 압축 + 재시도\n▸ 처리량 효율 — 한 요청당 전송 바이트 최대화\n▸ 신뢰성 — 네트워크 오류 재시도 메커니즘\n\n【정답 포인트】\n▸ 10바이트 × 10,000/초 = 100KB/초 → 소규모 레코드, 배치 필수\n▸ KPL = 자동 배칭(aggregation) → 여러 레코드를 한 요청에 포함\n▸ 레코드당 프로토콜 오버헤드 감소 → 처리량 효율 극대화\n▸ 지연 허용(몇 분) = 배치 대기 시간 유리\n▸ 자동 재시도 → 신뢰할 수 없는 네트워크 대응\n▸ Producer 수준의 관리 최소화\n\n【오답 체크】\n(A) Kinesis Agent = 로그 파일 수집 도구, 프로그래밍 애플리케이션용 X\n(C) Firehose = S3/Redshift 로드용, Kinesis Data Streams와는 다름\n(D) Kinesis SDK = 기본 API, 배칭/압축 자동화 X, 개발 필요\n\n【시험 포인트】\n▸ 소규모 레코드 대량 처리 → KPL 자동 배칭\n▸ KPL 주요 이점 → 처리량 효율(throughput optimization)\n▸ 신뢰성 + 성능 → KPL의 표준 적용 시나리오",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ FindMatches — Glue의 머신러닝 기반 중복/오류 감지\n▸ 퍼지 매칭 — 정확하지 않은 일치도 감지 (오타, 변형)\n▸ 반구조화된 데이터 — JSON, XML 등 스키마 유연 형식\n\n【정답 포인트】\n▸ 정확한 중복 제거 = SQL WHERE DISTINCT 사용 가능\n▸ 오류 표기 중복 = 퍼지 매칭 필요 → FindMatches 전문\n▸ FindMatches = Glue의 기본 제공 ML 기능, 설정만으로 사용\n▸ \"일반적인 오류 표기\" = 문자 변형, 오타 감지 필수\n▸ 최소 개발 오버헤드 (모델 학습 자동)\n\n【오답 체크】\n(B) Athena SQL = 정확 일치만 가능, 퍼지 매칭 미지원\n(C) Neptune ML = 그래프 기반, 일반 중복 제거용 과도\n(D) DynamoDB 글로벌 테이블 = 레플리카 관리, 중복 제거 X\n\n【시험 포인트】\n▸ Glue FindMatches = DEA 핵심 데이터 품질 도구\n▸ 반구조화된 데이터 품질 → ML 기반 접근\n▸ 오류 표기 감지 = 정확한 SQL보다 고급 기능 필요",
  ...
  },
  {
    ...
    "answer": "AC",
    "explanation": "【핵심 용어】\n▸ 중복 처리 — Kinesis의 at-least-once 전달 보장\n▸ 샤드 리밸런싱 — 샤드 수 변경 시 레코드 프로세서 재할당\n▸ 재시도 메커니즘 — 네트워크 오류 시 자동 재전송\n\n【정답 포인트】\n▸\n(A) Producer 타임아웃 → PutRecord 재시도 → KPL이 이미 전송된 레코드 재전송\n▸ Kinesis의 at-least-once 보장 = 중복 가능성 내재\n▸\n(C) 샤드 리밸런싱 → 레코드 프로세서 재시작 → 마지막 checkpoint 이후 레코드 재처리\n▸ KCL은 레코드 처리 후 checkpoint 저장 → 재시작 시 checkpoint부터 시작\n▸ 리밸런싱 중 checkpoint 저장 지연 가능 → 중복 처리\n\n【오답 체크】\n(B) IteratorAgeMilliseconds = 이터레이터 나이, 중복과 직접 무관\n(D) AggregationEnabled = KPL의 배칭 옵션, 중복 원인 아님\n(E) max_records = 배치 크기, 중복 원인 아님\n\n【시험 포인트】\n▸ Kinesis 중복 처리 = at-least-once 전달 특성 이해\n▸ Producer 재시도 = 타임아웃으로 인한 중복\n▸ 샤드 리밸런싱 = KCL checkpoint 관리 중요\n▸ 멱등성 처리 = 소비 측에서 중복 제거 책임",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ 주문 추적 — 시간 단위 쿼리, 분석 중심\n▸ Redshift vs DynamoDB — OLAP vs NoSQL\n▸ 개발 오버헤드 — 대시보드 구축, 데이터 모델링\n\n【정답 포인트】\n▸ 시간 단위 집계 쿼리 → 분석 쿼리에 최적화된 Redshift\n▸ 전체 이행 프로세스 상태 추적 → 정규화된 스키마, JOIN 쿼리 필요\n▸ Amazon Redshift + QuickSight = AWS 기본 분석 스택\n▸ Redshift의 BI 도구 통합 = QuickSight 최적화 (테이블 지정 가능)\n▸ 개발 최소 = AWS 관리형 서비스 조합\n\n【오답 체크】\n(B) DynamoDB = NoSQL, 복잡한 JOIN 쿼리 부적합\n▸ 시간 단위 집계 쿼리 비효율\n▸ QuickSight-DynamoDB 통합 약함\n(C) \n(D) DMS를 사용한 DynamoDB 적재 → 불필요한 복잡도\n▸ OLAP 쿼리(시간 단위 추적)에 DynamoDB 부적합\n▸\n(C) Grafana = 모니터링 도구, 분석 대시보드용 아님\n▸\n(D) 도 마찬가지로 데이터베이스 선택 부적절\n\n【시험 포인트】\n▸ 주문 추적 = 분석 쿼리 → Redshift 선택\n▸ OLAP(분석) vs OLTP(트랜잭션) 이해\n▸ 개발 오버헤드 = 기본 AWS 스택 우선\n▸ 관계형 쿼리(JOIN) 필요 → DynamoDB 피하기",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ S3 Event Notifications — S3 업로드 감지 자동 트리거\n▸ Glue + Athena + QuickSight — 경제적이고 완전 관리형 스택\n\n【정답 포인트】\n▸ 옵션 D는 소규모 파일(5KB)의 일일 증분 데이터 처리에 최적화되어 있습니다. S3 Event Notifications가 업로드를 감지하면 Lambda가 Glue를 트리거하여 처리합니다. Glue는 서버리스 ETL로 필요한 만큼만 비용을 지불하고, Athena는 S3에 저장된 데이터를 필요한 때만 쿼리합니다. QuickSight는 BI 시각화를 담당하며, 전체 스택이 프로비저닝 없이 자동 스케일링됩니다. 소규모 데이터의 경우 이는 최저 운영 비용을 제공합니다.\n\n【오답 체크】\n(A) EMR은 소규모 파일에 오버헤드가 크고 비용이 높습니다.\n(B) 프로비저닝된 EMR 클러스터는 일회 쿼리에 낭비적입니다.\n(C) Redshift Spectrum은 추가 비용이 발생하고, Step Functions도 Lambda가 더 간단합니다.\n(D) 가 S3 Event Notifications + 서버리스 스택의 조합으로 가장 효율적입니다.\n\n【시험 포인트】\n소규모 증분 데이터 + 일회성 쿼리 = 서버리스(Lambda + Glue + Athena) + QuickSight",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ AWS DataSync — 대규모 데이터 전송 자동화 서비스\n▸ EFS 로컬 마운팅 — 같은 리전에서의 성능 최적화\n\n【정답 포인트】\n▸ 요구사항의 핵심은 \"데이터가 팀의 리전 내에 남아야 함\"입니다. 옵션 D는 DataSync를 사용하여 다른 리전의 EFS 데이터를 팀의 리전으로 안전하게 전송합니다. 전송 후 Lambda 함수는 로컬 EFS를 마운트하여 지연 시간 없이 처리할 수 있습니다. DataSync는 네트워크 대역폭 최적화, 자동 체크섬 검증, 병렬 전송을 제공하므로 최소 작업으로 안정적인 데이터 이동이 가능합니다.\n\n【오답 체크】\n(A) VPC 피어링은 추가 네트워크 구성이 필요하고 리전 간 피어링은 복잡합니다.\n(B) EFS는 리전 간 네이티브 복제를 지원하지 않습니다.\n(C) Lambda를 모든 리전에 배포하고 S3 출력을 수동 처리하는 것은 요구사항을 만족하지 않습니다.\n(D) 가 데이터 지역성 요구사항을 충족하면서 최소 작업을 제공합니다.\n\n【시험 포인트】\n교차 리전 데이터 통합 + 지역성 제약 = AWS DataSync를 통한 중앙화",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Certificate Manager(ACM) — SSL/TLS 인증서의 자동 갱신 및 배포 서비스\n▸ 인증서 라이프사이클 관리 — 자동화된 갱신 및 배포\n\n【정답 포인트】\n▸ ACM은 SSL/TLS 인증서의 프로비저닝, 관리, 갱신을 완전히 자동화합니다. 인증서가 만료되기 전에 자동으로 갱신되고, AWS 서비스와의 통합으로 자동 배포가 가능합니다. 운영자는 인증서 갱신, 회전, 배포에 대해 전혀 개입할 필요가 없으므로 운영 오버헤드가 최소화됩니다. 또한 ACM은 AWS 서비스와의 통합으로 원활한 배포를 지원합니다.\n\n【오답 체크】\n(A) 자체 관리 인증서는 수동 갱신과 배포 프로세스가 필요합니다.\n(C) Secrets Manager는 인증서 저장 목적이며 자동 갱신을 제공하지 않습니다.\n(D) ECS Service Connect는 인증서 관리와 무관합니다.\n\n【시험 포인트】\n자동 갱신 + 자동 배포 + 최소 운영 오버헤드 → ACM이 정답입니다.",
  ...
  },
  {
    ...
    "answer": "AD",
    "explanation": "【핵심 용어】\n▸ AWS Glue DataBrew — PII 마스킹 기능이 있는 시각적 데이터 준비 도구\n▸ IAM — 접근 제어 및 권한 관리\n\n【정답 포인트】\n▸ 옵션 A와 D의 조합이 완전한 솔루션입니다. Glue DataBrew는 PII 데이터를 검색하고 자동으로 마스킹하는 기본 제공 레시피를 제공하여 낮은 유지 관리로 데이터 보호를 구현합니다. 동시에 IAM으로 사용자별 접근 권한을 관리하여 전처리 단계에서 필요한 사용자만 원본 PII 데이터에 액세스할 수 있도록 제어합니다. 이 조합은 자동화된 마스킹과 정책 기반 액세스 제어를 동시에 제공합니다.\n\n【오답 체크】\n(B) GuardDuty는 접근 패턴 모니터링이 주 기능이며 마스킹 기능이 없습니다.\n(C) Macie는 PII 발견에만 사용되며 마스킹이나 보호 기능이 없습니다.\n(E) 사용자 정의 스크립트는 높은 유지 관리 비용이 발생합니다.\n\n【시험 포인트】\nPII 마스킹 + 액세스 제어 → Glue DataBrew + IAM의 조합이 최적입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ EMR 보안 구성 — 암호화 설정을 포함하는 통합 보안 프로필\n▸ At-rest 암호화 — S3 데이터 암호화\n▸ In-transit 암호화 — PEM 파일 기반 TLS 암호화\n\n【정답 포인트】\n▸ 옵션 C는 단일 EMR 보안 구성에서 모든 암호화 설정을 통합합니다. KMS 키는 S3에서 저장 중 암호화를 처리하고, PEM 파일은 노드 간 전송 중 암호화를 처리합니다. 클러스터 생성 중에 이 하나의 보안 구성을 사용하면 모든 암호화 요구사항이 한 번에 적용됩니다. EMR 보안 구성은 \"at-rest 암호화\"로 S3 데이터를 암호화하고 \"in-transit 암호화\"로 전송 중 데이터를 보호합니다.\n\n【오답 체크】\n(A) 두 개의 보안 구성을 만들고 연결하는 것은 불필요하고 복잡합니다.\n(B) \"로컬 디스크 암호화\"는 S3 데이터 암호화가 아닙니다.\n(D) 옵션 C와 동일하지만 표현이 다릅니다. C가 더 정확합니다.\n\n【시험 포인트】\n단일 보안 구성 + KMS(저장 중) + PEM(전송 중) = EMR 암호화 완성",
  ...
  },
  {
    ...
    "answer": "AB",
    "explanation": "【핵심 용어】\n▸ Amazon Redshift ML — Redshift 내 네이티브 ML 기능\n▸ SageMaker 원격 엔드포인트 호출 — SQL에서 직접 예측 모델 실행\n\n【정답 포인트】\n▸ 옵션 A: Redshift ML은 Redshift 테이블의 데이터로부터 자동으로 모델을 학습하고 배포합니다. SQL 문제로 인벤토리 패턴을 분석하여 권장사항을 생성할 수 있습니다. 옵션 B: 외부 SageMaker 엔드포인트는 Redshift SQL 쿼리에서 직접 호출 가능하므로 배포된 실시간 ML 모델을 활용할 수 있습니다. 두 옵션 모두 SQL 기반으로 작동하여 데이터 이동 없이 실시간 예측을 가능하게 합니다.\n\n【오답 체크】\n(C) 오프라인 내보내기는 실시간 권장사항과 무관합니다.\n(D) SageMaker Autopilot은 대시보드 생성 기능이 없습니다.\n(E) 보관 기능은 권장사항이나 예측과 무관합니다.\n\n【시험 포인트】\n실시간 + 권장사항 + 예측 → Redshift ML + SageMaker 원격 호출의 조합",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ AWS Glue DataBrew — 코드 없는 데이터 준비 도구\n▸ 레시피 — 재사용 가능한 데이터 변환 규칙\n\n【정답 포인트】\n▸ DataBrew는 시각적 인터페이스로 복잡한 CSV 변환을 코드 없이 구현합니다. 열 이름 바꾸기, 열 제거, 행 필터링, 조건부 열 생성, 숫자 필터링 등 모든 변환이 기본 제공 함수로 지원됩니다. 레시피 기능으로 변환 규칙을 저장하고 다른 파일에 재사용할 수 있어 개발 시간이 최소화됩니다. Python이나 커스텀 로직 작성이 불필요합니다.\n\n【오답 체크】\n(A) Python 코드를 작성해야 하므로 개발 작업이 많습니다.\n(B) 크롤러는 스키마 추론 도구이지 변환 기능이 제한적입니다.\n(C) 워크플로우는 오버헤드가 크고 여러 작업 조율이 필요합니다.\n(D) 가 코드 없는 시각적 변환의 최적 선택입니다.\n\n【시험 포인트】\n최소 개발 작업 + 복잡한 CSV 변환 → AWS Glue DataBrew 레시피",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ ANALYZE COMPRESSION — Redshift 열의 최적 압축 인코딩을 분석하는 명령\n▸ 압축 인코딩 — 스토리지 효율성을 높이는 데이터 압축 방식\n\n【정답 포인트】\n▸ Redshift의 ANALYZE COMPRESSION 명령은 테이블의 각 열에 대해 현재 압축 인코딩과 권장되는 최적 인코딩을 비교 분석합니다. 출력은 각 열에 대해 현재 인코딩, 권장 인코딩, 예상 공간 절감률을 제공합니다. 데이터 엔지니어는 이 분석 결과를 기반으로 ALTER TABLE 명령으로 열의 인코딩을 수동으로 업데이트하여 스토리지 최적화를 달성합니다.\n\n【오답 체크】\n(A) ANALYZE는 테이블 통계만 수집하며 압축 분석은 하지 않습니다.\n(C) VACUUM REINDEX는 정렬 순서를 재정렬하는 명령입니다.\n(D) VACUUM RECLUSTER는 클러스터 재구성 명령입니다.\n\n【시험 포인트】\n압축 인코딩 최적화 = ANALYZE COMPRESSION + 수동 업데이트",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ S3 라이프사이클 정책 — 시간 기반 스토리지 계층 전환 자동화\n▸ S3 Standard-IA — 자주 액세스하지 않는 데이터를 위한 저비용 스토리지\n\n【정답 포인트】\n▸ 옵션 A는 두 가지 액세스 패턴을 완벽히 반영합니다. 처음 30일 동안은 S3 Standard로 빈번한 액세스를 지원하고, 30일 후 자동으로 S3 Standard-IA로 전환하여 저비용 스토리지로 보유합니다. 라이프사이클 정책이 자동으로 전환을 처리하므로 수동 개입이 필요 없습니다. 이는 규정 준수와 비용 최적화를 동시에 달성하는 가장 경제적인 방식입니다.\n\n【오답 체크】\n(B) Intelligent-Tiering은 액세스 패턴을 자동 분석하지만 고정 30일 경계를 적용할 수 없습니다.\n(C) Deep Archive는 30일 후에 필요할 수 있는 급속 접근을 지원하지 않습니다.\n(D) 처음 30일 동안 IA 요금을 지불하는 것은 불경제적입니다.\n\n【시험 포인트】\n고정 시간 기반 계층화 → 라이프사이클 정책 + Standard → IA 전환",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ LAC-A(Level-Aware Calculation - Aggregate) — 드릴다운과 무관하게 전체 집계 값을 표시\n▸ 드릴다운 — 데이터의 세부 수준으로 파고들기\n\n【정답 포인트】\n▸ QuickSight의 LAC-A 함수는 드릴다운 수준에 관계없이 전체 집계(합계, 평균 등)를 유지하면서 표시합니다. 사용자가 \"리전 → 도시 → 지점\"으로 드릴다운해도 항상 해당 리전의 총 수익이 표시됩니다. LAC-A는 계층 수준의 변화가 없어도 일정한 집계 값을 보여주는 것이 특징입니다. 반면 일반 계산 필드는 드릴다운에 따라 값이 변합니다.\n\n【오답 체크】\n(A) 테이블 계산은 행 단위 계산이며 집계 레벨 제어가 제한적입니다.\n(B) 간단한 계산 필드는 드릴다운에 따라 값이 변합니다.\n(D) LAC-W는 윈도우 함수로 행 범위 계산에 사용되며 이 경우와 맞지 않습니다.\n\n【시험 포인트】\n고정 집계 + 모든 드릴다운 수준 → LAC-A(Level-Aware Calculation - Aggregate)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Amazon Macie — 자동 민감 데이터 검색 및 분류 서비스\n▸ 자동 검색 — 수동 개입 최소화\n\n【정답 포인트】\n▸ Macie는 S3 버킷에 대한 자동화된 민감 데이터 검색을 수행합니다. PII(사회 보장 번호, 신용 카드, 개인 정보 등)를 자동으로 식별하고 분류하며, 발견한 데이터에 대한 보고서를 생성합니다. 한 번 활성화되면 Macie가 모든 S3 객체를 지속적으로 모니터링하여 새로운 PII 데이터를 자동 검출합니다. 수동 검사나 스크립트 작성이 필요 없어 가장 적은 수동 개입으로 요구사항을 충족합니다.\n\n【오답 체크】\n(B) CloudTrail은 감사 로그만 제공하며 PII 검색 기능이 없습니다.\n(C) Lambda 함수는 수동으로 작성하고 관리해야 하므로 개입이 많습니다.\n(D) 사용자 정의 SQL 쿼리는 수동 작성과 정기 실행이 필요합니다.\n\n【시험 포인트】\n자동 PII 검색 + 최소 수동 개입 → Amazon Macie의 자동 검색",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ CREATE TABLE AS (CTAS) — 기존 테이블을 기반으로 새 테이블 생성\n▸ WITH NO DATA — 스키마만 복사하고 데이터는 제외\n\n【정답 포인트】\n▸ 옵션 B의 쿼리는 \"CREATE TABLE AS SELECT * FROM old_table WITH NO DATA\"로, 기존 테이블의 스키마(컬럼, 타입, 메타데이터)를 완전히 복사하면서 데이터는 포함하지 않습니다. 빈 테이블을 생성하므로 데이터 처리 작업을 위한 새 테이블로 즉시 사용할 수 있습니다. WITH NO DATA 절이 핵심으로, 이것이 없으면 기존 1,000개 행이 모두 복사됩니다.\n\n【오답 체크】\n(A) LIKE 구문은 Athena에서 CREATE TABLE 문맥에서 표준 SQL이 아닙니다.\n(C) WITH NO DATA가 없으므로 모든 1,000개 행이 복사됩니다.\n(D) 테이블 이름 오류(old_cable)와 WHERE 1=1은 필요 없습니다.\n\n【시험 포인트】\n빈 테이블 복사 = CTAS + WITH NO DATA 절",
  ...
  },
  {
    ...
    "answer": "AC",
    "explanation": "【핵심 용어】\n▸ MSCK REPAIR TABLE — Athena 테이블과 S3 파티션을 동기화하는 명령\n▸ 파티션 불일치 — 데이터는 있지만 테이블이 인식하지 못하는 상황\n\n【정답 포인트】\n▸ 옵션 A와 C의 조합이 CloudTrail 로그 쿼리 문제의 전형적인 해결책입니다. 먼저 Athena 테이블이 올바른 S3 위치를 가리키는지 확인합니다(테이블 정의 오류). 그 후 MSCK REPAIR TABLE을 실행하여 S3의 실제 파티션(연도별)과 Athena 메타스토어의 파티션 정보를 동기화합니다. 파티션이 누락되면 Athena는 데이터를 찾을 수 없으므로 MSCK REPAIR TABLE이 파티션을 인식하도록 합니다.\n\n【오답 체크】\n(B) 타임아웃 증가는 네트워크 지연이 아닌 이상 도움이 되지 않습니다.\n(D) Athena 다시 시작은 메타데이터 문제를 해결하지 못합니다.\n(E) 테이블 삭제/재생성은 근본 원인을 해결하지 않습니다.\n\n【시험 포인트】\nAthena \"데이터 없음\" 반환 = S3 위치 확인 + 파티션 동기화(MSCK REPAIR TABLE)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Amazon MWAA(Managed Workflows for Apache Airflow) — Python 기반 워크플로우 오케스트레이션\n▸ DAG(Directed Acyclic Graph) — Airflow의 작업 정의 방식\n\n【정답 포인트】\n▸ MWAA는 Apache Airflow를 완전 관리형 서비스로 제공하며, Python으로 복잡한 ETL 워크플로우를 작성할 수 있습니다. Spark 작업(EMR), API 호출(Salesforce), 데이터 로드(Redshift)를 포함한 다양한 작업을 Python DAG로 정의합니다. Airflow는 내장 재시도 로직과 오류 처리 메커니즘을 제공하므로 복잡한 의존성 관리와 자동 재시도가 가능합니다. Python 프로그래머가 선호하는 선택지입니다.\n\n【오답 체크】\n(B) Step Functions는 JSON 기반 상태 머신이며 Python 코드 작성이 제한적입니다.\n(C) Glue는 ETL 작업 실행이 목적이며 오케스트레이션 전문 도구가 아닙니다.\n(D) EventBridge는 이벤트 기반 트리거링이 주 목적이며 복잡한 워크플로우 정의가 어렵습니다.\n\n【시험 포인트】\nPython 코드 기반 복잡한 ETL 오케스트레이션 → Amazon MWAA(Apache Airflow)",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Lambda 레이어(Lambda Layer) — 재사용 가능한 라이브러리 또는 코드를 여러 함수에 제공\n▸ 중앙 관리 — 한 곳에서 코드를 업데이트하고 모든 함수가 자동 적용\n\n【정답 포인트】\n▸ Lambda 레이어는 사용자 정의 Python 스크립트를 패키징하여 여러 Lambda 함수에 연결할 수 있는 메커니즘입니다. 스크립트를 레이어로 저장하면, 모든 참조 함수가 자동으로 최신 버전을 사용합니다. 스크립트 수정 시 레이어만 업데이트하면 되고, 개별 Lambda 함수를 수정할 필요가 없습니다. 이는 중앙화된 의존성 관리를 제공하며, 수동 업데이트를 완전히 제거합니다.\n\n【오답 체크】\n(A) S3 포인터는 런타임에 파일을 다운로드해야 하는 추가 작업이 필요합니다.\n(C) 환경 변수도 마찬가지로 런타임 다운로드가 필요합니다.\n(D) 별칭은 함수 버전 관리 도구이지 의존성 공유 메커니즘이 아닙니다.\n\n【시험 포인트】\n공유 라이브러리 중앙 관리 + 자동 적용 → Lambda 레이어가 정답입니다.",
    "en_q": "A data engineer maintains custom Python scripts that perform a data formatting process that many AWS Lambda functions use. When the data engineer needs to modify the Python scripts, the data engineer must manually update all the Lambda functions. The data engineer requires a less manual way to update the Lambda functions. Which solution will meet this requirement?",
    "en_opts": {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ S3 Object Lambda — S3 객체 데이터를 동적으로 변환하여 반환\n▸ Amazon Comprehend — NLP를 사용하여 PII(신용카드, 이름, 주소 등) 감지\n\n【정답 포인트】\nB는 \"가장 낮은 운영 오버헤드\"라는 조건을 최적화합니다.\n• S3 Object Lambda: 원본 데이터는 그대로 유지하되, 읽을 때 실시간으로 Comprehend를 통해 PII 제거\n• 별도 저장 불필요: 물리적 중복 데이터 생성 없음\n• 자동화된 흐름: Lambda 함수가 각 팀의 요청 시 즉시 처리\n\n【오답 체크】\n(A) Macie: 민감한 데이터 \"발견\"에 특화, 자동 제거 기능 없음\n(C) Kinesis Firehose: 스트리밍 데이터 수집용, 배치 데이터 처리에 부적합\n(D) DataBrew: 별도 버킷에 중복 저장 필요, 운영 오버헤드 증가\n\n【시험 포인트】\nS3 Object Lambda는 데이터 거버넌스 시나리오에서 실시간 변환/필터링의 핵심 솔루션. \"가장 낮은 오버헤드\" 조건에서 데이터 중복이나 추가 저장소 없이 접근 시점에 PII를 제거하는 우아한 패턴을 보인다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ CloudTrail — API 호출 및 사용자 활동의 감사 로그\n▸ CloudTrail Data Events — S3 GetObject, PutObject 등 객체 수준 작업 기록\n▸ EventName/Username — CloudTrail 이벤트에 포함된 필드로 \"누가\" \"무엇을\" 했는지 식별\n\n【정답 포인트】\nC가 정확한 이유:\n• CloudTrail은 객체 수준 API 호출을 기록하며 eventName과 userIdentity(username) 포함\n• IAM 역할 기반 접근 시도 감지: AssumeRole 이벤트와 권한 없는 접근 시도(Access Denied) 기록\n• CloudWatch Logs로 스트리밍하여 실시간 알람 설정 가능\n• 정책 위반 = 권한 없는 API 호출 시도를 CloudTrail이 캡처\n\n【오답 체크】\n(A) Config: AWS 리소스 설정 변경 추적용, 개별 사용자의 API 호출 감지 불가\n(B) CloudWatch Metrics: S3 요청 수 등 집계 메트릭만 제공, 개별 사용자 식별 불가\n(D) S3 Server Access Logs: HTTP 요청만 기록, 거부된 요청이나 IAM 정책 평가 정보 부족\n\n【시험 포인트】\n규정 준수 및 감사 요구 사항에서 CloudTrail은 필수. 특히 \"사용자 이름\" 추출이 필요한 경우 CloudTrail만이 userIdentity 정보를 제공한다. 정책 위반 감지는 Access Denied 이벤트를 필터링하여 구현.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ SUPER data type — Amazon Redshift의 네이티브 JSON/반정형 데이터 타입\n▸ \"LEAST EFFORT\" — 최소 노력, 즉 데이터 변환 없이 그대로 수용\n▸ JSON 필드 저장 — 스키마 변경 없이 JSON 값을 그대로 컬럼에 저장\n\n【정답 포인트】\nA가 최고의 선택인 이유:\n• Redshift SUPER 타입: JSON을 네이티브로 지원하므로 별도 파싱/평면화 불필요\n• 직접 로드: COPY 명령어로 JSON 필드를 SUPER 컬럼에 직접 로드 가능\n• 유연한 쿼리: JSON 필드를 dot notation으로 쿼리 가능 (예: column.field)\n• 최소한의 ETL: 데이터 변환 단계 제거\n\n【오답 체크】\n(B) Glue: JSON을 평면화 = 추가 ETL 단계, 운영 오버헤드 증가\n(C) Athena: Redshift 데이터 웨어하우스 요구사항 무시, 별도 쿼리 엔진\n(D) Lambda: 수동 평면화 개발, 가장 높은 노력 필요\n\n【시험 포인트】\nREDSHIFT 특화 질문. SUPER 데이터 타입은 Redshift가 반정형 데이터(JSON, Avro, Parquet)를 처리하기 위해 도입한 핵심 기능. \"가장 적은 노력\"이라는 조건에서 데이터 변환 없이 바로 적재할 수 있는 유일한 방법.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ AppFlow — Salesforce/SaaS 데이터 수집의 서버리스 커넥터\n▸ AWS Glue — 배치 ETL 도구, 온프레미스/RDS 데이터 추출 최적화\n▸ Step Functions — 서버리스 워크플로우 오케스트레이션\n\n【정답 포인트】\nC는 최적의 조합 이유:\n• AppFlow: SaaS(Salesforce) 통합에 특화, 사전 구성된 커넥터\n• Glue: 배치 ETL, RDS MySQL 데이터 추출에 최적 (DPU 기반 스케일링)\n• Step Functions: 서버리스, 저비용 오케스트레이션 (밤에 한 번 실행 = 비용 최소)\n• 비용: 일회성 배치 작업 = Lambda/Step Functions 선택\n\n【오답 체크】\n(A) MWAA: Airflow 관리형 서비스, 최소 비용이 Step Functions보다 높음\n(B) MWAA: 위와 동일, 추가로 Glue가 필수 아닌데 사용\n(D) Kinesis Streams: 배치 데이터에 부적합, 스트리밍 수집 = 비용 증가\n\n【시험 포인트】\n이종 소스 통합 + 배치 처리 + 서버리스 = AppFlow + Glue + Step Functions. MWAA는 복잡한 DAG 워크플로우(재시도, 조건부 분기 등)가 필요할 때 선택. 단순 밤 배치는 Step Functions가 경제적.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ S3 Lifecycle Policy — S3 객체의 생명 주기를 자동 관리하는 정책 (Transition, Expiration)\n▸ Expiration — 특정 기간 후 객체 자동 삭제\n▸ \"운영 오버헤드\" — 관리, 모니터링, 유지보수 필요성\n\n【정답 포인트】\nA가 명확한 정답인 이유:\n• 네이티브 S3 기능: 별도 코드/인프라 없음\n• 자동화: 정책 설정 후 자동 실행, 관리 불필요\n• 비용 효율: 실행 비용 없음 (Lambda는 호출당 청구)\n• 신뢰도: AWS 서비스의 기본 기능, 버그 위험 없음\n\n【오답 체크】\n(B) Lambda: 매일/매월 실행 설정 필요, CloudWatch 모니터링 필요, 오류 처리 로직 필요\n(C) EC2: 인스턴스 항상 실행 유지 = 비용 증가, 종료/시작 자동화 필수\n(D) Step Functions: 오버킬, Lifecycle만으로 충분한 작업에 복잡도 추가\n\n【시험 포인트】\n데이터 엔지니어 시험에서 S3 Lifecycle은 필수 개념. \"정기적 자동 삭제\" 요구사항 = Lifecycle Policy. LEAST OVERHEAD = 최소한의 코드/관리. AWS의 모든 \"자동\" 삭제 요구는 먼저 Lifecycle을 고려하라.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Step Functions State Types — 워크플로우 단계 정의 방식\n▸ Choice State — 입력 데이터 조건에 따라 다른 경로로 분기\n▸ Conditional Logic — if/else와 유사한 조건부 흐름\n\n【정답 포인트】\nB(Choice)가 정확한 이유:\n• 조건부 분기: \"데이터의 콘텐츠에 따라\" = 입력 값 검사 후 다른 작업 경로 선택\n• 예시: 데이터 타입이 \"order\"면 주문 처리, \"invoice\"면 청구 처리\n• Step Functions Choice State: JSON 경로 표현식으로 조건 정의\n• 비용: 모든 경로가 실행되지 않고 매칭된 경로만 실행\n\n【오답 체크】\n(A) Parallel: 여러 작업을 \"동시에\" 실행, 조건 없이 모두 실행\n(C) Task: 단일 작업 단계, 조건 불가능\n(D) Map: 배열의 각 항목에 대해 반복 실행, 조건부 분기 아님\n\n【시험 포인트】\nStep Functions 상태 타입은 각각 고유한 용도:\n• Parallel = 동시 실행\n• Choice = 조건부 분기\n• Task = 작업 실행\n• Map = 배열 반복\n\"콘텐츠에 따라 특정 단계 수행\" = Choice State의 정의적 용도.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ cloudtrail_logs — CloudTrail 로그를 Athena 테이블로 카탈로깅\n▸ errorcode — CloudTrail 이벤트의 오류 코드 필드\n▸ \"가장 최근 10개\" — 오류 발생 빈도 기준 상위 10개\n\n【정답 포인트】\nA가 정확한 이유:\n• \"errorcode is not null\" — 오류가 있는 이벤트만 필터링 (필수)\n• \"eventtime >= '2024-01-01T00:00:00Z'\" — 2024년 이후 기록\n• \"group by\" — eventname, errorcode, errormessage 조합으로 집계\n• \"order by TotalEvents desc\" — count(*) 기준 내림차순 (가장 빈번한 오류)\n• \"limit 10\" — 상위 10개 반환\n\n【오답 체크】\n(B) \"errorcode is not null\" 조건 누락 = errorcode가 NULL인 레코드도 포함 (오류 아닌 성공 이벤트)\n(C) \"order by eventname asc\" = 오류명 알파벳 순서, \"가장 최근\" 요구사항 불만족\n(D) \"limit 10\" 전에 \"order by\" 없음 = 10개 선택이 임의적, 상위 오류 보장 불가\n\n【시험 포인트】\nCloudTrail 쿼리 최적화:\n1. \"errorcode is not null\" 필수 (PII 데이터나 성공 로그 제외)\n2. 시간 범위 필터링으로 데이터 스캔 양 감소\n3. COUNT 및 GROUP BY로 오류 유형별 집계\n4. ORDER BY DESC + LIMIT로 상위 N개 추출\nATHENA와 CLOUDTRAIL은 DEA 시험 필수 조합.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Amazon Macie — S3 데이터의 민감한 정보(PII, 신용카드, API 키 등) 자동 감지\n▸ PII Detection — 이름, 이메일, 주소, 전화번호 등 식별\n▸ Automated Scanning — 정책 설정 후 자동 스캔\n\n【정답 포인트】\nC(Macie)가 정답인 이유:\n• 목적 일치: S3에 저장된 데이터의 PII 감지 전문 서비스\n• 자동화: Macie가 버킷을 자동으로 스캔, 규칙 활성화만 필요\n• PII 감지: 신용카드, 주민등록번호, 이름, 이메일 등 300+ 유형 식별\n• 워크플로우 통합: 발견 결과를 SNS/EventBridge로 전달, 수정 자동화 가능\n• LEAST OVERHEAD: 정책 설정 후 관리 최소화\n\n【오답 체크】\n(A) Textract: OCR 도구, 스캔된 문서 텍스트 추출용, PII 감지 불가\n(B) Storage Lens: S3 메트릭(저장 크기, 요청 수) 분석용, PII 감지 불가\n(D) Data Wrangler: 데이터 변환 및 전처리 도구, PII 감지 아님\n\n【시험 포인트】\nAWS의 데이터 보호 서비스 맵:\n• Macie = PII 감지\n• Comprehend = 텍스트 분석 (감정, 엔티티 추출)\n• Textract = OCR/문서 추출\n• Data Wrangler = 데이터 전처리\n\"S3에서 PII 감지\" = Macie는 필수 선택.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ RBAC (Role-Based Access Control) — 직무 역할(역할)에 따른 접근 제어\n▸ RLS (Row-Level Security) — 특정 행(row) 단위 접근 제한\n▸ CLS (Column-Level Security) — 특정 컬럼(column) 단위 접근 제한\n\n【정답 포인트】\nA(RBAC)가 정답인 이유:\n• 요구사항 분석: \"사용자의 직무 역할, 권한, 데이터 민감도\"\n• RBAC는 \"역할\"에 따라 권한 할당: 판매팀, 재무팀, 분석팀 등\n• 객체 수준: 테이블, 스키마, 데이터베이스에 대한 접근 권한 관리\n• 유연성: 사용자가 여러 역할 보유 가능, 역할 추가/변경 용이\n\n【오답 체크】\n(B) RLS: \"행 단위\" 접근, 예) 부서별 판매 데이터만 보기. 이 질문은 \"객체 접근\" = 테이블/스키마 수준\n(C) CLS: \"컬럼 단위\" 접근, 예) 급여 컬럼 숨기기. 과도하게 세분화\n(D) Dynamic Masking: 실시간 데이터 마스킹, RBAC보다 복잡\n\n【시험 포인트】\nRedshift 접근 제어 계층:\n• RBAC = 객체 권한 (테이블, 스키마)\n• RLS = 행 데이터 필터\n• CLS = 컬럼 데이터 필터\n• Masking = 데이터 변환\n\"직무 역할에 따른 객체 접근\" = RBAC는 기본 솔루션. 추가로 민감한 데이터는 RLS/CLS로 보강.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ AWS Glue Data Quality — 데이터 품질 규칙 정의 및 스코어링\n▸ DQDL (Data Quality Definition Language) — Glue 데이터 품질 규칙 언어\n▸ Amazon DataZone — 데이터 카탈로그 및 거버넌스\n▸ Glue Data Catalog — AWS Glue의 테이블 메타데이터 저장소\n\n【정답 포인트】\nC가 정답인 이유:\n• S3 데이터 레이크 사용: S3에 저장된 데이터 (Redshift 아님)\n• AWS Glue Data Catalog 사용: 메타데이터는 Glue Catalog에 저장\n• DQDL 규칙: 테이블별 데이터 품질 규칙 집합 정의\n• 매일 스케줄: 정기적 품질 점수 계산\n• DataZone 통합: AWS Glue 데이터 소스 등록 후 품질 구성 활성화\n\n【오답 체크】\n(A) Redshift 데이터 소스: 문제는 S3 데이터 레이크를 사용, Redshift가 아님\n(B) ETL 작업 내 규칙: DataZone과의 통합이 약함, 독립적 품질 작업 필요\n(D) ETL + Redshift: B의 오류 + Redshift 오류 조합\n\n【시험 포인트】\nDataZone 통합:\n1. Glue Data Catalog = 메타데이터 소스\n2. Glue Data Quality = 품질 평가\n3. DQDL = 규칙 정의 언어\n4. DataZone = 카탈로그 및 거버넌스 포털\nDataZone \"프로젝트 구성\" = Glue 데이터 소스 지정, 품질 구성 활성화로 스코어 연계.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】자동 확장, 처리량 최적화, 스파크 작업 병렬도, 리소스 할당, 비용 효율성, DPU 데이터 처리 단위, 워커 수 조정\n\n【정답 포인트】Glue 작업의 자동 확장(auto-scaling)을 활성화하면 데이터 볼륨에 따라 워커 수가 자동으로 조정됩니다. 초기 워커 수를 낮게 설정하고 최대값을 높게 설정하여 필요시 즉시 확장 가능하도록 합니다. 동시성과 병렬도를 최적화하면 처리량이 증가하고 비용이 절감됩니다. DPU(Data Processing Unit) 크기도 작업의 메모리와 CPU 특성에 맞게 조정합니다.\n\n【오답 체크】고정된 리소스로는 변동하는 데이터 볼륨에 효과적으로 대응할 수 없고 비용 낭비가 발생합니다.\n\n【시험 포인트】Glue 성능 최적화에서 자동 확장은 필수 고려사항입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】분자형 데이터 포맷, Parquet, ORC, Avro, 압축, 저장소 효율성, 쿼리 성능, 열 기반 저장, 포맷 최적화\n\n【정답 포인트】S3 데이터 레이크에 저장하는 데이터는 분자형 포맷을 사용해야 합니다. Parquet는 열 기반 저장으로 분석 쿼리 성능이 우수하고, ORC는 압축 효율이 높습니다. Snappy나 Gzip 압축을 적용하면 저장소 용량을 40~50% 절감할 수 있습니다. Glue로 변환할 때 이러한 포맷과 압축을 적용하면 저장소 비용과 쿼리 성능을 모두 개선할 수 있습니다.\n\n【오답 체크】CSV나 JSON 원본 포맷 그대로 저장하면 저장소 용량이 크고 쿼리 성능이 떨어집니다.\n\n【시험 포인트】데이터 레이크의 효율성은 저장 포맷 선택에 달려있습니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】데이터 계계 Data Lineage, 메타데이터 추적, 감사 추적 Audit Trail, 데이터 거버넌스, 규정 준수, 출처 추적, 변환 추적\n\n【정답 포인트】데이터 계보 관리는 데이터의 출처, 변환 과정, 최종 사용처를 추적하는 것입니다. AWS Glue는 작업 실행 시 자동으로 계보 정보를 기록합니다. AWS Lake Formation과 통합하여 권한 기반 접근을 제어합니다. CloudTrail로 모든 데이터 접근을 감사 로깅하면 규정 준수를 보장할 수 있습니다. 이는 데이터 거버넌스와 규정 준수의 핵심입니다.\n\n【오답 체크】계보 관리 없으면 데이터 품질 문제 발생 시 원인 파악이 어렵고 규정 준수가 불가능합니다.\n\n【시험 포인트】현대 데이터 거버넌스에서 계보 추적은 필수 요소입니다.",
  ...
  },
  {
    ...
    "answer": "BC",
    "explanation": "【핵심 용어】\n정답 BC번. 이 문제는 AWS 데이터 엔지니어링에서 비즈니스 요구사항을 정확히 분석하고 최적의 솔루션을 선택하는 능력을 평가합니다. 각 선택지의 기술적 특성과 장단점을 이해하는 것이 필수적입니다.\n\n【정답 포인트】\n정답 BC번이 선택되는 이유는 제시된 모든 요구사항을 가장 효율적이고 경제적으로 충족하기 때문입니다. AWS 시험에서는 단순히 하나의 서비스만 아는 것이 아니라, 여러 AWS 서비스의 특성과 제한사항을 정확히 비교하고 상황에 맞는 최적의 선택을 할 수 있는 능력을 평가합니다. 비용, 운영 오버헤드, 성능, 확장성, 보안 등 다양한 실무 요소를 종합적으로 고려하는 사고방식이 필수입니다.\n\n【오답 체크】\n다른 선택지들이 부적절한 이유를 분석하는 것이 문제 학습의 핵심입니다. 각 선택지는 특정 측면에서는 장점이 있지만, 주어진 구체적인 조건에서는 제약사항이나 추가 작업을 요구합니다. 선택지들 간의 미묘한 차이를 정확히 파악하고, 왜 어떤 선택지가 더 나은지 논리적으로 설명할 수 있어야 합니다.\n\n【시험 포인트】\nAWS 데이터 엔지니어 협회 자격증(DEA-C01) 시험의 전형적인 출제 패턴입니다. 이 유형의 문제를 효과적으로 풀기 위해서는 각 AWS 서비스의 사용 사례, 제한사항, 가격 모델을 정리하고, 비슷한 기능의 서비스들 간의 차이를 명확히 이해해야 합니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n데이터 중복 제거, AWS Glue, 정규화, 데이터 클렌징\n\n【정답 포인트】\n데이터 품질 개선을 위해 중복 제거는 필수 단계입니다. AWS Glue는 PySpark 또는 Scala를 사용하여 distinct() 연산으로 중복을 제거하고, 정규화된 형식으로 데이터를 변환할 수 있습니다. Glue는 관리형 ETL 서비스로 인프라 관리 없이 확장 가능한 데이터 처리를 제공하며, 스케줄링과 모니터링도 내장되어 있습니다.\n\n【오답 체크】\n다른 옵션들은 Glue만큼 효율적인 데이터 정제 기능을 제공하지 못합니다. Lambda는 메모리 제약이 있고, 대용량 데이터 처리에 부적절합니다.\n\n【시험 포인트】\n\"중복 제거\", \"데이터 클렌징\" 키워드와 함께 \"최소 운영 오버헤드\" 요구사항이 있으면 AWS Glue를 선택하세요. 시험에서는 데이터 품질 개선을 위한 적절한 도구 선택 문제가 자주 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nAmazon S3 Select, 비용 최적화, 쿼리 성능, 필터링\n\n【정답 포인트】\nS3에 저장된 대용량 CSV 파일에서 특정 조건의 데이터만 필요한 경우, Amazon S3 Select는 S3 객체 전체를 다운로드하지 않고 서버 측에서 필터링하여 필요한 데이터만 반환합니다. 이는 네트워크 전송량을 최대 80% 감소시키고 쿼리 성능을 크게 향상시킵니다. S3 Select는 추가 인프라 없이 즉시 사용 가능한 비용 효율적 솔루션입니다.\n\n【오답 체크】\n전체 파일을 다운로드한 후 로컬에서 필터링하는 방식은 불필요한 네트워크 대역폭을 낭비합니다.\n\n【시험 포인트】\n\"S3에서 선택적 쿼리\", \"비용 최적화\", \"성능 개선\" 키워드는 S3 Select를 가리킵니다. 시험에서는 데이터 레이크의 비용 최적화 문제가 점점 증가하고 있습니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n데이터 카탈로그, AWS Glue Data Catalog, 메타데이터 관리, 데이터 거버넌스\n\n【정답 포인트】\n다양한 AWS 서비스에서 생성되는 데이터의 중앙 집중식 메타데이터 관리는 데이터 거버넌스의 핵심입니다. AWS Glue Data Catalog는 데이터 리소스(테이블, 스키마, 위치)의 메타데이터를 자동으로 수집, 정리, 검색할 수 있게 해줍니다. 크롤러를 사용하여 S3, RDS, Redshift 등 다양한 소스의 메타데이터를 자동으로 감지하고 업데이트합니다.\n\n【오답 체크】\n수동 메타데이터 관리는 확장성이 낮고 오류 가능성이 높습니다.\n\n【시험 포인트】\n\"메타데이터 관리\", \"데이터 카탈로그\", \"자동화\" 키워드는 Glue Data Catalog를 선택하세요. 시험에서는 데이터 거버넌스와 메타데이터 관리 문제가 자주 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAmazon Athena, 쿼리 성능, 파티셔닝, 비용 최적화\n\n【정답 포인트】\nS3에 저장된 데이터를 쿼리할 때 성능과 비용을 동시에 최적화하려면 데이터 파티셔닝이 필수입니다. Amazon Athena는 S3 데이터에 대해 SQL 쿼리를 직접 수행하며, 파티션을 사용하면 쿼리가 필요한 파티션만 스캔하여 스캔 데이터량을 줄일 수 있습니다. 이는 쿼리 비용을 최대 95% 감소시킬 수 있습니다.\n\n【오답 체크】\n파티셔닝 없이 전체 데이터를 스캔하는 것은 비효율적이고 비용이 많이 듭니다.\n\n【시험 포인트】\n\"Athena\", \"파티셔닝\", \"비용 최적화\" 키워드 조합은 파티션 설계를 선택하게 합니다. 시험에서는 Athena 비용 최적화 문제가 자주 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n데이터 암호화, AWS KMS, 보안, 액세스 제어\n\n【정답 포인트】\n민감한 데이터의 보안을 위해 전송 중 암호화(TLS)와 저장 중 암호화(KMS) 모두 필요합니다. AWS KMS를 사용하면 고객 관리 키로 S3 객체를 암호화할 수 있고, 세밀한 접근 제어도 가능합니다. 데이터 엔지니어는 KMS 키에 대한 암호화/복호화 권한만 부여하고, 전체 S3 접근 권한을 제한하는 방식으로 최소 권한 원칙을 적용할 수 있습니다.\n\n【오답 체크】\nS3 기본 암호화만으로는 세밀한 키 관리와 접근 제어가 불가능합니다.\n\n【시험 포인트】\n\"민감한 데이터\", \"암호화\", \"접근 제어\" 키워드는 KMS를 선택하게 합니다. 시험에서는 보안 요구사항이 강조되는 추세입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n데이터 품질 모니터링, 이상치 탐지, 자동 경보, 운영 효율성\n\n【정답 포인트】\n데이터 품질을 지속적으로 모니터링하려면 자동화된 이상치 탐지와 경보 시스템이 필수입니다. AWS Glue 데이터 품질 규칙을 사용하여 데이터 패턴을 정의하고, CloudWatch와 SNS를 연동하여 규칙 위반 시 자동으로 알림을 발송할 수 있습니다. 이는 품질 문제를 조기에 감지하여 하위 시스템에의 영향을 최소화합니다.\n\n【오답 체크】\n수동 모니터링은 지연시간이 크고 일관성이 없습니다.\n\n【시험 포인트】\n\"품질 모니터링\", \"자동\", \"경보\" 키워드는 자동화된 품질 규칙을 선택하게 합니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n데이터 레이크 아키텍처, 원본 데이터, 정제된 데이터, 메타데이터\n\n【정답 포인트】\n데이터 레이크는 다양한 소스에서 원본 데이터를 수집하는 \"Bronze\" 계층, 데이터 품질과 스키마 적용을 한 \"Silver\" 계층, 비즈니스 로직이 적용된 \"Gold\" 계층으로 구성됩니다(medallion architecture). 이러한 계층 구조는 데이터 거버넌스, 추적 가능성, 재사용성을 보장합니다. AWS Glue Data Catalog에서 각 계층의 메타데이터를 관리합니다.\n\n【오답 체크】\n계층 없는 평면 구조는 데이터 거버넌스가 어렵습니다.\n\n【시험 포인트】\n\"medallion architecture\", \"계층 구조\", \"메타데이터\" 키워드는 계층별 설계를 선택하게 합니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n데이터 거버넌스, 액세스 제어, 감사 로깅, 규정 준수\n\n【정답 포인트】\n규정 준수와 데이터 거버넌스를 위해서는 세밀한 액세스 제어와 감시 능력이 필수입니다. AWS Lake Formation을 사용하면 역할 기반 액세스 제어(RBAC)를 쉽게 구성할 수 있고, CloudTrail과 통합하여 모든 데이터 접근을 감사 로그에 기록합니다. 이는 규제 기관의 감시 요구사항을 충족하고 데이터 유출 위험을 최소화합니다.\n\n【오답 체크】\n기본 IAM 정책만으로는 데이터 레벨의 세밀한 제어가 어렵습니다.\n\n【시험 포인트】\n\"거버넌스\", \"감사\", \"규정 준수\" 키워드는 Lake Formation을 선택하게 합니다. 시험에서는 보안과 거버넌스 문제가 점점 증가하는 추세입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】Bedrock Knowledge Bases는 완전 관리형 RAG(Retrieval-Augmented Generation) 서비스로, 외부 지식 기반을 LLM과 통합하여 소스 기반 응답을 생성합니다. 다국어 모델 지원과 자동 인용 추적이 핵심 기능입니다. 서버리스 아키텍처로 벡터 DB 구축·운영이 불필요하며, Knowledge Bases가 임베딩과 검색을 자동으로 처리합니다.\n\n【정답 포인트】B가 정답인 이유는 \"최소 운영 오버헤드\" 요구사항을 완벽히 충족하기 때문입니다. Bedrock Knowledge Bases는 AWS의 완전 관리형 RAG 솔루션으로, 교과서를 자동으로 임베드하고 인덱싱하며 소스 인용을 자동 추적합니다. Claude 등 기본 LLM의 인용 능력을 활용하며, 다국어 응답 생성도 내장되어 있습니다. 서버 관리가 전혀 필요 없으므로 운영 비용이 최소입니다.\n\n【오답 체크】A는 OpenSearch Serverless와 Lambda로 벡터 DB를 직접 구축해야 하므로 운영 오버헤드가 높고, C는 Comprehend·Kendra·Lambda 세 서비스를 연동해야 하므로 복잡성이 증가하며, D는 SageMaker 미세 조정에 훈련·배포 운영 비용이 매우 높습니다. Knowledge Bases는 이러한 복잡성을 모두 제거합니다.\n\n【시험 포인트】\"최소 운영 오버헤드\" + \"소스 인용\" + \"다국어\"라는 세 조건이 나타나면 Bedrock Knowledge Bases를 우선 선택하세요. 완전 관리형 서비스가 항상 운영 효율성에서 우수하며, 여러 서비스 연동보다는 통합 솔루션이 시험에 자주 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】Secrets Manager는 자격증명 관리 전담 서비스로, 자동 로테이션을 기본 기능으로 제공하는 반면 Parameter Store는 단순 구성 데이터 저장용입니다. SSE-KMS는 KMS 마스터 키로 암호화하여 키 제어가 가능하고, IAM 정책은 세분화된 액세스 제어를 제공합니다.\n\n【정답 포인트】B가 정답인 이유는 세 가지 요구사항(S3 액세스 제어, AWS 관리형 암호화, 자격증명 자동 로테이션)을 모두 충족하기 때문입니다. IAM 정책은 S3 액세스 제어의 표준이며, SSE-KMS는 AWS 관리형 키로 고급 암호화를 제공하고 KMS 권한을 세분화 관리할 수 있습니다. 가장 중요한 것은 Secrets Manager가 자격증명 자동 로테이션을 네이티브 기능으로 지원한다는 점입니다.\n\n【오답 체크】A는 Lambda 환경 변수에 자격증명을 저장하므로 보안 위험(평문 노출 가능)이 있으며, C는 Parameter Store가 정적 저장만 가능하고 자동 로테이션을 기본 지원하지 않으므로 추가 구현이 필요합니다. D는 SSE-S3가 AWS 관리형이지만 키 로테이션 통제가 불가능하고 Parameter Store는 역시 자동 로테이션 부족입니다.\n\n【시험 포인트】\"자격증명 자동 로테이션\"이 나타나면 즉시 Secrets Manager를 선택하세요. Parameter Store는 수동 로테이션만 지원하므로 시험에서는 항상 오답이 됩니다. Secrets Manager의 자동 로테이션은 Lambda 함수 기반으로 작동하며 완전 자동화된 보안 관리의 핵심입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】DynamoDB는 완전 서버리스 NoSQL 데이터베이스로 자동 확장을 지원하며, DAX(DynamoDB Accelerator)는 인메모리 캐시 서비스로 밀리초 응답시간을 보장합니다. 반정형 데이터는 JSON 문서 형식으로 저장되며, 밀리초는 전통적 데이터베이스나 S3로는 달성 불가능한 성능 요구사항입니다.\n\n【정답 포인트】D가 정답인 이유는 세 조건을 모두 충족하기 때문입니다. DynamoDB는 완전 서버리스이므로 프로비저닝이 불필요하고, 반정형 데이터(JSON) 저장에 최적화되어 있으며, 자동 확장으로 읽기 부하를 자동 처리합니다. DAX는 인메모리 캐시로 평균 1밀리초의 응답시간을 제공하며, 드문 쓰기와 빈번한 읽기라는 액세스 패턴에 완벽합니다. 운영 오버헤드는 거의 없습니다.\n\n【오답 체크】A는 S3 Transfer Acceleration을 활성화해도 응답시간이 수백 밀리초 이상이므로 밀리초 요구사항을 충족 불가능하며, B(Iceberg)는 데이터 레이크 포맷으로 OLTP용이 아니므로 밀리초 응답이 불가능합니다. C(RDS)는 관리형이지만 서버리스가 아니고 Aurora Serverless라도 밀리초를 보장할 수 없습니다.\n\n【시험 포인트】\"밀리초\" + \"서버리스\" + \"반정형 데이터\" + \"높은 읽기\"라는 조건이 나타나면 DynamoDB + DAX를 우선 선택하세요. 초저지연 응답은 인메모리 캐시 필수이며, DynamoDB는 이를 네이티브로 지원하는 유일한 AWS 데이터베이스입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】테이블 수준 잠금(table-level lock)은 동시성 문제로, 같은 테이블에 동시 액세스 시 발생합니다. Redshift 데이터 공유는 물리적 복제 없이 메타데이터 기반으로 데이터를 공유하며, 별도 클러스터는 각각 독립적인 잠금 영역을 제공합니다. 이를 통해 ETL과 분석 워크로드를 완전히 분리할 수 있습니다.\n\n【정답 포인트】D가 정답인 이유는 동시성 경합(concurrency contention)의 근본 원인을 제거하기 때문입니다. 별도의 Redshift 클러스터를 구성하면 ETL 작업과 분석 쿼리가 서로 다른 잠금 영역에서 독립적으로 실행되어 테이블 잠금 경합이 완전히 해소됩니다. 교차 데이터베이스 쿼리와 데이터 공유 기능은 원본 데이터에 접근하면서도 복제 오버헤드 없이 데이터 일관성을 유지합니다.\n\n【오답 체크】A(구체화된 뷰 + 오프피크 스케줄)는 시간대 제한만 하므로 실제 잠금 문제를 해결하지 못합니다. B는 Redshift가 읽기 복제본을 지원하지 않으므로 불가능하며, C(Spectrum)는 S3 데이터용이므로 Redshift 내부 테이블 잠금 경합을 해결할 수 없습니다.\n\n【시험 포인트】\"동시성 문제\" + \"테이블 잠금\"이 언급되면 즉시 리소스(클러스터) 분리를 고려하세요. Redshift 데이터 공유는 최신 기능이며, 시험에서 \"최고 성능\" 솔루션으로 자주 출제됩니다. 시간 스케줄링\n(A) 은 우회(workaround)일 뿐 근본 해결이 아닙니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】Parquet은 컬럼 지향(columnar) 포맷으로 필요한 컬럼만 선택적으로 읽을 수 있으며, 분할(partitioning)은 파티션 프루닝으로 스캔 범위를 줄이고, 정렬은 같은 값이 연속되도록 하여 압축률을 높입니다. S3 데이터 레이크에서 Parquet은 ORC보다 Athena와 Spark 모두에 최적화되어 있습니다.\n\n【정답 포인트】C가 정답인 이유는 센서 카테고리별 복잡한 쿼리의 성능을 극대화하기 때문입니다. Parquet의 컬럼 압축으로 전송 데이터량을 최소화하고, 센서 카테고리별 분할은 \"센서 카테고리 특화 쿼리\" 요구사항과 정렬되어 파티션 프루닝을 극대화합니다. 같은 카테고리만 읽으므로 불필요한 데이터 스캔이 완전히 제거되며, 날짜별 정렬로 같은 카테고리 내 시계열 데이터가 최적화됩니다.\n\n【오답 체크】A(ORC)는 S3 + Athena 조합에서는 Parquet보다 선호도가 낮으며, B와 D(CSV)는 전체 행을 읽어야 하므로 가장 느리고 비효율적입니다. B는 추가로 운영 상태별 정렬이므로 센서 카테고리 쿼리에 부적합합니다.\n\n【시험 포인트】\"S3 데이터 레이크\" + \"복잡한 쿼리\" + \"특정 컬럼 필터\"라는 조건이 나타나면 Parquet + 주요 필터 컬럼(여기선 센서 카테고리)으로 분할을 우선 선택하세요. 분할 전략이 S3 쿼리 성능의 핵심이며, 주요 필터 컬럼으로 분할하면 나머지 컬럼은 정렬 순서와 무관하게 성능이 결정됩니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】저장 프로시저(stored procedure)는 데이터베이스 내에서 여러 SQL 문을 트랜잭션으로 실행하는 메커니즘으로, 원자성(atomicity)을 보장하고 네트워크 오버헤드를 제거합니다. UPSERT 패턴(UPDATE 또는 INSERT)은 DELETE와 INSERT의 조합으로 데이터 최신화를 구현합니다.\n\n【정답 포인트】D가 정답인 이유는 Redshift 테이블의 UPDATE/DELETE 작업에 가장 직접적이고 효율적이기 때문입니다. 저장 프로시저 내에서 DELETE 및 INSERT를 단일 트랜잭션으로 실행하면, 원자성이 보장되어 부분 업데이트로 인한 데이터 불일치가 발생하지 않으며, 다운스트림 애플리케이션이 항상 일관된 데이터를 읽습니다. Redshift 내부 실행이므로 네트워크 왕복이 없어 성능이 최고입니다.\n\n【오답 체크】A(Spectrum)는 S3 데이터용 쿼리 엔진이므로 Redshift 테이블 직접 수정이 불가능하고, B는 Unload → Athena 수정 → Copy라는 다단계 프로세스로 과도한 라운드 트립과 복잡성이 발생하며, C(Federated Query)는 읽기 쿼리 최적화이지 쓰기/삭제 성능이 부족합니다.\n\n【시험 포인트】\"Redshift 테이블 업데이트\" + \"일관성 보장\"이 언급되면 저장 프로시저를 우선 선택하세요. Spectrum, Athena, Federated는 모두 읽기 워크로드용이며, Redshift 내부 데이터 수정은 네이티브 저장 프로시저가 최고 성능입니다. 트랜잭션 격리와 원자성이 핵심입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】구독 필터(subscription filter)는 CloudWatch Logs에서 원하는 로그만 필터링하여 지정된 대상(Firehose 등)으로 직접 스트리밍합니다. Firehose의 데이터 변환(Data Transformation) 기능은 내장 Lambda 통합으로 로그 포맷을 변환하며, 신뢰성 높은 배치 처리를 보장합니다.\n\n【정답 포인트】B가 정답인 이유는 올바른 파이프라인 아키텍처를 구성하기 때문입니다. 구독 필터는 CloudWatch에서 관련 로그만 사전 필터링하므로 불필요한 데이터 전송이 없고, Firehose는 이를 S3로 스트리밍하며, Firehose의 내장 Lambda 변환 기능(Data Transformation)이 로그를 분석팀이 원하는 형식으로 자동 변환합니다. 전체 흐름이 신뢰성 높은 스트리밍으로 최적화되어 있습니다.\n\n【오답 체크】A는 S3에 도착한 후 필터링하므로 불필요한 데이터가 이미 S3에 저장되어 비효율적이고, C는 \"Firehose 스트림을 모니터링\"이라는 표현이 불명확하며 표준 아키텍처가 아닙니다. D는 CloudWatch Logs가 태그 기반 필터를 지원하지 않으므로 불가능합니다.\n\n【시험 포인트】\"CloudWatch → Firehose\" 연결이 나타나면 구독 필터를 우선 선택하세요. Firehose의 Data Transformation(Lambda 변환)은 내장 기능이며 배치 처리로 안정성이 높습니다. S3 후처리\n(A) 는 데이터 낭비이며, 태그 필터\n(D) 는 CloudWatch 미지원 기능입니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】Kinesis Data Streams는 AWS 완전 관리형 스트리밍 서비스로 밀리초 지연과 자동 확장을 제공하며, 비용은 초당 레코드 수 기반 청구(사용량 기반)입니다. MSK는 오픈소스 Kafka로 성능 우수하나 자체 관리 소비자가 필요하므로 운영 복잡도가 높습니다.\n\n【정답 포인트】A가 정답인 이유는 \"최소 지연\" + \"확장 가능\" + \"비용 효율\" 세 조건을 균형있게 충족하기 때문입니다. Kinesis Data Streams는 실시간 스트리밍으로 밀리초 지연을 보장하고, 자동으로 수백만 이벤트/초를 처리하며, 3개 가용영역 복제로 내구성을 보장합니다. Lambda는 서버리스 처리로 자동 확장하고 호출 기반 청구되어 비용 효율적입니다. Redshift는 분석 저장소입니다.\n\n【오답 체크】B는 10분 폴링으로 실시간이 아니며 지연이 크고, C는 S3 Event Notifications의 로그 파일 도착 지연으로 실시간 수집이 불가능합니다. D(MSK)는 최고 성능이나 자체 관리 소비자로 인해 운영 복잡도와 비용이 높아 \"비용 효율\" 요구사항에 부적합합니다.\n\n【시험 포인트】\"실시간\" + \"최소 지연\" + \"수백만 이벤트\" + \"비용 효율\"이 나타나면 Kinesis Data Streams를 우선 선택하세요. MSK는 초고성능 필요 시에만 선택하며, 폴링(Glue, EventBridge)은 분 단위 지연이므로 실시간이 아닙니다. Kinesis는 AWS의 기본 스트리밍 선택지입니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】쿼리 결과 재사용(Query Result Reuse)은 Athena 워크그룹 설정으로 동일한 SQL 텍스트와 매개변수로 실행된 쿼리의 이전 결과를 캐시에서 즉시 반환하는 기능입니다. 첫 실행만 S3 스캔 비용이 발생하고 재실행은 비용이 제로입니다.\n\n【정답 포인트】C가 정답인 이유는 \"동일 매개변수로 여러 번\"이라는 조건을 정확히 해결하기 때문입니다. 쿼리 결과 재사용을 활성화하면 첫 실행 시 풀 스캔(비용 발생)을 하고, 이후 동일 쿼리는 캐시된 결과를 밀리초 내에 반환하므로 스캔 비용이 제거됩니다. TTL(Time To Live)을 설정하여 캐시 유효 기간을 제어할 수 있으며, 가장 간단하고 효과적인 최적화입니다.\n\n【오답 체크】A(JSON 변환)는 포맷 개선이지만 반복 쿼리 최적화는 아니며, B(EMR 사전 처리)는 복잡하고 추가 비용이 발생합니다. D(Redshift Spectrum)는 다른 쿼리 엔진이므로 Athena의 결과 재사용 기능을 활용할 수 없습니다.\n\n【시험 포인트】\"동일 매개변수로 여러 번\" 쿼리가 언급되면 쿼리 결과 재사용을 우선 선택하세요. Athena의 기본 최적화 기능이며, Workgroup 설정으로 5분 안에 활성화 가능합니다. 파티셔닝이나 포맷 변경보다 즉각적인 성능 향상을 제공합니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】AWS Glue Studio는 서버리스 ETL 플랫폼으로 비주얼 및 PySpark 코드 모두를 지원하며, Glue 데이터 연결(Job parameter)은 외부 Python 패키지를 작업 런타임에 자동 주입합니다. 부트스트랩 작업은 EMR에서만 필요한 추가 구성입니다.\n\n【정답 포인트】A가 정답인 이유는 \"최소 운영 오버헤드\"를 가장 잘 충족하기 때문입니다. Glue Studio는 완전 관리형 서버리스 서비스로 인프라 프로비저닝이 불필요하고, 자동으로 수백만 레코드를 처리하며, Glue 데이터 연결로 사용자 정의 라이브러리 의존성을 간단히 관리합니다. 스케줄링도 내장되어 있으며, Redshift 로더도 기본 지원되어 ETL 파이프라인 전체를 AWS Glue로 통합할 수 있습니다.\n\n【오답 체크】B(EC2 Auto Scaling)는 AMI 관리, 스케일링 정책 설정, 패치 관리 등 높은 운영 복잡도를 요구하므로 \"최소 오버헤드\" 위배입니다. C(EMR)는 클러스터 프로비저닝, 부트스트랩 스크립트 관리, 클러스터 수명 주기 관리가 필요하고 복잡합니다. D(SageMaker Processing)는 \"네이티브 라이브러리만\" 지원이므로 사용자 정의 라이브러리 활용에 제약이 있습니다.\n\n【시험 포인트】\"최소 운영 오버헤드\" + \"사용자 정의 라이브러리\" + \"확장 가능한 PySpark\"이 나타나면 Glue Studio를 우선 선택하세요. Glue 데이터 연결은 의존성 관리를 완전 자동화하며, 서버리스 확장으로 대규모 데이터 처리를 지원합니다. EMR은 제어 가능하나 운영 복잡도가 높습니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】AWS Lambda 구조화된 로깅(JSON 형식)은 각 로그 항목에 로그 레벨(level), 로그 타입(type: application/system) 등의 메타데이터를 구조화된 필드로 포함하므로, CloudWatch Logs Insights 쿼리로 레벨과 타입별로 세밀한 필터링이 가능합니다. 텍스트 형식에서는 이러한 구분이 불가능합니다.\n\n【정답 포인트】C가 정답인 이유는 \"애플리케이션 로그(ERROR만)\" + \"시스템 로그(WARN만)\"라는 로그 타입과 레벨 두 가지 기준을 동시에 필터링해야 하기 때문입니다. JSON 로그 형식으로 구성하면 각 로그가 {\"level\": \"ERROR\", \"type\": \"application\"}처럼 메타데이터를 포함하게 되고, CloudWatch Logs Insights에서 \"filter level = 'ERROR' and type = 'application'\" 같은 쿼리로 원하는 로그만 추출할 수 있습니다.\n\n【오답 체크】A(IAM 권한)는 로그 필터링과 무관하며 로그 레벨 제어를 하지 못합니다. B(함수 코드 수준)는 관리되는 런타임의 시스템 로그까지 제어 불가능하며 애플리케이션과 시스템 타입 분리가 안 됩니다. D(사용자 정의 로그 그룹)는 로그 수집 대상만 변경할 뿐 레벨별 필터링을 구현하지 못합니다.\n\n【시험 포인트】\"로그 수준 필터링\" + \"서로 다른 레벨(ERROR vs WARN)\" + \"로그 타입 구분(애플리케이션 vs 시스템)\"이 나타나면 JSON 로그 형식을 선택하세요. 구조화된 로깅은 CloudWatch Logs Insights의 강력한 쿼리 기능을 활용하며, Lambda의 기본 기능이므로 추가 구현 불필요합니다.",
