...
// 각 문제: 한글 번역 + 【핵심 용어】【정답 포인트】【오답 체크】【시험 포인트】 구조의 상세 해설
...
window.MLA_QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "AB",
    "explanation": "【핵심 용어】\n▸ Binary Encoding — 카테고리를 이진 비트로 표현 (2진수 1개=1 열)\n▸ Label Encoding — 카테고리를 정수로 매핑 (예=1, 아니오=0)\n▸ One-hot Encoding — 각 카테고리별 별도 열 생성 (차원성 증가)\n\n【정답 포인트】\n▸ \"차원성 증가 금지\" → (A) Binary, (B) Label encoding 만 가능합니다. 둘 다 2개 카테고리를 1개 차원으로 표현합니다.\n\n【오답 체크】\n(C) One-hot은 2개 값당 2개 열로 차원성이 2배 증가합니다.\n(D) Statistical imputation은 결측값 처리용이며 인코딩이 아닙니다.\n(E) Tokenization은 텍스트 처리용이며 예/아니오 이진 값 인코딩 부적절합니다.\n\n【시험 포인트】\n차원성 유지 필수 → Binary/Label encoding / 차원성 확대 가능 → One-hot",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Factorization Machines — 희소 데이터에서 특성 간 상호작용을 학습하는 추천 알고리즘\n▸ k-NN — 밀집 데이터의 유사도 기반 분류/회귀용\n\n【정답 포인트】\n▸ \"고차원 희소 데이터\" + \"추천 모델\" → Factorization Machines가 정확한 선택입니다. 협업 필터링과 콘텐츠 기반 필터링 모두 지원하며 희소성 극복 특화되어 있습니다.\n\n【오답 체크】\n(A) k-NN은 희소 고차원 데이터에서 거리 메트릭 신뢰성 낮습니다.\n(C) PCA는 차원 축소용이며 예측 모델이 아닙니다.\n(D) seq2seq는 시계열/기계번역용이며 추천 알고리즘이 아닙니다.\n\n【시험 포인트】\n희소 데이터 추천 → Factorization Machines 는 SageMaker 알고리즘 선택의 중요 문제입니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ SageMaker Training Compiler — 대규모 모델 훈련 최적화로 메모리/컴퓨트 효율성 향상\n▸ Deep Learning Containers (DLC) — 사전 최적화된 컨테이너 이미지\n\n【정답 포인트】\n▸ \"LLM 훈련\" + \"실시간 대화형 배포\" → Training Compiler로 최적화된 훈련 + Real-time endpoint로 즉시 응답 구조가 최적입니다. 대규모 모델의 훈련 성능 극대화 및 실시간 사용자 상호작용 지원합니다.\n\n【오답 체크】\n(B) DLC는 훈련 최적화가 Compiler보다 약하며 배포 공식입니다.\n(C) Notebook Jobs는 대규모 LLM 훈련에 부적절합니다. Async inference는 실시간 대화 부적합합니다.\n(D) Studio는 개발 환경이며 훈련용 아님입니다. Batch transform은 비대화형입니다.\n\n【시험 포인트】\nLLM 훈련 + 실시간 배포 → Training Compiler + Real-time endpoint 는 명확합니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Shadow Variant — 프로덕션 트래픽 100%를 샘플링하며 실제 사용자 영향 없음\n▸ Canary Deployment — 제한된 트래픽 비율로 점진적 전환\n\n【정답 포인트】\n▸ \"위험 최소화\" + \"트래픽 중단 금지\" + \"프로덕션 검증\" → Shadow variant가 유일한 완벽한 솔루션입니다. 실제 사용자 영향 없이 v2를 100% 실시간 데이터로 검증하며 Canary보다 안전합니다.\n\n【오답 체크】\n(A) (B) Canary 배포는 최소 1% 실제 사용자에게 v2 배포하므로 위험 존재합니다.\n(C) 별도 엔드포인트 + 데이터 캡처는 복잡하고 신뢰성 낮습니다.\n\n【시험 포인트】\n무중단 프로덕션 검증 → Shadow variant 는 SageMaker 배포 전략의 핵심입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ DPL(차이 비율) — 특정 세그먼트에서 양성 클래스의 비율이 전체 데이터셋과 비교할 때 얼마나 다른지를 측정\n▸ 오버샘플링 — 소수 클래스 샘플을 증가시켜 클래스 불균형 해소\n▸ 언더샘플링 — 다수 클래스 샘플을 감소시켜 클래스 불균형 해소\n\n【정답 포인트】\n▸ DPL 값이 +0.9로 매우 높음 → 40~45세 구간에서 양성 클래스가 과도하게 대표됨\n▸ 과도하게 대표되는 세그먼트를 줄이려면 언더샘플링 적용 필요\n▸ 문제는 특정 연령대(40~45세)에 양성 클래스가 집중된 것이므로 해당 구간을 언더샘플링하여 균형 맞춤\n\n【오답 체크】\n(A) 오버샘플링하면 이미 과도한 양성 클래스를 더 증가시켜 불균형 악화\n(C) 다른 연령대의 양성 클래스를 줄이면 40~45세의 편향은 더 심해짐\n(D) 음성 클래스를 다루는 것은 문제가 되는 양성 클래스의 과도한 표현을 해결하지 못함\n\n【시험 포인트】\nDPL 값 해석 → 높은 DPL은 세그먼트에서 양성이 과도 → 언더샘플링으로 correction",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Inter-container traffic encryption — SageMaker 분산 학습에서 컨테이너 간 통신 암호화\n▸ 네트워크 격리 — VPC 내에서 리소스 격리, 암호화 기능 없음\n▸ VPC 플로우 로그 — 네트워크 트래픽 모니터링 및 로깅만 수행\n\n【정답 포인트】\n▸ 분산 학습에서 여러 인스턴스 간 통신이 발생 → 컨테이너 간 데이터 전송 암호화 필요\n▸ SageMaker는 inter-container traffic encryption 옵션 제공\n▸ CreateModel API에서 EnableInterContainerTrafficEncryption=true로 설정하여 TLS 암호화 적용\n\n【오답 체크】\n(A) 네트워크 격리는 네트워크 경계를 정의하지만 암호화를 제공하지 않음\n(B) 보안 그룹은 방화벽 규칙 관리이며 자체적으로 데이터 암호화하지 않음\n(D) VPC 플로우 로그는 트래픽 로깅만 하며 암호화 기능 없음\n\n【시험 포인트】\n분산 학습 + 암호화 필요 → inter-container traffic encryption 옵션",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Data drift — 시간 경과에 따라 프로덕션 데이터의 분포가 학습 데이터와 달라지는 현상\n▸ Concept drift — 타겟 변수와 피처 간의 관계가 변하는 현상\n▸ Model degradation — 시간에 따른 모델 성능 저하\n\n【정답 포인트】\n▸ 모델이 장기간 성능 유지 → 학습 데이터 부족은 원인 아님\n▸ 갑작스러운 성능 저하 → 실시간 데이터 변화로 인한 drift 발생\n▸ 프로덕션 데이터의 특성이 학습 시점의 데이터와 다르면 모델 성능 급감\n\n【오답 체크】\n(A) 이미 학습 완료되어 있으며 긴 시간 성공 → 초기 데이터 부족은 아님\n(C) 리소스 제약은 점진적 성능 저하 발생, 급격한 저하 아님\n(D) 오버피팅은 학습/테스트 성능 불일치로 초기에 나타남, 배포 후 갑작스럽지 않음\n\n【시험 포인트】\n장기 배포 후 갑작스러운 성능 저하 → Data drift 의심 → SageMaker Model Monitor로 감지",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ DeepAR — SageMaker 내장 알고리즘, Parquet 형식 선호\n▸ RecordIO-Protobuf — Amazon 머신러닝 형식, 주로 이미지/텍스트용\n▸ Gzip — 범용 압축 포맷, Parquet과 호환성 우수\n▸ XZ — 높은 압축률이나 Parquet 호환성 낮음\n\n【정답 포인트】\n▸ DeepAR 알고리즘은 시계열 데이터 처리 → Parquet 형식 권장\n▸ Parquet는 구조화된 데이터에 최적화된 칼럼 기반 형식\n▸ gzip은 Parquet과 표준 호환성 제공하며 좋은 압축률 달성\n\n【오답 체크】\n(A) Snappy는 빠른 압축이지만 최적 압축률 제공 안 함\n(B) RecordIO-Protobuf는 시계열 데이터에 부적합, DeepAR용 아님\n(C) XZ는 높은 압축률이나 Parquet 처리 시 호환성 문제, 느린 처리\n\n【시험 포인트】\nDeepAR 시계열 → Parquet 형식 + gzip 압축 조합",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Parquet — 칼럼 기반 저장 형식, 쿼리 성능 최적화에 탁월\n▸ CSV — 행 기반 텍스트 형식, 대용량 데이터 쿼리 시 느림\n▸ AWS Glue ETL — 관리형 데이터 변환 서비스, 사용 용이\n▸ 운영 오버헤드 최소 — 인프라 관리 불필요한 서비스 선택\n\n【정답 포인트】\n▸ CSV는 전체 행을 읽어야 하므로 대용량 데이터 쿼리 비효율적\n▸ Parquet는 필요한 칼럼만 읽음 → IO 50-90% 감소\n▸ AWS Glue는 완전 관리형 서비스 → 인프라 관리 필요 없음\n▸ 한 번의 ETL 작업으로 모든 파일 변환 가능\n\n【오답 체크】\n(A) 파일 분할은 성능 개선 미미, 데이터 형식 최적화 아님\n(B) 칼럼 삭제는 쿼리 성능 근본 개선 아님, 형식 최적화 필요\n(D) EMR은 클러스터 관리 필요 → 운영 오버헤드 높음\n\n【시험 포인트】\n최소 오버헤드 + 쿼리 성능 → Glue ETL + Parquet 조합",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ DPL — 클래스 라벨의 비율 차이를 측정, 클래스 불균형 감지용\n▸ MSE — 회귀 모델의 예측 오차 측정, 분류 불균형과 무관\n▸ Silhouette score — 클러스터링 품질 평가, 분류 불균형 감지 안 함\n▸ SSIM — 이미지 유사도 측정, 표 형식 데이터 분석에 부적합\n\n【정답 포인트】\n▸ \"클래스 불균형 확인\" → 클래스 라벨의 분포 이상 확인 필요\n▸ DPL은 특정 세그먼트에서 양성 클래스 비율이 전체 데이터와 얼마나 다른지 측정\n▸ DPL 값이 크면 클래스 불균형 심함 → 편향 위험 높음\n\n【오답 체크】\n(A) MSE는 회귀용 손실 함수, 클래스 라벨 불균형 감지 불가\n(C) Silhouette score는 비지도 학습(클러스터링)용, 분류 문제와 무관\n(D) SSIM은 이미지 처리용, 테이블 형식 분류 데이터에 미적용\n\n【시험 포인트】\n사전학습 편향 측정 = DPL로 클래스 라벨 비율 불균형 확인",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Multi-model endpoint — 단일 엔드포인트에서 여러 모델 호스팅, 동적 로딩\n▸ Multi-container endpoint — 고정된 여러 컨테이너, 메모리 상주\n▸ Single-model endpoint — 모델당 별도 엔드포인트, 높은 비용\n▸ 추론 비용 최적화 — 컴퓨팅 리소스 효율성 극대화\n\n【정답 포인트】\n▸ \"여러 ML 모델\" + \"비용 최적화\" → 공유 리소스 활용 필수\n▸ Multi-model endpoint는 같은 프레임워크의 모델들을 동일 컨테이너에서 로드/언로드\n▸ 사용하지 않는 모델을 메모리에서 제거 → 메모리 절약, 비용 감소\n▸ 단일 엔드포인트 관리 → 인프라 오버헤드 최소\n\n【오답 체크】\n(A) Multi-container는 모든 컨테이너 메모리 상주 → 비용 높음\n(C) Sequential invocation은 성능 저하, 동시 요청 처리 불가\n(D) 모델당 별도 엔드포인트 → 가장 비용 비효율적\n\n【시험 포인트】\n같은 프레임워크 + 여러 모델 + 비용 최적화 → Multi-model endpoint",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Amazon A2I — 인간 검수 워크플로우, 머신 예측 + 사람 판단 결합\n▸ Amazon Rekognition — 컴퓨터 비전 서비스, 객체/동작 감지\n▸ SageMaker Ground Truth — 대규모 라벨 지정 서비스, 자동화 제한적\n▸ SageMaker JumpStart — 사전학습 모델 마켓플레이스, 라벨 지정 직접 지원 안 함\n\n【정답 포인트】\n▸ 비디오 움직임 분류 → 컴퓨터 비전 + 자동 감지 필요\n▸ Amazon Rekognition이 자동으로 비디오의 동작 감지\n▸ A2I가 Rekognition 결과에 대한 인간 검수 제공 → 라벨 지정 시간 대폭 단축\n▸ 자동 + 수동 검수 조합 → 4개월 대비 훨씬 빠른 처리 가능\n\n【오답 체크】\n(A) Ground Truth는 수동 라벨 지정 조율, 자동화 부족\n(B) JumpStart는 모델 사용/학습용, 라벨 지정 시간 단축 기능 없음\n(C) Data Wrangler는 데이터 변환 도구, 라벨 지정 가속 기능 없음\n\n【시험 포인트】\n비디오 라벨 시간 단축 → Rekognition 자동 감지 + A2I 인간 검수",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ SageMaker Clarify — 모델 편향/공정성 모니터링, 피처 중요도 분석\n▸ SageMaker Debugger — 학습 과정 디버깅, 배포 후 이상 감지 제한\n▸ X-Ray — API/요청 추적, 성능 저하 원인 분석 불충분\n▸ Model drift — 시간 경과에 따른 성능 저하, 편향 악화로 인한 경우 많음\n\n【정답 포인트】\n▸ \"시간 경과에 따른 성능 저하\" → drift 또는 편향 증가 가능성\n▸ SageMaker Clarify는 배포 후 모델의 편향 변화를 지속 모니터링\n▸ 피처 속성 편향 감지 → 모델 재학습 자동 트리거 가능\n▸ 수요 예측은 사회경제적 변화 → 편향 악화 가능성 높음\n\n【오답 체크】\n(A) Debugger는 학습 중 이상 감지, 배포 후 자동 재학습 트리거 기능 약함\n(B) X-Ray는 요청 지연 시간/에러율 추적, 모델 편향 감지 불가\n(C) Ground Truth는 데이터셋 큐레이션, 자동 재학습 트리거 기능 없음\n\n【시험 포인트】\n장기 배포 + 자동 재학습 + 편향 모니터링 → SageMaker Clarify",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Amazon Rekognition eye gaze detection — 눈동자 움직임/응시 방향 감지\n▸ 운전자 산만함 지표 — 눈 돌림, 도로 이탈, 스마트폰 사용\n▸ 최소 운영 노력 — 관리형 서비스, 커스텀 개발 최소화\n▸ 이미지 분석 — MP4 비디오 스트림에서 실시간 프레임 처리\n\n【정답 포인트】\n▸ 차량 카메라 → 이미지/비디오 데이터 수집 가능\n▸ Rekognition은 관리형 컴퓨터 비전 서비스, 즉시 사용 가능\n▸ Eye gaze detection은 운전 집중도 판단 → 산만함 감지 직결\n▸ 별도 모델 개발/학습 불필요 → 최소 운영 노력\n\n【오답 체크】\n(B) SageMaker는 커스텀 모델 학습 필요 → 개발 노력 높음\n(C) 타사 시스템 + Rekognition 통합 → 복잡도 증가, 비용 높음\n(D) Comprehend는 텍스트 분석, 카메라 영상 처리 불가\n\n【시험 포인트】\n비디오 카메라 + 운전자 안전 + 최소 운영 → Rekognition eye gaze detection",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ AWS CDK — Python/TypeScript로 Infrastructure as Code, 프로그래밍 친화적\n▸ AWS CloudFormation — JSON/YAML 기반 Infrastructure as Code\n▸ AWS CLI — 명령형 도구, 반복 가능한 IaC 부족\n▸ Amazon EKS — AWS 관리형 Kubernetes, 컨트롤 플레인 자동 관리\n\n【정답 포인트】\n▸ \"Python을 사용하여 프로비저닝\" → CDK는 Python/TypeScript 지원 (CloudFormation은 JSON/YAML)\n▸ \"컨트롤 플레인 관리 불필요\" → EKS는 완전 관리형 서비스\n▸ \"반복 가능한 방식\" → Infrastructure as Code 필수\n▸ CDK는 Python 코드로 EKS 생성 자동화 가능\n\n【오답 체크】\n(A) CloudFormation은 JSON/YAML, Python 아님. EC2 직접 관리 → 컨트롤 플레인 관리 필요\n(B) CLI는 반복 가능한 IaC 제공 못 함, 수동 명령 필요\n(D) CloudFormation은 Python 기반 아님 (YAML/JSON)\n\n【시험 포인트】\nKubernetes + Python IaC + 관리형 → AWS CDK + Amazon EKS",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ 오버피팅 — 학습 데이터에 과도 적응, 새 데이터 성능 저하\n▸ 교차 검증(Cross-validation) — 학습/테스트 데이터 분할 최적화\n▸ 정규화(Regularization) — L1/L2 제약으로 복잡도 제어\n▸ SageMaker Experiments — 하이퍼파라미터 조정 추적 및 비교\n\n【정답 포인트】\n▸ 학습 정확도 높음 + 테스트 성능 저하 → 오버피팅 특징\n▸ 오버피팅 해결 → 정규화 + 교차 검증 필수\n▸ Experiments로 여러 정규화 강도/모델 버전 체계적 비교\n▸ 최적 하이퍼파라미터 찾기 → 성능 개선 검증 가능\n\n【오답 체크】\n(A) 복잡도 증가 → 오버피팅 악화, 근본 해결 아님\n(C) 배포 전 오버피팅 해결 필요, 배포 후 조정은 비효율\n(D) 데이터 증가만으로는 부족, 모델 복잡도 제어 필요\n\n【시험 포인트】\n오버피팅(train high, test low) → 정규화 + 교차검증 + Experiments",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Blue/Green 배포 — 구 버전(Blue) / 신 버전(Green) 전환 배포\n▸ Canary 배포 — 작은 트래픽 비율로 신 버전 테스트\n▸ Shadow testing — 신 버전에서 동일 요청 처리, 결과는 사용자에게 미제공\n▸ 프로덕션 영향 최소 — 기존 사용자 경험 변화 없음\n\n【정답 포인트】\n▸ \"기존 모델 사용자에게 영향 주지 않고\" → 기존 모델은 그대로 운영\n▸ \"프로덕션 시나리오에서 평가\" → 실제 트래픽으로 테스트\n▸ Shadow testing은 100% 트래픽을 신 모델에도 전달 → 정확 성능 측정\n▸ 신 모델 결과는 사용자 응답에 미포함 → 위험 제로\n\n【오답 체크】\n(A) Linear shifting도 일부 사용자가 신 모델 영향 받음\n(B) Canary도 실제 사용자 트래픽 일부 신 모델로 라우팅 → 영향 가능성\n(C) Rolling은 점진적 전환, 기존 모델 일부 중단\n\n【시험 포인트】\n기존 무영향 + 실제 트래픽 테스트 → Shadow testing 100%",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS Glue — 관리형 ETL 서비스, Data Catalog로 메타데이터 자동 저장\n▸ Glue Crawler — 데이터 스키마 자동 감지, 카탈로그 자동 생성\n▸ Data Catalog — 모든 데이터 자산의 중앙 메타데이터 저장소\n▸ 최소 수동 작업 — 자동화된 스키마 감지 필수\n\n【정답 포인트】\n▸ \"스키마 발견\" + \"메타데이터 저장\" → Glue Crawler 자동 수행\n▸ Glue Crawler가 S3 데이터 스캔 → 스키마 자동 감지\n▸ 감지된 스키마 → Data Catalog에 자동 저장\n▸ 분석 도구(Athena, EMR 등)가 Catalog 활용 → 최소 설정\n\n【오답 체크】\n(B) Data Wrangler는 시각적 변환 도구, 메타데이터 자동 저장 기능 약함\n(C) Athena는 쿼리 엔진, 스키마 감지 기능 제한적, 메타데이터 관리 필요\n(D) EC2 직접 관리 → 운영 오버헤드 높음, 메타데이터 저장 수동\n\n【시험 포인트】\n최소 수동 작업 + 스키마 자동 감지 + 메타데이터 저장 → AWS Glue Crawler",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Parquet — 칼럼 기반 저장 형식, 배치/실시간 모두 최적화\n▸ CSV — 행 기반 텍스트, 전체 행 스캔 필요, 쿼리 성능 낮음\n▸ JSON — 반구조화 데이터, 파싱 오버헤드 높음\n▸ Athena — S3 파일 직접 쿼리, 형식 선택이 성능 좌우\n\n【정답 포인트】\n▸ 배치 처리 → 대용량 데이터 스캔 효율성\n▸ 실시간 쿼리 → 낮은 지연 시간 필요\n▸ Parquet는 칼럼 프루닝 → 필요 칼럼만 읽음\n▸ 스냅시 압축 내장 → IO 50-90% 감소\n▸ Athena + Parquet = 배치/실시간 모두 최상의 성능\n\n【오답 체크】\n(A) CSV는 전체 행 읽기 필수, 대규모 데이터 배치 처리 느림\n(C) 중첩 JSON은 파싱 오버헤드 높음, 실시간 쿼리 느림\n(D) 역직렬화 JSON도 동일하게 성능 저하\n\n【시험 포인트】\nAthena + 배치/실시간 혼합 → Parquet이 유일한 최적 선택",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Early stopping — 검증 오류가 증가하기 시작할 때 학습 종료(과적합 방지)\n▸ Overfitting — 학습 데이터에 과도하게 특화되어 검증 성능 저하\n▸ Performance peak and decline — 검증 세트에서 과적합 신호\n\n【정답 포인트】\n▸ \"performance peaks early then declines\" = 과적합 전형 증상\n▸ 초기 성공 + 점진 저하 → 검증 오류 증가 시나리오\n▸ 과적합 방지 = Early stopping(학습 중단 시점 결정)\n\n【오답 체크】\n(A) 레이어 수 증가: 모델 복잡도 ↑ = 과적합 위험 증가(악화)\n(C) 뉴런 수 추가: 모델 용량 ↑ = 더 심한 과적합 유발\n(D) 편향/분산 검토: 근본 원인 분석이지만 해결책 아님\n\n【시험 포인트】\n학습 곡선에서 train loss ↓하지만 val loss ↑ = 과적합. Early stopping은 이 지점에서 학습 중단. SageMaker에서 검증 메트릭 모니터링으로 자동 구현 가능. 엔터테인먼트 추천 시스템 같은 복잡 모델의 표준 정규화 기법.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ F1 score — Precision과 Recall의 조화평균(불균형 데이터에 적합)\n▸ Accuracy — 전체 정확률(다수 클래스 편향, 불균형에 부적합)\n▸ Class imbalance — 소수 클래스 성능 저하 현상\n▸ Image augmentation — 기존 이미지 변형으로 가상 샘플 생성\n\n【정답 포인트】\n▸ \"poor on one class\" + \"extremely small fraction\" → 불균형 문제\n▸ 불균형 데이터 → F1 score(소수 클래스 성능 중시)\n▸ 이미지 데이터 → 이미지 증강(SMOTE는 수치형에 최적)\n▸ 메트릭 선택이 핵심: Accuracy는 소수 클래스 무시 가능\n\n【오답 체크】\n(A) 정확도 최적화: 소수 클래스 1%라면 정확도는 다수 클래스만으로도 99% 가능(소수 클래스 개선 무시)\n(C) SMOTE: 수치형 테이블 데이터용(이미지 처리에는 부자연스러운 합성)\n(D) F1 + SMOTE: F1은 맞으나 이미지에는 증강이 표준\n\n【시험 포인트】\n불균형 분류 문제의 명제: (1) 메트릭 = Recall/F1(소수 클래스 가중), (2) 샘플링 = 이미지는 증강, 수치는 SMOTE. 소수 클래스가 극소수면 Accuracy는 거짓 신뢰도 줄 수 있음. F1은 두 클래스 성능 균형.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ k-means clustering — 비지도 학습으로 데이터를 k개 그룹으로 분할\n▸ Segmentation — 유사한 특성의 객체 그룹화(환자 위험군 분류)\n▸ Hyperparameter k — 클러스터 개수(직접 제어 가능)\n\n【정답 포인트】\n▸ \"segment patients into groups\" → 클러스터링 알고리즘\n▸ \"determine the number of groups by using hyperparameters\" → k 값 설정\n▸ \"similar risk factors\" → 동질 그룹화(k-means의 목표)\n\n【오답 체크】\n(A) XGBoost: 지도 학습(분류/회귀), 세분화 아님\n(C) DeepAR: 시계열 예측, 환자 세분화와 무관\n(D) RCF: 이상 탐지, 일반적 세분화 아님\n\n【시험 포인트】\n의료 환자 세분화 = k-means 표준. k는 사용자가 정의한 그룹 수(예: 저위험/중위험/고위험 = k=3). k-means는 거리 기반 응집도(cohesion) 최대화. 위험 요소 수치화 후 k-means로 동질 환자군 발굴. 개인화 치료 계획 수립 기초.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Document embedding — 문서를 고정 크기 수치 벡터로 변환(검색용)\n▸ Instruction tuning — 모델을 특정 명령에 반응하도록 학습(생성용)\n▸ Bedrock knowledge base — S3의 문서를 벡터화하여 검색 가능하게 구성\n▸ Cohere Embed — 다국어 임베딩 모델(RAG에 최적)\n\n【정답 포인트】\n▸ RAG 시스템 = 문서 검색(embedding) + 생성(LLM)\n▸ \"process PDF files and store in vector store\" → embedding 필수\n▸ Cohere Embed Multilingual — Bedrock 지식 기반의 표준 임베딩 모델\n▸ 다국어 지원 = 글로벌 PDF 처리 가능\n\n【오답 체크】\n(A) Titan Text Premier: 텍스트 생성 모델(embedding 아님)\n(B) Mistral 7B Instruct: 생성 모델, embedding 기능 없음\n(C) Claude Sonnet: 대화형 LLM(문서 임베딩 아님)\n\n【시험 포인트】\nBedrock의 RAG 구현: (1) 임베딩 모델로 문서 벡터화 = Cohere Embed, (2) 벡터 저장소 저장(OpenSearch, Pinecone 등), (3) 쿼리 임베딩 후 유사 문서 검색, (4) LLM으로 답변 생성. Cohere Embed는 768차원 벡터로 의미 정보 압축.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS Glue interactive sessions — 서버리스 Spark 환경으로 ETL 작업 실행\n▸ PII detection transform — Glue의 PII 자동 감지 및 제거 기능\n▸ Petabyte-scale — 대규모 분산 처리(Spark 필수)\n▸ Unstructured data — 텍스트, 문서 등 형태 없는 데이터\n\n【정답 포인트】\n▸ \"petabytes of unstructured data\" → 대규모 분산 처리 = Spark\n▸ \"identify and remove PII\" → Glue의 Detect PII 변환 기능\n▸ Glue interactive sessions → 서버리스(비용 효율), 확장 자동\n▸ SageMaker ML 학습 준비 → Glue에서 전처리\n\n【오답 체크】\n(B) SageMaker Canvas: 노코드 머신러닝, 대규모 ETL 부적합\n(C) Clarify: 모델 편향성/공정성 분석, PII 제거 기능 없음\n(D) Comprehend DetectEntities: NLP 개체 인식(PII 제거 자동화 아님, 비용도 높음)\n\n【시험 포인트】\nPII 규제 준수(GDPR, HIPAA) 시나리오에서 Glue Detect PII는 자동 마스킹/제거. 페타바이트 규모는 Spark 분산 처리 필수(단일 머신 불가). Glue interactive sessions로 빠르게 프로토타입 → SageMaker로 모델 학습 파이프라인 구성. 의료/금융 데이터 처리 표준 접근.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ SageMaker Model Monitor — 프로덕션 모델의 데이터 드리프트(distribution shift) 감지\n▸ Feature distribution drift — 입력 데이터 분포 변화(마케팅 캠페인 같은 사건 후)\n▸ SageMaker Clarify — 데이터 편향 및 변화 분석 도구\n▸ Proactive monitoring — 성능 문제 발생 전 조기 감지\n\n【정답 포인트】\n▸ \"sharp drop in performance\" → 데이터 분포 변화 신호(캠페인 후 고객 행동 변화)\n▸ \"detect feature distribution changes\" = SageMaker Clarify\n▸ \"near real-time input validation\" = Model Monitor(거의 즉시 드리프트 감지)\n▸ \"before future campaigns\" → 선제적 모니터링(조기 경보)\n\n【오답 체크】\n(B) CloudWatch + feature attribution: 속성 추적은 설명성용(성능 저하 예측 아님)\n(C) Clarify bias detection + latency alarm: 편향은 한 가지 요인, 지연은 성능 저하 원인 아님\n(D) Constraints monitoring + error logs: 로그 분석은 사후 대응, 선제적 감지 아님\n\n【시험 포인트】\n마케팅 캠페인 후 성능 저하의 근본 원인 = 고객 행동 변화(예: 새로운 구매층 유입)로 인한 feature distribution shift. Model Monitor는 입력 데이터 통계(평균, 분포)를 기준값과 비교하여 드리프트 자동 감지. Clarify로 원인 분석(어느 feature가 변했나?), Model Monitor로 모니터링(실시간 경보). 추천 시스템의 성능 저하 대응 표준 절차.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Stratified split — 각 클래스(월, 지역)의 분포를 학습/검증에 균등 유지\n▸ Seasonality — 특정 기간(계절)의 높은 판매량(모델이 학습하지 못한 패턴)\n▸ Distribution mismatch — 학습과 검증 데이터의 분포 불일치\n▸ Normalization — 수치 피처 스케일 조정(여기서는 부차적)\n\n【정답 포인트】\n▸ \"두 달 매년 높은 판매\" = 월별 계절성 패턴 존재\n▸ 무작위 분할(random split) → 검증셋에 이상값(높은 판매 월) 과다 포함 가능\n▸ 층화된 분할(stratified) → 각 월/지역 비율을 동등하게 유지\n▸ 학습과 검증의 분포 일치 = 공정한 성능 평가\n\n【오답 체크】\n(A) 이상값 제거: 높은 판매는 유효한 패턴이지 잡음 아님(정보 손실)\n(C) 전체 정규화: 월별 편차 무시하고 스케일만 조정(계절성 미반영)\n(D) 지역별 정규화: 지역 내 편차 감소하나 월 패턴 해결 안 함\n\n【시험 포인트】\n시계열 분류 + 계절성 = 층화된 분할 필수. 임의 분할에서 검증셋이 높은 판매 월 과다/과소 포함 시 검증 정확도 왜곡. Stratified split은 month 및 region 같은 그룹 변수의 비율을 학습/검증에 동일하게 배분. 20년 월간 데이터(240개)에서 계절성 학습 필요.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ SageMaker Data Wrangler — S3, Athena, Redshift, Snowflake 등 다양한 소스 직접 연결\n▸ Data source connectivity — 단일 도구에서 여러 데이터베이스 쿼리 가능\n▸ ETL automation — 데이터 로드, 변환, 검증 통합\n\n【정답 포인트】\n▸ \"directly from S3, Athena, and Snowflake\" → 다중 소스 지원 필수\n▸ SageMaker Data Wrangler = 통합 데이터 수집 + 변환 도구\n▸ 직접 로드(Direct load) = DataSync 같은 중간 도구 불필요\n▸ \"load historical data\" → 배치 또는 스트림 가능\n\n【오답 체크】\n(A) AWS Glue DataBrew: 데이터 정제 도구(소스 연결 기능 제한적)\n(B) SageMaker Pipelines + DataSync: Pipelines은 ML 오케스트레이션, DataSync는 온프레미스-AWS 동기화(클라우드 데이터 로드에 부적합)\n(C) Feature Store + Spark: Feature Store는 ML 피처 저장소(S3/Athena 직접 연결보다 사후 관리용)\n\n【시험 포인트】\nSageMaker Data Wrangler는 UI 기반으로 (1) S3, Athena, Snowflake 쿼리, (2) 데이터 로드, (3) 결측치/이상값 처리, (4) 인코딩, (5) SageMaker 학습용 내보내기를 원클릭으로 처리. 다중 데이터 소스 통합 시나리오의 표준 솔루션. 코드 최소화, 속도 최대화.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ SKLearnProcessor — SageMaker에서 scikit-learn 스크립트 실행 도구\n▸ Processing job — 데이터 전처리를 EC2 인스턴스에서 실행\n▸ Custom scikit-learn script — 사용자 정의 변환 로직\n▸ SageMaker Python SDK — AWS 통합 개발 환경\n\n【정답 포인트】\n▸ \"custom scikit-learn script\" → SKLearnProcessor 필수\n▸ \"prepare data and train\" → Processing job(전처리) + Training job(학습)\n▸ SageMaker Python SDK = 모든 작업 오케스트레이션\n▸ scikit-learn 환경 자동 구성(Docker 이미지 관리 불필요)\n\n【오답 체크】\n(A) AWS Lambda: 함수 시간 제한(15분), 대규모 데이터 처리 부적합\n(C) HuggingFaceProcessor: Hugging Face 모델용(scikit-learn 아님)\n(D) AWS Glue ETL: Glue 맞춤형 레시피(scikit-learn 스크립트 직접 실행 아님)\n\n【시험 포인트】\nSageMaker는 각 프레임워크별 Processor 클래스 제공: SKLearnProcessor(scikit-learn), PyTorchProcessor(PyTorch), XGBoostProcessor(XGBoost) 등. SKLearnProcessor는 (1) 로컬 스크립트 → S3에 업로드, (2) EC2 인스턴스에서 실행, (3) 결과 저장을 자동 처리. 데이터 과학자의 기존 scikit-learn 코드를 SageMaker로 바로 적용 가능.",
  ...
  },
  {
    ...
    "answer": "AC",
    "explanation": "【핵심 용어】\n▸ bedrock:InvokeModel — Bedrock 모델(Titan, Claude 등) 호출 권한\n▸ sqs:ReceiveMessage — SQS 큐에서 메시지 수신\n▸ sqs:DeleteMessage — SQS 큐에서 메시지 제거\n▸ bedrock:Get* — 모델 정보 조회(호출 아님)\n\n【정답 포인트】\n▸ 애플리케이션 흐름: (1) SQS 메시지 읽기 → (2) Bedrock 이미지 생성 → (3) 메시지 삭제\n▸ (1) 필요 권한: sqs:ReceiveMessage (A 제외, C와 묶임)\n▸ (2) 필요 권한: bedrock:InvokeModel (Bedrock 모델 실행)\n▸ (3) 필요 권한: sqs:DeleteMessage (C에 포함)\n\n【오답 체크】\n(B) bedrock:Get*: 메타데이터/정보 조회만(모델 호출 아님)\n(D) sqs:GetQueueAttributes: 큐 속성 조회(메시지 수신/삭제 아님)\n(E) sagemaker:PutRecord*: SageMaker Feature Store용(Bedrock/SQS와 무관)\n\n【시험 포인트】\nIAM 최소 권한 원칙(Least Privilege): (1) SQS 작업 = sqs:ReceiveMessage + sqs:DeleteMessage, (2) Bedrock 작업 = bedrock:InvokeModel. Get/List 권한과 실제 작업 권한 구분 필수. 2-선택 문제에서 AC는 각각 독립적인 작업(SQS와 Bedrock)을 담당.",
