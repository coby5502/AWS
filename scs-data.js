// AWS Certified Security - Specialty (SCS-C03) 한국어 문제 데이터
...
window.SCS_QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Amazon Macie — S3의 민감한 데이터 발견(PII, 금융정보 등) 전문 서비스\n▸ AWS Security Hub — 보안 발견사항의 중앙화된 수집 및 관리\n▸ Delegated Administrator — 조직 전체 계정의 권한을 대표 계정에 위임\n\n【정답 포인트】\n▸ 보안 계정을 Macie와 Security Hub 위임 관리자로 설정\n▸ Macie → S3 민감한 데이터 자동 발견 (PII 감지 전문)\n▸ Macie findings → Security Hub로 자동 게시\n▸ 결과: 한 위치(Security Hub)에서 조직 전체의 민감한 데이터 가시성\n▸ 관리 오버헤드 최소 → Macie 위임 관리로 자동 활성화\n\n【오답 체크】\n(B) Inspector — S3 민감한 데이터 발견 기능 부족 (주로 EC2/컨테이너 취약점)\n(C) Inspector + Trusted Advisor — S3 민감 데이터 전문 서비스 아님\n(D) Trusted Advisor — 단순 모범 사례 권장, 민감한 데이터 감지 기능 없음\n\n【시험 포인트】\n▸ Macie 고유 기능: S3 민감한 데이터 자동 분류 및 감지\n▸ 위임 관리자 패턴: 보안 계정 중앙화로 다중 계정 관리\n▸ Security Hub 중앙화: 조직 전체 보안 발견사항 단일 보기",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ GuardDuty DNS finding — VPC DNS 리졸버에 대한 비정상적인 DNS 쿼리 탐지\n▸ Custom DHCP OpenDNS — VPC의 기본 Amazon DNS 리졸버를 대체하는 외부 DNS\n▸ VPC Flow Logs — GuardDuty 발견과 무관한 네트워크 흐름 로깅\n\n【정답 포인트】\n▸ GuardDuty DNS finding 생성 조건: VPC의 Amazon DNS 리졸버(169.254.169.253) 사용\n▸ VPC DHCP 옵션에 사용자 지정 DNS(예: OpenDNS)가 설정되면 → Amazon DNS 리졸버 우회\n▸ GuardDuty가 감지할 DNS 트래픽 없음 → 발견 생성되지 않음\n▸ DNS 쿼리가 외부 리졸버로 가서 GuardDuty 모니터링 범위 벗어남\n\n【오답 체크】\n(A) VPC Flow Logs — GuardDuty 발견과 독립적 (GuardDuty는 자체 탐지 메커니즘)\n(C) GuardDuty-Security Hub 통합 — 발견이 생성되었다면 자동 게시 (발견 자체 문제)\n(D) Cross-Region 집계 — 단일 리전 환경이므로 관계없음\n\n【시험 포인트】\n▸ GuardDuty DNS finding: Amazon DNS 리졸버 트래픽 분석 기반\n▸ DHCP 옵션 설정: VPC 기본 DNS 리졸버 변경 시 GuardDuty 발견 영향\n▸ 테스트 실패 시나리오: 환경 구성(DHCP 옵션)이 원인이 될 수 있음",
  ...
  },
  {
    ...
    "answer": "BC",
    "explanation": "【핵심 용어】\n▸ IAM password policy — AWS 계정 사용자 비밀번호 규칙 (최소 길이, 복잡성 등)\n▸ Cognito password policy — Cognito 사용자 풀의 비밀번호 정책\n▸ On-premises Active Directory — 회사 네트워크의 독립적 ID 관리\n▸ SCP (Service Control Policy) — 조직 수준 권한 제어 (비밀번호 정책 미관여)\n\n【정답 포인트】\n▸ (B) Cognito 구성 → AWS에서 호스팅되는 사용자 풀의 비밀번호 정책 설정\n▸ (C) 온프레미스 AD → IAM과 연합된 외부 ID 공급자의 정책 설정\n▸ IAM은 (A)에서 설정 가능하지만, 선택지에 포함되지 않은 이유 검토 필요\n▸ 세 ID 공급자 각각에 독립적인 정책 설정 메커니즘 필요\n\n【오답 체크】\n(A) IAM password policy — 설정 가능하나, 문제에서 누락된 선택지 조합\n(D) SCP — 비밀번호 정책 적용 불가능 (권한 제어 도구, 정책 강제 도구 아님)\n(E) IAM 정책 — 비밀번호 길이 조건 사용 불가능 (정책 수행 불가 범위)\n\n【시험 포인트】\n▸ 다중 IdP 환경: 각 공급자별로 독립적인 정책 구성 필요\n▸ Cognito password policy — 사용자 풀 설정에서 직접 구성\n▸ 온프레미스 AD — AWS 외부이므로 해당 시스템의 정책 설정\n▸ SCP 오해: 조직 권한 제어, 비밀번호 정책 강제 불가",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Credential stuffing — 자동화된 도구를 사용한 대량의 로그인 시도\n▸ User agent filtering — HTTP 헤더의 User-Agent 문자열로 요청 식별\n▸ AWS WAF string matching — 특정 문자열 패턴을 찾아 규칙 적용\n\n【정답 포인트】\n▸ 공격 특성: 많은 IP 주소 + 특정 에뮬레이터 User-Agent\n▸ IP 기반 차단 불가능 → IP 주소가 분산되어 있음 (B 비효과적)\n▸ User-Agent 기반 차단 가능 → 모든 공격이 동일한 에뮬레이터 사용\n▸ WAF는 지정된 User-Agent 문자열을 차단하여 에뮬레이터 공격 방지\n▸ 합법적 로그인은 다른 User-Agent 사용 → 영향 없음\n\n【오답 체크】\n(A) CloudWatch 알람 + SNS → 알림만 생성, 공격 차단 안 됨\n(B) 보안 그룹 변경 → 분산된 IP 차단 불가능, 합법적 사용자도 차단 위험\n(D) 합법적 UA 허용 규칙 → 부정(negative) 규칙이므로 보수적이고, 새 User-Agent 추가 시 불편\n\n【시험 포인트】\n▸ WAF pattern matching: User-Agent, HTTP 헤더로 공격 감지\n▸ 자동화 공격 특성: 동일한 도구/설정으로 식별 가능\n▸ 블랙리스트 규칙 (C) > 화이트리스트 규칙 (D) 실용성",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ SQL injection attack — 악의적 SQL 코드를 입력하여 데이터베이스 조작\n▸ AWS WAF SQL injection detection — managed rule로 SQL 문법 탐지\n▸ Application Load Balancer (ALB) — WAF 적용 가능한 계층 7 로드 밸런서\n▸ Route 53 → ALB → EC2 → RDS 아키텍처 재설계\n\n【정답 포인트】\n▸ 24시간 제약 → 애플리케이션 패치 개발/배포 불가능 (C 탈락)\n▸ 최소한의 노력 → 기존 인스턴스 활용, 새 배포 아키텍처\n▸ ALB + WAF 조합 (A) → SQL injection 규칙 적용, 운영 중단 없음\n▸ 단계:\n  1. ALB 생성 (기존 EC2를 대상으로)\n  2. WAF 웹 ACL 생성 및 ALB에 적용\n  3. 테스트 후 Route 53 업데이트 (다운타임 최소)\n  4. EC2 보안 그룹 잠금 (인터넷 직접 접근 차단)\n\n【오답 체크】\n(B) CloudFront — 한 인스턴스만 원본으로 설정 (다중 인스턴스 미지원), 로드 밸런싱 불가\n(C) 소스 코드 패치 — 24시간 내 개발/테스트/배포 현실적 불가능\n(D) EC2에 WAF 직접 적용 — WAF는 ALB/CloudFront/API Gateway에만 적용 가능\n\n【시험 포인트】\n▸ WAF 적용 대상: ALB, CloudFront, API Gateway, AppSync (EC2 직접 불가)\n▸ 빠른 재설계: 기존 인프라를 WAF 보호 계층 앞에 배치\n▸ 운영 연속성: DNS 업데이트로 무중단 전환",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ EC2 Instance Connect — 브라우저 기반 안전한 SSH 연결 (키 쌍 불필요)\n▸ Host key validation — EC2 Instance Connect가 호스트의 진정성 확인\n▸ AWS trusted host keys database — AWS가 유지관리하는 신뢰할 수 있는 호스트 공개 키\n\n【정답 포인트】\n▸ EC2 Instance Connect 동작 방식:\n  1. 브라우저에서 인스턴스 선택\n  2. AWS가 인스턴스에 임시 SSH 공개 키 주입\n  3. 호스트 키로 인스턴스 진정성 검증 (AWS 신뢰 DB와 비교)\n▸ 호스트 키 회전 후 → AWS 신뢰 DB에 새 키 등록 필요\n▸ 새 호스트 키를 AWS 신뢰 데이터베이스에 수동 업로드 (B)\n\n【오답 체크】\n(A) KMS import — 호스트 키와 무관, SSH 키 암호화 용도\n(C) SSMManagedInstanceCore — Systems Manager Session Manager 정책, EC2 Instance Connect와 무관\n(D) SSH 키 쌍 생성 — EC2 Instance Connect는 키 쌍 불필요 (인스턴스 connect 방식)\n\n【시험 포인트】\n▸ EC2 Instance Connect: 호스트 키 검증이 중요한 보안 메커니즘\n▸ 호스트 키 회전 후: AWS 신뢰 DB 업데이트 필수\n▸ Instance Connect vs 일반 SSH: 임시 키 주입 방식의 차이",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Amazon Macie — S3 민감한 데이터 자동 발견 및 분류 전문 서비스\n▸ Managed data identifiers — PII, 금융정보 등 미리 정의된 패턴\n▸ Amazon EventBridge — Macie 발견사항을 SNS로 라우팅\n\n【정답 포인트】\n▸ Macie 선택 이유:\n  1. 관리형 데이터 식별자: 사전 구성된 500+ 패턴 (Lambda 커스텀 필요 없음)\n  2. 자동 분류: 발견 즉시 자동 실행\n  3. EventBridge 통합: 발견사항 → SNS 자동 연결\n▸ 최소 구현 노력:\n  - Macie 활성화 (1단계)\n  - EventBridge 규칙 생성 (1단계)\n  - 총 2단계로 완료\n\n【오답 체크】\n(A) AWS Config — S3 모니터링 기능 부족, 민감 데이터 자동 감지 불가\n(B) Lambda 함수 — 커스텀 패턴 개발, 정규식 작성, 유지보수 오버헤드 증가\n(D) GuardDuty — S3 데이터 보안이 아닌 위협 탐지 (민감 데이터 식별 미전문)\n\n【시험 포인트】\n▸ 민감한 데이터 감지 전문 → Macie 필수 선택\n▸ 최소 노력 = 관리형 기능 활용 (Lambda 커스텀 개발 피하기)\n▸ 기존 SNS 주제 활용 → EventBridge로 자동 연결",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Firewall Manager — AWS Organizations 전체 WAF 정책의 중앙화 관리\n▸ AWS Bot Control — bot 트래픽 탐지 및 차단 관리형 규칙 그룹\n▸ Managed rule groups — AWS에서 제공하는 SQL injection, DDoS, bot 등 규칙 모음\n\n【정답 포인트】\n▸ 12개 계정, 기존 + 미래 API 보호 → 조직 전체 정책 필요\n▸ Firewall Manager 선택 이유:\n  1. 조직 수준 정책 → 모든 계정의 WAF 일괄 관리\n  2. Bot Control + SQL 관리형 규칙 → 두 가지 위협 동시 차단\n  3. 자동 적용 → 새로운 API Gateway 자동 보호\n  4. 최소 운영 오버헤드 → 정책 하나로 12개 계정 관리\n▸ API Gateway stage 리소스 유형 → 모든 API 버전 자동 포함\n\n【오답 체크】\n(A) 각 API마다 WAF 생성 → 운영 오버헤드 증가 (12개 이상)\n(C) Service Catalog + Config 수정 → 복잡한 파이프라인, 관리 비용 높음\n(D) Security Hub + Lambda → 수동 감지 후 프로비저닝, 실시간 보호 지연\n\n【시험 포인트】\n▸ Firewall Manager: 조직 규모의 WAF 관리의 표준 솔루션\n▸ 최소 운영 오버헤드: 정책 우선순위, 자동 적용이 핵심\n▸ 다중 계정 환경: Firewall Manager의 원래 목적",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Cross-VPC peering — 두 VPC 간 직접 네트워크 연결\n▸ Security group reference — 다른 VPC의 보안 그룹을 참조하여 세밀한 제어\n▸ 최소 권한 원칙 — 필요한 인스턴스만 접근 가능하도록 제한\n\n【정답 포인트】\n▸ 문제 핵심: \"일부\" EC2 인스턴스만 DB 접근 필요\n▸ C의 솔루션:\n  1. 애플리케이션 VPC: 아웃바운드 용도의 보안 그룹 (inbound rule 불필요)\n  2. 데이터베이스 VPC: 애플리케이션 보안 그룹을 참조하는 inbound rule\n  3. 결과: 애플리케이션 SG 할당된 인스턴스만 DB 포트 1521 접근 가능\n▸ 동적 대응: EC2 생성/종료 시 보안 그룹만 관리 (CIDR 고정 불필요)\n\n【오답 체크】\n(A) CIDR 범위 → 애플리케이션 VPC 전체 범위 허용 (일부만 필요한 요구사항 미충족)\n(B) 인바운드 규칙 in app VPC — 데이터베이스는 응답 불필요, 보안 그룹 관계 역\n(D) IP 주소 범위 + 모든 트래픽 — 포트 1521만 허용해야 하는데 전체 범위 개방\n\n【시험 포인트】\n▸ 보안 그룹 참조 (sg-xxx) → 크로스 VPC 세밀 제어의 핵심\n▸ 아웃바운드 vs 인바운드: 필요한 방향에만 규칙 적용\n▸ CIDR vs SG reference: 동적 환경에는 SG 참조가 확장성 우수",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Cross-account access — 서로 다른 AWS 계정 간의 권한 위임\n▸ AssumeRole — IAM 역할 임시 자격증명으로 권한 획득\n▸ Trust relationship — 다른 계정의 주체가 역할을 맡도록 허용\n\n【정답 포인트】\n▸ 정답 (D): 프로덕션 계정의 IAM \"역할\"\n▸ 정답 선택 이유:\n  1. 개발자 계정 사용자 → 역할 가정 (AssumeRole)\n  2. 역할의 임시 자격증명 획득 (STS)\n  3. 프로덕션 리소스에 임시로 접근\n  4. 세션 만료 시 자동 권한 박탈\n▸ 보안상 우수:\n  - IAM 사용자 비밀번호 공유 불필요\n  - 임시 자격증명 (토큰 기반)\n  - 감사 추적 명확 (역할 가정 로그)\n\n【오답 체크】\n(A) 공유 비밀번호 → 패스워드 유출 위험, 권한 추적 어려움\n(B) 개발자 계정의 역할 → 개발자가 프로덕션 역할을 \"가정\"할 수 없음 (역할은 다른 주체를 가정)\n(C) 프로덕션의 IAM 사용자 + 공유 자격증명 → 비밀번호 관리 위험, 권한 박탈 불가능\n\n【시험 포인트】\n▸ 크로스 계정 최선의 실행: 프로덕션 계정의 \"역할\" 기반\n▸ AssumeRole 패턴: 임시 권한 위임의 표준 방식\n▸ IAM 사용자 vs 역할: 크로스 계정에서는 역할 필수",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Temporary S3 bucket policy — S3 버킷에 대한 읽기 액세스 즉시 차단\n▸ Explicit Deny — 모든 주체의 접근을 거부하는 정책 문구\n▸ 임시 완화 조치 — 취약점 패치 전까지의 긴급 차단\n\n【정답 포인트】\n▸ 현재 상황:\n  - 공격자가 이미 임시 자격증명 확보 (STS token)\n  - 4시간 내에만 패치 가능\n  - 데이터 유출 진행 중 → 즉시 차단 필요\n▸ 솔루션 (D):\n  1. S3 버킷 정책 추가: s3:GetObject 모두 거부\n  2. 모든 주체(*)의 읽기 접근 차단\n  3. 공격자의 임시 자격증명도 정책 거부 (효과)\n  4. 4시간 후 정책 제거 (패치 적용 후)\n▸ 즉시성: 정책 적용 → 바로 효과 발생\n\n【오답 체크】\n(A) 세션 취소 — STS 토큰은 이미 발급되어 취소 불가능 (토큰 유효 기간 내 사용 가능)\n(B) 보안 그룹 변경 — 네트워크 차단이지, 이미 발급된 API 자격증명 사용 막지 못함\n(C) Macie + Object Lock — 감지/보호이지 기존 데이터 다운로드 차단 불가 (사후 처리)\n\n【시험 포인트】\n▸ 자격증명 유출 대응: 네트워크 차단이 아닌 정책 기반 차단\n▸ 버킷 정책 우선순위: IAM 정책보다 버킷 정책 거부가 우선\n▸ 긴급 완화: 임시 정책으로 즉시 접근 차단 후 제거",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Cost Anomaly Detection — 머신러닝으로 비용 이상 패턴 자동 감지\n▸ Real-time alert — 이상 비용 발생 시 실시간 알림\n▸ 초기 탐지 (early detection) — 문제 비용 축적 전 빠른 발견\n\n【정답 포인트】\n▸ 문제 핵심: \"가장 빨리(EARLIEST) 감지\"\n▸ Cost Anomaly Detection (B) 선택 이유:\n  1. 자동화 머신러닝 모델 → 비용 패턴 학습\n  2. 실시간 모니터링 → 일일/주간 실행이 아닌 24/7 감시\n  3. 임계값 기반 알림 → 예상 비용 대비 % 초과 시 즉시 통보\n  4. SNS 통합 → 실시간 알림 (이메일/SMS)\n▸ 빠른 탐지 = 비용 손실 최소화\n\n【오답 체크】\n(A) EventBridge + Lambda 시간 실행 → 정기적 실행이지 실시간 아님 (최소 1시간 지연)\n(C) Cost Explorer 수동 검토 → 매일 확인해도 이미 비용 누적됨 (후행 지표)\n(D) VPC Flow Logs + 타사 분석 → 네트워크 이상 감지일 뿐, 비용 증가 원인 파악 딜레이\n\n【시험 포인트】\n▸ Cost Anomaly Detection: 비용 모니터링의 표준 (early detection 목적)\n▸ Machine Learning 기반: 정상 패턴 대비 비교로 정확한 감지\n▸ 실시간 vs 정기 실행: \"earliest detection\" = 자동화된 실시간 모니터링",
  ...
  },
  {
    ...
    "answer": "AD",
    "explanation": "【핵심 용어】\n▸ API Gateway Access Logging — 구조화된 로그를 실시간으로 CloudWatch Logs로 전송\n▸ CloudWatch Logs Insights — 스트림된 로그를 SQL 기반 쿼리로 분석\n\n【정답 포인트】\n▸ (A) 액세스 로깅 활성화 → CloudWatch Logs에 구조화된 로그 저장\n▸ (D) Logs Insights 사용 → 파싱 없이 직관적 쿼리로 패턴 분석\n▸ 조합: 로그 수집 + 분석 도구로 최소 노력 달성\n\n【오답 체크】\n(B) CloudTrail은 API 호출 감사(API 관리)용이지, 액세스 패턴 분석용 아님\n(C) S3 + Athena는 동작하지만 구성 복잡 (E) 메트릭은 차원 통계만 제공\n\n【시험 포인트】\n가장 간단한 분석: Access Log + Logs Insights\nCloudTrail은 감시 목적, Athena는 과도함",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ CloudTrail Multi-Region Trail — 모든 AWS 리전의 API 활동을 자동으로 모니터링\n▸ Region-Specific Trail — 단일 리전만 로깅\n\n【정답 포인트】\n▸ (D) 기존 추적을 multi-region으로 변경 → 자동으로 신규 리전 포함\n▸ 운영 오버헤드 최소: 단 1개 추적만 관리\n\n【오답 체크】\n(A) 새 추적 생성 → 관리해야 할 추적이 여러 개\n(B) S3 버킷 변경은 로깅 수집과 무관\n(C) 새로운 추적 생성 = 추가 관리 필요\n\n【시험 포인트】\nCloudTrail은 기존 추적을 \"Region\" 설정으로 All Regions 전환 가능\n새 리전 추가 시 자동 감지 → 비용/관리 효율적",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ KMS Key Grant — 임시 권한 부여 메커니즘, 쉬운 취소 가능\n▸ Key Policy — 영구적 정책, 수정 후 취소 복잡\n▸ Key Material Export — 보안 위험, 권장되지 않음\n\n【정답 포인트】\n▸ (C) Grant 사용 → 시간제한/임시 권한 부여 가능\n▸ 취소 간편 → revoke() 한 호출로 즉시 철회\n▸ 운영 오버헤드 최소화\n\n【오답 체크】\n(A) 키 자료 내보내기 = 보안 위반, 권장되지 않음\n(B) 키 정책 수정 → 관리 번거로움, 정책 일관성 유지 어려움\n(D) 온프레미스 키 생성 → 복잡하고 비효율적\n\n【시험 포인트】\nGrant는 임시 권한 위임 도구, Key Policy는 영구적 권한\n일시적 액세스 = Grant 최선",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Tag Policy — AWS Organizations에서 태그 규제 및 강제\n▸ SCP (Service Control Policy) — API 행동 거부/허용 정책\n▸ Tag Compliance — \"enforce\" 설정으로 정책 위반 자동 차단\n\n【정답 포인트】\n▸ (C) Tag Policy 활성화 → 승인된 값만 강제\n▸ \"enforce\" 옵션 → 위반 작업 자동 차단\n▸ SCP (aws:RequestTag.CostCenter null 거부) → null 값 생성 차단\n▸ 조합: 태그 정책 + SCP = 완벽한 규제\n\n【오답 체크】\n(A) Config Guard = 식별만 함, 강제 없음\n(B) EventBridge+Lambda = 감지 후 처리(반응형), 사전 차단 아님\n(D) Lambda는 이미 생성된 태그 감지 후 차단 불가능\n\n【시험 포인트】\nTag Policy는 AWS Organizations 기본 규제 도구\nenforce = 정책 위반 작업 자동 거부\nSCP로 보완 (null 방지)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS RAM (Resource Access Manager) — AWS 조직 내 리소스 공유\n▸ CloudHSM Cluster — VPC 서브넷에서만 액세스 가능한 네트워크 리소스\n▸ VPC 서브넷 ID 공유 — HSM 네트워크 접근성 확보\n\n【정답 포인트】\n▸ (A) RAM으로 서브넷 ID 공유 → 크로스 계정 VPC 액세스\n▸ 보안 그룹 규칙 추가 → 신규 계정의 클라이언트 IP 허용\n▸ 네트워크 레벨 공유 = CloudHSM 액세스 가능\n\n【오답 체크】\n(B) IAM으로 HSM 클러스터 공유 불가 (리소스 기반 권한 없음)\n(C) STS는 인증용, HSM 네트워크 접근과 무관\n(D) HSM ID 공유는 의미 없음, 반드시 서브넷 ID 필요\n\n【시험 포인트】\nCloudHSM은 네트워크 어플라이언스 (VPC 종속)\nVPC 서브넷 공유 → RAM 사용\n보안 그룹으로 접근 제어 추가",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS Config Aggregator (Organization-wide) — 전 조직 멀티 계정/리전 모니터링\n▸ Config Remediation — Lambda 자동 대응으로 비준수 리소스 수정\n▸ SCP — 신규 생성 차단만 가능, 기존 리소스 재구성 불가\n\n【정답 포인트】\n▸ (A) 조직 차원 수집기 → 모든 계정/리전 감시\n▸ Lambda + Config Findings → 기존 비준수 자동 재구성\n▸ 향후 신규 비준수도 자동 감지 및 대응\n\n【오답 체크】\n(B) SCP는 신규 생성만 차단, 기존 버킷 재구성 불가\n(C) 현재 계정/리전만 범위 → 향후 신규 리전/계정 확장 시 누락\n(D) 제한된 범위 + SCP는 기존 리소스 수정 불가\n\n【시험 포인트】\n기존 + 향후 비준수 대응 = Aggregator + Lambda 자동화\nOrganization-wide 범위 필수\nSCP로는 이미 생성된 리소스 제어 불가",
  ...
  },
  {
    ...
    "answer": "ACF",
    "explanation": "【핵심 용어】\n▸ External ID — 크로스 계정 역할 가정 시 추가 인증 메커니즘\n▸ sts:AssumeRole — 크로스 계정 역할 가정 권한\n▸ Role ARN — 가정 대상 역할의 고유 식별자\n\n【정답 포인트】\n▸ (A) 외부 ID 불일치 → 역할 가정 실패 (신뢰 정책 위반)\n▸ (C) sts:AssumeRole 권한 없음 → 역할 가정 권한 부족\n▸ (F) 역할 ARN 오류 → 잘못된 역할 대상\n\n【오답 체크】\n(B) 비밀번호는 IAM 크로스 계정 역할과 무관 (STS 사용)\n(D) EC2 역할이 대상 계정 역할일 필요 없음 (원본 계정 역할)\n(E) 시크릿 키는 원본 계정 IAM 사용자/역할 것, 대상 아님\n\n【시험 포인트】\n크로스 계정 역할 가정 3요소: External ID + Permission + ARN 정확성\nIAM 비밀번호, EC2 역할과는 직접 관련 없음",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Well-Architected Framework — Reliability, Security, Operational Excellence 등 5개 기둥\n▸ Well-Architected Tool — 아키텍처 리뷰 및 개선 권고\n▸ Resilience 증명 — 설계 개선 문서화\n\n【정답 포인트】\n▸ (B) Well-Architected Tool 사용 → 3개 기둥 포함 탄력성 평가\n▸ 위험 식별 및 완화 계획 → 아키텍처 개선\n▸ 문서화된 개선사항 → 감시자 증거 제출\n\n【오답 체크】\n(A) Audit Manager는 규정 준수 평가용, 탄력성 설계 아님\n(C) Inspector는 취약점 스캔용, 아키텍처 탄력성 평가 아님\n(D) Fault Injection은 탄력성 테스트 도구, 단순 개선 증거 부족\n\n【시험 포인트】\nWell-Architected Tool = 아키텍처 지원 프레임워크 기반 개선\n\"설계 개선 문서\" 제출이 감시 증거\nOperational Excellence + Security + Reliability = 탄력성 입증",
  ...
  },
  {
    ...
    "answer": "AB",
    "explanation": "【핵심 용어】\n▸ Site-to-Site VPN — IPsec 암호화 터널, 공인 인터넷 사용\n▸ AWS Direct Connect — 전용 네트워크 연결, 저지연, IPsec 미지원\n▸ 조합 전략 — 암호화 + 저지연 동시 만족\n\n【정답 포인트】\n▸ (A) Site-to-Site VPN → IPsec 암호화 요구사항 만족\n▸ (B) Direct Connect → 저지연 전용 회선, 데이터베이스 지연 민감성 해결\n▸ 조합: VPN (암호화) + Direct Connect (성능)\n\n【오답 체크】\n(C) VPN CloudHub는 다중 VPN 사이트 연결용, 지연 개선 없음\n(D) VPC 피어링은 온프레미스 지원 안 함\n(E) NAT 게이트웨이는 암호화/지연 제어 불가\n\n【시험 포인트】\n규정 + 암호화 = VPN (IPsec 기본)\n데이터베이스 지연 민감 = Direct Connect (저지연)\n하이브리드 연결 모두 필요 시 VPN + Direct Connect 조합",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ s3-default-encryption-kms — 기본 암호화 KMS 설정 확인 규칙\n▸ s3-bucket-ssl-requests-only — HTTPS 전송 암호화만 확인 (KMS 아님)\n▸ SCP vs Bucket Policy — 조직 차원 vs 개별 버킷 정책\n\n【정답 포인트】\n▸ (A) s3-default-encryption-kms 규칙 → KMS 암호화 설정 확인\n▸ SCP로 sts:PutObject 제어 → 조직 차원 강제\n▸ 정책 일관성: 모든 버킷에 동일한 규칙 적용\n\n【오답 체크】\n(B) s3-default-encryption-kms는 맞지만, \"deny\" 조건이 SSE-S3일 때 → 논리 역전\n(C) ssl-requests-only는 \"https\" 확인용, KMS 암호화 검증 안 함\n(D) ssl-requests-only + 버킷 정책 → 잘못된 규칙 선택\n\n【시험 포인트】\nKMS 암호화 강제 = s3-default-encryption-kms\nOrganization-wide 강제 = SCP 사용 (각 버킷 정책보다 효율적)\nSSL ≠ 암호화 (전송 vs 저장)",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ GuardDuty Finding — 보안 위협 감지 이벤트\n▸ EventBridge — AWS 서비스 이벤트를 Lambda 등으로 라우팅\n▸ Config Rule vs GuardDuty Finding — Config는 리소스 상태, GuardDuty는 위협 탐지\n\n【정답 포인트】\n▸ (C) EventBridge로 GuardDuty CryptoCurrency 추적 감지\n▸ 규칙: 추적 이벤트 발생 시 Lambda 호출\n▸ Lambda가 격리 보안 그룹으로 인스턴스 이동 (실시간 대응)\n\n【오답 체크】\n(A) GuardDuty는 Lambda 직접 호출 기능 없음 (EventBridge 필수)\n(B) AWS Config 규칙은 리소스 구성 규정준수 확인용\n    (구성 변경과 GuardDuty 위협은 별개)\n(D) EventBridge + Config는 구성 변경만 감지 (위협 탐지 아님)\n\n【시험 포인트】\nGuardDuty Finding 자동화 = EventBridge 사용 필수\nEventBridge로 GuardDuty → Lambda 연결\nConfig는 리소스 상태 모니터링 (보안 위협 아님)",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ RPO (Recovery Point Objective) — 1시간 = 최대 1시간 데이터 손실 허용\n▸ AWS Backup — EC2, EBS, S3 등 자동 백업 (스케줄 지정)\n▸ CloudFormation + CodeCommit — 인프라 및 구성 재구성 자동화\n\n【정답 포인트】\n▸ (A) AWS Backup 매시간 = RPO 1시간 충족\n▸ EC2 + EBS + S3 모두 백업\n▸ CloudFormation 템플릿 + CodeCommit = 빠른 재배포\n▸ 랜섬웨어 복구: 백업 복원 + 자동 재배포\n\n【오답 체크】\n(B) 일일 백업 = RPO 1시간 미충족 (최대 24시간 손실)\n(C) Security Lake + Hub는 감지/로깅용, 복구 도구 아님\n(D) 4시간 스냅샷 = RPO 초과, GuardDuty는 탐지만 함 (복구 아님)\n\n【시험 포인트】\nRPO 1시간 → 매시간 백업 필수\nAWS Backup이 가장 간편한 예약 백업\nCloudFormation = 재해복구 자동화의 핵심",
    "en_q": "A company runs an application on a fleet of Amazon EC2 instances. The company can remove instances from the fleet without risk to the application. All EC2 instances use the same security group named ProdFleet. Amazon GuardDuty and AWS Config are active in the company's AWS account. A security engineer needs to provide a solution that will prevent an EC2 instance from sending outbound traffic if GuardDuty generates a cryptocurrency finding event. The security engineer creates a new security group named isolate that contains no outbound rules. The security engineer configures an AWS Lambda function to remove an EC2 instance from the ProdFleet security group and add it to the isolate security group. Which additional step will meet this requirement?",
    "en_opts": {
