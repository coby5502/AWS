window.SOA_QUESTIONS = [
  {
    "id": 1,
    "question": "CloudOps 엔지니어가 다음의 AWS CloudFormation 템플릿을 검토하고 있습니다. 스택 생성이 실패하는 이유는 무엇입니까?",
    "options": {
      ...
      "D": "CloudFormation 템플릿에서 VPC가 지정되지 않았습니다."
    },
    "answer": "C",
  ...
  {
    "id": 2,
    "question": "회사가 AWS 워크로드와 연관된 리소스에 사용자 정의 태그를 적용했습니다. 태그 적용 후 20일이 지났을 때, 회사는 AWS Cost Explorer 콘솔에서 해당 태그를 사용하여 뷰를 필터링할 수 없다는 것을 발견했습니다. 이 문제의 원인은 무엇입니까?",
    "options": {
      "A": "Cost Explorer에서 태그를 사용하여 뷰를 필터링할 수 있으려면 최소 30일이 필요합니다.",
      "B": "회사가 비용 할당을 위해 사용자 정의 태그를 활성화하지 않았습니다.",
      "C": "회사가 AWS Cost and Usage Report를 생성하지 않았습니다.",
      "D": "회사가 AWS Budgets에서 예산을 생성하지 않았습니다."
    },
    "answer": "B",
  ...
  {
    "id": 3,
    "question": "환경에 100개의 Amazon EC2 Windows 인스턴스가 배포되어 있습니다. 모든 EC2 인스턴스에 기본 설정 파일로 CloudWatch 에이전트가 설치되어 로그 파일을 캡처하고 있습니다. 이제 50개의 인스턴스에 존재하는 DHCP 로그 파일을 캡처해야 합니다. 이 새로운 요구사항을 충족하는 가장 효율적인 운영 방식은 무엇입니까?",
    "options": {
      "A": "DHCP 로그를 캡처하기 위한 추가 CloudWatch 에이전트 설정 파일을 생성합니다. AWS Systems Manager Run Command를 사용하여 append-config 옵션으로 각 EC2 인스턴스의 CloudWatch 에이전트를 재시작합니다.",
      "B": "각 EC2 인스턴스에 관리자 권한으로 로그인합니다. baseline 로그 파일과 DHCP 로그 파일을 CloudWatch로 푸시하는 PowerShell 스크립트를 생성합니다.",
      "C": "각 EC2 인스턴스에서 CloudWatch 에이전트 설정 파일 마법사를 실행합니다. baseline 로그 파일이 포함되었는지 확인하고 마법사 생성 중에 DHCP 로그 파일을 추가합니다.",
      "D": "각 EC2 인스턴스에서 CloudWatch 에이전트 설정 파일 마법사를 실행하고 고급 세부 수준을 선택합니다. 이는 운영 체제 로그 파일을 캡처합니다."
    },
    "answer": "A",
  ...
  {
    "id": 4,
    "question": "회사가 Amazon S3 버킷에 백업을 저장하고 있습니다. 백업 생성 후 최소 3개월 동안은 삭제되지 않아야 합니다. CloudOps 엔지니어가 이 요구사항을 충족하기 위해 해야 할 일은 무엇입니까?",
    "options": {
      "A": "모든 사용자에 대해 s3:DeleteObject 작업을 거부하는 IAM 정책을 구성합니다. 객체가 작성된 후 3개월이 지나면 정책을 제거합니다.",
      "B": "Compliance Mode에서 새로운 S3 버킷의 S3 Object Lock을 활성화합니다. 모든 백업을 새 S3 버킷에 3개월의 보존 기간으로 배치합니다.",
      "C": "기존 S3 버킷의 S3 Versioning을 활성화합니다. S3 Lifecycle 규칙을 구성하여 백업을 보호합니다.",
      "D": "Governance Mode에서 새로운 S3 버킷의 S3 Object Lock을 활성화합니다. 모든 백업을 새 S3 버킷에 3개월의 보존 기간으로 배치합니다."
    },
    "answer": "B",
  ...
  {
    "id": 5,
    "question": "CloudOps 엔지니어가 애플리케이션 컴포넌트 간의 통신을 문제 해결하고 있습니다. 회사가 VPC Flow Logs를 Amazon CloudWatch Logs에 발행하도록 구성했으나, CloudWatch Logs에 로그가 없습니다. VPC Flow Logs가 CloudWatch Logs로 발행되는 것을 막을 수 있는 것은 무엇입니까?",
    "options": {
      "A": "Flow Log용 IAM 역할에 연결된 IAM 정책에서 logs:CreateLogGroup 권한이 누락되었습니다.",
      "B": "Flow Log용 IAM 역할에 연결된 IAM 정책에서 logs:CreateExportTask 권한이 누락되었습니다.",
      "C": "VPC가 IPv6 주소용으로 구성되었습니다.",
      "D": "VPC가 AWS 계정의 다른 VPC와 피어링되었습니다."
    },
    "answer": "A",
  ...
  {
    "id": 6,
    "question": "회사가 레거시 애플리케이션을 AWS로 마이그레이션하고 있습니다. 회사가 여러 가용 영역에 걸쳐 Amazon EC2 인스턴스에 레거시 애플리케이션을 수동으로 설치하고 구성했습니다. 회사가 애플리케이션용 Application Load Balancer(ALB)를 설정했습니다. 회사가 대상 그룹 라우팅 알고리즘을 가중 무작위로 설정했습니다. 애플리케이션에 세션 선호도가 필요합니다. 회사가 애플리케이션을 배포한 후, 사용자가 레거시 버전에는 없던 무작위 애플리케이션 오류를 보고했습니다. 대상 그룹 상태 검사에는 오류가 표시되지 않습니다. 회사가 애플리케이션 오류를 해결해야 합니다. 이 요구사항을 충족할 해결책은 무엇입니까?",
    "options": {
      "A": "대상 그룹의 라우팅 알고리즘을 최소 미처리 요청으로 설정합니다.",
      "B": "대상 그룹의 이상 탐지 완화를 켭니다.",
  ...
  {
    "id": 7,
    "question": "회사가 Point-in-Time 복구, Backtracking 및 자동 백업을 활성화한 Amazon Aurora MySQL DB 클러스터를 사용하고 있습니다. CloudOps 엔지니어가 이전 72시간 내의 특정 복구 지점으로 DB 클러스터를 롤백할 수 있어야 합니다. 복구를 동일한 프로덕션 DB 클러스터에서 완료해야 합니다. 이 요구사항을 충족할 해결책은 무엇입니까?",
    "options": {
  ...
  {
    "id": 8,
    "question": "CloudOps 엔지니어가 실패한 AWS CloudFormation 스택 생성을 문제 해결하고 있습니다. 문제를 식별하기 전에 스택과 그 리소스가 삭제됩니다. 향후 배포를 위해 CloudOps 엔지니어가 CloudFormation이 성공적으로 생성한 리소스를 보존해야 합니다. CloudOps 엔지니어가 이 요구사항을 충족하기 위해 해야 할 일은 무엇입니까?",
    "options": {
      "A": "스택 생성 중에 DisableRollback 파라미터의 값을 False로 설정합니다.",
      "B": "스택 생성 중에 OnFailure 파라미터의 값을 DO_NOTHING으로 설정합니다.",
      "C": "스택 생성 중에 DO_NOTHING의 롤백 트리거가 있는 롤백 구성을 지정합니다.",
      "D": "스택 생성 중에 OnFailure 파라미터의 값을 ROLLBACK으로 설정합니다."
    },
    "answer": "B",
  ...
  {
    "id": 9,
    "question": "회사가 Elastic Load Balancing(ELB) 로드 밸런서 뒤의 Amazon EC2 인스턴스에서 공개 웹 애플리케이션을 실행할 계획입니다. 회사의 보안팀이 AWS Certificate Manager(ACM) 인증서를 사용하여 웹 사이트를 보호하려고 합니다. 로드 밸런서가 HTTP 요청을 자동으로 HTTPS로 리디렉션해야 합니다. 이 요구사항을 충족할 해결책은 무엇입니까?",
    "options": {
      "A": "포트 80에서 HTTPS 리스너가 하나 있는 Application Load Balancer를 생성합니다. SSL/TLS 인증서를 리스너 포트 80에 연결합니다. HTTP에서 HTTPS로 요청을 리디렉션하는 규칙을 생성합니다.",
      "B": "포트 80에 HTTP 리스너가 하나, 포트 443에 HTTPS 프로토콜 리스너가 하나 있는 Application Load Balancer를 생성합니다. SSL/TLS 인증서를 리스너 포트 443에 연결합니다. 포트 80에서 포트 443으로 요청을 리디렉션하는 규칙을 생성합니다.",
      "C": "포트 80과 포트 443에 TCP 리스너가 두 개 있는 Application Load Balancer를 생성합니다. SSL/TLS 인증서를 리스너 포트 443에 연결합니다. 포트 80에서 포트 443으로 요청을 리디렉션하는 규칙을 생성합니다.",
      "D": "포트 80과 포트 443에 TCP 리스너가 두 개 있는 Network Load Balancer를 생성합니다. SSL/TLS 인증서를 리스너 포트 443에 연결합니다. 포트 80에서 포트 443으로 요청을 리디렉션하는 규칙을 생성합니다."
    },
    "answer": "B",
  ...
  {
    "id": 10,
    "question": "회사가 AWS Organizations를 사용하여 AWS 계정 집합을 관리하고 있습니다. 회사가 조직에서 조직 단위(OU)를 설정했습니다. 애플리케이션 OU는 다양한 애플리케이션을 지원합니다. CloudOps 엔지니어가 사용자가 CostCenter-Project 태그가 없는 Amazon EC2 인스턴스를 애플리케이션 OU의 모든 계정에 시작하는 것을 방지해야 합니다. 제한이 애플리케이션 OU의 계정에만 적용되어야 합니다. 이 요구사항을 충족할 해결책은 무엇입니까?",
    "options": {
      ...
      "C": "CostCenter-Project 태그가 있을 때 ec2:RunInstances 작업을 허용하는 정책이 있는 IAM 역할을 생성합니다. IAM 역할을 애플리케이션 OU 계정의 IAM 사용자에 연결합니다.",
  ...
  {
    "id": 11,
    "question": "회사가 300개 이상의 Linux 기반 인스턴스에서 비즈니스 애플리케이션을 실행하고 있습니다. 각 인스턴스에 AWS Systems Manager Agent(SSM Agent)가 설치되어 있습니다. 회사는 앞으로 인스턴스 수가 증가할 것으로 예상합니다. 모든 비즈니스 애플리케이션 인스턴스는 동일한 사용자 정의 태그를 가지고 있습니다. CloudOps 엔지니어가 모든 비즈니스 애플리케이션 인스턴스에서 명령을 실행하여 비공개 리포지토리에서 패키지를 다운로드하고 설치하려고 합니다. 리포지토리에 압도적인 영향을 주지 않기 위해 CloudOps 엔지니어가 한 번에 최대 30개의 다운로드만 발생하도록 해야 합니다. 이 요구사항을 가장 효율적인 운영 방식으로 충족할 해결책은 무엇입니까?",
    "options": {
      ...
      "B": "사용자 정의 태그가 있는 인스턴스 ID 목록을 읽는 Systems Manager Run Command 문서를 자동으로 실행하는 AWS Lambda 함수를 사용합니다. Lambda 함수의 예약 동시성을 30으로 설정합니다.",
      "C": "패키지를 다운로드하고 설치하는 Systems Manager Run Command 문서를 사용합니다. 속도 제어를 사용하여 동시성을 30으로 설정합니다. Run Command 문서의 일부로 사용자 정의 태그를 사용하여 대상을 지정합니다.",
      "D": "사용자 정의 태그가 있는 인스턴스 ID 목록을 읽는 Systems Manager Run Command 문서를 자동으로 실행하는 AWS Step Functions의 병렬 워크플로우 상태를 사용합니다. 병렬 상태 수를 30으로 설정합니다. Step Functions 워크플로우를 10번 실행합니다."
    },
    "answer": "C",
  ...
  {
    "id": 12,
    "question": "회사가 여러 AWS 리전 간의 지연 시간 기반 라우팅을 사용하는 Amazon Route 53을 사용하여 복원력을 제공합니다. 회사가 Route 53을 지연 시간 기반 라우팅으로 사용하여 트래픽을 가장 가까운 리전으로 전송합니다. 각 리전 내에서 가중 A 레코드는 여러 가용 영역에 걸쳐 트래픽을 분산합니다. 최근 업데이트 중에 일부 가용 영역 엔드포인트가 비정상이 되었습니다. Route 53은 계속해서 트래픽을 비정상 엔드포인트로 라우팅했습니다. 회사가 향후 이 문제가 발생하는 것을 방지해야 합니다. 이 요구사항을 충족할 해결책은 무엇입니까?",
    "options": {
      "A": "최근 업데이트 중에 트래픽을 받은 가중 레코드 각각에 대해 Route 53 건강 검사를 추가합니다.",
      "B": "업데이트 중에 트래픽이 가야 할 리전의 Route 53 레코드의 가중치를 증가시킵니다.",
      "C": "모든 레코드를 모든 리전에서 균일하게 지연 시간 기반 라우팅을 사용하도록 재구성합니다.",
      "D": "지연 시간 기반 라우팅의 TTL 값을 줄여 변경사항을 더 빨리 감지합니다."
    },
    "answer": "A",
  ...
  {
    "id": 13,
    "question": "회사가 모든 Amazon EC2 Windows 인스턴스가 AWS 계정에서 시작될 때 타사 에이전트가 설치되어 있는지 확인해야 합니다. 회사가 AWS Systems Manager를 사용하며, Windows 인스턴스에 적절한 태그가 지정되어 있습니다. 회사가 업데이트가 사용 가능할 때 타사 에이전트에 대한 정기적 업데이트를 배포해야 합니다. 최소한의 운영 노력으로 이 요구사항을 충족하는 단계의 조합은 무엇입니까? (2개 선택)",
    "options": {
      "A": "타사 에이전트용 Systems Manager Distributor 패키지를 생성합니다.",
      "B": "Windows의 태그 값을 포함하는 Systems Manager OpsItem을 생성합니다. Systems Manager Inventory를 OpsItem에 연결합니다.",
      "C": "AWS Lambda 함수를 생성합니다. Lambda 함수를 프로그래밍하여 각 인스턴스에 로그인하고 필요에 따라 타사 에이전트를 설치하거나 업데이트합니다.",
      "D": "AWS-RunRemoteScript 문서를 실행하는 Systems Manager State Manager 연관을 생성합니다. 타사 에이전트 패키지의 세부정보를 입력합니다.",
      "E": "AWS-ConfigureAWSPackage 문서를 실행하는 Systems Manager State Manager 연관을 생성합니다. 타사 에이전트 패키지의 세부정보를 입력합니다. Windows의 적절한 태그 값을 기반으로 인스턴스 태그를 지정합니다."
    },
    "answer": "AE",
  ...
  {
    "id": 14,
    "question": "회사가 두 AWS 리전에서 사용자 정의 Amazon Machine Images(AMI)로부터 Amazon EC2 인스턴스를 배포했습니다. 회사가 모든 인스턴스를 AWS Systems Manager에 등록했습니다. 회사가 일부 인스턴스의 운영 체제에 심각한 제로데이 악용이 있다는 것을 발견했습니다. 하지만 회사는 영향받는 인스턴스가 몇 개인지 알 수 없습니다. CloudOps 엔지니어가 영향받는 EC2 인스턴스에 운영 체제 패치를 배포할 해결책을 구현해야 합니다. 최소한의 운영 오버헤드로 이 요구사항을 충족할 해결책은 무엇입니까?",
    "options": {
      "A": "Systems Manager Patch Manager에서 패치 기준선을 정의합니다. Patch Manager 스캔을 사용하여 영향받는 인스턴스를 식별합니다. 각 리전에서 Patch Now 옵션을 사용하여 영향받는 인스턴스를 업데이트합니다.",
      "B": "AWS Config를 사용하여 영향받는 인스턴스를 식별합니다. Systems Manager Patch Manager에서 패치 기준선을 정의합니다. Patch Manager에서 Patch Now 옵션을 사용하여 영향받는 인스턴스를 업데이트합니다.",
      "C": "Systems Manager Compliance 이벤트에 반응하는 Amazon EventBridge 규칙을 생성합니다. EventBridge 규칙을 구성하여 영향받는 인스턴스에서 패치 기준선을 실행합니다.",
      "D": "AWS Config를 사용하여 영향받는 인스턴스를 식별합니다. 원하는 패치로 기존 EC2 AMI를 업데이트합니다. 새 AMI로부터 인스턴스를 수동으로 시작하여 양쪽 리전의 영향받는 인스턴스를 대체합니다."
    },
    "answer": "A",
  ...
  {
    "id": 15,
    "question": "회사가 Amazon EC2 인스턴스에서 FTP 서버를 호스팅합니다. 회사의 AWS 환경에서 AWS Security Hub는 인스턴스에 연결된 Security Group에서 FTP 포트가 공개적으로 노출되어 있기 때문에 Security Hub 문제를 Amazon EventBridge로 전송합니다. CloudOps 엔지니어는 Security Hub 문제 및 유사한 노출된 포트 문제를 자동으로 수정하는 솔루션을 원합니다. CloudOps 엔지니어는 이벤트 기반 접근 방식을 사용하려고 합니다. 이러한 요구사항을 충족하는 솔루션은 어느 것입니까?",
    "options": {
      "A": "기존 EventBridge 이벤트를 구성하여 노출된 포트가 있는 EC2 인스턴스를 중지합니다.",
      "B": "FTP 서버에 대한 cron 작업을 생성하여 AWS Lambda 함수를 호출합니다. Lambda 함수를 구성하여 식별된 EC2 인스턴스의 Security Group을 수정하고 공개 액세스를 허용하는 인스턴스를 제거합니다.",
      "C": "FTP 서버에 대한 cron 작업을 생성하여 AWS Lambda 함수를 호출합니다. Lambda 함수를 구성하여 서버를 FTP 대신 SFTP를 사용하도록 수정합니다.",
      "D": "기존 EventBridge 이벤트를 구성하여 AWS Lambda 함수를 호출합니다. 함수를 구성하여 공개 액세스를 허용하는 Security Group 규칙을 제거합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ EventBridge — AWS 이벤트 기반 서비스, Security Hub 문제를 감지하여 실시간 트리거\n▸ Lambda — 서버리스 컴퓨팅으로 자동 실행\n▸ Security Group Rule — Security Group의 특정 규칙만 제거\n\n【정답 포인트】\n▸ \"이벤트 기반\"→EventBridge로 Security Hub 문제를 캡처\n▸ \"자동 해결\"→Lambda 함수로 Security Group 규칙만 제거 (인스턴스 중지 X)\n▸ \"유사한 노출된 포트\"→이벤트 패턴으로 모든 노출 문제에 자동 대응\n\n【오답 체크】\n(A) 인스턴스 중지는 재수정이 필요하며, 근본적인 보안 규칙 제거가 아님\n(B) cron 작업은 스케줄 기반이지 이벤트 기반이 아니며, 인스턴스 제거는 과격함\n(C) cron 작업은 스케줄 기반이며, 모든 노출된 포트(HTTP, RDP 등)에 대응 불가\n\n【시험 포인트】\n▸ \"이벤트 기반\" vs \"스케줄 기반\" 구분 (EventBridge vs cron)\n▸ Security Hub 문제→자동 트리거 패턴 이해\n▸ \"노출된 포트 제거\"의 가장 간단한 방법 = Security Group 규칙 삭제\n▸ 함정: B/C는 cron 작업 사용으로 스케줄 기반이므로 탈락",
  ...
  {
    "id": 16,
    "question": "회사는 고성능 컴퓨팅(HPC) 가상 머신(VM)을 여러 개 Amazon EC2 인스턴스로 AWS에 마이그레이션하려고 계획합니다. CloudOps 엔지니어는 이 배포에 사용할 배치 그룹을 식별해야 합니다. 이 전략은 네트워크 지연시간을 최소화하고 HPC VM 간 네트워크 처리량을 최대화해야 합니다. 이러한 요구사항을 충족하기 위해 CloudOps 엔지니어가 선택해야 할 전략은 무엇입니까?",
    "options": {
      "A": "인스턴스를 하나의 Availability Zone에 있는 cluster placement group에 배포합니다.",
      "B": "인스턴스를 두 개의 Availability Zone에 있는 partition placement group에 배포합니다.",
      "C": "인스턴스를 하나의 Availability Zone에 있는 partition placement group에 배포합니다.",
      "D": "인스턴스를 두 개의 Availability Zone에 있는 spread placement group에 배포합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Cluster Placement Group — 단일 AZ 내 인스턴스 모음, 최저 지연시간 & 최고 처리량\n▸ Partition Placement Group — 분산된 파티션 그룹, 대규모 분산 워크로드용\n▸ Spread Placement Group — 인스턴스를 분산 배치, 고가용성(지연 증가)\n\n【정답 포인트】\n▸ \"HPC\" \"최소화 네트워크 지연\" \"최대화 처리량\"→Cluster가 유일한 선택\n▸ Cluster Placement Group = 단일 AZ 내 극저지연 네트워크 토폴로지\n▸ 클러스터 배치 그룹은 높은 대역폭, 저지연 EC2 클러스터 상호 간 통신 최적화\n\n【오답 체크】\n(B) Partition은 여러 AZ 지원이지만, 파티션 간 높은 지연시간 발생\n(C) Partition 단일 AZ도 지연시간이 Cluster보다 높음\n(D) Spread는 물리적으로 분산되어 지연시간 최악\n\n【시험 포인트】\n▸ HPC→극저지연 필요→Cluster Placement Group 암기\n▸ 배치 그룹 3가지 유형의 사용 사례 구분\n▸ \"단일 AZ\" = 지연시간 최소, \"다중 AZ\" = 가용성 증대지만 지연증가\n▸ 함정: Partition은 빅데이터/메시지큐용, HPC는 아님",
  ...
  {
    "id": 17,
    "question": "회사는 AWS Organizations를 사용하여 AWS의 여러 계정을 관리합니다. 회사의 보안 팀은 네이티브 AWS 서비스를 사용하여 Center for Internet Security(CIS) AWS Foundations Benchmark에 대해 모든 AWS 계정을 정기적으로 스캔하려고 합니다. 이러한 요구사항을 충족하기 위한 가장 운영상 효율적인 방법은 무엇입니까?",
    "options": {
      "A": "중앙 보안 계정을 AWS Security Hub 관리자 계정으로 지정합니다. Security Hub 관리자 계정에서 초대를 보내고 멤버 계정에서 초대를 수락하는 스크립트를 생성합니다. 새 계정이 생성될 때마다 스크립트를 실행합니다. Security Hub를 구성하여 CIS AWS Foundations Benchmark 스캔을 실행합니다.",
      "B": "Amazon Inspector를 사용하여 모든 계정에 걸쳐 CIS AWS Foundations Benchmark를 실행합니다.",
      "C": "중앙 보안 계정을 Amazon GuardDuty 관리자 계정으로 지정합니다. GuardDuty 관리자 계정에서 초대를 보내고 멤버 계정에서 초대를 수락하는 스크립트를 생성합니다. 새 계정이 생성될 때마다 스크립트를 실행합니다. GuardDuty를 구성하여 CIS AWS Foundations Benchmark 스캔을 실행합니다.",
      "D": "AWS Security Hub 관리자 계정을 지정합니다. 조직의 새 계정을 자동으로 멤버 계정이 되도록 구성합니다. CIS AWS Foundations Benchmark 스캔을 활성화합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ AWS Security Hub — 보안 표준 및 컴플라이언스 모니터링 (CIS Benchmark 지원)\n▸ AWS Organizations Integration — 새 계정 자동 등록\n▸ CIS AWS Foundations Benchmark — 구성 모범 사례 검사\n\n【정답 포인트】\n▸ \"가장 운영상 효율적\"→자동 통합 (수동 스크립트 X)\n▸ \"모든 계정\" \"정기적 스캔\"→Security Hub의 조직 정책 기능\n▸ \"새 계정 생성 시마다\"→자동 멤버쉽 기능으로 스크립트 불필요\n\n【오답 체크】\n(A) 수동 스크립트로 초대 관리 필요, 자동화 부족\n(B) Inspector는 EC2 취약점 스캔용, CIS Benchmark 전용 아님\n(C) GuardDuty는 위협 탐지(악성 행위), CIS Benchmark 실행 불가\n\n【시험 포인트】\n▸ Security Hub = CIS Benchmark 표준 지원의 네이티브 서비스\n▸ Organizations 통합 = 자동 멤버쉽 등록 (A의 수동 스크립트 제거)\n▸ GuardDuty ≠ 구성 컴플라이언스 (위협 탐지만 수행)\n▸ Inspector ≠ CIS Benchmark (EC2/ECR 취약점만 스캔)\n▸ 함정: A는 수동 스크립트 때문에 \"운영상 효율적\"이 아님",
  ...
  {
    "id": 18,
    "question": "회사의 사용자가 필요한 것보다 더 많은 볼륨 성능 용량을 가진 Amazon EC2 인스턴스를 배포합니다. CloudOps 엔지니어는 인스턴스와 연결된 모든 Amazon Elastic Block Store(Amazon EBS) 볼륨을 검토하고 IOPS 및 처리량을 기반으로 비용 최적화 권장사항을 만들어야 합니다. CloudOps 엔지니어가 가장 운영상 효율적인 방식으로 이러한 요구사항을 충족하기 위해 어떻게 해야 합니까?",
    "options": {
      "A": "EC2 콘솔의 모니터링 그래프를 사용하여 EBS 볼륨의 메트릭을 봅니다. 각 볼륨의 프로비저닝된 공간에 대한 소비된 공간을 검토합니다. 활용도가 낮은 볼륨을 식별합니다.",
      "B": "EC2 콘솔에서 EC2 인스턴스를 중지합니다. EC2 인스턴스 타입을 Amazon EBS 최적화로 변경합니다. EC2 인스턴스를 시작합니다.",
      "C": "AWS Compute Optimizer에 가입합니다. 메트릭이 수집될 충분한 시간을 허용합니다. Compute Optimizer의 EBS 볼륨 결과를 검토합니다.",
      "D": "EC2 인스턴스에 fio 도구를 설치하고 필요한 워크로드를 근사하는 .cfg 파일을 생성합니다. 벤치마크 결과를 사용하여 프로비저닝된 EBS 볼륨이 가장 적절한 타입인지 여부를 판단합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ AWS Compute Optimizer — 머신러닝 기반 리소스 최적화 제안 도구\n▸ EBS 볼륨 분석 — IOPS, 처리량, 스토리지 활용도 자동 검사\n▸ Cost Optimization Recommendations — 데이터 기반 최적화 제안\n\n【정답 포인트】\n▸ \"가장 운영상 효율적\"→자동화된 분석 도구 (수동 모니터링 X)\n▸ \"IOPS와 처리량\"→Compute Optimizer가 EBS 볼륨 분석 수행\n▸ \"모든 EBS 볼륨 검토\"→자동 스캔 & 권장사항 (스크립트 불필요)\n\n【오답 체크】\n(A) EC2 콘솔 그래프는 수동 비교 필요, 자동 최적화 제안 없음\n(B) 인스턴스 중지/변경은 EBS 볼륨 타입 분석과 무관\n(D) fio 벤치마크는 수동 테스트, 모든 볼륨에 확장 불가\n\n【시험 포인트】\n▸ Compute Optimizer = 최적화 제안의 \"자동화\" 도구\n▸ \"충분한 시간 허용\" = 머신러닝 수렴 필요 (최소 14일)\n▸ EBS 볼륨 권장사항 = Compute Optimizer 기본 기능\n▸ 함정: A는 수동 분석이라 비효율적, D는 벤치마킹 비용 높음",
  ...
  {
    "id": 19,
    "question": "CloudOps 엔지니어는 회사의 현재 및 미래의 모든 Amazon S3 버킷에서 로깅이 활성화되도록 보장해야 합니다. S3 버킷에서 로깅이 활성화되지 않은 경우 자동화된 프로세스가 S3 버킷에 대해 로깅을 활성화해야 합니다. 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "AWS Trusted Advisor를 사용하여 로깅이 활성화되지 않은 S3 버킷 확인을 수행합니다. 로깅이 활성화되지 않은 S3 버킷에 대해 로깅을 활성화하도록 확인을 구성합니다.",
      "B": "모든 현재 및 미래의 S3 버킷에서 로깅을 활성화하도록 요구하는 S3 버킷 정책을 구성합니다.",
      "C": "s3-bucket-logging-enabled AWS Config 관리형 규칙을 사용합니다. AWS Lambda 함수를 사용하는 수정 액션을 추가하여 로깅을 활성화합니다.",
      "D": "s3-bucket-logging-enabled AWS Config 관리형 규칙을 사용합니다. AWS-ConfigureS3BucketLogging AWS Systems Manager Automation runbook을 사용하는 수정 액션을 추가하여 로깅을 활성화합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ AWS Config — 리소스 컴플라이언스 모니터링 & 자동 수정\n▸ Config Remediation — 비준수 리소스 자동 해결\n▸ AWS Systems Manager Automation — 사전 검증된 runbook (AWS 공식)\n\n【정답 포인트】\n▸ \"현재 및 미래\"→Config로 지속적 모니터링\n▸ \"자동화된 프로세스\"→Remediation 액션으로 자동 수정\n▸ \"AWS-ConfigureS3BucketLogging\"→AWS 공식 검증 runbook (신뢰성 높음)\n\n【오답 체크】\n(A) Trusted Advisor는 실시간 수정 불가, 권장사항만 제공\n(B) 버킷 정책은 로깅 활성화 강제 불가, 설정만 가능\n(C) Lambda로 수정도 가능하지만, AWS 공식 runbook이 더 안정적\n\n【시험 포인트】\n▸ Config Managed Rules = 자동 검사 & 수정 매커니즘\n▸ AWS Systems Manager Automation = AWS 공식 검증 솔루션\n▸ \"현재 및 미래\"의 미래 버킷도 자동 스캔 (Config의 기본)\n▸ C vs D 비교: 둘 다 Config 사용하지만, D가 검증된 runbook이라 우수\n▸ 함정: Trusted Advisor는 자동 수정 불가, 버킷 정책은 강제 불가",
  ...
  {
    "id": 20,
    "question": "회사는 수백만 명의 구독자를 보유하고 있습니다. 회사의 마케팅 부서는 매주 토요일에 구독자에게 알림을 보내는 프로세스를 자동화하려고 합니다. 회사는 이미 Amazon Simple Notification Service(Amazon SNS)를 사용하여 구독자에게 알림을 보내는 메커니즘을 가지고 있습니다. 하지만 회사는 역사적으로 구독자에게 수동으로 알림을 보냈습니다. CloudOps 엔지니어는 스케줄에 따라 알림을 자동으로 보내는 솔루션이 필요합니다. 이러한 요구사항을 가장 운영상 효율적인 방식으로 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "새로운 Amazon EC2 인스턴스를 시작합니다. AWS SDK를 사용하여 매주 토요일에 구독자에게 SNS 알림을 보내도록 cron 작업을 구성합니다.",
      "B": "매주 토요일에 트리거되는 Amazon EventBridge 규칙을 생성합니다. SNS 주제에 알림을 게시하도록 규칙을 구성합니다.",
      "C": "매주 토요일에 구독자에게 알림을 보내는 메시지 팬아웃에 SNS 구독을 생성합니다.",
      "D": "AWS Step Functions 스케줄링 기능을 사용하여 매주 토요일에 Step Functions 단계를 실행합니다. SNS 주제에 메시지를 게시하도록 단계를 구성합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ EventBridge Rules — 시간 기반 이벤트 트리거 (cron 표현식 지원)\n▸ SNS Topic — 메시지 게시/구독 메커니즘\n▸ Serverless — EC2 없이 완전 관리형 서비스\n\n【정답 포인트】\n▸ \"스케줄에 따라\"→EventBridge cron 규칙 (토요일 지정)\n▸ \"가장 운영상 효율적\"→서버리스 (EC2 유지보수 X)\n▸ \"기존 SNS\"→EventBridge→SNS 통합 (즉시 사용 가능)\n\n【오답 체크】\n(A) EC2 인스턴스 실행 비용, 패치/관리 오버헤드 발생\n(C) \"메시지 팬아웃\"은 SNS 기능이 아님, 구독자가 이미 있음\n(D) Step Functions 스케줄링도 가능하지만, EventBridge가 더 간단\n\n【시험 포인트】\n▸ \"정기적 스케줄\" = EventBridge (cron) vs EC2 (cron 작업)\n▸ Serverless 선호도 = EventBridge > EC2\n▸ SNS 통합 대상 = EventBridge 네이티브 지원\n▸ 함정: A는 운영 부담 증가, D는 오버엔지니어링",
  ...
  {
    "id": 22,
    "question": "Amazon EC2 인스턴스가 Amazon Simple Queue Service(Amazon SQS) 큐를 사용하는 애플리케이션을 실행 중입니다. CloudOps 엔지니어는 애플리케이션이 SQS 큐에서 메시지를 읽고, 쓰고, 삭제할 수 있도록 보장해야 합니다. 이러한 요구사항을 가장 안전한 방식으로 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "sqs:SendMessage 권한, sqs:ReceiveMessage 권한, sqs:DeleteMessage 권한을 적절한 큐에 허용하는 IAM 정책이 있는 IAM 사용자를 생성합니다. IAM 사용자의 자격증명을 애플리케이션 구성에 포함합니다.",
      "B": "sqs:SendMessage 권한, sqs:ReceiveMessage 권한, sqs:DeleteMessage 권한을 적절한 큐에 허용하는 IAM 정책이 있는 IAM 사용자를 생성합니다. IAM 사용자의 액세스 키와 시크릿 액세스 키를 EC2 인스턴스의 환경 변수로 내보냅니다.",
      "C": "EC2 인스턴스가 AWS 서비스를 호출할 수 있도록 하는 IAM 역할을 생성하고 연결합니다. 적절한 큐에 sqs:* 권한을 허용하는 IAM 정책을 역할에 연결합니다.",
      "D": "EC2 인스턴스가 AWS 서비스를 호출할 수 있도록 하는 IAM 역할을 생성하고 연결합니다. sqs:SendMessage 권한, sqs:ReceiveMessage 권한, sqs:DeleteMessage 권한을 적절한 큐에 허용하는 IAM 정책을 역할에 연결합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ IAM Role (Instance Profile) — EC2 인스턴스에 임시 자격증명 제공\n▸ Principle of Least Privilege — 필요한 권한만 부여\n▸ Credential Exposure Risk — 자격증명이 코드/로그에 노출될 위험\n\n【정답 포인트】\n▸ \"가장 안전한 방식\"→IAM 역할 (자격증명 저장 X)\n▸ \"EC2에서 SQS\"→IAM 역할 기반 임시 자격증명\n▸ \"3가지 권한\"→sqs:SendMessage, ReceiveMessage, DeleteMessage (sqs:* 아님)\n\n【오답 체크】\n(A) IAM 사용자 자격증명 포함 = 보안 위험 (코드 노출 시 영구 키)\n(B) 환경 변수에 액세스 키 저장 = 자격증명 노출 위험\n(C) sqs:* = 최소 권한 원칙 위반 (DeleteQueue, PurgeQueue 등 추가)\n\n【시험 포인트】\n▸ \"EC2 + AWS 서비스\" = IAM 역할 (항상 최선)\n▸ \"최소 권한\" = 필요한 액션만 (SendMessage, ReceiveMessage, DeleteMessage)\n▸ IAM 사용자 자격증명 = EC2에서 금지 사항\n▸ 환경 변수 = 자격증명 저장소로 부적절\n▸ 함정: C는 역할 사용하지만, sqs:*로 과도한 권한",
  ...
  {
    "id": 24,
    "question": "회사는 VPC와 회사의 온프레미스 데이터 센터에서 컴퓨팅 리소스를 운영합니다. 회사는 이미 VPC와 온프레미스 데이터 센터 간에 AWS Direct Connect 연결을 가지고 있습니다. CloudOps 엔지니어는 VPC의 Amazon EC2 인스턴스가 온프레미스 데이터 센터의 호스트에 대한 DNS 이름을 해석할 수 있도록 보장해야 합니다. 이 요구사항을 가장 적게 지속적인 유지보수로 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "Amazon Route 53 프라이빗 호스팅 영역을 생성합니다. 온프레미스 데이터 센터의 호스트의 호스트이름과 IP 주소로 영역을 채웁니다.",
      "B": "Amazon Route 53 Resolver 아웃바운드 엔드포인트를 생성합니다. 전달해야 할 도메인 이름에 대해 온프레미스 DNS 서버의 IP 주소를 추가합니다.",
      "C": "Amazon Route 53 Resolver에서 역방향 DNS 쿼리에 대한 전달 규칙을 설정합니다. VPC에 대해 enableDnsHostnames 속성을 true로 설정합니다.",
      "D": "온프레미스 호스트의 호스트이름과 IP 주소를 각 EC2 인스턴스의 /etc/hosts 파일에 추가합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Route 53 Resolver Outbound Endpoint — VPC에서 온프레미스 DNS로 쿼리 전달\n▸ Forwarding Rule — 특정 도메인의 DNS 쿼리를 외부 DNS로 전달\n▸ Direct Connect — VPC ↔ 온프레미스 네트워크 연결\n\n【정답 포인트】\n▸ \"최소 유지보수\"→자동화 (수동 /etc/hosts 유지 X)\n▸ \"온프레미스 DNS\"→Route 53 Resolver 아웃바운드 엔드포인트\n▸ \"도메인 기반 전달\"→온프레미스 DNS 서버로 쿼리 자동 라우팅\n\n【오답 체크】\n(A) Route 53 프라이빗 호스팅 영역 = 수동 레코드 유지 필요 (온프레미스 변경 시마다)\n(C) \"역방향 DNS\" + \"enableDnsHostnames\" = 구성이 명확하지 않고, 역 DNS만 대한 것\n(D) /etc/hosts 파일 = 각 EC2 인스턴스마다 수동 관리 (새 서버 추가 시마다)\n\n【시험 포인트】\n▸ \"온프레미스 호스트\" \"DNS 이름 해석\" = Route 53 Resolver Outbound\n▸ \"최소 유지보수\" = 중앙화된 DNS 전달 규칙 (개별 수동 관리 X)\n▸ \"도메인 기반 조건부 전달\" = Forwarding Rule의 핵심\n▸ Private Hosted Zone = VPC 내부 리소스용 (온프레미스 동기화 필요)\n▸ 함정: A는 수동 유지, D는 확장성 없음, C는 역 DNS만 다룸",
  ...
  {
    "id": 25,
    "question": "회사는 Amazon EC2 인스턴스에서 웹 애플리케이션을 호스팅합니다. 웹 서버 로그는 Amazon CloudWatch Logs에 게시됩니다. 로그 이벤트는 동일한 구조를 가지며 사용자 요청과 연결된 HTTP 응답 코드를 포함합니다. 회사는 웹 서버가 HTTP 404 응답을 반환하는 횟수를 모니터링해야 합니다. 이러한 요구사항을 충족하는 가장 운영상 효율적인 솔루션은 무엇입니까?",
    "options": {
      "A": "웹 서버가 HTTP 404 응답을 반환하는 횟수를 세는 CloudWatch Logs Metric Filter를 생성합니다.",
      "B": "웹 서버가 HTTP 404 응답을 반환하는 횟수를 세는 CloudWatch Logs Subscription Filter를 생성합니다.",
      "C": "지난 1시간 동안 로그 이벤트의 404 코드를 세는 CloudWatch Logs Insights 쿼리를 실행하는 AWS Lambda 함수를 생성합니다.",
      "D": "지난 1시간 동안 로그 이벤트의 404 코드를 세는 CloudWatch Logs Insights 쿼리를 실행하는 스크립트를 생성합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ CloudWatch Logs Metric Filter — 로그 패턴 매칭으로 메트릭 생성\n▸ Subscription Filter — 로그 필터로 다른 서비스(Lambda 등)로 전달\n▸ CloudWatch Logs Insights — 임시 쿼리 도구 (연속 모니터링 X)\n\n【정답 포인트】\n▸ \"모니터링 필요\"→지속적인 메트릭 필요\n▸ \"HTTP 404 횟수\"→메트릭 필터로 자동 집계\n▸ \"가장 운영상 효율적\"→자동화된 메트릭 필터 (수동 쿼리 X)\n\n【오답 체크】\n(B) Subscription Filter = 로그를 다른 서비스로 전달용 (메트릭 생성 아님)\n(C) Lambda + Insights = 정기적 쿼리 (메트릭 필터보다 복잡)\n(D) 스크립트 실행 = 수동 관리 & 운영 부담 증가\n\n【시험 포인트】\n▸ \"정기적 모니터링\" = Metric Filter (자동 메트릭)\n▸ \"Subscription vs Metric\" = Subscription은 데이터 라우팅, Metric은 메트릭 생성\n▸ Insights = 임시 분석용 (지속 모니터링 아님)\n▸ 메트릭 필터 → CloudWatch Alarm → SNS/Auto Scaling 연결 가능\n▸ 함정: C/D는 수동 쿼리 필요로 비효율적",
  ...
  {
    "id": 26,
    "question": "회사는 Application Load Balancer 뒤에서 Amazon EC2 인스턴스에서 실행되는 내부 웹 애플리케이션을 가지고 있습니다. 인스턴스는 단일 Availability Zone의 Amazon EC2 Auto Scaling 그룹에서 실행됩니다. CloudOps 엔지니어는 애플리케이션을 고가용성으로 만들어야 합니다. CloudOps 엔지니어가 이 요구사항을 충족하기 위해 어떤 조치를 취해야 합니까?",
    "options": {
      "A": "최대 부하 시 필요한 용량을 충족하도록 Auto Scaling 그룹의 최대 인스턴스 수를 증가합니다.",
      "B": "최대 부하 시 필요한 용량을 충족하도록 Auto Scaling 그룹의 최소 인스턴스 수를 증가합니다.",
      "C": "Auto Scaling 그룹을 업데이트하여 같은 AWS Region의 두 번째 Availability Zone에서 새 인스턴스를 시작합니다.",
      "D": "Auto Scaling 그룹을 업데이트하여 두 번째 AWS Region의 Availability Zone에서 새 인스턴스를 시작합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ High Availability — 여러 가용 영역(AZ)에 리소스 분산\n▸ Single AZ Risk — 한 AZ 장애 시 전체 중단\n▸ Multi-AZ Deployment — 같은 리전 내 여러 AZ 배치\n\n【정답 포인트】\n▸ \"고가용성\"→AZ 장애 대비 필요\n▸ \"같은 리전 두 번째 AZ\"→지연 최소 & 비용 효율\n▸ Auto Scaling 그룹 = 다중 AZ 설정으로 자동 분산\n\n【오답 체크】\n(A) 최대 인스턴스만 증가 = 일반적 부하는 대응 X\n(B) 최소 인스턴스 증가 = AZ 장애 대비 아님\n(D) 두 번째 리전 = 지연 증가 & 비용 높음 (리전 간 데이터 전송료)\n\n【시험 포인트】\n▸ \"고가용성\" = 다중 AZ (같은 리전)\n▸ \"단일 AZ → 고가용성\" = 두 번째 AZ 추가\n▸ 리전 vs AZ = 리전 간은 재해복구, AZ는 고가용성\n▸ Auto Scaling 그룹에 다중 AZ 설정 = 자동 분산\n▸ 함정: A/B는 용량 증가만, D는 과도한 지연",
  ...
  {
    "id": 27,
    "question": "CloudOps 엔지니어는 Amazon EC2에서 실행되는 간단하고 공개 웹사이트를 만들고 있습니다. CloudOps 엔지니어는 기존 공개 서브넷에 EC2 인스턴스를 생성하고 인스턴스에 Elastic IP 주소를 할당했습니다. 다음으로, CloudOps 엔지니어는 새로운 Security Group을 생성하고 인스턴스에 적용하여 0.0.0.0/0에서 들어오는 HTTP 트래픽을 허용합니다. 마지막으로, CloudOps 엔지니어는 새로운 Network ACL을 생성하고 서브넷에 적용하여 0.0.0.0/0에서 들어오는 HTTP 트래픽을 허용합니다. 하지만 인터넷에서 웹사이트에 액세스할 수 없습니다. 이 문제의 원인은 무엇입니까?",
    "options": {
      "A": "CloudOps 엔지니어가 새로운 Network ACL에서 ephemeral port 반환 트래픽을 허용하는 아웃바운드 규칙을 생성하지 않았습니다.",
      "B": "CloudOps 엔지니어가 포트 80에서 HTTP 트래픽을 허용하는 Security Group에서 아웃바운드 규칙을 생성하지 않았습니다.",
      "C": "EC2 인스턴스에 할당된 Elastic IP 주소가 변경되었습니다.",
      "D": "포트 80에서 들어오는 HTTP 트래픽을 거부하는 규칙을 포함하는 추가 Network ACL이 서브넷과 연결되어 있습니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Network ACL — 서브넷 수준 상태 비저장 방화벽 (인바운드/아웃바운드 모두)\n▸ Security Group — 인스턴스 수준 상태 저장 방화벽 (아웃바운드 기본 허용)\n▸ Ephemeral Port — 클라이언트의 응답 포트 (1024-65535)\n\n【정답 포인트】\n▸ \"새로운 Network ACL\" \"HTTP 인바운드\"→아웃바운드 필요\n▸ \"클라이언트 응답\"→ephemeral 포트로 돌아옴\n▸ \"상태 비저장\" ACL→양방향 규칙 모두 명시 필요\n\n【오답 체크】\n(B) Security Group은 상태 저장 (아웃바운드 기본 허용), HTTP 응답 자동 허용\n(C) Elastic IP는 할당 후 변경 안 함 (재부팅 후에도 유지)\n(D) 추가 ACL이 있어도, \"새로운 ACL\"을 적용했으므로 이미 반영됨\n\n【시험 포인트】\n▸ \"새로운 Network ACL 적용\" = 기존 ACL 삭제됨 (D 오답)\n▸ ACL은 상태 비저장 = 응답 트래픽(Ephemeral 포트)도 명시 필요\n▸ Security Group은 상태 저장 = 아웃바운드 자동 허용\n▸ \"클라이언트 → 서버 (포트 80)\" + \"서버 → 클라이언트 (ephemeral)\" 모두 필요\n▸ 함정: Security Group만 설정했으면 OK, 하지만 새 ACL로 아웃바운드 ACL 규칙 제거됨",
  ...
  {
    "id": 28,
    "question": "회사는 AWS Systems Manager를 사용하여 대규모 Amazon EC2 인스턴스 플릿을 관리하려고 합니다. 회사는 인스턴스를 프라이빗 서브넷에 호스팅합니다. 회사는 최소 권한 원칙을 따라 액세스 권한을 할당합니다. 모든 프라이빗 서브넷은 NAT Gateway를 통해 인터넷 연결성을 가지고 있습니다. CloudOps 엔지니어는 최신 버전의 Systems Manager Agent(SSM Agent)를 설치합니다. 하지만 EC2 인스턴스는 Systems Manager Fleet Manager에 나타나지 않습니다. CloudOps 엔지니어는 이 문제를 해결해야 합니다. 이 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "NAT Gateway를 공개 서브넷에 배포된 NAT Instance로 교체합니다. 프라이빗 서브넷의 경로 테이블을 업데이트하여 NAT Instance를 사용합니다.",
      "B": "Systems Manager용 VPC Endpoint를 생성합니다. 프라이빗 서브넷의 경로 테이블에서 NAT Gateway를 통한 인터넷 경로를 제거합니다.",
      "C": "인스턴스와 연결된 EC2 Instance Profile에 AmazonSSMManagedInstanceCore AWS 관리형 정책을 연결합니다.",
      "D": "인스턴스와 연결된 EC2 Instance Profile에 ssm:*에 대한 모든 작업을 허용하는 사용자 정의 정책을 연결합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ SSM Agent — Systems Manager와 통신하는 EC2 에이전트\n▸ IAM Instance Profile — EC2 인스턴스에 권한 부여\n▸ AmazonSSMManagedInstanceCore — Systems Manager 기본 권한\n▸ VPC Endpoint — AWS 서비스에 대한 프라이빗 연결\n\n【정답 포인트】\n▸ \"Fleet Manager에 나타나지 않음\"→SSM Agent와 Systems Manager 통신 불가\n▸ \"SSM Agent 설치됨\"→권한 문제 (IAM 정책 확인 필요)\n▸ \"최소 권한 원칙\"→AmazonSSMManagedInstanceCore 정책 필요\n\n【오답 체크】\n(A) NAT Gateway → NAT Instance = 비용 증가 & 문제 미해결\n(B) VPC Endpoint는 도움이 되지만, NAT Gateway 제거 필수 아님\n(D) ssm:* = 최소 권한 원칙 위반 (불필요한 권한 포함)\n\n【시험 포인트】\n▸ \"Fleet Manager에 인스턴스 미표시\" = IAM 권한 부족\n▸ SSM Agent = 자체 설치 & 실행 (에이전트만으로 부족)\n▸ IAM Role (Instance Profile) = 인스턴스가 Systems Manager와 통신하기 위한 권한\n▸ AmazonSSMManagedInstanceCore = Systems Manager 최소 권한\n▸ 함정: VPC Endpoint는 이미 NAT Gateway로 인터넷 접근 가능하므로 불필요",
  ...
  {
    "id": 29,
    "question": "한 회사가 수천 개의 경보 시스템에서 알림을 수집하는 애플리케이션을 운영 중입니다. 알림에는 경보 알림과 정보 알림이 포함됩니다. 정보 알림에는 시스템 무장 프로세스, 해제 프로세스, 센서 상태 정보가 포함됩니다. 모든 알림은 Amazon Simple Queue Service(Amazon SQS) 큐에 메시지로 저장됩니다. Auto Scaling 그룹에 속한 Amazon EC2 인스턴스가 메시지를 처리합니다. CloudOps 엔지니어는 경보 알림을 정보 알림보다 우선순위를 높여 처리하는 솔루션을 구현해야 합니다. 어느 솔루션이 이러한 요구사항을 충족합니까?",
    "options": {
      "A": "Auto Scaling 그룹을 조정하여 큐의 메시지 수가 많을 때 더 빠르게 확장되도록 설정합니다.",
      "B": "Amazon Simple Notification Service(Amazon SNS) 팬아웃 기능을 Amazon SQS와 함께 사용하여 모든 EC2 인스턴스에 알림을 병렬로 전송합니다.",
      "C": "Amazon DynamoDB Stream을 추가하여 메시지 처리를 가속화합니다.",
      "D": "경보 알림용 큐와 정보 알림용 큐를 각각 생성합니다. 애플리케이션을 업데이트하여 경보 알림 큐에서 먼저 메시지를 수집하도록 합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Amazon SQS — 메시지 큐 서비스로 FIFO 또는 Standard 큐 모드 지원\n▸ 우선순위 처리 — 중요도가 높은 작업을 먼저 처리하는 패턴\n▸ Auto Scaling — 워크로드에 따라 EC2 인스턴스 수를 자동 조정\n\n【정답 포인트】\n▸ \"별도 큐\" → 경보와 정보 알림을 분리하여 우선순위 제어 가능\n▸ \"먼저 수집\" → 애플리케이션 로직에서 경보 큐를 먼저 폴링하도록 구현\n▸ 우선순위 기반 처리 — 메시지 큐의 구조적 분리가 가장 직관적이고 효율적\n\n【오답 체크】\n(A) Auto Scaling 속도 조정 — 우선순위 결정과 무관하며, 단순히 인스턴스 수 증가만 가능\n(B) SNS 팬아웃 — 모든 메시지를 병렬로 전송하므로 우선순위 적용 불가능\n(C) DynamoDB Stream — 처리 속도를 높이지만 우선순위 메커니즘 제공 안 함\n\n【시험 포인트】\n▸ SQS 기반 우선순위 처리는 \"별도 큐 + 폴링 순서\" 패턴이 표준\n▸ 아키텍처 설계 문제에서 \"우선순위\" 요구사항이 있으면 리소스 분리 고려\n▸ SNS/SQS 팬아웃은 \"브로드캐스팅 및 확장성\" 목적용이지 우선순위 아님",
  ...
  {
    "id": 30,
    "question": "한 회사가 AWS Trusted Advisor를 사용하여 보안 및 규정 준수를 구현하고 있습니다. 회사의 CloudOps 팀은 접근 가능한 Trusted Advisor 검사 목록을 검증하고 있습니다. 사용 가능한 Trusted Advisor 검사 수에 영향을 미치는 요인은 무엇입니까?",
    "options": {
      "A": "최소한 하나 이상의 Amazon EC2 인스턴스가 실행 중 상태에 있는지 여부",
      "B": "AWS Support 플랜",
      "C": "AWS Organizations 서비스 제어 정책(SCP)",
      "D": "AWS 계정 루트 사용자가 Multi-Factor Authentication(MFA)을 활성화했는지 여부"
    },
    "answer": "B",
  ...
  {
    "id": 31,
    "question": "CloudOps 엔지니어가 AWS CloudFormation 템플릿으로 VPC를 성공적으로 배포했습니다. CloudOps 엔지니어는 AWS Organizations로 관리되는 여러 계정에 동일한 템플릿을 배포하고자 합니다. 운영 오버헤드를 최소화하면서 이 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
  ...
  {
    "id": 32,
    "question": "한 회사의 애플리케이션이 인터넷 서비스 공급자에서 app.example.com으로 호스팅되고 있습니다. 회사는 Amazon Route 53으로 관리하는 자신이 소유한 www.company.com을 사용하여 애플리케이션에 접근하고자 합니다. 이를 해결하기 위해 생성해야 하는 Route 53 레코드는 무엇입니까?",
    "options": {
      "A": "A 레코드",
      "B": "Alias 레코드",
      "C": "CNAME 레코드",
  ...
  {
    "id": 33,
    "question": "한 회사가 애플리케이션 데이터를 캐시하기 위해 Amazon ElastiCache(Redis OSS)를 사용합니다. CloudOps 엔지니어는 캐시의 복원력을 높이는 솔루션을 구현해야 합니다. 솔루션은 또한 복구 시간 목표(RTO)를 최소화해야 합니다. 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "ElastiCache(Redis OSS)를 ElastiCache(Memcached)로 바꿉니다.",
      "B": "매 시간마다 백업을 시작하는 Amazon EventBridge 규칙을 생성합니다. 필요할 때 백업을 복구합니다.",
      "C": "두 번째 가용 영역에 읽기 복제본을 생성합니다. ElastiCache(Redis OSS) Replication 그룹에서 Multi-AZ를 활성화합니다.",
      "D": "자동 백업을 활성화합니다. 필요할 때 백업을 복구합니다."
    },
    "answer": "C",
  ...
  {
    "id": 34,
    "question": "한 회사가 여러 AWS 계정을 운영합니다. CloudOps 엔지니어는 샌드박스 계정을 사용하여 프로덕션 계정에서 사용할 IAM 정책을 만들고 검증합니다. CloudOps 엔지니어는 AWS CloudFormation을 사용하여 테스트를 위해 정책을 샌드박스 계정에 배포합니다. 테스트가 통과하면 CloudOps 엔지니어는 정책을 프로덕션 환경에 배포합니다. CloudOps 엔지니어는 샌드박스 계정과 프로덕션 계정 모두에서 AWS CloudTrail을 구성했습니다. CloudOps 엔지니어는 CloudFormation으로 배포한 후 IAM 정책의 변경사항을 감지하고자 합니다. CloudOps 엔지니어는 정책 변경에 대한 알림을 받아야 합니다. 최소한의 관리 노력으로 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "CloudTrail을 구성하여 CloudTrail이 IAM 정책 변경을 감지할 때 CloudOps 엔지니어에게 이메일 알림을 보내도록 합니다.",
      "B": "Amazon EventBridge 규칙을 생성하여 AWS Lambda 함수를 호출하고, CloudFormation 스택에서 드리프트를 확인하도록 합니다. 함수가 드리프트를 감지하면 Amazon Simple Notification Service(Amazon SNS)를 사용하여 CloudOps 엔지니어에게 알림을 보내도록 구성합니다.",
      "C": "AWS Identity and Access Management Access Analyzer를 사용하여 프로덕션 계정의 IAM 정책이 연결된 IAM 역할에 대한 CloudTrail 활동을 기반으로 정책을 생성합니다. 결과를 샌드박스 계정의 IAM 정책과 비교합니다. 정책이 다르면 CloudOps 엔지니어에게 알림을 보냅니다.",
      "D": "IAM 정책을 Amazon S3 버킷의 JSON 문서로 저장합니다. AWS Lambda 함수를 사용하여 IAM 정책을 S3 버킷에 저장된 JSON 문서와 주기적으로 비교합니다."
    },
    "answer": "B",
  ...
  {
    "id": 35,
    "question": "금융 회사가 Amazon S3 버킷에 기밀 데이터를 저장합니다. 회사는 Amazon QuickSight를 사용하여 데이터를 분석하고 대시보드 보고서를 생성합니다. 회사는 QuickSight로의 모든 데이터 액세스 및 연결이 회사의 VPC 네트워크 경계 내에 남아 있어야 합니다. 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "QuickSight를 위한 Interface VPC 엔드포인트를 생성합니다. AWS PrivateLink를 사용하여 VPC 내에서 QuickSight에 연결하도록 엔드포인트를 구성합니다. S3 데이터를 가리키는 매니페스트 파일을 생성합니다. QuickSight가 S3 버킷에 액세스할 수 있도록 권한을 부여합니다.",
      "B": "QuickSight를 위한 VPC 엔드포인트를 설정합니다. Amazon EC2 인스턴스를 프록시로 사용하여 VPC와 QuickSight 간의 직접 연결을 설정합니다. S3 데이터를 가리키는 매니페스트 파일을 생성합니다. 매니페스트를 EC2 인스턴스에 저장합니다. QuickSight가 EC2 인스턴스에 액세스할 수 있도록 권한을 부여합니다.",
      "C": "Amazon S3 VPC Gateway 엔드포인트를 구성합니다. QuickSight의 모든 데이터를 엔드포인트를 통해 라우팅하여 데이터를 전송합니다. QuickSight가 S3 버킷에 액세스할 수 있도록 권한을 부여합니다.",
      "D": "회사의 VPC에서 NAT 게이트웨이를 구성합니다. QuickSight의 모든 데이터를 NAT 게이트웨이를 통해 라우팅하여 데이터를 전송합니다. QuickSight가 S3 버킷에 액세스할 수 있도록 권한을 부여합니다."
    },
    "answer": "A",
  ...
  {
    "id": 36,
    "question": "한 회사가 온프레미스 파일 서버를 AWS로 이전하고 있습니다. 파일 서버에 저장된 모든 데이터는 가용 영역이 사용 불가능해지거나 시스템 유지보수가 수행될 때도 액세스할 수 있어야 합니다. 사용자는 SMB 프로토콜을 통해 파일 서버와 상호작용할 수 있어야 합니다. 사용자는 또한 Windows ACL을 사용하여 파일 권한을 관리할 수 있어야 합니다. 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
    ...
    "en_opts": {
      ...
      "D": "Deploy two Amazon FSx for Windows File Server Single-AZ file systems. Configure Microsoft Distributed File System Replication (DFSR)."
    },
  ...
  {
    "id": 37,
    "question": "한 회사의 웹 애플리케이션이 매 밤 여러 번 성능 문제를 겪고 있습니다. 근본 원인 분석 결과 Amazon EC2 Linux 인스턴스에서 CPU 사용률이 5분간 갑자기 증가하는 것으로 나타났습니다. CloudOps 엔지니어는 더 많은 CPU를 소비하는 서비스 또는 프로세스의 프로세스 ID(PID)를 찾아야 합니다. CloudOps 엔지니어가 최소한의 노력으로 프로세스 사용률 정보를 수집하기 위해 해야 할 일은 무엇입니까?",
    "options": {
      "A": "Amazon CloudWatch 에이전트 procstat 플러그인을 구성하여 CPU 프로세스 메트릭을 캡처합니다.",
      "B": "매 분마다 실행되어 PID를 캡처하고 알림을 전송하는 AWS Lambda 함수를 구성합니다.",
      "C": ".pem 키를 사용하여 매 밤 EC2 인스턴스에 로그인한 다음, top 명령을 실행합니다.",
  ...
  {
    "id": 38,
    "question": "한 회사는 최종 사용자에게 웹사이트의 가용성을 모니터링해야 합니다. 회사는 웹사이트의 가동 시간이 99% 미만으로 떨어지면 Amazon Simple Notification Service(Amazon SNS) 알림을 제공하는 솔루션이 필요합니다. 모니터링은 웹사이트의 사용자 경험에 대한 정확한 관점을 제공해야 합니다. 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "CloudWatch Logs 로그 그룹에 발행된 웹사이트의 로그를 기반으로 Amazon CloudWatch 경보를 생성합니다. HTTP 4xx 및 5xx 오류의 수가 지정된 임계값을 초과하면 SNS 알림을 발행하도록 경보를 구성합니다.",
      "B": "CloudWatch에 발행된 웹사이트의 메트릭을 기반으로 Amazon CloudWatch 경보를 생성합니다. 변칙 탐지를 기반으로 SNS 알림을 발행하도록 경보를 구성합니다.",
      "C": "Amazon CloudWatch Synthetics 하트비트 모니터링 카나리를 생성합니다. 카나리를 최종 사용자의 웹사이트 URL과 연결합니다. 카나리에 대한 CloudWatch 경보를 생성합니다. SuccessPercent 메트릭 값이 99% 미만이면 SNS 알림을 발행하도록 경보를 구성합니다.",
      "D": "Amazon CloudWatch Synthetics 끊어진 링크 검사기 모니터링 카나리를 생성합니다. 카나리를 최종 사용자의 웹사이트 URL과 연결합니다. 카나리에 대한 CloudWatch 경보를 생성합니다. SuccessPercent 메트릭 값이 99% 미만이면 SNS 알림을 발행하도록 경보를 구성합니다."
    },
    "answer": "C",
  ...
  {
    "id": 39,
    "question": "한 회사가 여러 가용 영역에 걸쳐 Amazon EC2 Auto Scaling을 사용합니다. 회사는 EC2 인스턴스가 프라이빗 서브넷에 프로비전되도록 해야 합니다. 회사는 최근 VPC의 NAT 게이트웨이 수를 1개로 줄여 클라우드 인프라를 최적화했습니다. 인프라 업데이트 후 일부 EC2 인스턴스가 인터넷 연결을 잃었습니다. CloudOps 엔지니어는 연결 문제를 해결해야 합니다. 이 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
      "A": "기존 NAT 게이트웨이를 동일한 서브넷의 NAT 인스턴스로 바꿉니다.",
      "B": "VPC 라우팅 테이블을 업데이트하여 인터넷 트래픽이 기존 NAT 게이트웨이를 대상으로 하도록 합니다.",
      "C": "VPC 라우팅 테이블을 업데이트하여 인터넷 트래픽이 인터넷 게이트웨이를 대상으로 하도록 합니다.",
  ...
  {
    "id": 40,
    "question": "한 회사가 AWS CloudFormation을 사용하여 AWS의 Amazon EC2 인스턴스 스택을 관리합니다. CloudOps 엔지니어는 누군가가 스택을 삭제하더라도 인스턴스 및 모든 인스턴스 데이터를 유지해야 합니다. 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
  ...
  {
    "id": 41,
    "question": "CloudOps 엔지니어가 Auto Scaling 그룹에 속한 Amazon EC2 인스턴스에 애플리케이션을 배포할 준비를 하고 있습니다. 애플리케이션은 의존성을 설치해야 합니다. 애플리케이션 업데이트가 매주 발행됩니다. CloudOps 엔지니어는 정기적으로 애플리케이션 업데이트를 적용하는 솔루션을 구현해야 합니다. 솔루션은 또한 Amazon Machine Image(AMI) 생성 중에 취약점 검사를 수행해야 합니다. 이러한 요구사항을 충족하는 가장 운영 효율적인 솔루션은 무엇입니까?",
    "options": {
      "A": "Packer를 사용하는 스크립트를 생성합니다. 스크립트를 실행하는 cron 작업을 예약합니다.",
      "B": "EC2 인스턴스에 애플리케이션 및 해당 의존성을 설치합니다. EC2 인스턴스의 AMI를 생성합니다.",
      "C": "사용자 정의 레시피가 있는 EC2 Image Builder를 사용하여 애플리케이션 및 해당 의존성을 설치합니다.",
      "D": "Amazon EventBridge 스케줄 규칙을 사용하여 EC2 CreateImage API 작업을 호출합니다."
    },
    "answer": "C",
  ...
  {
    "id": 42,
    "question": "한 회사가 여러 가용 영역에 분산된 Amazon EC2 인스턴스 세트에서 애플리케이션을 호스팅할 계획입니다. 애플리케이션은 초당 수백만 개의 요청으로 확장할 수 있어야 합니다. CloudOps 엔지니어는 EC2 인스턴스에 트래픽을 분산하는 솔루션을 설계해야 합니다. 솔루션은 각 가용 영역에 대해 단일 정적 IP 주소를 사용하면서 급격하고 변동성이 높은 트래픽 패턴을 처리하도록 최적화되어야 합니다. 이러한 요구사항을 충족하는 솔루션은 무엇입니까?",
    "options": {
  ...
  {
    "id": 43,
    "question": "한 회사가 Amazon EC2 인스턴스에서 실행되는 상태 기반(Stateful) 웹 애플리케이션을 Auto Scaling 그룹으로 호스팅합니다. 인스턴스들은 단일 Target Group을 가진 Application Load Balancer(ALB)를 통해 실행되고 있으며, ALB는 Amazon CloudFront 배포의 Origin으로 구성되어 있습니다. 사용자들이 웹 애플리케이션에서 무작위로 로그아웃된다고 보고했습니다. CloudOps 엔지니어가 이 문제를 해결하기 위해 취해야 할 조치의 조합은 무엇인가요? (2가지 선택)",
    "options": {
      "A": "ALB Target Group의 로드 밸런싱 알고리즘을 최소 미결정 요청(Least Outstanding Requests)으로 변경",
      "B": "CloudFront 배포의 캐시 동작에서 Cookie Forwarding 구성",
      "C": "CloudFront 배포의 캐시 동작에서 Header Forwarding 구성",
      "D": "ALB Listener Rule에서 그룹 레벨의 Stickiness 활성화",
      "E": "ALB Target Group에서 Sticky Sessions 활성화"
    },
    "answer": "BE",
    "explanation": "【핵심 용어】\n▸ Application Load Balancer Sticky Sessions — 동일 사용자 요청을 같은 백엔드 인스턴스로 라우팅하는 기능\n▸ Cookie Forwarding — CloudFront가 쿠키를 Origin(ALB)으로 전달, 세션 식별 가능\n▸ Round Robin / Least Outstanding Requests — ALB의 기본 로드 밸런싱 알고리즘 (세션 유지 X)\n\n【정답 포인트】\n▸ \"동일 사용자 → 같은 인스턴스\" → ALB Sticky Sessions 활성화 필수\n▸ CloudFront → ALB 경로에서 쿠키 손실 방지 → CloudFront에서 Cookie Forwarding 활성화\n▸ B(Cookie Forwarding) + E(Sticky Sessions) 조합으로 세션 일관성 보장\n\n【오답 체크】\n(A) Least Outstanding Requests 알고리즘은 분산 효율 기준의 로드 밸런싱일 뿐 세션 유지 기능 없음. 오히려 분산 강화로 문제 악화.\n(C) Header Forwarding은 일반 HTTP 헤더만 전달하며 쿠키 정보를 보존하지 않아 ALB Sticky Sessions 식별 불가.\n(D) ALB Target Group 수준의 그룹 Stickiness 개념은 존재하지 않음. Sticky Sessions은 Target Group 속성으로 활성화.\n\n【시험 포인트】\n▸ \"동일 사용자 동일 서버\" → ALB Sticky Sessions + CloudFront Cookie Forwarding 조합 필수\n▸ CloudFront 앞단 시 쿠키 포워딩 누락이 세션 단절의 흔한 원인\n▸ ALB Sticky Sessions은 application-based(쿠키) / duration-based 두 가지 모드 지원",
  ...
  {
    "id": 44,
    "question": "한 글로벌 회사가 AWS Organizations의 조직을 이용하여 여러 AWS 계정을 관리합니다. 규정 준수를 위해 회사는 5개의 AWS Region에 워크로드 환경을 배포하고 있으며, 각 Region마다 별도의 AWS 계정을 보유하고 있습니다. 회사는 모든 환경의 VPC를 디렉토리 역할을 하는 중앙 공유 VPC와 공유 모니터링 VPC에 연결해야 합니다. 공유 계정은 각각 별도의 AWS 계정에 있습니다. 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "중앙 공유 AWS 계정에 Transit Gateway를 생성합니다. 회사의 AWS 계정들과 Transit Gateway를 공유합니다. 모든 VPC를 중앙 Transit Gateway에 연결합니다.",
      "B": "회사가 리소스를 배포한 모든 Region에 별도의 Transit Gateway를 생성합니다. Transit Gateway들을 회사의 AWS 계정들과 공유합니다. 각 Region의 VPC를 같은 Region의 Transit Gateway에 연결합니다. Transit Gateway들을 서로 피어링합니다. 모든 라우팅 테이블에 적절한 경로를 생성합니다.",
      "C": "공유 VPC를 위한 Virtual Private Gateway를 생성합니다. 워크로드 VPC를 위한 Customer Gateway를 생성합니다. 디렉토리 VPC, 모니터링 VPC 및 모든 워크로드 VPC 사이에 AWS Site-to-Site VPN 연결을 구성합니다.",
      "D": "중앙 공유 VPC, 공유 모니터링 VPC, 그리고 모든 워크로드 VPC 사이에 VPC Peering Connection을 생성합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Transit Gateway — Region 내 및 Region 간 VPC/온프레미스를 연결하는 중앙 허브(Spoke 모델)\n▸ Transit Gateway Peering — 서로 다른 Region의 Transit Gateway 간 연결\n▸ VPC Peering — 1:1 양방향 연결로, 다중 VPC 연결 시 메싱 그래프 복잡도 지수 증가\n▸ Site-to-Site VPN — 온프레미스 환경 연결용, 5개 Region × 2개 공유 VPC = 최소 10개 VPN 터널 필요\n\n【정답 포인트】\nB(Region별 Transit Gateway + Peering): 5개 Region 각각에 Transit Gateway 배포 → 같은 Region의 모든 VPC(워크로드 + 공유)를 로컬 Transit Gateway에 연결 → Region 간 Transit Gateway Peering으로 전역 연결 → 확장성 우수, 중앙집중식 관리 최소화\n\n【오답 체크】\n(A) 중앙 Transit Gateway는 단일 Region의 VPC만 직접 연결 가능하며, 5개 Region 분산 배포에는 Cross-Region 능력 부족\n(C) Site-to-Site VPN은 온프레미스-AWS 간 용도이며, 각 VPC마다 별도 VPN 터널 필요로 관리 복잡도 극대\n(D) 5개 Region × 3개 VPC(워크로드 1개 + 공유 2개) = 15개 VPC를 모두 Peering하면 조합 수(15×14/2=105)로 관리 불가능\n\n【시험 포인트】\n▸ 다중 VPC 다중 Region 연결 → Transit Gateway Peering이 표준 패턴\n▸ Region별 배포 = 각 Region의 지연 시간 최소화 + 로컬 라우팅 테이블 관리\n▸ VPC Peering은 메시 수가 지수함수적으로 증가하므로 3개 이상 VPC 연결 시 Transit Gateway 선택",
  ...
  {
    "id": 45,
    "question": "한 회사가 Amazon RDS for PostgreSQL 데이터베이스에 중요한 정보를 저장합니다. 회사는 피크 쇼핑 시간에 성능 저하, CPU 높은 이용률, 증가된 쿼리 지연시간, 그리고 연결 타임아웃이 발생함을 알아챘습니다. 또한 회사는 피크 시간에 사용자 연결의 급증을 식별했습니다. 연결 급증은 데이터베이스의 읽기 성능에 영향을 미칩니다. 회사는 데이터베이스 성능 문제를 해결하려고 합니다. 이 요구사항을 충족할 조치의 조합은 무엇인가요? (2가지 선택)",
    "options": {
      "A": "Amazon RDS Performance Insights를 사용하여 데이터베이스 성능에 가장 영향을 미치는 SQL 쿼리를 분석합니다. 분석 결과를 바탕으로 SQL 쿼리를 수정합니다.",
      "B": "Amazon CloudWatch Logs Insights를 사용하여 데이터베이스 쿼리를 분석하고 성능 병목을 식별합니다. 분석 결과를 바탕으로 쿼리를 수정합니다.",
      "C": "단일 Availability Zone을 가진 Amazon RDS for PostgreSQL을 사용합니다.",
      "D": "Connection Pooling을 완전히 비활성화하여 피크 시간에도 모든 사용자 연결이 동등하게 처리되도록 합니다.",
      "E": "Connection Pooling을 갖춘 RDS Proxy를 구현합니다."
    },
    "answer": "AE",
    "explanation": "【핵심 용어】\n▸ RDS Performance Insights — 실시간 DB 성능 모니터링, 느린 쿼리 식별 및 대기 이벤트 분석\n▸ RDS Proxy — Connection Pooling으로 데이터베이스 연결 재사용, 연결 오버헤드 감소(최대 1,000배)\n▸ CloudWatch Logs Insights — 로그 쿼리 분석 도구지만, DB 성능 메트릭(DB Load, Active Sessions) 직접 분석 불가\n▸ 연결 급증 — 애플리케이션이 새로운 연결을 계속 생성하면 데이터베이스 리소스(메모리, CPU) 소진\n\n【정답 포인트】\nA(Performance Insights): 실시간으로 느린 쿼리, 대기 이벤트(Lock, I/O), DB Load 분석 → 쿼리 최적화로 근본적 처리량 개선\nE(RDS Proxy): 애플리케이션 연결을 풀 내 기존 연결로 재사용 → 새 연결 생성 오버헤드 제거, CPU/메모리 절감\n\n【오답 체크】\n(B) CloudWatch Logs Insights는 텍스트 로그 분석 도구로, 데이터베이스의 성능 메트릭(DB Load, Wait Events)을 직접 측정하지 않음\n(C) 다중 AZ는 가용성 향상이지 피크 시간 성능 개선과 무관하며, CPU/메모리 병목은 해결되지 않음\n(D) Connection Pooling 비활성화는 반대 방향으로, 각 사용자 연결이 독립적 리소스를 점유하므로 성능 악화\n\n【시험 포인트】\n▸ 데이터베이스 성능 = 느린 쿼리(쿼리 최적화) + 연결 오버헤드(RDS Proxy)\n▸ Performance Insights는 어느 쿼리가 병목인지 식별 → 개발자가 쿼리 개선\n▸ RDS Proxy는 연결 수를 물리적으로 줄여 데이터베이스 리소스 절감\n▸ 연결 급증은 성능 저하의 주요 원인이므로 RDS Proxy 필수",
  ...
  {
    "id": 46,
    "question": "한 회사가 Application Load Balancer(ALB)를 통해 3개의 Amazon EC2 인스턴스에서 웹 애플리케이션을 실행합니다. 회사는 무작위 트래픽 증가 기간이 애플리케이션의 성능 저하를 유발함을 알아챘습니다. CloudOps 엔지니어는 증가된 트래픽에 대응하기 위해 애플리케이션을 확장해야 합니다. 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "Amazon CloudWatch 알람을 생성하여 애플리케이션 지연시간을 모니터링하고, 원하는 임계값에 도달하면 각 EC2 인스턴스의 크기를 증가시킵니다.",
      "B": "Amazon EventBridge Rule을 생성하여 애플리케이션 지연시간을 모니터링하고, 원하는 임계값에 도달하면 ALB에 EC2 인스턴스를 추가합니다.",
      "C": "애플리케이션을 Auto Scaling 그룹의 EC2 인스턴스에 배포하고 Target Tracking Scaling Policy를 설정합니다. ALB를 Auto Scaling 그룹에 연결합니다.",
      "D": "애플리케이션을 Auto Scaling 그룹의 EC2 인스턴스에 배포하고 Scheduled Scaling Policy를 설정합니다. ALB를 Auto Scaling 그룹에 연결합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ 수평 확장(Horizontal Scaling) — 인스턴스 개수를 늘려 처리량 증가\n▸ 수직 확장(Vertical Scaling) — 단일 인스턴스의 크기/성능을 증가(한계 존재)\n▸ Target Tracking Scaling Policy — 특정 메트릭(CPU, ALB Target Response Time)에 따라 자동 확장\n▸ Scheduled Scaling Policy — 미리 정한 시간에 확장, 예측 가능한 트래픽 패턴용\n▸ EventBridge — 이벤트 기반 워크플로우지만, 자동 확장은 전담 서비스 아님\n\n【정답 포인트】\nC(Auto Scaling + Target Tracking): 무작위 트래픽 급증에 대응하려면 메트릭 기반 동적 확장 필요 → Target Tracking Policy가 특정 메트릭(ALB Target Response Time, RequestCountPerTarget)을 모니터링 → 자동으로 인스턴스 개수 조정 → ALB 연결로 즉시 로드 밸런싱\n\n【오답 체크】\n(A) 수직 확장은 한계가 있고, CloudWatch 알람만으로는 자동 실행 불가능(Lambda 또는 Systems Manager 추가 필요)\n(B) EventBridge는 이벤트 라우팅 서비스이며, Auto Scaling의 전담 기능(Target Tracking, 조정 정책)을 대체하지 못함\n(D) Scheduled Scaling은 고정 시간 기반이므로 무작위 트래픽 급증에 대응 불가\n\n【시험 포인트】\n▸ 무작위 트래픽 = 메트릭 기반 동적 확장(Target Tracking) 선택\n▸ 예측 가능한 시간대(오전 9-11시) = Scheduled Scaling\n▸ Auto Scaling이 수평 확장의 표준 패턴(확장 그룹 내 인스턴스 개수 조정)\n▸ ALB와 Auto Scaling 통합이 필수(로드 밸런싱 + 동적 인스턴스 관리)",
  ...
  {
    "id": 47,
    "question": "한 회사의 15분에 실행되던 보고서 작업이 이제 1시간이 걸리고 있습니다. 한 애플리케이션이 보고서를 생성합니다. 애플리케이션은 Amazon EC2 인스턴스에서 실행되고 Amazon RDS for MySQL 데이터베이스에서 데이터를 추출합니다. CloudOps 엔지니어가 RDS 인스턴스의 Amazon CloudWatch 대시보드를 확인했을 때, 보고서가 실행되지 않는 경우에도 Read IOPS 메트릭이 높은 것을 발견했습니다. CloudOps 엔지니어는 RDS 인스턴스의 성능과 가용성을 개선해야 합니다. 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "RDS 인스턴스 앞에 Amazon ElastiCache 클러스터를 구성합니다. 보고서 작업을 ElastiCache 클러스터에서 쿼리하도록 수정합니다.",
      "B": "RDS Read Replica를 배포합니다. 보고서 작업을 Reader Endpoint에서 쿼리하도록 수정합니다.",
      "C": "Amazon CloudFront 배포를 생성합니다. RDS 인스턴스를 Origin으로 설정합니다. 보고서 작업을 CloudFront 배포에서 쿼리하도록 수정합니다.",
      "D": "RDS 인스턴스의 크기를 증가시킵니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Read Replica — 쓰기는 Primary, 읽기는 Replica에서 처리하여 I/O 분산\n▸ 읽기 IOPS 지속 — 보고서 작업만의 문제가 아니라 백그라운드 작업도 높은 읽기 IOPS 유발\n▸ ElastiCache — 애플리케이션 캐싱용이지만, 기존 쿼리 코드 대규모 수정 필요\n▸ CloudFront — 정적 웹 콘텐츠용이며, 동적 데이터베이스 쿼리에는 부적합\n\n【정답 포인트】\nB(Read Replica): Read Replica가 Primary 데이터베이스와 동기화되므로 최신 데이터 반영 → 보고서 생성 작업은 Replica의 Reader Endpoint로 재지정 → Primary는 애플리케이션의 쓰기/트랜잭션만 처리 → IOPS 부하 분산 → Replica가 별도 인스턴스이므로 가용성도 향상\n\n【오답 체크】\n(A) ElastiCache는 애플리케이션 로직이 캐시 계층을 인식해야 하므로, 기존 SQL 쿼리를 Redis/Memcached 명령어로 재작성 필요 → 높은 개발 비용\n(C) CloudFront는 웹 페이지, 이미지 같은 정적 콘텐츠 캐싱용이며, 동적 데이터베이스 쿼리 응답은 캐싱하지 않음\n(D) 인스턴스 크기 증가는 수직 확장이지만, 읽기 부하 자체는 증가하므로 단기 해결책일 뿐 확장성 제한\n\n【시험 포인트】\n▸ 읽기 집약적 워크로드(보고서, 분석) = Read Replica 분리\n▸ Replica는 비동기이므로 약간의 지연 가능하지만, 분석/보고서는 실시간 일관성 요구 낮음\n▸ \"보고서 작업 + 백그라운드 IOPS\" 조합 = 읽기 트래픽이 주 원인이므로 분산 필수\n▸ Read Replica는 \"최소 개발 노력\"으로 IOPS 분산 달성",
  ...
  {
    "id": 48,
    "question": "AWS에서 여러 워크로드를 실행하는 한 회사가 DNS 기반 위협 보호를 구현하여 보안 태세를 강화하고 싶습니다. 회사는 DNS 기반 공격을 차단해야 합니다. 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "AWS Shield Advanced를 배포하여 악의적 DNS 쿼리를 필터링하고 차단합니다. 도메인 필터링 정책을 설정합니다.",
      "B": "AWS WAF를 사용하여 DNS 트래픽을 검사하고 악의적 도메인을 찾습니다. 알려진 위협을 차단하는 사용자 정의 규칙을 생성합니다.",
      "C": "Amazon Route 53 Resolver를 구성하여 DNS 쿼리를 Route 53 Resolver DNS Firewall Advanced로 전달하여 위협을 탐지하고 필터링합니다.",
      "D": "AWS Config를 구성하여 DNS 쿼리 및 DNS 트래픽 패턴을 모니터링합니다. AWS Lambda 함수를 사용하여 악의적 도메인에 대한 액세스를 방지합니다."
    },
    "answer": "C",
  ...
  {
    "id": 49,
    "question": "CloudOps 엔지니어가 프로덕션 데이터베이스의 복사본을 마이그레이션 계정과 공유하려고 합니다. 프로덕션 데이터베이스는 Amazon RDS DB 인스턴스에 호스팅되며 production-rds-key라는 별칭을 가진 AWS Key Management Service(AWS KMS) 키로 저장 시 암호화됩니다. CloudOps 엔지니어가 최소한의 관리 오버헤드로 이 요구사항을 충족하려면 무엇을 해야 하나요?",
    "options": {
      "A": "프로덕션 계정에서 RDS DB 인스턴스의 스냅샷을 생성합니다. production-rds-key KMS 키의 KMS 키 정책을 수정하여 마이그레이션 계정의 루트 사용자에게 액세스 권한을 부여합니다. 스냅샷을 마이그레이션 계정과 공유합니다.",
      "B": "마이그레이션 계정에서 RDS Read Replica를 생성합니다. KMS 키 정책을 구성하여 production-rds-key KMS 키를 마이그레이션 계정에 복제합니다.",
      "C": "프로덕션 계정에서 RDS DB 인스턴스의 스냅샷을 생성합니다. 스냅샷을 마이그레이션 계정과 공유합니다. 마이그레이션 계정에서 동일한 별칭을 가진 새로운 KMS 키를 생성합니다.",
      "D": "기본 데이터베이스 도구 집합을 사용하여 RDS DB 인스턴스를 Amazon S3로 내보냅니다. S3 버킷과 프로덕션 계정과 마이그레이션 계정 간 교차 계정 액세스를 위한 S3 Bucket Policy를 생성합니다. 기본 데이터베이스 도구 집합을 사용하여 Amazon S3에서 새로운 RDS DB 인스턴스로 데이터베이스를 가져옵니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ RDS 스냅샷 공유 — 스냅샷과 암호화 KMS 키 접근 권한 모두 필요\n▸ KMS 교차 계정 액세스 — KMS 키 정책에서 대상 계정의 루트 또는 역할에 kms:Decrypt, kms:DescribeKey 권한 부여\n▸ 마이그레이션 계정 루트 사용자 — 교차 계정 권한의 기본 보안 주체\n▸ Read Replica 교차 계정 — KMS 키 복제 개념이 없음(옵션 B 오류)\n\n【정답 포인트】\nA(스냅샷 공유 + KMS 정책): ① RDS 스냅샷 생성(암호화 유지) → ② production-rds-key의 KMS 정책에 마이그레이션 계정 루트 권한 추가(kms:Decrypt 등) → ③ 스냅샷을 마이그레이션 계정과 공유 → 마이그레이션 계정에서 스냅샷 복원 시 기존 KMS 키 자동 사용 → \"최소 관리 부담\" 충족\n\n【오답 체크】\n(B) RDS는 \"계정 간 Read Replica 생성\" 미지원이며, \"KMS 키 복제\"는 KMS 기능이 아님\n(C) 스냅샷 공유 후 마이그레이션 계정에서 새 KMS 키 생성 → 기존 데이터 암호 해독 불가능(다른 키로 재암호화 필요, 복잡함)\n(D) S3 Export → 형식 변환 → Import는 관리 오버헤드 최대이며, 스냅샷 방식이 훨씬 간단\n\n【시험 포인트】\n▸ 암호화된 RDS 스냅샷 공유 = KMS 키 정책 + 스냅샷 공유 2단계 필수\n▸ 마이그레이션 계정은 스냅샷 복원 시 자동으로 기존 KMS 키 사용(새 키 생성 불필요)\n▸ \"최소 관리 부담\" = 스냅샷 기반 복사(옵션 A가 가장 간단)\n▸ S3 Export는 데이터 형식 변환이 필요하므로 오버헤드 증가",
  ...
  {
    "id": 51,
    "question": "개발자가 Amazon Linux Amazon Machine Image(AMI)를 사용하여 타사 애플리케이션을 호스팅하는 EC2 인스턴스를 시작합니다. 애플리케이션이 가끔 불안정해집니다. CloudOps 엔지니어는 이용률이 15분 동안 90% 이상일 때마다 EC2 인스턴스를 자동으로 재부팅하고 개발자에게 재부팅을 알리는 솔루션이 필요합니다. 최소한의 관리 노력으로 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "인스턴스의 CPU 이용률을 평가하는 Amazon CloudWatch 알람을 구성합니다. 알람이 활성화될 때 AWS Lambda 함수를 호출하도록 알람을 구성합니다. Lambda 함수가 Amazon Simple Notification Service(Amazon SNS) 주제에 메시지를 발행하고 EC2 인스턴스를 재부팅하도록 구성합니다. 개발자를 SNS 주제에 구독합니다.",
      "B": "인스턴스의 CPU 이용률을 평가하는 Amazon CloudWatch 알람을 생성합니다. 알람이 Amazon Simple Notification Service(Amazon SNS) 주제에 알림을 발행하고 EC2 인스턴스를 재부팅하는 EC2 Action을 수행하도록 구성합니다. 개발자를 SNS 주제에 구독합니다.",
      "C": "인스턴스의 CPU 이용률을 평가하는 Amazon CloudWatch 알람을 생성합니다. 알람이 AWS Systems Manager Action을 호출하여 Incident를 생성하고 개발자에게 알리며 재부팅을 요청하도록 구성합니다.",
      "D": "Amazon Simple Notification Service(Amazon SNS) 주제에 메시지를 발행하고 EC2 인스턴스를 재부팅하는 AWS Systems Manager Runbook Script를 생성합니다. 개발자를 SNS 주제에 구독합니다. 인스턴스의 CPU 이용률이 15분 이상 90% 이상으로 유지될 때 Systems Manager Runbook을 실행하도록 Amazon CloudWatch 알람을 구성합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ CloudWatch Alarm — 메트릭 임계값 기반 자동 트리거\n▸ EC2 Auto Recovery (recover) — CloudWatch Alarm 액션으로 인스턴스 자동 복구\n▸ Systems Manager Automation Runbook — 자동화된 운영 작업 실행 (수동 트리거 가능)\n▸ AWS::EC2::Reboot — CloudWatch Alarm 자체 액션 (개발 코드 불필요)\n\n【정답 포인트】\n▸ \"EC2 자동 재부팅 + 최소 개발 노력\" → CloudWatch Alarm + EC2 Reboot 액션 조합\n▸ 알람 액션에 EC2 Reboot을 직접 지정 → Lambda·Runbook 작성 불필요\n▸ 가장 단순한 네이티브 통합으로 운영 오버헤드 최소\n\n【오답 체크】\n(A) Lambda 함수 개발 + 실행 역할 권한 설정 + EC2 재부팅 호출 코드 필요. 코드 유지보수 부담 발생.\n(C) Systems Manager Incident는 사람의 개입을 전제로 한 Incident 관리 도구. 자동 재부팅과 무관.\n(D) Automation Runbook 작성 + CloudWatch ↔ Runbook 연결 구성이 추가로 필요해 B 옵션보다 복잡, \"최소 노력\" 위배.\n\n【시험 포인트】\n▸ EC2 재부팅/복구 → CloudWatch Alarm 네이티브 액션이 가장 단순\n▸ Lambda·Runbook·Incident는 모두 추가 작업이 들어가는 옵션 → 우선순위 낮음\n▸ \"최소 운영 노력\" 키워드 → 네이티브 통합 우선 검토",
  ...
  {
    "id": 52,
    "question": "한 회사의 마이크로서비스가 Application Load Balancer(ALB)를 통해 실행되는 Amazon EC2 인스턴스 세트에서 실행됩니다. CloudOps 엔지니어는 Amazon Route 53을 사용하여 ALB URL을 example.com에 매핑하는 레코드를 생성해야 합니다. 이 요구사항을 충족할 레코드 유형은 무엇인가요?",
    "options": {
      "A": "A 레코드",
      "B": "AAAA 레코드",
      "C": "Alias 레코드",
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ A 레코드 — 도메인을 IPv4 주소(예: 203.0.113.1)로 매핑\n▸ AAAA 레코드 — 도메인을 IPv6 주소로 매핑\n▸ CNAME 레코드 — 도메인을 다른 도메인으로 매핑(Canonical Name), 루트 도메인(example.com) 불가\n▸ Alias 레코드 — Route 53 전용, AWS 리소스(ALB, CloudFront, S3 등)에 직접 연결 가능, 루트 도메인 지원\n\n【정답 포인트】\nC(Alias 레코드): ALB는 DNS 이름(예: my-alb-123456.us-east-1.elb.amazonaws.com)을 가지므로 → Route 53 Alias 레코드로 example.com을 ALB의 DNS 이름에 매핑 → Alias는 AWS 리소스 직결로 자동 IP 업데이트(수동 관리 불필요)\n\n【오답 체크】\n(A) A 레코드는 IPv4 고정 주소용이며, ALB는 관리형 서비스로 IP 변경 가능성 → Alias가 더 적합\n(B) AAAA 레코드는 IPv6 매핑용이며, 일반적 ALB는 IPv4 기본\n(D) CNAME은 루트 도메인(example.com) 사용 불가(CNAME은 서브도메인 전용, www.example.com만 가능) + AWS 리소스 직결 미지원\n\n【시험 포인트】\n▸ ALB → Route 53 = Alias 레코드 표준 패턴\n▸ Alias 레코드는 AWS 리소스 매핑 전용(ALB, CloudFront, S3 정적 웹사이트 호스팅 등)\n▸ 루트 도메인 매핑 = A 또는 Alias (CNAME 불가)\n▸ CNAME은 외부 도메인(타사 호스팅, GitHub Pages) 매핑용",
  ...
  {
    "id": 53,
    "question": "한 애플리케이션이 Application Load Balancer(ALB)를 통해 Amazon EC2 인스턴스에서 실행됩니다. 애플리케이션은 시작 후 로컬 캐시를 채우는 데 최대 2분이 걸립니다. 애플리케이션은 시작 후 몇 초 후 Target Group 상태 확인에서 정상으로 보고됩니다. CloudOps 엔지니어는 일부 인스턴스가 재부팅된 후 각 인스턴스가 정상으로 보고되면 즉시 같은 양의 트래픽을 받음을 관찰합니다. 애플리케이션은 로컬 캐시가 채워지는 동안 점진적으로 증가하는 트래픽 할당이 필요합니다. 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "Target Group 속성 slow_start.duration_seconds를 120초로 변경합니다. 인스턴스를 재부팅하기 전에 Target Group에서 인스턴스를 등록 취소합니다. 인스턴스를 재부팅한 후 Target Group에 인스턴스를 등록합니다.",
      "B": "Target Group의 HealthCheckTimeoutSeconds 파라미터를 120초로 변경합니다. 인스턴스를 재부팅하기 전에 Target Group에서 인스턴스를 등록 취소합니다. 인스턴스를 재부팅한 후 Target Group에 인스턴스를 등록합니다.",
      "C": "상태 확인 상태를 모니터링하는 Amazon CloudWatch 알람을 구성합니다. 상태 확인이 실패하면 EC2 인스턴스를 재시작하는 알람 액션을 구성합니다. loadbalancing.algorithm.type Target Group 속성을 weighted_random으로 변경합니다.",
      "D": "Amazon EC2 Auto Scaling 그룹을 생성합니다. 기존 EC2 인스턴스를 Auto Scaling 그룹에 연결합니다. EC2 Auto Scaling 라이프사이클 훅을 구성하여 시작 인스턴스를 Pending Wait 상태로 이동합니다. 로컬 캐시가 채워지면 애플리케이션을 업데이트하여 라이프사이클 훅을 완료합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Slow Start Mode — ALB가 신규 등록 인스턴스에 처음 1~3분간 점진적으로 트래픽 할당(100%까지 선형 증가)\n▸ slow_start.duration_seconds — Slow Start 지속 시간 설정(기본 300초, 최대 3600초)\n▸ 상태 확인 정상 — 애플리케이션 준비 상태와 무관하게 상태 확인 통과 시 정상 판정\n▸ 라이프사이클 훅 — Auto Scaling 인스턴스 상태 전이 중 대기(Pending Wait)\n\n【정답 포인트】\nA(Slow Start Mode): slow_start.duration_seconds = 120초 설정 → 인스턴스 재부팅 후 Target Group 등록(상태 확인 통과) → ALB가 120초 동안 트래픽를 0%에서 100%까지 점진적 증가 → 로컬 캐시 채우는 2분(120초) 동안 충분한 Warm-up 시간 확보 → \"최소 코드 변경\" (ALB 속성 설정만)\n\n【오답 체크】\n(B) HealthCheckTimeoutSeconds는 상태 확인 응답 대기 시간이며, 신규 인스턴스 트래픽 점진적 할당과 무관\n(C) CloudWatch 알람 + weighted_random은 상태 재시작 및 분산 방식 변경이지, 신규 인스턴스 Warm-up과 무관\n(D) 라이프사이클 훅은 애플리케이션 코드 수정 필요(complete-lifecycle-action 호출) → 관리 오버헤드 증가 + 기존 인스턴스 Auto Scaling 그룹 통합의 복잡함\n\n【시험 포인트】\n▸ 신규 인스턴스 Warm-up (Startup Time) = Slow Start Mode(ALB 기본 제공)\n▸ 2분 Warm-up → slow_start.duration_seconds = 120초 이상 설정\n▸ 라이프사이클 훅은 Custom Logic 필요 시(애플리케이션이 캐시 채우기 완료 신호 보낼 때)\n▸ \"최소 오버헤드\" = Slow Start Mode 설정 (속성 변경만)",
  ...
  {
    "id": 54,
    "question": "한 회사가 Amazon EC2 인스턴스에서 실행되는 애플리케이션을 운영합니다. 애플리케이션은 Amazon Aurora PostgreSQL 데이터베이스에서 데이터를 저장하고 검색합니다. 개발자가 실수로 데이터베이스에서 테이블을 삭제하여 애플리케이션 오류가 발생합니다. 2시간 후 CloudOps 엔지니어는 데이터를 복구하고 애플리케이션을 다시 작동시켜야 합니다. 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "Aurora Backtrack 기능을 사용하여 데이터베이스를 2시간 전의 지정된 시간으로 되감습니다.",
      "B": "기존 데이터베이스에서 Point-in-Time Recovery를 수행하여 2시간 전의 지정된 시점으로 데이터베이스를 복원합니다.",
      "C": "Point-in-Time Recovery를 수행하고 새 데이터베이스를 생성하여 2시간 전의 지정된 시점으로 데이터베이스를 복원합니다. 애플리케이션을 새 데이터베이스 엔드포인트로 재구성합니다.",
  ...
  {
    "id": 55,
    "question": "한 회사가 AWS에서 여러 워크로드를 실행합니다. 회사는 특정 AWS Region에서 모니터링할 5개의 AWS Trusted Advisor 서비스 할당량 메트릭을 식별합니다. 회사는 리소스 사용이 서비스 할당량의 60%를 초과할 때마다 이메일 알림을 수신하려고 합니다. 이 요구사항을 충족할 솔루션은 무엇인가요?",
    "options": {
      "A": "각 Trusted Advisor 서비스 할당량 메트릭마다 1개씩 5개의 Amazon CloudWatch 알람을 생성합니다. 사용이 할당량의 60%를 초과할 때마다 이메일 알림을 위해 Amazon Simple Notification Service(Amazon SNS) 주제를 구성합니다.",
      "B": "각 Trusted Advisor 서비스 할당량 메트릭마다 1개씩 5개의 Amazon CloudWatch 알람을 생성합니다. 사용이 할당량의 60%를 초과할 때마다 이메일 알림을 위해 Amazon Simple Queue Service(Amazon SQS) 큐를 구성합니다.",
      "C": "AWS Health Dashboard를 사용하여 각 Trusted Advisor 서비스 할당량 메트릭을 모니터링합니다. 사용이 할당량의 60%를 초과할 때마다 이메일 알림을 위해 Amazon Simple Queue Service(Amazon SQS) 큐를 구성합니다.",
      "D": "AWS Health Dashboard를 사용하여 각 Trusted Advisor 서비스 할당량 메트릭을 모니터링합니다. 사용이 할당량의 60%를 초과할 때마다 이메일 알림을 위해 Amazon Simple Notification Service(Amazon SNS) 주제를 구성합니다."
    },
    "answer": "A",
  ...
  {
    "id": 56,
    "question": "CloudOps 엔지니어가 Amazon CloudWatch Synthetics의 구현을 문제 해결하고 있습니다. CloudWatch Synthetics 결과를 Amazon S3 버킷으로 전송해야 합니다. CloudOps 엔지니어가 인터넷 게이트웨이가 연결된 VPC에서 실행되는 기존 Canary의 구성을 복사했습니다. 그러나 CloudOps 엔지니어는 인터넷 액세스가 없는 Private VPC에서 Canary를 성공적으로 시작할 수 없습니다. CloudOps 엔지니어가 Private VPC에서 Canary를 성공적으로 실행하려면 무엇을 해야 하나요?",
    "options": {
      "A": "VPC에서 DNS 해석 옵션과 DNS 호스트명 옵션을 켜집니다. VPC에 synthetics:GetCanaryRuns 권한을 추가합니다. S3 버킷에서 CloudWatch Synthetics 역할에 IgnorePublicAcls 권한을 추가합니다.",
      "B": "VPC에서 DNS 해석 옵션과 DNS 호스트명 옵션을 끕니다. Amazon S3에 대한 Gateway VPC Endpoint를 생성합니다. CloudWatch Synthetics가 S3 Endpoint를 사용할 수 있도록 권한을 추가합니다.",
      "C": "VPC에서 DNS 해석 옵션과 DNS 호스트명 옵션을 끕니다. Canary에 Security Group을 추가하여 DNS 포트의 아웃바운드 트래픽을 허용합니다. CloudWatch Synthetics가 S3 버킷에 쓸 수 있도록 권한을 추가합니다.",
      "D": "VPC에서 DNS 해석 옵션과 DNS 호스트명 옵션을 켜집니다. CloudWatch를 위한 Interface VPC Endpoint를 생성합니다. Amazon S3를 위한 Gateway VPC Endpoint를 생성합니다. CloudWatch Synthetics가 두 Endpoint를 사용할 수 있도록 권한을 추가합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ CloudWatch Synthetics — Canary 스크립트로 웹 애플리케이션 가용성 모니터링\n▸ Private VPC(인터넷 액세스 없음) — NAT Gateway/IGW 없음, AWS 서비스 접근 = VPC Endpoint 필수\n▸ Gateway Endpoint — S3, DynamoDB 전용(프리티어)\n▸ Interface Endpoint — CloudWatch, CloudWatch Logs, Systems Manager 등(요금 발생)\n▸ DNS 호스트명 — VPC 내 EC2에 DNS 이름 할당하여 Endpoint 접근 가능\n\n【정답 포인트】\nD(Interface + Gateway Endpoint): ① DNS 해석(enable_dns_hostnames=true) + DNS 지원(enable_dns_support=true) 활성화 → ② CloudWatch Interface Endpoint(Canary 메트릭 발행) → ③ S3 Gateway Endpoint(결과 쓰기) → ④ 권한 추가(s3:PutObject, cloudwatch:PutMetricData) → Private VPC에서 AWS 서비스 접근 완성\n\n【오답 체크】\n(A) synthetics:GetCanaryRuns은 실제 권한이 아니며, VPC에 권한 추가 불가(IAM 역할에만 추가)\n(B) DNS 옵션 비활성화하면 Endpoint DNS 이름 해석 불가능 → 반대 방향의 오류\n(C) DNS 비활성화 + DNS 포트 아웃바운드는 모순이며, 근본 해결책 아님(Endpoint 없음)\n\n【시험 포인트】\n▸ Private VPC CloudWatch Synthetics = CloudWatch + S3 Endpoint 필수\n▸ CloudWatch는 Interface Endpoint(eni-xxxx.us-east-1.vpce.amazonaws.com)\n▸ S3는 Gateway Endpoint(s3.us-east-1.vpce.amazonaws.com)\n▸ DNS 호스트명 활성화 = Endpoint DNS 이름 해석 가능\n▸ Canary가 메트릭 발행 + 결과 저장 = 2개 Endpoint 모두 필요",
  ...
  {
    "id": 57,
    "question": "회사에서 Internet Gateway를 포함한 VPC에 AWS 인프라를 배포했습니다. VPC에는 public subnet과 private subnet이 있습니다. Amazon RDS for MySQL DB 인스턴스가 private subnet에 배포되었고, AWS Lambda 함수는 같은 private subnet을 사용하여 DB 인스턴스에 연결해 데이터를 조회합니다. 개발자가 Lambda 함수를 수정하여 Amazon SQS 큐에 메시지를 발행하는 기능을 추가했습니다. 이 변경 후 Lambda 함수가 SQS 큐에 메시지를 발행하려 할 때 타임아웃되었습니다. 이 문제를 해결할 수 있는 솔루션은 어느 것입니까? (2개 선택)",
    "options": {
      "A": "Lambda 함수가 VPC에 연결되지 않도록 재구성합니다.",
      "B": "RDS proxy를 배포하고, Lambda 함수를 프록시를 통해 DB 인스턴스에 연결하도록 구성합니다.",
      "C": "NAT gateway를 배포하고, private subnet의 route table을 모든 트래픽을 NAT gateway로 라우팅하도록 업데이트합니다.",
      "D": "VPC에서 Amazon SQS용 interface endpoint를 생성합니다.",
      "E": "VPC에서 Amazon SQS용 gateway endpoint를 생성합니다."
    },
    "answer": "CD",
    "explanation": "【핵심 용어】\n▸ NAT gateway — private subnet의 인스턴스가 인터넷 및 AWS 서비스로 아웃바운드 트래픽을 전송할 수 있게 하는 관리형 서비스\n▸ VPC Endpoint — VPC 내에서 AWS 서비스에 직접 접근할 수 있는 프라이빗 연결\n▸ VPC Endpoint 유형: Gateway(S3, DynamoDB), Interface(대부분의 AWS 서비스)\n\n【정답 포인트】\n▸ \"Private subnet\" → 인터넷 접근 불가 → SQS 호출 시 경로 문제\n▸ \"NAT gateway\" → 아웃바운드 경로 제공, 인터넷 기반 AWS 서비스 접근 가능 (옵션 C)\n▸ \"SQS endpoint\" → VPC 내 프라이빗 경로로 AWS 서비스 접근 (옵션 D가 Interface Endpoint)\n▸ 2개 선택 → C와 D 조합: NAT gateway 또는 Interface Endpoint 중 하나가 아닌, 두 가지 접근 방식 동시 제시\n\n【오답 체크】\n(A) Lambda를 VPC에서 제거 — DB 인스턴스 쿼리 불가능. RDS가 private subnet에 있으므로 VPC 연결 필수. 요구사항 충돌.\n(B) RDS proxy — DB 연결 풀 관리용. SQS 접근 문제와 무관. 네트워크 경로 해결 불가.\n(E) Gateway Endpoint — S3, DynamoDB에만 사용. SQS는 Interface Endpoint 필요.\n\n【시험 포인트】\n▸ Private 리소스의 아웃바운드 경로 문제 진단 필수\n▸ NAT gateway vs VPC Endpoint 선택: NAT(인터넷 경로), Interface Endpoint(AWS 서비스 직접)\n▸ Endpoint 유형 구분: Gateway(S3/DynamoDB만), Interface(SQS 포함 대부분)\n▸ 두 가지 해결책 모두 가능 — 선택은 아키텍처에 따라 결정",
  ...
  {
    "id": 58,
    "question": "회사의 보안 정책은 수신 SSH 트래픽을 정의된 주소 세트로 제한하도록 요구합니다. 회사는 AWS Config 규칙을 사용하여 Security Group이 제한되지 않은 수신 SSH 트래픽을 허용하는지 확인합니다. CloudOps 엔지니어가 비준수 리소스를 발견하고 Security Group을 수동으로 수정했습니다. CloudOps 엔지니어는 다른 비준수 리소스의 복구를 자동화하고 싶어합니다. 이 요구사항을 충족하는 운영상 가장 효율적인 솔루션은 무엇입니까?",
    "options": {
      "A": "AWS Config 규칙의 상태 메트릭에 대해 Amazon CloudWatch 알람을 생성합니다. Security Group에서 비준수 규칙을 제거할 수 있는 AWS Lambda 함수를 생성하고, 알람 작업을 Lambda 함수 호출로 구성합니다.",
      "B": "AWS Config 규칙에서 자동 복구 작업을 구성하고, AWS-DisableIncomingSSHOnPort22 복구 작업을 지정합니다.",
      "C": "AWS Config 구성 항목 변경 이벤트에 대한 Amazon EventBridge 규칙을 구성합니다. Security Group에서 비준수 규칙을 제거할 수 있는 AWS Lambda 함수를 생성하고, 규칙을 Lambda 함수 호출로 구성합니다.",
      "D": "Security Group의 인바운드 규칙을 분석하여 제한되지 않은 SSH 액세스를 확인할 수 있는 AWS Lambda 함수를 생성합니다. Security Group에서 비준수 규칙을 제거하도록 Lambda 함수를 구성하고, 매 시간마다 Lambda 함수를 호출하도록 Amazon EventBridge 규칙을 구성합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Config 자동 복구 — 비준수 리소스를 자동으로 수정하는 AWS Config의 기본 기능\n▸ AWS-DisableIncomingSSHOnPort22 — SSH 포트 22의 인바운드 규칙을 자동 제거하는 AWS 관리형 복구 작업\n▸ 운영상 효율성 — 최소 개발 및 관리 오버헤드로 자동화 달성\n\n【정답 포인트】\n▸ \"자동 복구\" → AWS Config의 내장 기능 활용 (옵션 B 핵심)\n▸ \"AWS-DisableIncomingSSHOnPort22\" → AWS 관리형 복구 작업, 즉시 사용 가능\n▸ \"가장 운영상 효율적\" → 최소 구현 (CloudWatch + Lambda 같은 커스텀 솔루션 불필요)\n▸ 자동 복구는 Config 규칙 위반 감지 시 즉시 실행, 수동 개입 제거\n\n【오답 체크】\n(A) CloudWatch 알람 + Lambda 조합 — 수동 구현 필요, Config 자동 복구보다 복잡. Security Group 수정 로직 직접 개발 필요.\n(C) EventBridge + Lambda — Config 변경 감지 후 처리하는 방식. 자동 복구 없이 이벤트 기반 커스텀 로직 필요. A보다 복잡.\n(D) 매시간 Lambda 실행 — 주기적 검사, 실시간 복구 아님. 비준수 상태가 1시간 지속 가능. 가장 비효율적.\n\n【시험 포인트】\n▸ AWS Config \"자동 복구\" 기능 이해 — AWS 관리형 복구 작업 목록 숙지\n▸ SSH 관련 표준 복구 작업: AWS-DisableIncomingSSHOnPort22 (포트 22)\n▸ 효율성 비교: 관리형 복구 > 커스텀 이벤트 기반 > 폴링 기반\n▸ \"운영상 효율적\"이라는 표현 → 최소 코드, 관리형 서비스 우선",
  ...
  {
    "id": 59,
    "question": "CloudOps 엔지니어가 Amazon CloudFront 웹 배포, Application Load Balancer (ALB), Amazon RDS, Amazon EC2를 VPC에서 사용하는 웹 애플리케이션을 유지관리하고 있습니다. 모든 서비스에 로깅이 활성화되어 있습니다. CloudOps 엔지니어는 웹 애플리케이션의 HTTP Layer 7 상태 코드를 조사해야 합니다. 어느 로그 소스에 상태 코드가 포함되어 있습니까? (2개 선택)",
    "options": {
      "A": "VPC Flow Logs",
      "B": "AWS CloudTrail 로그",
      "C": "ALB 액세스 로그",
  ...
  {
    "id": 60,
    "question": "프로세스 오류로 인해 CPU를 100% 사용하는 문제가 있습니다. CloudOps 엔지니어가 문제가 2분 이상 발생할 때 Amazon EC2 인스턴스를 자동으로 재시작하려고 합니다. 이를 어떻게 수행할 수 있습니까?",
    "options": {
      "A": "기본 모니터링을 사용하여 EC2 인스턴스에 대해 Amazon CloudWatch 알람을 생성하고, 인스턴스 재시작 작업을 추가합니다.",
      "B": "상세 모니터링을 사용하여 EC2 인스턴스에 대해 Amazon CloudWatch 알람을 생성하고, 인스턴스 재시작 작업을 추가합니다.",
      "C": "EC2 인스턴스를 재시작할 AWS Lambda 함수를 생성하고, 매 2분마다 예약 기반으로 호출되도록 구성합니다.",
      "D": "EC2 인스턴스를 재시작할 AWS Lambda 함수를 생성하고, EC2 상태 검사로 호출되도록 구성합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ 기본 모니터링 — 5분 간격 메트릭 (CPU, Network 등 기본 메트릭만)\n▸ 상세 모니터링 — 1분 간격 메트릭 (기본과 동일하나 \"2분\" 조건 평가 가능)\n▸ CloudWatch 알람 — 메트릭 조건 기반 자동 조치 (재시작, SNS 등)\n▸ \"2분 이상\" → 데이터 포인트 2개 이상 필요, 상세 모니터링 필수\n\n【정답 포인트】\n▸ \"2분 이상 100%\" → 알람이 조건을 평가하려면 최소 2개 데이터 포인트 필요\n▸ \"상세 모니터링\" → 1분마다 메트릭 수집 (기본은 5분), 2분 조건 충족 가능\n▸ \"CloudWatch 알람\" + \"재시작 작업\" → 자동 복구의 표준 방식 (옵션 B)\n▸ CPU 메트릭은 기본/상세 모두 제공, 상세가 빈도 높음\n\n【오답 체크】\n(A) 기본 모니터링 — 5분 간격 데이터. \"2분 조건\" 평가 불가. 1개 데이터 포인트만으로 결정 → 오양성 높음.\n(C) Lambda 2분마다 폴링 — Scheduled rule로 실행. 실제 CPU 상태 측정 아님. 상태 검사 없이 주기적 재시작 → 운영 효율성 낮음.\n(D) EC2 상태 검사 — 인스턴스 하드웨어 상태(실패/성공)만 확인. CPU 사용률 모니터링 아님. 프로세스 CPU 증가 감지 불가.\n\n【시험 포인트】\n▸ 모니터링 빈도 구분: 기본(5분), 상세(1분). \"2분\" 요구 → 상세 선택 필수\n▸ CloudWatch 알람 활용 — 메트릭 기반 자동 조치 (가장 효율적)\n▸ EC2 상태 검사는 인스턴스 가용성용, CPU 용도 아님\n▸ \"자동 재시작\" → CloudWatch 알람의 표준 작업 (Lambda 폴링보다 우수)",
  ...
  {
    "id": 61,
    "question": "CloudOps 엔지니어가 여러 IAM 사용자에게 IAM 정책을 연결하여 AWS 서비스에 대한 액세스를 제공하고 싶어합니다. CloudOps 엔지니어는 또한 정책을 변경하고 새 버전을 생성할 수 있어야 합니다. 이 요구사항을 충족하는 작업 조합은 어느 것입니까? (2개 선택)",
    "options": {
      "A": "사용자를 IAM service-linked role에 추가하고, 역할에 정책을 연결합니다.",
      "B": "사용자를 IAM 사용자 그룹에 추가하고, 그룹에 정책을 연결합니다.",
      "C": "AWS 관리형 정책을 생성합니다.",
      "D": "Customer managed 정책을 생성합니다.",
      "E": "Inline 정책을 생성합니다."
    },
    "answer": "BD",
    "explanation": "【핵심 용어】\n▸ 사용자 그룹 — 여러 사용자에게 정책을 일괄 관리하는 메커니즘\n▸ Customer managed 정책 — 사용자가 생성/관리하는 정책, 버전 관리 가능 (최대 5개 버전)\n▸ AWS managed 정책 — AWS에서 제공, 사용자가 버전 변경 불가 (AWS가 업데이트)\n▸ Inline 정책 — 1:1 정책, 버전 관리 불가, 삭제 시 정책도 함께 삭제\n\n【정답 포인트】\n▸ \"여러 IAM 사용자\" → 개별 정책 연결 비효율 → 그룹 사용 (옵션 B)\n▸ \"정책 변경\" + \"새 버전 생성\" → 버전 관리 기능 필요 → Customer managed 정책 (옵션 D)\n▸ 2개 선택 조합: B(그룹) + D(정책 타입) 필수\n▸ Customer managed 정책만 버전 관리 가능 (생성, 설정, 삭제)\n\n【오답 체크】\n(A) Service-linked role — 특정 AWS 서비스 자동 생성 역할. 사용자 추가 개념 없음. 수동 관리 불가.\n(C) AWS managed 정책 — AWS에서 관리, 버전 변경 권한 없음. 새로운 버전 생성 불가(읽기 전용).\n(E) Inline 정책 — 각 사용자/그룹에 직접 정책 포함. 버전 관리 메커니즘 없음. 수정 시 새 버전 생성 아님(덮어쓰기).\n\n【시험 포인트】\n▸ IAM 정책 유형 숙지: AWS managed(버전 없음), Customer managed(버전 관리), Inline(버전 없음)\n▸ \"버전 관리\" 요구 → 항상 \"Customer managed 정책\" 선택\n▸ \"여러 사용자\" + \"중앙 관리\" → 그룹 사용 (개별 정책 추적 용이)\n▸ 2개 선택 문제 → 정책 적용 방식(그룹) + 정책 타입(Customer managed) 구분 필수",
  ...
  {
    "id": 62,
    "question": "회사에서 Amazon EC2 Auto Scaling 그룹의 Amazon EC2 인스턴스에서 애플리케이션을 실행하고 있습니다. 긴 부팅 스크립트로 인해 스케일 아웃 작업이 완료되는 데 오래 걸립니다. CloudOps 엔지니어는 Auto Scaling 그룹을 과도하게 프로비저닝하지 않으면서 스케일 아웃 작업에 필요한 시간을 줄이는 솔루션을 구현해야 합니다. 이 요구사항을 충족하는 솔루션은 어느 것입니까?",
    "options": {
      "A": "Launch configuration을 더 큰 인스턴스 크기로 변경합니다.",
      "B": "Auto Scaling 그룹의 최소 인스턴스 수를 증가합니다.",
      "C": "Auto Scaling 그룹에 Predictive scaling policy를 추가합니다.",
      "D": "Auto Scaling 그룹에 Warm pool을 추가합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Warm pool — 미리 시작된 인스턴스를 Standby 상태로 유지. 스케일 아웃 시 즉시 활성화 (부팅 시간 제거)\n▸ 부팅 스크립트 — 인스턴스 초기화 시간 소비 (Application Load, Configuration 등)\n▸ 스케일 아웃 지연 — 부팅 완료까지 트래픽 처리 불가\n▸ 과도 프로비저닝 — 평상시 불필요한 높은 최소 인스턴스 수\n\n【정답 포인트】\n▸ \"스케일 아웃 시간\" + \"과도 프로비저닝 금지\" → Warm pool 최적\n▸ Warm pool — 미리 준비된 인스턴스 (부팅 완료), 필요 시 활성화 (Standby → InService)\n▸ 부팅 스크립트 시간 문제 → Warm pool이 완전히 초기화된 상태로 해결\n▸ 평상시 낮은 비용 + 스케일 아웃 빠름 = 최고 효율\n\n【오답 체크】\n(A) 더 큰 인스턴스 — 부팅 시간 감소 아님. 비용 증가, 크기 자체는 스케일 속도와 무관.\n(B) 최소 인스턴스 수 증가 — 평상시 항상 실행 → 과도 프로비저닝 (요구사항 위배). 스케일 아웃 지연도 해결 안 함.\n(C) Predictive scaling — 미래 트래픽 예측 후 미리 스케일 아웃. 현재 상황에서는 여전히 부팅 시간 필요. 예측 오류 시 비효율.\n\n【시험 포인트】\n▸ Warm pool은 EC2 Auto Scaling의 부팅 시간 문제 해결 최적화\n▸ 스케일 아웃 성능 개선 방법: 1)Warm pool(우선), 2)더 간단한 부팅 스크립트, 3)AMI 최적화\n▸ 비용 효율성 + 성능 → Warm pool 선택. 최소 인스턴스 증가는 평상시 낭비\n▸ \"과도 프로비저닝 금지\" 조건이 핵심 — 이를 피하면서 속도 개선하는 유일한 방법은 Warm pool",
  ...
  {
    "id": 63,
    "question": "CloudOps 엔지니어가 public subnet과 private subnet을 포함한 새로운 VPC를 생성했습니다. CloudOps 엔지니어가 private subnet에 11개의 Amazon EC2 인스턴스를 성공적으로 시작했습니다. CloudOps 엔지니어가 같은 subnet에 EC2 인스턴스 1개를 더 시작하려고 시도했으나, 사용 가능한 IP 주소가 충분하지 않다는 오류 메시지가 나타났습니다. CloudOps 엔지니어가 더 많은 EC2 인스턴스를 배포하기 위해 해야 할 일은 무엇입니까?",
    "options": {
      "A": "Private subnet을 편집하여 CIDR block을 /27로 변경합니다.",
      "B": "Private subnet을 편집하여 두 번째 Availability Zone으로 확장합니다.",
      "C": "Private subnet에 추가 Elastic IP 주소를 할당합니다.",
      "D": "필요한 EC2 인스턴스를 보유할 새로운 private subnet을 생성합니다."
    },
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ VPC CIDR — 기본값 /16 (65,536개 IP), Subnet은 /24 (256개 IP) 등으로 분할\n▸ Subnet IP 고갈 — 할당된 CIDR 범위 내 주소 부족. Subnet 확장 불가 (생성 후 고정)\n▸ 새 Subnet — 다른 CIDR 범위의 독립적 Subnet, AZ별로 생성 가능\n▸ Elastic IP — 퍼블릭 IP 주소. Private 인스턴스에는 Private IP 필요 (EIP 도움 안 됨)\n\n【정답 포인트】\n▸ \"IP 주소 충분하지 않음\" → 현재 Subnet의 CIDR 범위 초과\n▸ \"Subnet CIDR 변경\" 불가능 — Subnet 생성 후 수정/확장 불가 (AWS 정책)\n▸ \"새 private Subnet\" → 다른 CIDR 블록으로 생성. 추가 IP 주소 공간 확보 (옵션 D)\n▸ 새 Subnet은 같은 AZ 또는 다른 AZ에 생성 가능\n\n【오답 체크】\n(A) CIDR block 변경(/27) — 더 작은 범위로 축소, IP 수 감소. 확장 불가. Subnet 생성 후 수정 불가능.\n(B) 두 번째 AZ로 확장 — 단일 Subnet은 1개 AZ에만 존재. 새 AZ 추가 = 새 Subnet 생성 필요 (B 자체 불가).\n(C) Elastic IP 할당 — 퍼블릭 IP 주소. Private 인스턴스의 Private IP 부족과 무관. Private IP 부족 해결 안 함.\n\n【시험 포인트】\n▸ Subnet CIDR \"변경 불가\" 규칙 필수 인지 — 생성 후 고정\n▸ IP 고갈 해결 유일한 방법 → 새로운 CIDR 범위의 Subnet 생성\n▸ VPC 설계 시 적절한 CIDR 블록 크기 계획 중요 (사후 수정 불가)\n▸ Elastic IP ≠ Private IP. Private IP 부족 문제에는 효과 없음",
  ...
  {
    "id": 64,
    "question": "CloudOps 엔지니어가 커스텀 애플리케이션 이벤트 세트에 대한 이벤트 인프라를 구축해야 합니다. 이벤트를 처리하기 위해 AWS Lambda 함수로 전송되어야 합니다. CloudOps 엔지니어는 이벤트를 기록하고 이벤트 유형 또는 이벤트 시간으로 나중에 재생할 수 있어야 합니다. 이 요구사항을 충족하는 솔루션은 어느 것입니까?",
    "options": {
      "A": "Amazon EventBridge custom event bus를 생성하고, custom event bus에 Archive를 생성합니다. 규칙을 생성하여 커스텀 이벤트를 Lambda 함수로 전송합니다.",
      "B": "Amazon EventBridge default event bus에 Archive를 생성합니다. 패턴 매칭을 사용하여 커스텀 이벤트를 기록합니다. 규칙을 생성하여 커스텀 이벤트를 Lambda 함수로 전송합니다.",
      "C": "Amazon EventBridge default event bus에 Archive를 생성합니다. EventBridge Pipe를 생성하여 커스텀 이벤트를 수집하고 Archive에 저장합니다. 규칙을 생성하여 커스텀 이벤트를 Lambda 함수로 전송합니다.",
      "D": "Amazon CloudWatch Logs에서 로그 그룹을 생성합니다. Amazon EventBridge 규칙을 생성하여 커스텀 이벤트를 Lambda 함수와 로그 그룹으로 전송합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ EventBridge Archive — 이벤트를 JSON 형식으로 저장. 이벤트 유형/시간으로 재생 가능\n▸ Custom event bus — 사용자 정의 이벤트 소스 전용. Default event bus(AWS 서비스 이벤트)와 분리\n▸ EventBridge Pipe — 이벤트 수집/변환/라우팅. Archive 저장과는 다름\n▸ 패턴 매칭 — 이벤트 필터링, Archive 생성과 무관\n\n【정답 포인트】\n▸ \"커스텀 애플리케이션 이벤트\" → Custom event bus 필수 (Default bus는 AWS 관리 이벤트)\n▸ \"이벤트 기록\" + \"유형/시간별 재생\" → Archive 기능 (옵션 A)\n▸ \"Lambda 함수 전송\" + \"아카이빙\" 동시 지원 → Custom event bus + Archive + Rule (옵션 A 완전 구현)\n\n【오답 체크】\n(B) Default event bus — AWS 서비스(EC2, RDS, S3) 이벤트용. 커스텀 애플리케이션 이벤트 구독 불가. 패턴 매칭은 필터링일 뿐 Archive 기능 아님.\n(C) EventBridge Pipe — 이벤트 변환/라우팅용. Archive 저장 기능 없음 (Pipe 목적과 다름). Default bus는 커스텀 이벤트 부적절.\n(D) CloudWatch Logs — 텍스트 로그 저장. 구조화된 이벤트 재생 불가. \"이벤트 유형별 재생\" 기능 없음. JSON Archive와 다름.\n\n【시험 포인트】\n▸ EventBridge event bus 선택: Default(AWS 서비스 이벤트), Custom(사용자 정의 이벤트)\n▸ Archive = 이벤트 영구 저장 + 유형/시간별 재생 검색 기능\n▸ EventBridge Pipe ≠ Archive. Pipe는 변환/라우팅, Archive는 저장소\n▸ 커스텀 애플리케이션 이벤트 → 항상 \"Custom event bus\" 필요",
  ...
  {
    "id": 65,
    "question": "CloudOps 엔지니어가 us-west-2의 회사 기존 인프라에 대한 AWS CloudFormation 템플릿을 가지고 있습니다. CloudOps 엔지니어가 템플릿을 사용하여 eu-west-1에서 새로운 Stack을 시작하려고 시도했으나, Stack이 부분적으로만 배포되고 오류 메시지가 나타난 후 롤백되었습니다. 이 템플릿이 배포에 실패하는 이유는 무엇입니까? (2개 선택)",
    "options": {
      "A": "템플릿이 eu-west-1에서 사용할 수 없는 IAM 사용자를 참조했습니다.",
      "B": "템플릿이 eu-west-1에서 사용할 수 없는 Amazon Machine Image (AMI)를 참조했습니다.",
      "C": "템플릿이 리소스를 배포하기 위한 적절한 권한 수준을 가지지 않았습니다.",
      "D": "템플릿이 eu-west-1에 존재하지 않는 서비스를 요청했습니다.",
      "E": "CloudFormation 템플릿은 기존 서비스를 업데이트하는 데만 사용될 수 있습니다."
    },
    "answer": "BD",
    "explanation": "【핵심 용어】\n▸ Region 간 리소스 호환성 — AMI, 서비스 가용성은 Region별로 다름\n▸ AMI ID — Region별 고유 ID. 한 Region의 AMI ID를 다른 Region에 사용 불가\n▸ 서비스 가용성 — 일부 AWS 서비스는 특정 Region에만 지원 (예: Outposts, 특정 ML 서비스)\n▸ 권한 문제 — CloudFormation 역할 권한 부족 (Stack 생성 실패, Region과 무관)\n\n【정답 포인트】\n▸ \"us-west-2에서 성공\" + \"eu-west-1 실패\" → Region별 차이가 원인\n▸ \"AMI 참조\" → us-west-2의 AMI ID ≠ eu-west-1 AMI ID (옵션 B)\n▸ \"서비스 부재\" → 일부 서비스는 특정 Region만 지원 (옵션 D)\n▸ \"IAM 사용자\" → 글로벌 리소스, Region 간 동일 (옵션 A 오답)\n▸ \"권한 부족\" → 템플릿 오류 아님, CloudFormation 실행 권한 이슈 (옵션 C 이유 아님)\n\n【오답 체크】\n(A) IAM 사용자는 글로벌 리소스. eu-west-1에 없다는 개념 자체 불가능. IAM은 모든 Region에서 접근 가능.\n(C) 권한 부족 — CloudFormation 역할의 IAM 권한 문제. 템플릿 자체 오류 아님. \"Stack 배포 실패\"의 원인은 될 수 있지만 \"템플릿이\" 문제라는 표현과 맞지 않음.\n(E) CloudFormation은 새 Stack 생성(Create), 업데이트(Update), 삭제(Delete) 모두 가능. \"기존 서비스 업데이트만\" 오답.\n\n【시험 포인트】\n▸ Region별 리소스 차이 인식 — AMI, 서비스 가용성 확인 필수\n▸ 템플릿 마이그레이션 — Region 간 리소스 ID 수정 필요 (AMI, 서비스 체크)\n▸ 글로벌 vs Regional 리소스 구분: IAM(글로벌), EC2/S3/RDS(Regional)\n▸ 권한 문제 vs 템플릿 오류 구분 — 권한은 실행 환경, 템플릿은 설정 자체",
  ...
  {
    "id": 66,
    "question": "회사의 웹 사이트가 Amazon EC2 Linux 인스턴스에서 실행됩니다. 웹 사이트는 Amazon S3 버킷에서 PDF 파일을 제공해야 합니다. 계정 수준에서 S3 버킷에 대한 모든 Public 액세스가 차단되었습니다. 회사는 웹 사이트 사용자가 PDF 파일을 다운로드할 수 있도록 허용해야 합니다. 이 요구사항을 가장 적은 관리 노력으로 충족하는 솔루션은 어느 것입니까?",
    "options": {
      "A": "s3:list* 및 s3:get* 권한을 허용하는 정책이 있는 IAM 역할을 생성합니다. 역할을 EC2 인스턴스에 할당합니다. 회사 직원을 지정하여 요청된 PDF 파일을 EC2 인스턴스로 다운로드하고 파일을 웹 사이트 사용자에게 전달합니다. 로컬 파일을 주기적으로 삭제하기 위해 AWS Lambda 함수를 생성합니다.",
      "B": "S3 버킷을 가리키는 OAC(Origin Access Control)를 사용하는 Amazon CloudFront 배포를 생성합니다. CloudFront 배포로부터의 연결을 허용하도록 S3 버킷에 Bucket Policy를 적용합니다. 사용자가 PDF 파일을 요청할 때 배포 URL과 객체 경로를 포함하는 다운로드 URL을 제공하도록 회사 직원을 지정합니다.",
      "C": "S3 버킷 권한을 변경하여 소스 S3 버킷에서 Public 액세스를 허용합니다. 사용자가 PDF 파일을 요청할 때 PDF 파일 URL을 제공하도록 회사 직원을 지정합니다.",
      "D": "Public subnet에 IAM Instance Profile을 가진 EC2 인스턴스를 배포합니다. EC2 인스턴스의 서명된 URL을 사용하여 웹 사이트 사용자에게 S3 버킷에 대한 임시 액세스를 제공합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ CloudFront 배포 — CDN을 통한 콘텐츠 전달. 엣지 로케이션에서 캐싱\n▸ OAC(Origin Access Control) — CloudFront만 S3 버킷 액세스 가능. 직접 액세스 차단\n▸ Bucket Policy — 특정 서비스/IAM에게만 S3 액세스 권한 부여\n▸ \"최소 관리 노력\" → 자동화, 수동 작업 최소화\n\n【정답 포인트】\n▸ \"Public 액세스 차단\" + \"사용자가 PDF 다운로드\" → S3 직접 접근 불가 → 프록시 필요\n▸ \"CloudFront\" + \"OAC\" → CloudFront만 S3 접근, 사용자는 CloudFront 통한 접근 (보안 + 캐싱)\n▸ \"최소 관리 노력\" → 자동화된 CDN 솔루션 (CloudFront)\n▸ 회사 직원 개입 최소 — URL 제공만 하면 자동 처리\n\n【오답 체크】\n(A) EC2 + IAM 역할 — 수동 다운로드/전달 필요. 확장성 낮음. 회사 직원 부담 높음. Lambda로 로컬 파일 삭제 = 추가 복잡성.\n(C) Public 액세스 허용 — \"계정 수준 차단\" 정책 위반. 보안 위험. 누구나 S3 직접 접근 가능.\n(D) 서명된 URL — 임시 액세스 제공. 기술적으로는 가능하지만, EC2가 서명 로직 처리 필요 (운영 복잡성). OAC 방식이 더 단순하고 자동화됨.\n\n【시험 포인트】\n▸ \"최소 관리 노력\" → CloudFront + OAC 조합 우선 검토\n▸ OAC의 이점 — CloudFront만 S3 접근 가능, 사용자는 CDN 통함 (보안 + 성능)\n▸ 서명된 URL vs OAC: 서명 URL(동적 권한 필요), OAC(고정 정책, 설정 후 자동)\n▸ 수동 작업 제거가 핵심 — EC2 직원 개입 최소화",
  ...
  {
    "id": 67,
    "question": "금융 서비스 회사가 고객 이미지를 us-east-1 Region의 Amazon S3 버킷에 저장합니다. 규정 준수를 위해 회사는 모든 기존 객체가 두 번째 AWS Region의 S3 버킷으로 복제되도록 보장해야 합니다. 객체 복제가 실패하면 회사는 해당 객체에 대해 복제를 재시도할 수 있어야 합니다. 이 요구사항을 충족하는 솔루션은 어느 것입니까?",
    "options": {
      "A": "Amazon S3 Cross-Region Replication (CRR)를 구성합니다. Amazon S3 live replication을 사용하여 기존 객체를 복제합니다.",
      "B": "Amazon S3 Cross-Region Replication (CRR)를 구성합니다. S3 Batch Replication을 사용하여 기존 객체를 복제합니다.",
      "C": "Amazon S3 Cross-Region Replication (CRR)를 구성합니다. S3 Replication Time Control (RTC)을 사용하여 기존 객체를 복제합니다.",
      "D": "S3 Lifecycle 규칙을 사용하여 객체를 두 번째 Region의 대상 버킷으로 이동합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ CRR(Cross-Region Replication) — 새 객체 자동 복제. \"기존\" 객체는 복제 안 함 (CRR만으로는 불가)\n▸ S3 Batch Replication — 기존 객체 일괄 복제. 실패 시 재시도 가능\n▸ S3 라이브 복제 — 실시간 복제, 기존 객체 미포함\n▸ S3 RTC(Replication Time Control) — 복제 시간 보장 (99.99% 15분 내 복제). 기존 객체용 아님\n\n【정답 포인트】\n▸ \"모든 기존 객체\" + \"두 번째 Region\" → 과거 객체 복제 필요\n▸ \"복제 실패\" + \"재시도\" → Batch Replication의 핵심 기능 (실패 처리)\n▸ CRR 기본 설정 + S3 Batch Replication → 기존 객체 일괄 처리\n▸ \"규정 준수\" → 모든 데이터 복제 완료 필수\n\n【오답 체크】\n(A) S3 라이브 복제 — 실시간 새 객체만 복제. \"기존\" 객체 미포함. 규정 준수 불가.\n(C) S3 RTC — 복제 시간 보장 서비스. 새 객체에만 적용. \"기존 객체\" 복제 기능 아님. 복제 속도 보장일 뿐.\n(D) S3 Lifecycle 규칙 — 객체 이동(같은 S3 내 스토리지 클래스). 다른 Region 복제 불가. 지역 간 복제는 CRR/Batch Replication만 가능.\n\n【시험 포인트】\n▸ CRR 한계 인식 — 새 객체만 복제. 기존 객체 = S3 Batch Replication 필수\n▸ S3 Batch Replication — 실패 재시도, 진행 상황 추적, 보고서 생성 지원\n▸ \"기존\" vs \"신규\" 구분이 핵심 — 날짜별 복제 정책 필수\n▸ Lifecycle ≠ CRR. Lifecycle은 스토리지 클래스 전환, 복제 아님",
  ...
  {
    "id": 68,
    "question": "회사에서 Network Load Balancer (NLB) 뒤에 메모리 최적화 Amazon EC2 인스턴스를 사용하여 애플리케이션을 실행합니다. 회사는 AWS 제공 Red Hat Enterprise Linux (RHEL) Amazon Machine Image (AMI)에서 EC2 인스턴스를 시작했습니다. CloudOps 엔지니어는 5분 간격으로 RAM 사용률을 모니터링해야 합니다. CloudOps 엔지니어는 들어오는 로드에 따라 EC2 인스턴스가 적절하게 scale in 및 scale out되도록 해야 합니다. 이 요구사항을 충족하는 솔루션은 어느 것입니까?",
    "options": {
      "A": "EC2 인스턴스에 상세 모니터링을 구성합니다. EC2 인스턴스에 Amazon CloudWatch 에이전트를 구성합니다. EC2 Auto Scaling 그룹과 mem_active 메트릭을 기반으로 하는 Auto Scaling 정책을 생성합니다.",
      "B": "EC2 인스턴스에 상세 모니터링을 구성합니다. 상세 모니터링 기능이 제공하는 mem_used_percent 메트릭을 사용합니다. CloudWatch 에이전트가 데이터를 업로드할 수 있도록 하는 IAM 역할을 생성합니다. EC2 Auto Scaling 그룹과 mem_used_percent 메트릭을 기반으로 하는 Auto Scaling 정책을 생성합니다.",
      "C": "EC2 인스턴스에 기본 모니터링을 구성합니다. EC2 인스턴스에 Amazon CloudWatch 에이전트를 구성합니다. CloudWatch 에이전트가 데이터를 업로드할 수 있도록 하는 IAM 역할을 생성합니다. EC2 Auto Scaling 그룹과 mem_used_percent 메트릭을 기반으로 하는 Auto Scaling 정책을 생성합니다.",
      "D": "EC2 인스턴스에 기본 모니터링을 구성합니다. 모니터링을 위해 표준 mem_used_percent 메트릭을 사용합니다. EC2 Auto Scaling 그룹과 mem_used_percent 메트릭을 기반으로 하는 Auto Scaling 정책을 생성합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ 기본 모니터링 — AWS 제공 메트릭 (CPU, Network). RAM 메트릭 없음\n▸ 상세 모니터링 — AWS 제공 메트릭 (기본과 동일), 1분 간격\n▸ CloudWatch 에이전트 — EC2 인스턴스에서 커스텀 메트릭(RAM, 디스크 등) 수집 후 CloudWatch 업로드\n▸ mem_used_percent — 커스텀 메트릭. CloudWatch 에이전트만 제공 (기본/상세 모니터링 아님)\n\n【정답 포인트】\n▸ \"RAM 사용률\" → AWS 기본 메트릭 부재 → CloudWatch 에이전트 필수\n▸ \"mem_used_percent\" → 에이전트가 수집하는 커스텀 메트릭\n▸ \"5분 간격\" → 기본 모니터링이 5분 간격 제공 (상세 = 1분)\n▸ \"IAM 역할\" → 에이전트가 CloudWatch에 데이터 업로드하려면 권한 필요\n▸ 조합: 기본 모니터링 + CloudWatch 에이전트 + IAM 역할 + mem_used_percent (옵션 C)\n\n【오답 체크】\n(A) mem_active 메트릭 — 일반적이지 않음. mem_used_percent (표준 커스텀 메트릭) 아님. CloudWatch 에이전트 설정해도 에이전트 업로드 권한(IAM) 없음.\n(B) 상세 모니터링 + mem_used_percent — 상세 모니터링은 \"상세 메트릭\"을 제공하지만 mem_used_percent 없음. \"상세 모니터링 기능이 제공\"은 오답. 에이전트 필요.\n(D) 기본 모니터링만 — mem_used_percent 메트릭 제공 불가 (에이전트 없음). \"표준 mem_used_percent\"는 존재하지 않음. 에이전트로만 가능.\n\n【시험 포인트】\n▸ 기본/상세 모니터링의 한계 — OS 내부 메트릭(RAM, 디스크) 미지원\n▸ 커스텀 메트릭 = CloudWatch 에이전트 필수\n▸ 에이전트 업로드 권한 = IAM 역할 필수 (\"cloudwatch:PutMetricData\" 허용)\n▸ mem_used_percent는 \"커스텀\" 메트릭, 기본/상세와 구분",
  ...
  {
    "id": 69,
    "question": "다국적 회사가 AWS Organizations의 조직을 사용하여 여러 AWS Region의 200개 이상의 멤버 계정을 관리합니다. 회사는 모든 AWS 리소스가 특정 보안 요구사항을 충족하도록 보장해야 합니다. 회사는 ap-southeast-2 Region에서 EC2 인스턴스를 배포하면 안 됩니다. 회사는 모든 멤버 계정에서 root 사용자 작업을 완전히 차단해야 합니다. 회사는 관리자를 포함한 누구도 AWS CloudTrail 로그를 삭제하지 못하도록 방지해야 합니다. 회사는 모든 기존 및 향후 계정에 자동으로 적용할 수 있는 중앙 집중식 관리 솔루션이 필요합니다. 이 요구사항을 충족하는 솔루션은 어느 것입니까?",
    "options": {
      "A": "각 계정에서 복구 작업이 있는 AWS Config 규칙을 생성합니다. Account root 사용자를 위한 IAM 권한 경계를 구현합니다.",
      "B": "조직 전체에서 AWS Security Hub를 활성화합니다. 보안 요구사항을 강제하는 커스텀 보안 표준을 생성합니다. AWS CloudFormation StackSets를 사용하여 표준을 조직의 모든 계정에 배포합니다. Security Hub 자동 복구 작업을 설정합니다.",
      "C": "계정 거버넌스를 위해 AWS Control Tower를 사용합니다. Region deny 제어를 구성합니다. SCP(Service Control Policy)를 사용하여 root 사용자 액세스를 제한합니다.",
      "D": "AWS Firewall Manager를 보안 정책으로 구성하여 보안 요구사항을 충족합니다. 보안 정책 위반을 감지하기 위해 조직 전체 적합성 팩이 있는 AWS Config Aggregator를 사용합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ AWS Control Tower — 조직의 계정 거버넌스, 자동 규정 준수, Guardrails 적용\n▸ Guardrails — Region 제한, root MFA, CloudTrail 필수 등 기본 제어\n▸ SCP(Service Control Policy) — 조직 차원의 권한 제한. 모든 계정에 강제\n▸ Region deny 제어 — 특정 Region 접근 차단 (Control Tower)\n\n【정답 포인트】\n▸ \"중앙 집중식\" + \"모든 기존/향후 계정\" → AWS Control Tower 필수\n▸ \"ap-southeast-2 배포 금지\" → Control Tower \"Region deny\" (Region Deny Guardrails)\n▸ \"root 사용자 차단\" → SCPs로 root 액션 명시적 거부\n▸ \"CloudTrail 로그 삭제 금지\" → SCPs로 \"cloudtrail:DeleteTrail\" 거부 (모든 사용자 포함 관리자)\n▸ 3가지 요구사항을 1개 솔루션(Control Tower + SCP)으로 처리\n\n【오답 체크】\n(A) AWS Config + 권한 경계 — 각 계정별 구성 필요. \"중앙 집중식\" 아님. root 사용자 차단 불완전 (권한 경계는 IAM 사용자용). \"기존/향후 계정 자동 적용\" 불가.\n(B) Security Hub + StackSets — 컴플라이언스 감지/보고용. \"차단\" 기능 약함. Region 거부, root 차단, CloudTrail 보호는 주요 기능 아님. CloudFormation 템플릿 유지 필요.\n(D) Firewall Manager + Config — 네트워크 보안(WAF, Shield) 관리용. Region 제한, root 권한 제어 기능 제한적. \"중앙 집중식 강제\" 수준이 Control Tower보다 낮음.\n\n【시험 포인트】\n▸ Control Tower는 \"조직 전체 거버넌스\" 최우선 선택 (200+ 계정)\n▸ SCP의 힘 — root 포함 \"모든 주체\"에 적용. 권한 경계보다 강력\n▸ Region deny는 Control Tower Guardrails 기능\n▸ 3가지 독립적 요구사항 → 1개 통합 솔루션 필요 (Control Tower)\n▸ SCP는 \"거부\" 기반 (Blacklist), IAM 권한 경계는 \"허용\" 기반 (Whitelist)",
  ...
  {
    "id": 70,
    "question": "회사에서 프로덕션 및 비프로덕션 워크로드를 실행하기 위해 수백 개의 Amazon EC2 On-Demand Instance 및 Spot Instance를 사용합니다. 회사는 EC2 인스턴스에 AWS Systems Manager Agent (SSM Agent)를 설치하고 구성합니다. 최근 인스턴스 패치 작업 중에 인스턴스가 바쁘거나 다운되어 있어서 일부 인스턴스가 패치되지 않았습니다. 회사는 모든 인스턴스의 현재 패치 버전을 나열하는 보고서를 생성해야 합니다. 이 요구사항을 가장 운영상 효율적으로 충족하는 솔루션은 어느 것입니까?",
    "options": {
      ...
      "C": "AWS Config를 사용하여 SSM Agent의 출력을 사용하는 EC2 인스턴스 구성 변경을 추적합니다. 패치 버전을 확인하는 커스텀 규칙을 생성합니다. 패치되지 않은 모든 인스턴스의 보고서를 생성합니다.",
      "D": "AWS Config를 사용하여 SSM Agent의 출력을 사용하는 EC2 인스턴스의 패치 상태를 모니터링합니다. 패치가 설치되었는지 확인하는 구성 준수 규칙을 생성합니다. 모든 인스턴스의 보고서를 생성합니다."
    },
    "answer": "A",
  ...
  {
    "id": 71,
    "question": "CloudOps 엔지니어가 AWS Compute Optimizer를 사용하여 Amazon EC2 인스턴스 집합에 대한 권장사항을 생성하고 있습니다. 일부 인스턴스는 새로 출시된 인스턴스 타입을 사용하고 있으며, 다른 인스턴스는 이전 인스턴스 타입을 사용하고 있습니다. 분석이 완료된 후 CloudOps 엔지니어는 일부 EC2 인스턴스가 Compute Optimizer 대시보드에서 누락되었음을 알게 됩니다. 이 문제의 가능성 있는 원인은 무엇입니까?",
    "options": {
      "A": "누락된 인스턴스는 분석을 위한 불충분한 Amazon CloudWatch 메트릭 데이터를 가지고 있습니다.",
      "B": "Compute Optimizer가 누락된 인스턴스의 인스턴스 타입을 지원하지 않습니다.",
      "C": "Compute Optimizer는 이미 누락된 인스턴스를 최적화된 것으로 고려합니다.",
      "D": "누락된 인스턴스는 Windows 운영 체제를 실행하고 있습니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Compute Optimizer — EC2/EBS/Lambda 등 리소스 최적화 권장사항 제공\n▸ Supported Instance Types — Compute Optimizer가 분석 가능한 인스턴스 타입 목록 (구형/특수 타입 일부 제외)\n▸ CloudWatch Metric Requirement — 분석을 위해 최소 14일·일정 메트릭 필요\n\n【정답 포인트】\n▸ \"일부 인스턴스 누락\" → 대시보드 미표시 = Compute Optimizer 분석 대상 외\n▸ \"구형/신규 인스턴스 타입 혼재\" 힌트 → 미지원 타입은 분석 불가, 결과에서 빠짐\n▸ 미지원 타입은 누락(missing) 처리 (Optimized·Under-provisioned 등 분류 결과조차 부여 X)\n\n【오답 체크】\n(A) CloudWatch 메트릭이 부족하면 누락이 아니라 \"데이터 불충분(Insufficient data)\" 상태로 표시되어 대시보드에 보임.\n(C) 이미 최적화된 인스턴스도 대시보드에 \"Optimized\" 상태로 표시됨 → 제외 사유 아님.\n(D) Compute Optimizer는 OS 종류(Windows/Linux)에 무관하게 동일하게 분석함.\n\n【시험 포인트】\n▸ \"Compute Optimizer 누락\" 패턴 → 인스턴스 타입 미지원 의심\n▸ \"데이터 불충분\"은 별도 상태로 표시 (누락과 구분)\n▸ 분석 가능 여부 = 인스턴스 타입 지원 + 충분한 메트릭 양쪽 필요",
  ...
  {
    "id": 72,
    "question": "회사가 Amazon VPC에서 워크로드를 실행하고 있으며 CloudWatch Logs를 구성했습니다. 회사는 회사의 AWS 계정에서 비정상적인 API 활동과 보안 이벤트를 자동으로 탐지할 수 있는 솔루션이 필요합니다. 이 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
  ...
  {
    "id": 73,
    "question": "회사가 수백 개의 이미지를 생성하여 Amazon S3 버킷에 업로드합니다. 회사는 수동으로 이미지를 항상 켜진 Amazon EC2 인스턴스로 복사하여 처리합니다. 일반적으로 각 이미지를 처리하는 데 30초에서 120초가 소요됩니다. CloudOps 엔지니어는 이미지가 S3 버킷에 도착하는 즉시 처리하도록 이미지 처리 솔루션을 자동화하려고 합니다. 가장 비용 효율적으로 이러한 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "S3 Event Notifications를 구성하여 이미지가 S3 버킷에 업로드될 때 EC2 인스턴스를 호출합니다. EC2 인스턴스에서 이미지 처리 솔루션을 실행하여 이미지를 처리합니다.",
      "B": "S3 Event Notifications를 구성하여 Amazon EventBridge 규칙을 호출합니다. EventBridge 규칙을 구성하여 사전 구성된 AWS Glue ETL 작업을 시작하여 이미지를 처리합니다.",
      "C": "S3 Event Notifications를 구성하여 새 이미지가 원본 S3 버킷에 업로드될 때 이미지 처리 로직을 실행하는 AWS Lambda 함수를 호출합니다.",
      "D": "S3 Event Notifications를 구성하여 이미지가 S3 버킷에 업로드될 때 EC2 인스턴스로 지원되는 Amazon Elastic Container Service (Amazon ECS) 컨테이너의 작업을 호출합니다. ECS 작업을 구성하여 이미지를 처리합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ S3 Event Notifications  — 객체 업로드 시 AWS 서비스 자동 호출\n▸ Lambda Invocation  —  30~120초 작업에 최적화된 이벤트 기반 컴퓨팅\n▸ Cost Efficiency  — 사용량에 따른 요금 (서버리스 vs 항상 켜진 EC2)\n\n【정답 포인트】\n▸ \"30-120초 처리 + 가장 비용 효율적\" → Lambda는 실행 시간만 청구되며, 항상 켜진 EC2보다 훨씬 저렴합니다. 이벤트 기반으로 자동 확장되어 처리량에 대응합니다.\n\n【오답 체크】\n(A) EC2는 항상 켜있어서 유휴 시간도 비용 발생합니다. 이미지 도착이 간헐적이면 EC2 활용률이 낮아 비효율적입니다.\n(B) Glue ETL은 데이터 변환/통합용 고비용 서비스로, 단순 이미지 처리에는 과도합니다.\n(D) ECS도 EC2 기반 백엔드로 항상 비용 발생하며, Lambda보다 관리 오버헤드가 큽니다.\n\n【시험 포인트】\n▸ \"비용 효율\" + \"자동화\" → Lambda 선택\n▸ Glue ≠ 단순 이미지 처리",
  ...
  {
    "id": 74,
    "question": "회사가 AWS Organizations의 조직을 사용하여 여러 AWS 계정을 관리합니다. 회사는 조직의 모든 계정에서 특정 이벤트를 새 수신자 계정으로 전송하여 AWS Lambda 함수가 이벤트를 처리할 수 있도록 해야 합니다. CloudOps 엔지니어는 Amazon EventBridge를 구성하여 이벤트를 새 수신자 계정의 us-west-2 리전의 대상 이벤트 버스로 라우팅해야 합니다. CloudOps 엔지니어는 발신자 계정 및 수신자 계정에서 지정된 이벤트와 일치하는 규칙을 만듭니다. 규칙은 이벤트 패턴에 계정 매개변수를 지정하지 않습니다. CloudOps 엔지니어는 발신자 계정에서 대상 이벤트 버스에 대한 PutEvents 작업을 허용하는 IAM 역할을 만듭니다. us-east-1 리전에서 시작되는 첫 번째 테스트 이벤트는 수신 계정의 Lambda 함수에서 처리되지 않습니다. 이벤트가 처리되지 않는 가능성 있는 이유는 무엇입니까?",
    "options": {
      "A": "발신자 계정 및 수신자 계정에는 EventBridge용 Interface VPC 엔드포인트가 필요합니다.",
      "B": "대상 Lambda 함수가 다른 AWS 리전에 있으며 EventBridge에서 지원하지 않습니다.",
  ...
  {
    "id": 75,
    "question": "개발자가 Amazon S3 버킷에서 Versioning을 활성화합니다. 개발자가 버킷에서 쓰기 작업을 시도할 때 HTTP 404 NoSuchKey 오류가 발생합니다. CloudOps 엔지니어가 이 문제를 해결해야 합니다. 이 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "S3 버킷에서 Versioning을 비활성화하고 쓰기 작업을 다시 시도합니다.",
      "B": "버킷 정책을 수정하여 버전 관리된 객체에 대한 쓰기 작업을 허용합니다.",
      "C": "Versioning을 활성화한 후 최소 15분을 기다린 다음 쓰기 작업을 수행합니다.",
      "D": "버킷에서 S3 Transfer Acceleration을 활성화합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ S3 Versioning  — 객체 버전 관리 기능 활성화\n▸ NoSuchKey Error  — 객체를 찾을 수 없음 에러\n▸ Replication Propagation  — 설정 변경 반영 지연\n\n【정답 포인트】\n▸ \"버전 관리 활성화 후 즉시 오류\" → AWS는 S3 버킷 설정 변경(버전 관리 활성화) 후 모든 리전에 반영되는 데 시간 지연(보통 15분)이 발생합니다. 충분한 시간 대기 후 재시도하면 정상 작동합니다.\n\n【오답 체크】\n(A) 비활성화하면 버전 관리 기능을 사용할 수 없으므로, 요구사항(버전 관리)을 포기하는 것입니다.\n(B) S3 버킷 정책은 버전 관리 활성화와 무관하며, 쓰기 권한 문제는 아닙니다.\n(D) Transfer Acceleration은 전송 속도 최적화이며, 버전 관리 설정 지연과 무관합니다.\n\n【시험 포인트】\n▸ AWS 설정 변경 = 반영 시간 지연 발생 패턴\n▸ 즉시 오류 → 설정 반영 대기 필요",
  ...
  {
    "id": 76,
    "question": "회사가 회사의 Amazon S3 버킷에 대해 Cross-Region Replication (CRR)을 구현합니다. S3 버킷은 us-east-1 리전에 있습니다. 회사는 Amazon S3 관리형 키 (SSE-S3)를 사용한 서버 측 암호화를 사용하여 버킷의 데이터를 보호합니다. CloudOps 엔지니어가 백업을 저장할 새로운 AWS 계정을 만듭니다. 모든 백업 버킷은 us-west-2 리전에 있습니다. CloudOps 엔지니어는 원본 버킷 및 대상 버킷에서 Versioning을 활성화합니다. CloudOps 엔지니어는 원본 계정에서 s3.amazonaws.com에 대한 IAM 역할을 만듭니다. CloudOps 엔지니어는 원본 버킷에서 읽기 작업, 대상 버킷에서 복제 작업, 대상 버킷의 키를 사용한 암호화 작업을 수행할 권한을 IAM 역할에 부여합니다. 대상 버킷 정책은 IAM 역할이 복제 및 읽기 작업을 수행하도록 허용합니다. 복제 구성이 완료된 후 CloudOps 엔지니어는 객체가 복제되지 않는 것을 확인합니다. 객체가 복제되지 않는 가능성 있는 이유는 무엇입니까?",
    "options": {
      "A": "IAM 역할 및 Bucket Policy는 ObjectOwnerOverrideToBucketOwner 권한을 가져야 합니다.",
      "B": "원본 버킷 및 대상 버킷의 객체는 다중 리전 키로 암호화되어야 합니다.",
      "C": "원본 계정 및 대상 계정에서 Amazon S3용 Gateway VPC 엔드포인트를 만들어야 합니다.",
      "D": "대상 버킷은 AWS KMS 키 (SSE-KMS)를 사용한 서버 측 암호화를 사용해야 합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Cross-Region Replication (CRR) — 서로 다른 리전 S3 버킷 간 자동 복제\n▸ Cross-Account Replication — 원본·대상 버킷이 서로 다른 AWS 계정에 있는 경우의 복제\n▸ ObjectOwnerOverrideToBucketOwner — 복제된 객체 소유권을 대상 버킷 소유자에게 이양하는 권한\n▸ s3:ObjectOwnerOverrideToBucketOwner — IAM 역할과 버킷 정책 양쪽에 부여 필요\n\n【정답 포인트】\n▸ \"다른 계정 + CRR + 객체 미복제\" → 객체 소유권 충돌이 핵심 원인\n▸ 원본 계정 소유 객체가 대상 계정 버킷에 복제될 때 대상 계정이 객체 관리 권한을 갖지 못해 복제 실패\n▸ IAM 역할과 대상 버킷 정책 모두에 ObjectOwnerOverrideToBucketOwner 권한이 있어야 복제 성공\n\n【오답 체크】\n(B) 다중 리전 KMS 키는 SSE-KMS 사용 시 옵션이며, 현 시나리오는 SSE-S3 사용 중. 소유권 문제와 무관.\n(C) Gateway VPC 엔드포인트는 VPC 내부 트래픽 최적화용으로 S3 복제 필수 요건이 아님. 복제는 AWS 백본 사용.\n(D) SSE-S3 → SSE-KMS 변경은 암호화 방식 변경일 뿐. 객체 소유권 문제는 그대로 남아 복제 실패 지속.\n\n【시험 포인트】\n▸ \"Cross-Account CRR + 복제 안 됨\" → ObjectOwnerOverrideToBucketOwner 권한 누락 의심\n▸ IAM 역할 정책 + 대상 버킷 정책 양쪽 모두에 권한이 있어야 함 (한쪽만 있으면 실패)\n▸ 객체 소유권 = 크로스 계정 복제의 가장 흔한 함정",
  ...
  {
    "id": 77,
    "question": "회사가 공용 서브넷과 프라이빗 서브넷을 포함하는 VPC를 가지고 있습니다. 회사는 Amazon Linux Amazon Machine Image (AMI)를 사용하고 AWS Systems Manager Agent (SSM Agent)가 설치된 Amazon EC2 인스턴스를 프라이빗 서브넷에 배포합니다. EC2 인스턴스는 아웃바운드 트래픽만 허용하는 Security Group에 있습니다. CloudOps 엔지니어는 권한이 있는 관리자 그룹이 인스턴스를 인터넷에 노출하지 않고 SSH를 통해 인스턴스에 연결할 수 있는 기능을 제공해야 합니다. 이 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "프라이빗 서브넷에서 EC2 Instance Connect 엔드포인트를 만듭니다. Security Group을 업데이트하여 인바운드 SSH 트래픽을 허용합니다. 권한이 있는 관리자를 위한 IAM 그룹을 만듭니다. PowerUserAccess 관리형 정책을 IAM 그룹에 할당합니다.",
      "B": "프라이빗 서브넷에서 Systems Manager 엔드포인트를 만듭니다. Security Group을 업데이트하여 Systems Manager 엔드포인트가 연결된 프라이빗 네트워크에서의 SSH 트래픽을 허용합니다. 권한이 있는 관리자를 위한 IAM 그룹을 만듭니다. PowerUserAccess 관리형 정책을 IAM 그룹에 할당합니다.",
      "C": "공용 서브넷에서 EC2 Instance Connect 엔드포인트를 만듭니다. Security Group을 업데이트하여 프라이빗 네트워크에서의 SSH 트래픽을 허용합니다. 권한이 있는 관리자를 위한 IAM 그룹을 만듭니다. PowerUserAccess 관리형 정책을 IAM 그룹에 할당합니다.",
      "D": "공용 서브넷에서 Systems Manager 엔드포인트를 만듭니다. EC2 인스턴스에 대해 AmazonSSMManagedInstanceCore 권한이 있는 IAM 역할을 만듭니다. 권한이 있는 관리자를 위한 IAM 그룹을 만듭니다. AmazonEC2ReadOnlyAccess IAM 정책을 IAM 그룹에 할당합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ EC2 Instance Connect Endpoint  — SSH/RDP 접근을 위한 프라이빗 엔드포인트\n▸ Systems Manager Session Manager  — 웹/CLI 기반 인스턴스 접근\n▸ Bastion Host 대체 — 인터넷 노출 없는 보안 접근\n\n【정답 포인트】\n▸ \"SSH 접근 + 인터넷 미노출\" → EC2 Instance Connect 엔드포인트는 프라이빗 서브넷에서 SSH를 인터넷 노출 없이 제공합니다. Systems Manager와 달리 기존 SSH 클라이언트/도구와 호환됩니다. PowerUserAccess는 관리자 권한 제공합니다.\n\n【오답 체크】\n(B) Systems Manager 엔드포인트는 Session Manager(웹/CLI) 기반이며, SSH 직접 연결은 지원하지 않습니다.\n(C) 퍼블릭 서브넷 엔드포인트는 아키텍처 요구사항에 맞지 않으며, 프라이빗 서브넷에 배치해야 합니다.\n(D) Systems Manager 엔드포인트는 Session Manager용이고, AmazonEC2ReadOnlyAccess는 읽기 전용이라 SSH 세션 시작 권한이 부족합니다.\n\n【시험 포인트】\n▸ \"SSH\" → EC2 Instance Connect 엔드포인트\n▸ \"웹 기반 접근\" → Systems Manager Session Manager",
  ...
  {
    "id": 78,
    "question": "회사가 us-east-1 리전의 Application Load Balancer (ALB) 뒤에 Amazon EC2 인스턴스에서 웹 기반 애플리케이션을 실행합니다. 전 세계 사용자가 애플리케이션에 액세스합니다. 북미 외부의 사용자는 높은 지연 시간과 불안정한 애플리케이션 성능을 보고합니다. 회사는 모든 글로벌 사용자를 위해 지연 시간 및 애플리케이션 성능을 개선해야 합니다. 이 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "ALB 앞에서 AWS Global Accelerator를 사용합니다.",
      "B": "ALB 앞에 Network Load Balancer (NLB)를 배포합니다.",
      "C": "ALB를 Network Load Balancer (NLB)로 바꿉니다.",
      "D": "Amazon Route 53 헬스 체크를 구성하여 지연 시간 임계값을 기반으로 AWS 리전 간에 장애 조치합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS Global Accelerator  — Anycast IP + 엣지 로케이션을 통한 글로벌 성능 최적화\n▸ Latency Reduction  — 전 세계 사용자 연결 최적화\n▸ Single Region Limitation  — 단일 리전 배포의 성능 한계\n\n【정답 포인트】\n▸ \"글로벌 지연 + 단일 리전\" → AWS Global Accelerator는 AWS 글로벌 네트워크를 활용하여 지연 시간을 최소화합니다. 엣지 로케이션에서 가장 가까운 경로를 선택하여 모든 지역 사용자에게 최적 성능을 제공합니다.\n\n【오답 체크】\n(B) NLB는 프로토콜/처리량 최적화(L4)이며, 지역 간 성능 최적화 기능이 없습니다.\n(C) ALB → NLB 교체는 로드 밸런서 타입만 변경될 뿐, 글로벌 지연 개선에 효과 없습니다.\n(D) Route 53은 DNS 기반 리전 간 장애 조치이며, 단일 리전 내 성능 개선에는 도움이 안 됩니다.\n\n【시험 포인트】\n▸ \"글로벌 사용자 + 지연\" → Global Accelerator 필수\n▸ \"단일 리전\" = Global Accelerator/CloudFront 검토",
  ...
  {
    "id": 80,
    "question": "CloudOps 엔지니어는 여러 AWS 계정에 걸쳐 AWS 리소스에 일관되게 태그를 지정해야 합니다. 회사는 AWS Organizations의 조직을 사용하여 중앙에서 계정을 관리합니다. 회사는 비용 배치 태그를 구현하여 각 비즈니스 단위에 할당된 비용을 정확하게 추적하려고 합니다. 가장 적은 운영 오버헤드로 이러한 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "Organizations 태그 정책을 사용하여 모든 리소스에 필수 태깅을 강제합니다. AWS 결제 및 비용 관리 콘솔에서 비용 배치 태그를 활성화합니다.",
      "B": "AWS CloudTrail 이벤트를 구성하여 AWS Lambda 함수를 호출하고 태그가 없는 리소스를 탐지하고 미리 정의된 규칙에 따라 자동으로 태그를 할당합니다.",
      "C": "AWS Config를 사용하여 태깅 규정 준수를 평가합니다. AWS Budgets를 사용하여 비용 배치를 위한 태그를 적용합니다.",
      "D": "AWS Service Catalog를 사용하여 미리 태그된 리소스만 프로비저닝합니다. AWS Trusted Advisor를 사용하여 조직 전체에 태깅을 강제합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ Organizations Tag Policies  — 중앙 집중식 태그 정책 관리\n▸ Mandatory Tagging  — 리소스 생성 시 필수 태그 적용\n▸ Cost Allocation Tags  — 비용 추적용 태그\n\n【정답 포인트】\n▸ \"일관된 태깅 + 운영 오버헤드 최소\" → Organizations 태그 정책은 조직 전체에 자동으로 태깅 규칙을 적용하므로, 각 계정별 수동 관리가 불필요합니다. 리소스 생성 시점부터 정책을 강제하여 규정 위반을 방지합니다.\n\n【오답 체크】\n(B) Lambda 함수로 사후 감지 및 태깅하는 방식은 수동 개입, 지연, 모니터링 오버헤드가 발생합니다.\n(C) Config는 준수 평가만 하며, 태그 적용을 강제하지 않습니다. Budgets도 적용 기능이 아닙니다.\n(D) Service Catalog는 신규 프로비저닝만 커버하며, 기존 리소스 태깅 문제를 해결하지 못합니다.\n\n【시험 포인트】\n▸ Organizations = 중앙 정책 관리\n▸ \"자동화 + 의무화\" → Organizations 정책 선택",
  ...
  {
    "id": 81,
    "question": "회사가 Ubuntu 운영 체제 (OS)를 실행하는 여러 Amazon EC2 인스턴스를 가지고 있습니다. 회사는 OS를 정기적으로 패치해야 합니다. CloudOps 엔지니어는 매주 수동으로 패치를 설치합니다. 회사는 Ubuntu를 실행하는 새로운 EC2 인스턴스를 지속적으로 추가합니다. CloudOps 엔지니어는 패칭 프로세스를 자동화해야 합니다. 가장 운영 효율적인 방식으로 이 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "SSH를 사용하여 EC2 인스턴스에 연결하고 패치를 설치하는 AWS Lambda 함수를 만듭니다. Lambda 함수를 구성하여 매주 실행되도록 합니다.",
      "B": "EC2 인스턴스에 AWS Systems Manager Agent (SSM Agent)를 설치합니다. Systems Manager Patch Manager를 구성하여 매주 인스턴스에 패치를 설치하도록 합니다.",
      "C": "AWS Systems Manager Inventory를 사용하여 패치되지 않은 EC2 인스턴스를 식별하고 OS 패치를 설치합니다.",
      "D": "매주 패치를 설치하는 cron 표현식이 있는 Amazon EventBridge 규칙을 만듭니다. EC2 인스턴스를 대상으로 하도록 EventBridge 규칙을 구성합니다. 대상 인스턴스에서 OS 업데이트를 실행하는 작업을 구성합니다."
    },
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ Systems Manager Patch Manager  — 패치 관리 자동화 서비스\n▸ SSM Agent  — EC2에서 Systems Manager 기능 활성화\n▸ Automated Patching  — 운영 오버헤드 최소화\n\n【정답 포인트】\n▸ \"정기 패칭 + 자동화 + 운영 효율\" → Systems Manager Patch Manager는 Linux 패치 자동 적용, 재부팅 스케줄링, 패치 규정 준수 추적을 제공합니다. SSM Agent 설치 후 AWS 콘솔에서 정책을 지정하면 모든 인스턴스에 자동 적용됩니다. 새 인스턴스도 자동으로 정책 대상에 포함됩니다.\n\n【오답 체크】\n(A) Lambda + SSH는 관리 복잡성이 높고, 네트워크 접근/인증 오버헤드가 발생합니다.\n(C) Inventory는 인벤토리 수집만 하며, 패치 자동 설치 기능이 없습니다. 수동 개입 필요합니다.\n(D) EventBridge는 이벤트 라우팅 서비스이며, 패치 관리 기능(스케줄, 규정 준수, 자동 재부팅)이 없습니다.\n\n【시험 포인트】\n▸ \"패치 관리\" = Patch Manager 필수\n▸ Systems Manager = EC2 운영 관리 통합 플랫폼",
  ...
  {
    "id": 82,
    "question": "글로벌 회사가 us-east-1 리전에서 중요한 주 워크로드를 실행합니다. 회사는 워크로드 장애 발생 시 최소한의 다운타임으로 비즈니스 연속성을 보장하려고 합니다. 회사는 워크로드를 두 번째 AWS 리전으로 복제하려고 합니다. CloudOps 엔지니어는 서비스 수준 계약을 충족하기 위해 RTO (Recovery Time Objective) 10분 이내 및 RPO (Recovery Point Objective) 0을 달성할 솔루션이 필요합니다. 이 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "두 번째 리전에서 실시간 데이터 복제를 제공하는 파일럿 라이트 아키텍처를 구현합니다. Amazon Route 53 헬스 체크 및 자동 DNS 장애 조치를 구성합니다.",
      "B": "두 번째 리전에서 정기적인 데이터 복제를 제공하는 웜 스탠바이 아키텍처를 구현합니다. Amazon Route 53 헬스 체크 및 자동 DNS 장애 조치를 구성합니다.",
      "C": "두 리전에 걸쳐 실시간 데이터 복제를 제공하는 액티브-액티브 아키텍처를 구현합니다. Amazon Route 53 헬스 체크 및 가중치 기반 라우팅 정책을 사용합니다.",
      "D": "데이터의 정기적인 백업을 생성하고 두 번째 리전에 있는 S3 버킷에 저장하는 사용자 정의 스크립트를 구현합니다. 워크로드 장애 시 백업을 사용하여 두 번째 리전에서 애플리케이션을 시작합니다."
    },
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ RTO (Recovery Time Objective) —  10분 이내\n▸ RPO (Recovery Point Objective) —  0 (데이터 손실 없음)\n▸ Active-Active Architecture  — 양쪽 리전에서 동시 운영\n▸ Real-time Replication  — 실시간 데이터 동기화\n\n【정답 포인트】\n▸ \"RTO < 10분 + RPO = 0\" → 액티브-액티브 아키텍처만이 두 조건을 동시에 충족합니다. 양쪽 리전이 동시에 운영되므로 장애 시 즉시 다른 리전으로 트래픽 전환 가능(RTO 최소화), 실시간 데이터 복제로 데이터 손실 방지(RPO 0)합니다.\n\n【오답 체크】\n(A) 파일럿 라이트는 실시간 복제하지만, 리전 2에 리소스가 최소한이라 시작 시간(RTO)이 10분 초과합니다.\n(B) 웜 스탠바이는 정기 복제(시간 단위)로 RPO > 0이며, 10분 이내 RTO 달성이 어렵습니다.\n(D) 정기 백업은 RPO가 크고(보통 시간 단위), 복구 시 애플리케이션 시작 시간이 10분 초과합니다.\n\n【시험 포인트】\n▸ RTO < 10분 + RPO = 0 → Active-Active 유일\n▸ Disaster Recovery 아키텍처 구분 필수",
  ...
  {
    "id": 83,
    "question": "CloudOps 엔지니어는 Amazon Elastic Kubernetes Service (Amazon EKS)에서 실행되는 공용 웹사이트의 특정 메트릭에 대한 관측성을 구성하려고 합니다. CloudOps 엔지니어는 지연 시간, 트래픽, 오류 및 포화 메트릭을 관찰하려고 합니다. CloudOps 엔지니어는 Service Level Objectives (SLOs)를 정의하고 Service Level Indicators (SLIs)를 모니터링하려고 합니다. CloudOps 엔지니어는 또한 메트릭, 로그 및 추적을 상관관계 지어 더 빠른 문제 해결을 지원하려고 합니다. 가장 적은 운영 노력으로 이러한 요구사항을 충족할 솔루션은 무엇입니까?",
    "options": {
      "A": "Amazon CloudWatch Application Signals를 사용하여 EKS 워크로드에 대해 지정된 메트릭을 자동으로 수집하고 모니터링합니다.",
      "B": "웹사이트에 대해 AWS Distro for OpenTelemetry를 구성하여 메트릭을 생성합니다. Amazon Managed Service for Prometheus를 사용하여 지정된 메트릭을 수집합니다. Amazon Managed Grafana를 사용하여 메트릭을 시각화합니다.",
      "C": "Amazon CloudWatch RUM 및 CloudWatch Synthetics canaries를 구성하여 EKS 워크로드에 대해 지정된 메트릭을 자동으로 수집하고 모니터링합니다.",
      "D": "Amazon CloudWatch Application Insights를 구성하여 일반적인 애플리케이션 성능 문제 및 이상을 탐지하고 EKS 워크로드에 대해 지정된 메트릭을 모니터링합니다."
    },
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ CloudWatch Application Signals  — SLO/SLI 자동 수집 및 모니터링\n▸ Latency, Traffic, Errors, Saturation (LTES) — Golden Signal 메트릭\n▸ Operational Effort Minimization  — 자동 측정, 낮은 오버헤드\n\n【정답 포인트】\n▸ \"SLO/SLI + 지표 상관관계 + 최소 운영 노력\" → CloudWatch Application Signals는 EKS에 자동으로 배포되어 LTES 메트릭, SLO/SLI를 즉시 수집합니다. OpenTelemetry 복잡성 없이, 메트릭/로그/추적 통합 제공으로 최소 운영 노력입니다.\n\n【오답 체크】\n(B) OpenTelemetry + Prometheus + Grafana는 수동 구성이 복잡하고, 통합 작업이 필요하여 운영 오버헤드가 높습니다.\n(C) RUM(사용자 경험) + Synthetics(합성 모니터링)은 관측 가능성 완성도가 낮고, SLO/SLI 자동 계산 기능이 없습니다.\n(D) Application Insights는 이상 탐지에 중점이며, SLO/SLI 정의 및 상관관계 분석이 부족합니다.\n\n【시험 포인트】\n▸ \"SLO + SLI + LTES\" → Application Signals\n▸ \"최소 노력\" = 기본 제공 자동화",
