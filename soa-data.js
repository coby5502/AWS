window.SOA_QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ 네트워크 ACL — 서브넷 수준 방화벽, 상태 비저장(Stateless), 인바운드/아웃바운드 명시적 규칙 필요\n▸ 보안 그룹 — 인스턴스 수준 방화벽, 상태 저장(Stateful), 인바운드 허용 시 아웃바운드 자동 허용\n▸ 임시 포트 (Ephemeral Port) — 클라이언트 요청 응답 시 사용 포트 범위(1024~65535)\n▸ 상태 비저장 vs 상태 저장 — 양방향 통신 처리 방식의 차이\n\n【정답 포인트】\n▸ HTTP 요청(포트 80) 인바운드: NACL에서 명시적으로 허용됨 ✓\n▸ 응답 경로: 서버가 클라이언트 임시 포트(1024~65535)로 응답 패킷 전송\n▸ NACL은 상태 비저장 → 아웃바운드도 명시적 규칙 필수\n▸ 아웃바운드 임시 포트 범위 규칙 없음 → 응답 차단 → 웹사이트 접근 불가\n▸ 보안 그룹은 상태 저장이므로 인바운드 허용 시 응답 자동 허용 (추가 규칙 불필요)\n\n【오답 체크】\n(B) 보안 그룹 아웃바운드: 보안 그룹은 상태 저장이므로 인바운드 허용하면 아웃바운드 자동 허용. 추가 규칙 불필요.\n(C) Elastic IP 변경: Elastic IP는 정적 주소, 할당 후 변경되지 않음.\n(D) 추가 NACL: 문제 명시 \"새 네트워크 ACL만 만들었다\"는 추가 NACL 없음.\n\n【시험 포인트】\nNACL 상태 비저장의 핵심: 요청과 응답이 서로 다른 규칙으로 취급. HTTP 80 인바운드만으로 부족—응답 경로의 임시 포트 아웃바운드 규칙도 동일하게 필요. 보안 그룹(상태 저장)과의 차이점이 시험 핵심.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ Systems Manager Agent (SSM Agent) — EC2 인스턴스 원격 관리 에이전트\n▸ Fleet Manager — SSM 중앙 관리 콘솔, 인스턴스 관리 및 모니터링 허브\n▸ AmazonSSMManagedInstanceCore — SSM 최소 권한 IAM 매니지드 정책\n▸ IAM 인스턴스 프로필 — EC2 인스턴스에 IAM 권한 부여 메커니즘\n▸ 최소 권한 원칙 — 필요한 최소 권한만 부여하는 보안 모범 사례\n\n【정답 포인트】\n▸ 증상: SSM Agent 설치 완료 → Fleet Manager에 인스턴스가 보이지 않음\n▸ 네트워크 확인: NAT 게이트웨이 이미 구성되어 있음 → 아웃바운드 연결 정상\n▸ 에이전트 설치: SSM Agent 최신 버전 설치됨 → 소프트웨어 정상\n▸ 남은 원인: IAM 권한 부족 (인스턴스 프로필에 필요 정책 연결 안 됨)\n▸ 해결책: EC2 인스턴스 프로필에 AmazonSSMManagedInstanceCore 정책 연결\n▸ 이 정책에 포함된 권한: SSM Session Manager, Run Command, 인스턴스 메타데이터 접근 등\n\n【오답 체크】\n(A) NAT 인스턴스 교체: 아키텍처 불필요한 변경. 네트워크는 이미 정상 작동 중.\n(B) VPC 엔드포인트: 가능한 최적화이지만 PRIMARY 문제는 IAM 권한. 엔드포인트는 선택사항.\n(D) ssm* 모든 권한: 과도한 권한 부여, 최소 권한 원칙 위배. AmazonSSMManagedInstanceCore가 정답.\n\n【시험 포인트】\nFleet Manager \"인스턴스 표시 안 됨\" 문제의 90% 원인은 IAM 권한 부족. SSM Agent 설치, 네트워크 연결 확인 후에도 문제면 즉시 IAM 권한 검증. AmazonSSMManagedInstanceCore는 Systems Manager 작업의 기본 매니지드 정책.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ SQS 메시지 우선순위 처리 — Standard SQS에는 기본 우선순위 기능 없음\n▸ Standard SQS — 순서 보장 없음, 우선순위 처리 불가능\n▸ FIFO SQS — 순서 보장, 메시지 그룹화 가능하지만 모든 메시지가 같은 속도 처리\n▸ 우선순위 큐 패턴 — 여러 개의 SQS 큐를 정책에 따라 관리\n▸ Auto Scaling — 리소스 용량 조정, 우선순위 제어와는 별개\n\n【정답 포인트】\n▸ 요구사항: \"알람 알림을 정보 알림보다 우선순위\"\n▸ SQS 우선순위 기본 패턴: 우선순위별로 별도 큐 생성\n▸ 설계: Alarm Queue + Information Queue 분리\n▸ 애플리케이션 로직: EC2 인스턴스가 먼저 Alarm 큐 폴링, 이후 Information 큐 폴링\n▸ 장점: 단순하고 명확한 우선순위 제어, 리소스 할당 조정 용이\n\n【오답 체크】\n(A) Auto Scaling: 용량 증가는 처리 속도만 개선, 메시지 처리 순서(우선순위) 제어 불가능.\n(B) SNS 팬아웃: 모든 인스턴스에 병렬 분산 전송, 우선순위 제어 불가능.\n(C) DynamoDB 스트림: 데이터 저장소 추가, 메시지 우선순위 처리 문제와 무관.\n\n【시험 포인트】\nSQS 우선순위 처리 = 다중 큐 패턴의 정석. Standard SQS에는 우선순위 개념 없음—따라서 아키텍처 수준에서 별도 큐로 분리. 클라이언트 코드에서 처리 순서만 제어하면 된다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Trusted Advisor — AWS 환경 최적화, 보안, 성능 점검 및 권장사항 제공 서비스\n▸ AWS 지원 플랜 — Basic, Developer, Business, Enterprise 4가지 수준\n▸ 코어 점검 (Core Checks) — Basic Support 또는 무료 플랜에서만 가능 (6가지: 보안 그룹, MFA 등)\n▸ 전체 점검 (All Checks) — Business, Enterprise Support에서만 가능 (100+ 항목)\n\n【정답 포인트】\n▸ Trusted Advisor 점검 범위는 AWS 지원 플랜에 따라 결정됨\n▸ Basic Support: 6가지 코어 점검만 제공 (비용 절감)\n▸ Business/Enterprise Support: 모든 점검 활성화 (100+ 포인트)\n▸ 더 높은 플랜 → 더 많은 점검 범위 제공\n▸ AWS 지원 플랜 = Trusted Advisor 가용 점검 수 결정 요인\n\n【오답 체크】\n(A) EC2 인스턴스 상태: 특정 점검의 결과에 영향, 전체 점검 수에는 영향 없음.\n(C) SCP (Service Control Policy): IAM 권한 제어 메커니즘, Trusted Advisor 점검 범위와 무관.\n(D) MFA 활성화: 보안 모범 사례, Trusted Advisor 점검 수 제어 안 함.\n\n【시험 포인트】\nTrusted Advisor 점검 범위 = AWS 지원 플랜 종속성. \"Trusted Advisor 점검\" 관련 문제는 무조건 \"지원 플랜\" 확인. Basic = 제한적, Business 이상 = 전체 점검.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ CloudFormation StackSets — 여러 AWS 계정/리전에 CloudFormation 스택 배포 기능\n▸ 자동화 배포 — 관리 계정에서 일회 설정 → 모든 멤버 계정에 자동 배포\n▸ 운영 오버헤드 최소화 — 반복 배포 자동화, 추가 코드 불필요\n▸ OrganizationAccountAccessRole — AWS Organizations 계정 간 역할 전환\n\n【정답 포인트】\n▸ 요구사항: \"여러 AWS Organizations 계정에 동일 템플릿 배포\"\n▸ \"운영상 오버헤드 최소\" → 자동화된 솔루션 필수\n▸ CloudFormation StackSets: CloudFormation 네이티브 기능\n▸ 구현: 관리 계정에서 StackSet 일회 생성 → 모든 멤버 계정에 자동 배포\n▸ 유지보수: StackSet 업데이트 → 모든 계정에 자동 반영\n▸ Organizations 통합: 계정 추가 시 자동 포함 가능 (Auto Deployment)\n\n【오답 체크】\n(A) OrganizationAccountAccessRole: 수동 배포 필요, 각 계정마다 반복, 오버헤드 증가.\n(B), \n(C) Lambda + CreateStack API: 커스텀 코드 작성, 유지보수 복잡도 증가.\n\n【시험 포인트】\n여러 계정 배포 = StackSets의 정석 용도. StackSets는 CloudFormation \"마법\" 기능—관리 계정에서 설정하면 자동으로 모든 멤버 계정에 스택 생성, 업데이트, 삭제 처리.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ CNAME (Canonical Name) — 도메인명을 다른 도메인명으로 매핑(정규 이름)\n▸ A 레코드 — 도메인명을 IP 주소(IPv4)로 매핑\n▸ AAAA 레코드 — 도메인명을 IP 주소(IPv6)로 매핑\n▸ 별칭 레코드 (Alias Record) — AWS 리소스(ALB, CloudFront, S3 등)로 매핑\n▸ PTR 레코드 — IP 주소를 도메인명으로 역매핑 (역 DNS)\n\n【정답 포인트】\n▸ 원본: www.company.com (자사 도메인, Route 53 관리)\n▸ 대상: app.example.com (외부 인터넷 공급자 호스팅 도메인)\n▸ 필요: 도메인명 → 도메인명 매핑 (도메인 이름 변경)\n▸ 해결책: CNAME 레코드 생성\n▸ 설정: www.company.com CNAME → app.example.com\n▸ 결과: www.company.com으로 접근 → app.example.com으로 자동 리다이렉트\n\n【오답 체크】\n(A) A 레코드: IP 주소 직접 매핑, 도메인명 변경 불가능.\n(B) 별칭 레코드: AWS 리소스 전용(CloudFront, ELB), 일반 외부 도메인 매핑 불가능.\n(D) PTR 레코드: 역방향 DNS (IP → 도메인), 정방향 매핑 아님.\n\n【시험 포인트】\nRoute 53 레코드 선택:\n- AWS 리소스 → 별칭 레코드\n- 외부 도메인 → CNAME\n- IP 주소 → A/AAAA 레코드\n이 문제는 CNAME의 핵심 용도: 도메인 이름 → 도메인 이름 재지정.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ 복원력 (Resilience) — 장애 발생 시에도 서비스 지속 가능성\n▸ RTO (Recovery Time Objective) — 서비스 복구까지 필요한 최대 시간\n▸ RPO (Recovery Point Objective) — 허용 가능한 데이터 손실 시간\n▸ Multi-AZ Replication — 여러 가용 영역에 동기식 복제, 자동 장애 조치\n▸ 읽기 복제본 (Read Replica) — 읽기 전용 복사본\n\n【정답 포인트】\n▸ 요구사항: \"복원력 높이기\" + \"RTO 최소화\"\n▸ \"RTO 최소화\" → 즉시 자동 장애 조치(failover) 필수\n▸ 백업/복원: 시간 소요 (RTO 증가) → 부적절\n▸ Multi-AZ Replication: 자동 failover, 다운타임 최소\n▸ 동기식 복제 → 데이터 손실 없음 (RPO ≈ 0), 빠른 복구\n▸ Redis Multi-AZ: 자동 장애 조치, ElastiCache 고가용성 표준\n\n【오답 체크】\n(A) Memcached 교체: Memcached는 영속성 없음, 캐시 복구 불가, 데이터 손실 위험.\n(B) EventBridge + 매시간 백업: 백업 간 데이터 손실 가능, RTO 길어짐.\n(D) 자동 백업 + 복원: RTO 길어짐, 데이터 손실 가능.\n\n【시험 포인트】\n\"RTO 최소화\" → 백업/복원 패턴 피하기. \"높은 복원력 + 낮은 RTO\" → Multi-AZ, 읽기 복제본, 자동 failover. Redis Multi-AZ는 ElastiCache 고가용성의 표준.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ CloudFormation 드리프트 (Drift) — CloudFormation 템플릿 정의 상태와 실제 리소스 상태의 불일치\n▸ Drift Detection — CloudFormation 스택이 템플릿과 동기화되지 않은 상태 감지\n▸ EventBridge — AWS 이벤트 기반 자동화 서비스 (스케줄, 규칙)\n▸ Lambda — 서버리스 함수 실행\n▸ SNS — 이메일/SMS/HTTP 알림 서비스\n\n【정답 포인트】\n▸ 요구사항: \"CloudFormation 배포 후 정책 변경 감지\"\n▸ \"가장 적은 관리 노력\" → CloudFormation 네이티브 기능 활용\n▸ 솔루션: CloudFormation Drift Detection 활용\n▸ 구현: EventBridge 규칙(스케줄) → Lambda 함수 → CheckStackDrift API → 드리프트 감지 → SNS 알림\n▸ 드리프트 감지: 수정됨, 삭제됨, 차이 있음 등 상태 구분\n\n【오답 체크】\n(A) CloudTrail 이메일: CloudTrail은 로깅 서비스, 이메일 알림 기능 없음.\n(C) Access Analyzer: IAM 정책 분석/생성 도구, Drift 감지 기능 아님.\n(D) S3 JSON 비교: 수동 비교, 자동화 불가능, 관리 노력 증가.\n\n【시험 포인트】\nCloudFormation 정책 변경 감지 = Drift Detection + EventBridge 스케줄 조합. CloudFormation Drift는 \"템플릿 정의 vs 실제 상태\" 비교하는 표준 기능. Access Analyzer는 정책 분석 도구이지, Drift 감지 도구 아님.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ VPC 엔드포인트 (VPC Endpoint) — AWS 서비스에 대한 비공개 연결\n▸ 인터페이스 엔드포인트 (Interface Endpoint) — ENI 기반, 프라이빗 IP 주소로 접근\n▸ 게이트웨이 엔드포인트 (Gateway Endpoint) — S3/DynamoDB 전용, 라우팅 테이블 기반\n▸ PrivateLink — AWS 서비스에 대한 비공개 액세스 기술\n▸ VPC 격리 — 데이터가 VPC 네트워크 경계 내에 유지\n\n【정답 포인트】\n▸ 요구사항: \"모든 데이터 액세스가 VPC 네트워크 경계 내에 유지\"\n▸ S3 버킷: QuickSight의 데이터 소스\n▸ 문제: 인터넷을 통한 공개 연결은 보안 위험\n▸ 해결책: VPC 엔드포인트 구성\n▸ 필요한 엔드포인트: S3 (Gateway Endpoint) + QuickSight API (Interface Endpoint)\n▸ 결과: 비공개 연결, VPC 경계 내 데이터 흐름, 인터넷 노출 없음\n\n【오답 체크】\n(다른 옵션들은 VPC 엔드포인트를 부분적으로만 구성하거나 다른 기술 제시)\n\n【시험 포인트】\nVPC 격리 + AWS 서비스 접근 = VPC 엔드포인트 활용. S3 Gateway (비용 없음, 라우팅) vs Interface (프라이빗 IP, 비용 있음) 차이 이해. QuickSight는 Interface Endpoint 필요.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAWS Config: AWS 리소스의 규정 준수를 자동으로 모니터링하고 규칙 위반을 감지하는 서비스. 자동 수정(Automatic Remediation): 비준수 리소스를 자동으로 복구하는 기능. AWS Systems Manager: AWS 리소스 관리 및 자동화를 위한 관리형 문서 제공. CloudWatch/EventBridge/Lambda: 이벤트 기반 자동화를 구현하는 서비스들.\n\n【정답 포인트】\nAWS Config의 자동 수정 기능은 AWS Systems Manager 문서를 활용하여 규칙 위반이 감지되는 즉시 비준수 리소스를 자동으로 수정합니다. 'AWS-DisableIncomingSSHOnPort22'는 SSH 접근 제한을 위해 AWS에서 공식 제공하는 관리형 문서로, Config 규칙 내에서 자동 수정 작업으로 설정하면 보안 그룹 규칙이 자동으로 수정됩니다. 이는 가장 운영 효율적이고 즉시 대응이 가능한 솔루션입니다.\n\n【오답 체크】\nA) CloudWatch 알람: 메트릭 기반 알람으로 Config 규칙 위반에 직접 대응하지 못하며 Lambda 함수 호출이 필요해 절차가 복잡함. C) EventBridge: Config 변경 이벤트로 Lambda를 호출하는 방식으로 구현이 복잡하고 Config 내장 자동 수정 기능보다 효율성이 낮음. D) EventBridge 스케줄: 시간 기반 실행으로 비준수 리소스에 대한 실시간 수정이 불가능하고 최대 1시간의 지연 발생.\n\n【시험 포인트】\nAWS Config의 자동 수정 기능, AWS Systems Manager 문서 활용, 규정 준수 자동화는 CloudOps 시험의 핵심 주제입니다. 특히 자동 수정 vs 수동 수정, 이벤트 기반 vs 시간 기반 자동화의 운영 효율성 차이 이해가 시험에서 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "CD",
    "explanation": "【핵심 용어】\nHTTP Layer 7 상태 코드: 애플리케이션 계층의 HTTP 응답 코드(200 OK, 404 Not Found, 500 Internal Server Error 등). VPC Flow Logs: OSI Layer 3/4 네트워크 트래픽 로그. CloudTrail: AWS 계정의 API 호출 로그. ALB/CloudFront: 애플리케이션 계층에서 작동하는 서비스들.\n\n【정답 포인트】\nHTTP Layer 7 상태 코드는 애플리케이션 계층의 HTTP 응답 정보입니다. ALB(Application Load Balancer)는 Layer 7에서 작동하므로 접근 로그에 HTTP 메서드, URI, 응답 상태 코드 등을 상세히 기록합니다. CloudFront는 CDN으로서 웹 배포 수준에서 모든 HTTP 요청과 응답을 처리하므로 CloudFront 접근 로그에도 HTTP 상태 코드가 포함됩니다. 따라서 HTTP 상태 코드를 조사하려면 ALB 로그와 CloudFront 로그를 확인해야 합니다.\n\n【오답 체크】\nA) VPC Flow Logs: OSI Layer 3/4의 네트워크 트래픽만 기록하므로 HTTP 상태 코드 정보를 포함하지 않음. B) CloudTrail: AWS API 호출 이력을 추적하는 서비스로 애플리케이션의 HTTP 응답 정보를 기록하지 않음. E) RDS 로그: 데이터베이스 활동만 기록하므로 HTTP 상태 코드와는 무관.\n\n【시험 포인트】\nOSI 계층별 로깅 소스의 정확한 이해가 중요합니다. 각 AWS 서비스가 기록하는 로그의 계층(Layer 3/4 vs Layer 7)과 내용을 정확히 구분하는 능력이 시험에서 빈번하게 출제되며, 특히 다층 아키텍처에서 문제 원인을 파악할 때 필수적입니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nCloudWatch 기본 모니터링: 5분 주기의 메트릭 수집. CloudWatch 자세한 모니터링: 1분 주기의 상세 메트릭 수집. 알람 임계값: CloudWatch 알람의 트리거 조건. EC2 Auto Recovery: 인스턴스의 자동 재시작 메커니즘.\n\n【정답 포인트】\nCloudWatch 기본 모니터링은 5분 간격으로 메트릭을 수집하는 반면, 자세한 모니터링(Detailed Monitoring)은 1분 간격으로 메트릭을 수집합니다. 문제에서 '2분 이상 지속될 때'라는 구체적인 시간 조건이 있으므로, 정확하게 감지하려면 1분 단위의 메트릭이 필수적입니다. 자세한 모니터링으로 CPU 사용률이 100%인 상태를 1분 단위로 감지할 수 있고, CloudWatch 알람이 임계값 초과 시 즉시 인스턴스 재시작 작업을 수행합니다.\n\n【오답 체크】\nA) 기본 모니터링(5분 주기): 2분 단위의 CPU 상태 변화를 정확히 포착할 수 없어 요구사항 미충족. C) Lambda 스케줄(2분): 실시간 조건 기반 감지가 아니라 단순 시간 기반 실행으로 CPU 문제와 무관하게 실행되어 비효율적. D) EC2 Health Check: 인스턴스 가용성을 확인하는 메커니즘으로 CPU 모니터링과 무관하여 CPU 100% 문제 감지 불가.\n\n【시험 포인트】\nCloudWatch 모니터링 주기(기본 5분 vs 자세한 1분)의 차이는 반드시 숙지해야 합니다. 시간 조건을 만족하는 모니터링 간격 선택, 알람 기반 자동 복구, 시간 기반 vs 조건 기반 트리거의 차이가 CloudOps 시험에서 자주 출제되는 중요 개념입니다.",
  ...
  },
  {
    ...
    "answer": "BD",
    "explanation": "【핵심 용어】\nIAM 사용자 그룹(User Group): 여러 사용자를 함께 관리하여 일괄적으로 정책을 적용할 수 있는 기능. 고객 관리형 정책(Customer Managed): 사용자가 직접 생성하고 관리하는 독립적인 IAM 정책. 정책 버전 관리(Policy Versioning): 정책 변경 후 이전 버전으로 롤백 가능한 기능. 서비스 연결 역할: AWS 서비스가 관리하는 특수 역할.\n\n【정답 포인트】\n여러 사용자에게 동일한 정책을 적용하면서 동시에 정책 버전을 관리해야 하는 요구사항을 충족하려면 두 가지 조건이 필수입니다. 첫째, 옵션 B의 IAM 사용자 그룹을 사용하면 그룹에 정책을 연결하여 그룹에 속한 모든 사용자가 자동으로 정책을 상속받습니다. 둘째, 옵션 D의 고객 관리형 정책을 생성하면 사용자가 정책을 직접 생성하고 관리할 수 있으며, 정책 버전 관리 기능을 지원하여 변경 후 이전 버전으로 롤백할 수 있습니다.\n\n【오답 체크】\nA) 서비스 연결 역할: AWS가 관리하는 특수한 역할로 사용자가 수정하거나 정책을 변경할 수 없음. C) AWS 관리형 정책: AWS가 직접 관리하고 업데이트하므로 사용자는 버전 제어를 할 수 없고 변경할 수 없음. E) 인라인 정책: 개별 사용자나 역할에만 적용되는 정책으로 다중 사용자 관리가 어렵고 버전 관리가 미지원.\n\n【시험 포인트】\nIAM 정책 유형별 특성(AWS 관리형, 고객 관리형, 인라인, 서비스 연결 역할) 이해가 필수입니다. 특히 다중 사용자 관리 시 IAM 그룹 활용, 정책 버전 관리를 통한 변경 사항 추적과 롤백, 정책 변경이 시험에서 빈번하게 출제되는 중요한 운영 관리 주제입니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nEC2 Auto Scaling: 수요에 따라 인스턴스 개수를 자동으로 증감하는 기능. Warm Pool: 미리 구성되고 초기화된 준비 인스턴스 풀. 부팅 시간(Bootstrap Time): 인스턴스 시작 후 애플리케이션이 사용 가능해질 때까지의 시간. 스케일아웃(Scale-out): 인스턴스 개수를 증가시키는 작업.\n\n【정답 포인트】\nWarm Pool은 EC2 Auto Scaling 그룹의 고급 기능으로, 미리 구성되고 초기화된 인스턴스들을 준비 상태로 유지했다가 스케일아웃이 필요할 때 빠르게 활성화합니다. 이를 통해 장시간 부팅 스크립트로 인한 시작 지연을 완전히 회피할 수 있으므로 응답 시간을 획기적으로 단축할 수 있습니다. 동시에 필요할 때만 인스턴스를 활성화하므로 항상 최소 인스턴스를 실행하는 경우보다 비용 효율적입니다.\n\n【오답 체크】\nA) 인스턴스 크기 증가: 더 큰 인스턴스는 부팅 시간을 단축하지 못함. B) 최소 인스턴스 증가: 항상 더 많은 리소스를 실행해야 하므로 과도한 프로비저닝으로 비용이 증가. C) 예측 스케일링: 미래의 부하를 예측하여 미리 스케일아웃하지만 각 인스턴스의 부팅 시간 자체는 단축하지 못함.\n\n【시험 포인트】\nEC2 Auto Scaling의 고급 기능(Warm Pool, 예측 스케일링, 타겟 트래킹)의 정확한 용도 구분이 중요합니다. 성능 요구사항과 비용 최적화를 동시에 만족하는 설계 패턴, 스케일링 전략의 성능 vs 비용 트레이드오프 이해가 CloudOps 시험에서 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\nAWS Lambda: 코드 실행 환경을 프로비저닝할 필요 없는 서버리스 컴퓨팅 서비스. 환경 변수: Lambda 함수의 구성 정보를 저장하는 메커니즘. AWS Secrets Manager: 민감한 정보를 안전하게 저장하고 회전하는 서비스. 보안: 로그에서 민감한 데이터 노출 방지.\n\n【정답 포인트】\nAWS Lambda 함수의 환경 변수에 민감한 정보(API 키, 데이터베이스 비밀번호)를 직접 저장하면 CloudWatch 로그와 X-Ray 추적에 평문으로 노출될 수 있으므로 심각한 보안 위험이 있습니다. AWS Secrets Manager나 Parameter Store를 사용하여 민감한 정보를 암호화하여 안전하게 관리하고, Lambda 함수는 필요한 시점에 이러한 서비스에서 값을 조회하는 것이 보안 모범 사례입니다. 이를 통해 로그 노출을 완전히 방지할 수 있습니다.\n\n【오답 체크】\n환경 변수에 직접 저장: CloudWatch 로그와 X-Ray에서 평문으로 추적 가능하고 노출 위험 높음. 코드에 하드코딩: 소스 코드 버전 관리 시스템에 노출될 수 있음. 암호화 미설정: 평문으로 저장되어 콘솔에서 볼 수 있음. Secrets Manager 대신 환경 변수: 보안 기준을 충족하지 못함.\n\n【시험 포인트】\nLambda 보안 모범 사례, Secrets Manager와 Parameter Store의 차이, 민감한 정보 관리 방식이 CloudOps 시험의 중요한 보안 주제입니다. 특히 서버리스 애플리케이션의 구성 관리와 보안을 동시에 고려하는 설계가 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\nAmazon S3 버킷 정책: 버킷 수준에서의 접근 제어 정책. IAM 정책: 사용자나 역할 수준에서의 접근 제어. 명시적 거부(Explicit Deny): 모든 허용 권한을 override하는 거부 명령. 효과적인 권한: 모든 정책의 조합 결과.\n\n【정답 포인트】\nAmazon S3의 접근 제어는 IAM 정책과 버킷 정책이 함께 평가됩니다. AWS의 기본 거부 원칙(Explicit Deny)에 따라 명시적 거부(Deny)가 모든 명시적 허용(Allow)보다 우선합니다. IAM 정책에서 S3 객체 접근을 허용하더라도, S3 버킷 정책이나 객체 ACL에서 명시적으로 거부하면 결국 접근이 완전히 거부됩니다. 따라서 효과적인 권한을 정확히 판단하려면 IAM, 버킷 정책, 객체 ACL을 모두 검토해야 합니다.\n\n【오답 체크】\nIAM 정책만으로 충분: S3 버킷 정책도 함께 고려해야 함. S3 버킷 정책만 적용: IAM 정책도 동시에 평가됨. 최신 정책이 우선: 명시적 거부가 최우선 순위. 권한 조합 규칙: 모든 정책이 동시에 평가되고 명시적 거부가 최우선.\n\n【시험 포인트】\nS3 접근 제어, IAM vs 버킷 정책, 명시적 거부 원칙이 CloudOps 시험의 필수 보안 주제입니다. 복잡한 권한 시나리오에서 효과적인 권한을 정확히 판단하는 능력이 자주 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "BD",
    "explanation": "【핵심 용어】\nAmazon CloudWatch 로그: 애플리케이션과 시스템 로그를 중앙에서 저장하는 서비스. 필터 패턴: 특정 텍스트나 패턴을 포함하는 로그 메시지를 검색하는 기능. 로그 그룹: 관련 있는 로그들을 논리적으로 모으는 단위. CloudWatch Logs Insights: 고급 쿼리 및 분석 기능.\n\n【정답 포인트】\nAmazon CloudWatch의 필터 패턴을 사용하여 특정 텍스트(ERROR, Exception, WARN 등)를 포함하는 로그를 효율적으로 검색할 수 있습니다. 필터 패턴은 정규표현식이나 간단한 텍스트 매칭을 지원하며, CloudWatch Logs Insights를 사용하면 더 복잡한 쿼리도 가능합니다. 필터 패턴은 모든 로그 스트림을 빠르게 검색하여 일치하는 항목을 즉시 반환합니다. 로그를 다운로드하거나 S3로 내보내는 방식보다 훨씬 효율적입니다.\n\n【오답 체크】\n전체 로그를 다운로드: 비효율적이고 매우 많은 시간이 소모됨. CloudTrail 로그 사용: AWS API 호출 로그로 애플리케이션 로그와 무관. S3로 내보내기: 필터링 없이 모든 로그를 저장만 함. 실시간 모니터링 불가능: 필터 패턴으로 즉시 검색 가능.\n\n【시험 포인트】\nCloudWatch 로그 관리, 필터 패턴을 통한 로그 분석, Logs Insights 활용이 CloudOps 시험의 중요한 주제입니다. 대규모 로그 환경에서 효율적인 검색과 모니터링 방법이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAmazon RDS 자동 백업: 매일 자동으로 생성되는 백업(기본 1~35일 보관). Point-in-Time Restore: 보관 기간 내의 어느 시점으로든 데이터베이스를 복구하는 기능. 스냅샷: 특정 시점의 데이터베이스 상태를 수동으로 캡처하는 기능. 백업 보관 기간: 복구 가능한 시간 범위.\n\n【정답 포인트】\nAmazon RDS의 자동 백업을 35일로 설정하면 최근 35일 이내의 어느 시점으로도 데이터베이스를 복구할 수 있습니다. Point-in-Time Restore는 우발적인 데이터 삭제나 손상된 데이터로부터 복구하는 가장 효과적이고 유연한 방법입니다. 백업 보관 기간은 데이터 보호 요구사항과 스토리지 비용 간의 균형을 고려하여 설정합니다. 스냅샷은 특정 시점만 보관하므로 복구 유연성이 매우 낮습니다.\n\n【오답 체크】\n자동 백업 비활성화: 백업이 없으면 복구 불가능함. 백업 보관 기간 1일: 복구 가능한 기간이 너무 짧음. 스냅샷만 사용: 특정 시점에 대해서만 복구 가능. 백업 암호화 미설정: 보안 규정 요구사항을 충족하지 못함.\n\n【시험 포인트】\nAmazon RDS 백업 전략, Point-in-Time Restore의 중요성, 자동 백업 vs 수동 스냅샷의 차이가 CloudOps 시험의 핵심 주제입니다. 비즈니스 연속성과 재해 복구 요구사항에 맞춘 백업 정책이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAmazon VPC 보안 그룹: 인스턴스 수준의 상태 저장 방화벽. 인바운드 규칙: 인스턴스로 들어오는 트래픽 제어. 아웃바운드 규칙: 인스턴스에서 나가는 트래픽 제어. 최소 권한 원칙: 필요한 접근만 허용하는 보안 원칙.\n\n【정답 포인트】\n보안 그룹은 인스턴스 수준의 상태 저장 방화벽으로, 명시된 규칙에 해당하는 트래픽만 허용합니다. 기본적으로 모든 인바운드 트래픽을 거부하고, HTTP(80), HTTPS(443), SSH(22) 등 필요한 포트와 프로토콜만 개방하는 규칙을 추가합니다. 아웃바운드는 기본적으로 모든 트래픽을 허용하지만, 필요시 특정 트래픽만 제한할 수 있습니다. 최소 권한 원칙에 따라 필요한 포트만 개방하여 보안을 강화합니다.\n\n【오답 체크】\n모든 포트를 개방: 심각한 보안 위협 발생. SSH에 대해 무제한 허용: 비인증 접근 시도에 노출. 규칙 순서가 중요: 보안 그룹은 규칙 순서와 무관하게 논리적 OR로 동작. 네트워크 ACL이 항상 필수: 보안 그룹으로 충분한 경우가 많음.\n\n【시험 포인트】\nAmazon VPC 보안 그룹 설정, 최소 권한 기반 접근 제어, 보안 그룹과 네트워크 ACL의 차이가 CloudOps 시험의 필수 보안 주제입니다. 실무 시나리오에서 적절한 보안 그룹 정책을 수립하는 능력이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAWS Systems Manager Parameter Store: 애플리케이션 구성 정보와 민감한 데이터를 안전하게 저장하는 서비스. SecureString: KMS를 사용하여 암호화된 민감 정보(비밀번호, API 키)를 저장하는 매개변수 타입. KMS 암호화: AWS Key Management Service를 사용한 고급 암호화. API 조회: 실행 시간에 Parameter Store에서 값을 동적으로 조회.\n\n【정답 포인트】\nAWS Systems Manager Parameter Store의 SecureString 타입은 민감한 정보(데이터베이스 비밀번호, API 키, 접근 토큰)를 AWS KMS를 사용하여 암호화하여 안전하게 저장합니다. 기본적으로 AWS 관리 KMS 마스터 키를 사용하여 암호화하며, 필요시 고객 관리 KMS 키를 지정할 수도 있습니다. 애플리케이션은 Parameter Store API를 호출하여 필요한 값을 동적으로 조회하고, AWS IAM 권한으로 접근을 엄격히 제어할 수 있습니다.\n\n【오답 체크】\n환경 변수에 저장: CloudWatch 로그와 배포 파이프라인에 평문으로 노출될 수 있음. 애플리케이션 코드에 하드코딩: 소스 코드 버전 관리에 민감 정보가 노출됨. 암호화 없이 저장: 평문으로 저장되어 접근 제어만으로 보호 부족. Secrets Manager 불필요: Parameter Store와 다른 용도이며 선택적.\n\n【시험 포인트】\nAWS Systems Manager Parameter Store 사용, SecureString 암호화, KMS 기반 민감 정보 관리가 CloudOps 시험의 중요한 보안 주제입니다. 애플리케이션 구성 관리와 보안을 동시에 고려하는 설계가 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAWS CloudTrail: 모든 AWS API 호출과 계정 활동을 기록하는 감사 로그 서비스. 이벤트 히스토리: 최근 90일간의 API 활동을 기본으로 보관하는 기능. 조직 추적(Organization Trail): AWS Organizations의 모든 계정과 리전에 걸친 활동을 통합 추적. 로그 파일 검증: CloudTrail 로그의 무결성을 암호화하여 보증.\n\n【정답 포인트】\nAWS CloudTrail은 모든 AWS API 호출의 상세한 이력을 기록하며, 기본적으로 최근 90일의 이벤트 히스토리를 제공합니다. 장기 보관이 필요한 경우 CloudTrail을 S3 버킷에 로깅하도록 구성하면 규정 준수 요구사항을 효과적으로 충족할 수 있습니다. 조직 추적(Organization Trail)을 활성화하면 AWS Organizations에 속한 모든 계정의 API 활동을 중앙에서 통합적으로 모니터링할 수 있으며, 로그 파일 검증을 통해 로그의 무결성을 보장합니다.\n\n【오답 체크】\n90일만 보관: 장기 규정 준수 요구사항을 충족하려면 S3로의 로깅이 필수. 단일 계정만 추적: 조직 추적으로 모든 계정을 동시에 감시 가능. CloudWatch 로그와 혼동: CloudTrail은 API 호출, CloudWatch Logs는 애플리케이션 로그. 로그 파일 검증 미설정: 규정 준수 및 보안 감시 요구사항 미충족.\n\n【시험 포인트】\nAWS CloudTrail 구성, 감시 범위와 보관 기간, 다중 계정 환경에서의 통합 모니터링, 규정 준수 요구사항 충족이 CloudOps 시험의 핵심 보안 주제입니다. 감시 범위와 장기 보관 전략이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\nAmazon EC2 T 시리즈: CPU 사용이 낮을 때는 저렴하고 필요 시 부스트 성능을 제공하는 burstable 인스턴스. CPU 크레딧: T 시리즈의 부스트 성능을 제공하기 위해 누적되고 사용되는 가상 단위. 기본선(Baseline): T 시리즈 인스턴스의 기본 CPU 할당 수준. 부스트 성능: 필요할 때 CPU 크레딧을 사용하여 제공하는 고성능.\n\n【정답 포인트】\nAmazon EC2의 T 시리즈 인스턴스는 burstable 성능을 제공하여 CPU 사용량이 기본선 이하일 때는 저렴하게 운영하고, 부하가 증가하면 CPU를 부스트할 수 있습니다. 낮은 CPU 사용률 기간에 CPU 크레딧을 누적하고, 높은 성능이 필요한 경우 누적된 크레딧을 사용하여 부스트 성능을 제공합니다. 크레딧이 부족하면 성능이 기본선 수준으로 제한되므로 지속적으로 높은 CPU가 필요한 워크로드에는 제한적입니다.\n\n【오답 체크】\n항상 높은 성능 제공: CPU 크레딧이 소진되면 기본선으로 제한됨. 모든 워크로드에 비용 효율적: 지속적으로 높은 CPU 필요 시 M/C 시리즈가 효율적. 크레딧 관리 불필요: 크레딧 소진 모니터링이 필수. 무제한 부스트 제공: 불가능하며 누적 크레딧에 기반.\n\n【시험 포인트】\nAmazon EC2 T 시리즈의 burstable 성능 모델, CPU 크레딧 관리의 중요성, 워크로드 특성에 따른 인스턴스 타입 선택이 CloudOps 시험의 주요 주제입니다. 비용과 성능의 적절한 균형을 맞추는 설계가 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAWS Elastic Beanstalk: 웹 애플리케이션과 서비스를 쉽게 배포하고 스케일링할 수 있는 완전 관리형 플랫폼. 환경: 애플리케이션의 배포 대상이 되는 AWS 리소스의 모음. 애플리케이션 버전: 특정 시점의 애플리케이션 코드 스냅샷. 배포 정책: 무중단 배포와 최소화된 다운타임을 위한 배포 방식.\n\n【정답 포인트】\nAWS Elastic Beanstalk는 구성 관리 없이 애플리케이션을 간단하게 배포할 수 있는 완전 관리형 플랫폼입니다. 애플리케이션을 업로드하면 필요한 EC2, RDS, Load Balancer 등의 기본 인프라가 자동으로 프로비저닝됩니다. 다양한 배포 정책(All at once, Rolling, Rolling with additional batch, Immutable, Blue/Green)을 지원하여 무중단 배포를 손쉽게 구현할 수 있으며, 애플리케이션 버전과 환경 설정을 버전 관리하므로 이전 버전으로의 빠른 롤백이 가능합니다.\n\n【오답 체크】\n완전 자동화만 가능: 세부적인 커스터마이제이션도 가능함. 단일 배포 방식만: 다양한 배포 정책으로 유연한 배포 지원. 롤백 불가능: 버전 관리로 쉬운 롤백 제공. 모니터링 불필요: CloudWatch와 통합되어 모니터링 가능.\n\n【시험 포인트】\nAWS Elastic Beanstalk의 배포 전략, 무중단 배포 구현, 애플리케이션 버전 관리와 롤백이 CloudOps 시험의 중요한 주제입니다. 빠른 배포와 운영상 편의성이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\nAWS Application Auto Scaling: CloudWatch 메트릭을 기반으로 AWS 리소스의 용량을 자동으로 조정하는 서비스. 타겟 트래킹 스케일링: 지정된 메트릭을 목표값 근처에서 유지하도록 자동 조정. 단계별 스케일링(Step Scaling): 메트릭의 임계값을 초과할 때 단계별로 리소스를 추가. 조정 냉각 시간: 스케일링 활동 후 다음 조정 전까지의 대기 시간.\n\n【정답 포인트】\nAWS Auto Scaling은 CloudWatch 메트릭을 실시간으로 모니터링하여 리소스를 자동으로 조정합니다. 타겟 트래킹 스케일링은 CPU 사용률이나 ALB 요청 수 등의 지정된 메트릭이 목표값 근처를 유지하도록 자동으로 용량을 조정합니다. 단계별 스케일링은 메트릭이 임계값을 초과할 때마다 단계별로 리소스를 추가합니다. 조정 냉각 시간을 설정하여 빈번한 확장과 축소로 인한 불안정성을 방지할 수 있습니다.\n\n【오답 체크】\n수동 조정만 가능: Auto Scaling으로 자동 조정 가능함. 고정 용량 유지: 가변 워크로드에는 비효율적. 메트릭 무관: 메트릭 기반 결정이 가장 효율적. 스케일링 즉시 반영: 냉각 시간으로 안정성과 비용 최적화 확보.\n\n【시험 포인트】\nAWS Auto Scaling의 정책(타겟 트래킹, 단계별 스케일링), 냉각 시간의 역할, 메트릭 선택이 CloudOps 시험의 핵심 주제입니다. 비용 최적화와 성능 유지의 균형이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAmazon DynamoDB: 완전 관리형 NoSQL 데이터베이스 서비스. 온디맨드 청구 모드: 실제 사용량에 따라 비용을 지불하는 방식. 프로비저닝 청구 모드: 사전에 읽기/쓰기 용량 단위를 예약하는 방식. 자동 스케일링: 프로비저닝 모드에서 트래픽에 따라 용량을 자동으로 조정하는 기능.\n\n【정답 포인트】\nAmazon DynamoDB는 온디맨드와 프로비저닝 두 가지 청구 모드를 제공합니다. 온디맨드 모드는 사용량만큼 비용을 지불하므로 트래픽 패턴을 예측하기 어려운 애플리케이션에 적합합니다. 프로비저닝 모드는 읽기/쓰기 용량을 사전에 예약하므로 트래픽이 예측 가능한 경우 더 비용 효율적입니다. 프로비저닝 모드에서 자동 스케일링을 활성화하면 트래픽에 따라 용량이 자동으로 조정되어 두 모드의 장점을 함께 얻을 수 있습니다.\n\n【오답 체크】\n온디맨드 모드만: 프로비저닝 모드도 비용 효율적인 경우 많음. 항상 비용 높음: 트래픽 패턴에 따라 적절한 모드 선택 필요. 스케일링 불가능: 자동 스케일링으로 유연성 확보. 완전 관리형 불가: DynamoDB는 완전 관리형 서비스.\n\n【시험 포인트】\nAmazon DynamoDB의 청구 모델, 성능 특성, 비용 최적화 전략이 CloudOps 시험의 중요한 주제입니다. 워크로드 특성에 따른 적절한 모드 선택과 스케일링 전략이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAWS Lambda: 코드 업로드만으로 자동 스케일링되는 서버리스 컴퓨팅 서비스. 콜드 스타트: 함수가 처음 호출될 때 초기 환경 준비로 인한 초기 지연. 메모리 할당량: Lambda 함수에 할당되는 메모리로 CPU도 함께 증가. 임시 스토리지(/tmp): Lambda 함수 실행 중 사용할 수 있는 임시 저장소(512MB 제한).\n\n【정답 포인트】\nAWS Lambda 함수의 성능은 할당된 메모리에 정비례합니다. 메모리 할당량이 높을수록 CPU 리소스도 함께 증가하여 코드 실행 속도가 빨라지고, 결과적으로 콜드 스타트 시간과 전체 실행 시간이 모두 단축됩니다. 임시 스토리지(/tmp)는 512MB까지 사용 가능하며, 함수 실행 중 임시 파일이나 캐시 데이터를 저장할 수 있습니다. 콜드 스타트는 완전히 회피할 수는 없지만 메모리 증가와 코드 최적화로 영향을 최소화할 수 있습니다.\n\n【오답 체크】\n콜드 스타트 완전 회피: 최소화만 가능하며 회피 불가능. 메모리와 성능 무관: 메모리 높으면 CPU도 증가하여 성능 향상. 임시 스토리지 무제한: 512MB 제약이 있으므로 관리 필요. 함수 크기 무관: 코드와 디펜던시 최소화로 콜드 스타트 단축.\n\n【시험 포인트】\nAWS Lambda의 성능 최적화, 메모리 설정의 영향, 콜드 스타트 완화 방법이 CloudOps 시험의 서버리스 주제입니다. 비용과 성능의 트레이드오프를 이해하는 서버리스 애플리케이션 설계가 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\nAmazon S3: 객체 저장소 서비스. 버전 관리: 각 객체의 모든 버전을 유지하여 이전 버전 복구 지원. 라이프사이클 정책: 일정 기간 후 자동으로 객체를 다른 스토리지 클래스로 이동하거나 삭제하는 규칙. 스토리지 클래스: 접근 빈도에 따른 가격 등급(Standard, IA, Glacier, Deep Archive). MFA Delete: 버전 삭제 시 추가 인증을 요구하는 보안 기능.\n\n【정답 포인트】\nAmazon S3 버전 관리를 활성화하면 각 객체의 모든 버전이 유지되므로 실수로 삭제된 파일을 언제든지 복구할 수 있습니다. 라이프사이클 정책을 사용하여 오래된 버전을 저렴한 스토리지 클래스(Glacier, Deep Archive)로 자동 이동시키거나 삭제할 수 있으므로 데이터 보호와 비용 최적화를 동시에 달성합니다. MFA Delete를 활성화하면 버전 삭제 시 추가 인증을 요구하여 보안을 크게 강화할 수 있습니다.\n\n【오답 체크】\n버전 관리 미사용: 실수 삭제 시 복구 불가능. 라이프사이클 정책 미설정: 저가 스토리지로의 자동 이동 불가능하여 비용 낭비. 모든 버전을 Standard에 보관: 불필요한 비용 증가로 최적화 실패. MFA Delete 선택: 보안 규정 요구사항에 따라 필수 설정.\n\n【시험 포인트】\nAmazon S3 버전 관리, 라이프사이클 정책, 스토리지 클래스 최적화, MFA Delete 보안이 CloudOps 시험의 중요한 주제입니다. 데이터 보호와 비용 효율성의 균형을 맞추는 S3 전략이 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\nAWS CodeDeploy: 온프레미스 및 클라우드 인스턴스로의 배포를 자동화하는 서비스. AppSpec 파일: 배포 과정에서 실행할 스크립트와 파일 매핑을 정의하는 설정 파일. 배포 그룹: 배포 대상이 되는 인스턴스들의 논리적 모음. 배포 설정: 한 번에 배포할 인스턴스 수와 성공 기준을 정의하는 정책.\n\n【정답 포인트】\nAWS CodeDeploy는 온프레미스 및 AWS 클라우드 인스턴스로 자동화된 배포를 제공합니다. AppSpec 파일(appspec.yaml)에 배포 전후 실행할 스크립트, 파일 매핑, 라이프사이클 이벤트를 정의합니다. 배포 설정으로 한 번에 배포할 인스턴스 수와 성공 기준을 정의하여 무중단 배포를 구현할 수 있습니다. CodePipeline과 통합하면 지속적 배포(CD) 파이프라인을 완성할 수 있습니다.\n\n【오답 체크】\n수동 배포만: CodeDeploy로 자동화 가능하여 인적 오류 감소. 배포 설정 불필요: 무중단 배포와 성공 기준 정의에 필수. AppSpec 파일 선택: 배포 지침 정의에 필수적. 실패 처리 없음: 자동 롤백 정책 설정 가능.\n\n【시험 포인트】\nAWS CodeDeploy를 통한 배포 자동화, AppSpec 파일 구성, 무중단 배포 전략이 CloudOps 시험의 DevOps 핵심 주제입니다. CI/CD 파이프라인 구축과 배포 자동화가 빈번하게 출제됩니다.",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ EC2 Instance Connect Endpoint — 프라이빗 EC2 인스턴스에 SSH/RDP 접근을 위해 Bastion 호스트 없이 인터넷 노출 없이 안전하게 연결 가능한 VPC 엔드포인트\n▸ Systems Manager VPC Endpoint — AWS Systems Manager 서비스와 통신하기 위한 VPC 엔드포인트\n▸ 아웃바운드 트래픽 보안 그룹 — 인바운드만 제한하고 아웃바운드만 허용하는 규칙\n\n【정답 포인트】\n▸ 인터넷 노출 없이 SSH 접근 → EC2 Instance Connect Endpoint 필수 선택\n▸ EC2 Instance Connect Endpoint는 프라이빗 서브넷에 배포되어야 함 (클라이언트와 같은 VPC 내 통신)\n▸ 보안 그룹을 인바운드 SSH 트래픽(포트 22) 허용하도록 업데이트 필수\n▸ PowerUserAccess는 과도한 권한이지만 EC2 Instance Connect 자체 접근에는 불필요 (실제로는 IAM 권한 불필요)\n▸ 정답의 핵심: 프라이빗 서브넷의 EC2 Instance Connect Endpoint + 보안 그룹 인바운드 설정\n\n【오답 체크】\n(B) Systems Manager Endpoint는 Systems Manager 서비스 통신용으로, SSH 직접 접근 메커니즘이 아님. Systems Manager는 Session Manager를 통한 간접 접근만 가능\n(C) 공개 서브넷의 EC2 Instance Connect Endpoint는 인스턴스를 인터넷에 노출할 위험 증가. 엔드포인트는 프라이빗 서브넷에 배포되어야 함\n(D) 공개 서브넷의 Systems Manager Endpoint와 Session Manager는 SSH 직접 접근이 아니라 Systems Manager 세션을 통한 셸 접근\n\n【시험 포인트】\n▸ VPC Endpoint 종류 구분 능력: EC2 Instance Connect vs Systems Manager\n▸ 엔드포인트 배포 위치의 중요성: 프라이빗 vs 공개 서브넷 선택\n▸ 보안 그룹 인바운드/아웃바운드 규칙 이해\n▸ 최소 권한 원칙(Least Privilege) 적용 능력",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS Global Accelerator — 전역 사용자 트래픽을 가장 가까운 AWS 엣지 로케이션으로 라우팅하여 지연 시간을 최소화하는 서비스\n▸ Anycast IP — Global Accelerator가 사용하는 정적 IP로 전 세계 에지에서 동일한 IP로 트래픽 수용\n▸ Application Load Balancer(ALB) — OSI 레이어 7 애플리케이션 계층에서 작동하는 로드 밸런서\n▸ Network Load Balancer(NLB) — OSI 레이어 4 전송 계층에서 초고성능 처리를 제공하는 로드 밸런서\n\n【정답 포인트】\n▸ 북미 외부 사용자의 높은 지연 시간 문제 → 지리적으로 분산된 전역 성능 개선이 핵심 필요\n▸ AWS Global Accelerator는 사용자와 가장 가까운 엣지 로케이션으로 트래픽을 우선 라우팅\n▸ ALB 앞에 Global Accelerator를 배치하면 레이어 7 기능은 유지하면서 전역 성능 향상\n▸ 단일 리전(us-east-1)에서도 AWS의 글로벌 네트워크 인프라를 활용하여 지연 시간 대폭 감소\n▸ 정답의 핵심: ALB의 기능 유지 + Global Accelerator의 전역 최적화\n\n【오답 체크】\n(B) NLB는 ALB보다 레이어 4에서 더 빠른 처리를 제공하지만, 지연 시간 문제의 원인(지리적 거리)은 해결 불가능\n(C) NLB로의 교체는 레이어 7 기능 손실(경로 기반 라우팅 등) + 전역 성능 개선 없음\n(D) Route 53 상태 확인 + 장애 조치는 다중 리전 아키텍처가 필수. 현재 단일 리전 시나리오에서 해결책이 아님\n\n【시험 포인트】\n▸ Global Accelerator vs CloudFront: 지연 시간 최소화는 Global Accelerator, 콘텐츠 캐싱은 CloudFront\n▸ 단일 리전 애플리케이션의 전역 성능 개선 전략\n▸ ALB와 Global Accelerator의 결합으로 레이어 7 기능 유지 + 전역 성능 향상 극대화",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ AWS Organizations Tag Policy — AWS Organizations에서 다중 계정 전체에 필수 태깅 규칙을 강제하는 정책\n▸ Cost Allocation Tags — AWS Billing and Cost Management에서 비용 추적 및 할당을 위해 활성화하는 태그\n▸ 비용 할당 — 태그 기반으로 사용 비용을 부서별/프로젝트별로 추적하고 청구하는 기능\n▸ 운영 오버헤드 — 관리 작업에 소요되는 시간과 리소스\n\n【정답 포인트】\n▸ 다중 계정 환경에서 일관된 태깅 강제 → AWS Organizations Tag Policy 사용 (중앙 집중식 제어)\n▸ Organizations Tag Policy는 아래쪽 조직 단위(OU)의 모든 리소스에 필수 태깅 자동 적용\n▸ Cost Allocation Tags를 활성화하면 태그 기반으로 비용을 자동으로 추적 및 할당\n▸ 수동 개입 없이 정책 기반 자동 태깅 → 운영 오버헤드 최소\n▸ 정답의 핵심: Organizations 정책 + Cost Allocation Tag 활성화로 자동화된 일관된 태깅\n\n【오답 체크】\n(B) Lambda 자동화는 CloudTrail 이벤트 감시 + Lambda 함수 개발/관리/모니터링 필요 → 운영 오버헤드 크게 증가\n(C) AWS Config는 태깅 준수 현황 모니터링만 가능하고, 필수 태깅 강제나 자동 적용 기능 없음. AWS Budgets는 비용 할당 불가능\n(D) Service Catalog는 프로비저닝 템플릿을 통한 사전 태깅만 가능하며, 기존 리소스나 다른 방법의 생성 리소스 제어 불가. Trusted Advisor는 권장사항만 제공\n\n【시험 포인트】\n▸ AWS Organizations 기능: 태그 정책(Tag Policy) vs 서비스 제어 정책(SCP) 구분\n▸ Cost Allocation Tags 활성화의 필수성과 비용 추적과의 연관성\n▸ 다중 계정 환경에서의 중앙 집중식 제어와 자동화 전략",
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "【핵심 용어】\n▸ AWS Systems Manager Agent(SSM Agent) — EC2 인스턴스를 Systems Manager로 관리하기 위해 설치되는 소프트웨어 에이전트\n▸ Systems Manager Patch Manager — AWS Systems Manager의 기능으로 EC2 인스턴스의 OS 패치를 중앙에서 자동으로 관리\n▸ 패치 스케줄 — 주간/월간 등 정기적으로 패치를 자동 설치하도록 구성하는 기능\n▸ 규정 준수 추적 — 어느 인스턴스가 패치되었는지, 어떤 패치가 설치되었는지 기록하고 보고\n\n【정답 포인트】\n▸ EC2 인스턴스 자동 패칭 → AWS Systems Manager Patch Manager 사용 (AWS 표준 솔루션)\n▸ SSM Agent는 Amazon Linux 2에 기본 포함되지만, Ubuntu는 별도 설치 필요\n▸ Patch Manager는 매주 패치 스케줄 설정으로 자동 설치 가능\n▸ 새로운 인스턴스 추가 시 SSM Agent 설치만으로 자동으로 Patch Manager의 패칭 대상에 포함 가능\n▸ 패치 결과 보고, 비준수 인스턴스 추적 등 운영 기능 제공\n▸ 정답의 핵심: SSM Agent + Patch Manager로 가장 간편하고 확장 가능한 자동화\n\n【오답 체크】\n(A) Lambda는 SSH 연결 설정, 인증서 관리, 에러 처리, 로깅 등 추가 개발 필요 → 복잡성과 유지보수 부담 증가. SSH 접근 실패 시 대응 메커니즘 필요\n(C) AWS Systems Manager Inventory는 리소스 메타데이터 조회 목적(설치된 애플리케이션, 네트워크 설정 등)이며, 패치 설치 자동화 기능 없음\n(D) EventBridge는 스케줄 기반 이벤트 트리거만 가능하고, 실제 패치 설치 명령 실행 메커니즘이 불명확하며 복잡한 설정 필요\n\n【시험 포인트】\n▸ AWS Systems Manager 서비스군: Patch Manager(패칭), Session Manager(대화형 세션), Run Command(일회성 명령) 용도 구분\n▸ SSM Agent의 필수성과 OS별 설치 방법(Amazon Linux 기본 포함, Ubuntu 수동 설치)\n▸ AWS 관리형 패칭 솔루션으로 운영 오버헤드 최소화 및 규정 준수 추적",
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "【핵심 용어】\n▸ RTO(Recovery Time Objective) — 목표 복구 시간: 재해 발생 후 서비스 복구까지 허용되는 최대 시간(10분 미만)\n▸ RPO(Recovery Point Objective) — 목표 복구 지점: 허용 가능한 최대 데이터 손실량(0 = 데이터 손실 없음)\n▸ Active-Active Architecture — 양쪽 리전이 동시에 트래픽을 처리하는 완전 활성 상태\n▸ 실시간 데이터 복제 — 데이터 변경이 발생하는 즉시 다른 리전으로 동기화되는 복제\n\n【정답 포인트】\n▸ RPO = 0 → 데이터 손실이 허용되지 않으므로 실시간 데이터 동기 복제 필수\n▸ RTO < 10분 → 이미 활성화된 리소스가 즉시 트래픽을 수용할 수 있어야 하므로 Active-Active 필수\n▸ Active-Active: 양쪽 리전(us-east-1 + 두 번째 리전)이 동시에 완전 활성화되어 있음\n▸ Route 53 가중치 라우팅: 양쪽 엔드포인트로 트래픽을 분산(예: 50:50)\n▸ 장애 발생 시: 현재 지역이 다운되어도 다른 지역이 즉시 트래픽 처리 가능\n▸ 정답의 핵심: RPO 0 + RTO 10분 미만을 동시에 충족하려면 Active-Active만 가능\n\n【오답 체크】\n(A) 파일럿 라이트: 리소스가 최소화되어 있어 복구(리소스 스케일업)에 시간 필요 → RTO 10분 이상 소요. RPO도 정기 복제만 가능하므로 RPO 0 달성 어려움\n(B) 웜 스탠바이: 리소스가 부분적으로 실행 중이므로 복구 시간 단축되지만 여전히 RTO 10분 이상. 정기적 복제라면 RPO = 0 불가능 (복제 간격 동안 데이터 손실)\n(D) 정기 백업: 백업 생성 간격이 있으므로 RPO = 0 불가능. 복구 시 백업에서 두 번째 리전으로 복원하는 과정 필요 → RTO 10분 이상 소요\n\n【시험 포인트】\n▸ 재해 복구 아키텍처 4가지: 백업/복원(가장 저비용, 높은 RTO/RPO), 파일럿 라이트, 웜 스탠바이, Active-Active(최고 비용, 최소 RTO/RPO)\n▸ RTO/RPO 요구사항 분석으로 적절한 아키텍처 선택\n▸ Active-Active 필요 조건: 데이터 동기화, 양쪽 지역 활성화, DNS 가중치 라우팅\n▸ 비용 vs 복구 효율성 트레이드오프 이해",
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "【핵심 용어】\n▸ CloudWatch Application Signals — AWS의 관리형 APM(Application Performance Monitoring) 서비스로 메트릭, 로그, 트레이스를 자동으로 수집하고 상호 연관\n▸ RED 메트릭 — Rate(트래픽), Error(오류), Duration(지연 시간) 측정 방식\n▸ SLO/SLI — SLO(서비스 수준 목표: 달성하려는 성능 목표), SLI(서비스 수준 지표: 실제 측정값)\n▸ 자동 계측 — 애플리케이션 코드 수정 없이 메트릭을 자동으로 수집하는 기능\n\n【정답 포인트】\n▸ 지정된 메트릭(지연 시간, 트래픽, 오류, 포화도) 자동 수집 → CloudWatch Application Signals 최적 선택\n▸ Application Signals는 EKS 워크로드에 대해 자동 계측 없이 메트릭 수집 시작 가능\n▸ 메트릭, 로그, 트레이스를 자동으로 상호 연관(Correlation)하여 빠른 문제 해결 지원\n▸ SLO/SLI 정의 및 모니터링 기능이 Application Signals에 내장\n▸ AWS 완전 관리형 서비스로 인프라 관리 오버헤드 최소(Prometheus, Grafana 운영 불필요)\n▸ 정답의 핵심: Application Signals의 자동화와 통합 기능으로 가장 낮은 운영 노력\n\n【오답 체크】\n(B) AWS Distro for OpenTelemetry는 수동으로 애플리케이션에 계측(instrumentation) 코드 추가 필요. Prometheus + Grafana 조합은 3개 서비스를 별도로 설정/관리해야 하므로 운영 복잡성 크게 증가. SLO/SLI 모니터링을 위해 추가 도구 필요\n(C) CloudWatch RUM은 클라이언트 측(브라우저) 메트릭만 수집하므로 포화도 등 서버 측 메트릭 불가. Synthetics 카나리는 외부 엔드포인트 모니터링용으로 내부 메트릭 수집 불가\n(D) CloudWatch Application Insights는 문제 감지(이상 탐지)는 가능하지만 SLO/SLI 모니터링 기능은 제한적. 지정된 메트릭 수집과 트레이싱 기능 부족\n\n【시험 포인트】\n▸ CloudWatch Application Signals: EKS/EC2 애플리케이션의 자동 APM 솔루션\n▸ OpenTelemetry vs Application Signals: 자동화 vs 수동 계측 선택\n▸ 메트릭, 로그, 트레이스 상호 연관의 중요성과 빠른 문제 해결\n▸ 관리형 서비스(Application Signals) vs 자체 운영(Prometheus+Grafana) 트레이드오프",
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "【핵심 용어】\n▸ Multi-AZ Deployment — RDS 자동 장애 조치(Failover)를 위해 스탠바이 복제본을 다른 가용 영역에 동기적으로 유지하는 배포 방식\n▸ Backup Retention Period — 자동 백업을 보관하는 기간(최소 1일, 최대 35일, 0 = 비활성화)\n▸ 자동 백업 — RDS가 매일 지정된 백업 윈도우 동안 자동으로 생성하는 백업\n▸ 스탠바이 복제본 — Multi-AZ에서 자동 장애 조치를 위해 다른 AZ에 유지되는 동기 복제본\n\n【정답 포인트】\n▸ RDS Multi-AZ는 자동 장애 조치를 위해 스탠바이 복제본을 다른 AZ에 항상 유지하고 동기화 필요\n▸ Multi-AZ 환경에서는 스탠바이 복제본 복구를 위해 자동 백업이 반드시 필요\n▸ 따라서 백업 보존 기간을 0으로 설정(완전 비활성화) 불가능 → 최소 1일 이상 필수\n▸ \"보존 기간이 1과 35 사이여야 한다\" 오류는 Multi-AZ 상태에서 0으로 설정 시도했을 때 발생\n▸ 자동 백업을 완전히 비활성화하려면 먼저 Multi-AZ 배포를 비활성화해야 함\n▸ 정답의 핵심: Multi-AZ가 활성화되면 백업 보존이 자동으로 강제됨\n\n【오답 체크】\n(A) 권한 부족 문제라면 \"Access Denied\" 또는 \"Permission Denied\" 같은 다른 오류 메시지 표시. 보존 기간 범위 오류와는 다른 유형의 오류\n(B) 읽기 복제본은 독립적인 RDS 인스턴스이며, 원본 데이터베이스의 백업 정책에 영향을 주지 않음\n(C) 기본 백업 윈도우 사용은 보존 기간 설정과 독립적. 백업 윈도우는 언제 백업을 수행할지 결정하고, 보존 기간은 얼마나 오래 보관할지 결정\n\n【시험 포인트】\n▸ RDS Multi-AZ의 제약 사항: 백업 보존 최소값 강제, 스탠바이 복제본 자동 유지\n▸ 백업 비활성화 시나리오: Multi-AZ 비활성화 후에만 보존 기간을 0으로 설정 가능\n▸ AWS 오류 메시지 해석: 보존 기간 범위 오류(1~35)는 Multi-AZ 활성화 상태의 표지\n▸ RDS 배포 모드별 백업 정책 이해: 단일 AZ vs Multi-AZ vs Read Replica",
